...
function buildframes($framename = ''){
	...
	//xiaojie 修改 2018/07/07  解决用户清除缓存之后，出现的管理员和超级管理员菜单问题
	//$frames = cache_build_frame_menu();	//1、直接加载文件判断
	$frames = cache_load('system_frame'); //2、根据菜单不同管理员的系统菜单判断
	$isfounder = empty($frames["system"])? '':$frames["system"]["section"]["welcome"]["founder"];
	if(empty($frames) || ($_W["isfounder"] && !$isfounder) || (!$_W["isfounder"] && $isfounder)) {
...
function site_safety_monitoring(){
	...
	if ($_W['isfounder']) {
			...
			if($safe=='safe'){
			$url = 'http://www.47cu.com/forum.php?mod=viewthread&tid=2512';
			return <<<EOF
$(function() {
//	var html = 
