...
function user_login_forward($forward = '') {
	...
	$login_location = array(
		// 'account' => url('home/welcome'), // xiaojie 注釋 2018/06/30 改为直接跳转商城首页
		'account' => url('site/entry/web', array('m' => 'wx_shop','r'=>"switchversion", "route"=>"shop")),
	...
	if(!empty($_W['isfounder']) && user_is_founder($_W['uid'])){
		$login_forward = url('account/display');
	}else{
		// .http://ytxcx1.iiio.top/web/index.php?c=site&a=entry&m=wx_shop&do=web&r=switchversion&route=shop
		//$login_forward =  url('site/entry/web', array('m' => 'wx_shop','r'=>"switchversion", "route"=>"shop"));
		$login_forward =  url('site/entry/web',array('m'=>'wx_shop'));
	}
