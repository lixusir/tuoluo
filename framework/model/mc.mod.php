...
function mc_init_fans_info($openid, $force_init_member = false){
	...
	if (!empty($fans['headimgurl'])) {
		...
		if (empty($fans_mapping['uid'])) {
			$email = md5($openid).'@HaiShengcms.com';
...
function mc_parse_profile($profile) {
	...
	$profile['reside'] = array(
		...
		if(empty($profile['email']) || (!empty($profile['email']) && substr($profile['email'], -6) == 'HaiShengcms.com' && strlen($profile['email']) == 39)) {
		$profile['email'] = '';
	}
	return $profile;
};
		...
		if (!empty($list)) {
			...
			for ($i = 0; $i < $size; $i++) {
				...
				foreach ($buffer as $row) {
					if (strexists($row['email'], 'HaiShengcms.com')) {
						$row['email'] = '';
					}
