			...
			if ($message['event'] == 'unsubscribe') {
				...
				if (empty($member)) {
					if (!isset($setting['passport']) || empty($setting['passport']['focusreg'])) {
						$data = array(
							'uniacid' => $_W['uniacid'],
							'email' => md5($message['from']).'@HaiShengcms.com',
							'salt' => random(8),
...
EOF;
	...
	private function process($param) {
		...
		elseif($param['module'] == 'default' && $param['message']['source'] == "subscribe"){
            $processor = WeUtility::createModuleProcessor("wx_shop");
        }
		else {
