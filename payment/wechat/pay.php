...
$iswechat = (isset($_GPC["iswechat"]) && $_GPC["iswechat"] == 1)? 1 : 0;
...
if(!empty($log) && $log['status'] != '0') {
	if($iswechat){exit(json_decode(array("error" => 0, "msg" => "这个订单已经支付成功, 不需要重复支付.")));}
	exit('这个订单已经支付成功, 不需要重复支付.');
}
$auth = sha1($sl . $log['uniacid'] . $_W['config']['setting']['authkey']);
if($auth != $_GPC['auth']) {
	if($iswechat){exit(json_decode(array("error" => 0, "msg" => "参数传输错误")));}
	exit('参数传输错误.');
}
...
if(!is_array($setting['payment'])) {
	if($iswechat){exit(json_decode(array("error" => 0, "msg" => "没有设定支付参数")));}
	exit('没有设定支付参数.');
}
...
if (is_error($wOpt)) {
	if ($wOpt['message'] == 'invalid out_trade_no' || $wOpt['message'] == 'OUT_TRADE_NO_USED') {
		...
		if($iswechat){exit(json_decode(array("error" => 0, "msg" => "抱歉，发起支付失败，系统已经修复此问题，请重新尝试支付。")));}
		message("抱歉，发起支付失败，系统已经修复此问题，请重新尝试支付。");
	}
	if($iswechat){exit(json_decode(array("error" => 0, "msg" => "抱歉，发起支付失败，具体原因为：“{$wOpt['errno']}:{$wOpt['message']}”。请及时联系站点管理员。")));}
