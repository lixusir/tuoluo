...
require IA_ROOT . '/addons/wx_shop/core/inc/functions.php';
require IA_ROOT . '/addons/wx_shop/core/inc/plugin_model.php';
require IA_ROOT . '/addons/wx_shop/core/inc/com_model.php';
new aliApy();
class aliApy 
{
	public $post;
		...
		if (!(empty($log)) && ($log['status'] == '0')) 
		{
			...
			if (!(is_error($site))) 
			{
				...
				if (method_exists($site, $method)) 
				{
					...
					pdo_update('wx_shop_order', array('paytype' => 22), array('uniacid' => $log['uniacid'], 'ordersn' => $log['tid']));
					...
					if ($result) 
					{
						...
						pdo_update('wx_shop_order', array('paytype' => 22, 'apppay' => ($this->isapp ? 1 : 0), 'transid' => $this->post['trade_no']), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
						exit('success');
					}
				}
			}
	...
	public function threen() 
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_threen_log') . ' WHERE `uniacid`=:uniacid and logno = :logno limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
		if (!($this->isapp) && ($this->post['sign_type'] == 'RSA')) 
		{
	...
	public function recharge() 
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_log') . ' WHERE `uniacid`=:uniacid and `logno`=:logno limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
		if (!($this->isapp) && ($this->post['sign_type'] == 'RSA')) 
		{
		...
		{
			pdo_update('wx_shop_member_log', array('status' => 1, 'rechargetype' => 'alipay', 'apppay' => ($this->isapp ? 1 : 0), 'transid' => $this->post['trade_no']), array('id' => $log['id']));
	...
	{
		global $_W;
		$apply = pdo_fetch('select * from ' . tablename('wx_shop_commission_apply') . ' where id=:id limit 1', array(':id' => $id));
	...
	{
		$log = pdo_fetch('select * from ' . tablename('wx_shop_member_log') . ' where id=:id limit 1', array(':id' => $id));
	...
	{
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order') . ' WHERE id = :id limit 1', array(':id' => $id));
		if (empty($item)) 
		{
			exit();
		}
		...
		$time = time();
		$goods = pdo_fetchall('SELECT g.id,g.credit, o.total,o.realprice FROM ' . tablename('wx_shop_order_goods') . ' o left join ' . tablename('wx_shop_goods') . ' g on o.goodsid=g.id ' . ' WHERE o.orderid=:orderid and o.uniacid=:uniacid', array(':orderid' => $item['id'], ':uniacid' => $item['uniacid']));
		...
		pdo_update('wx_shop_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $item['uniacid']));
		foreach ($goods as $g ) 
		{
			$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $g['id'], ':uniacid' => $item['uniacid']));
