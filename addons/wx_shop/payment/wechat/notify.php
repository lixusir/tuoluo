...
new WxShopWechatPay($get);
exit('fail');
class WxShopWechatPay
{
	public $get;
	public $type;
	public $total_fee;
	...
	public function order() 
	{
		...
		$tid = substr($tid, 0, 22);
		$order = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order') . ' WHERE ordersn = :ordersn AND uniacid = :uniacid', array(':ordersn' => $tid, ':uniacid' => $_W['uniacid']));
		...
		{
			pdo_update('wx_shop_order', array('paytype' => $paytype, 'isborrow' => $isborrow, 'borrowopenid' => $borrowopenid, 'apppay' => ($this->isapp ? 1 : 0)), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
	...
	public function recharge() 
	{
		...
		if ($OK) 
		{
			pdo_update('wx_shop_member_log', array('status' => 1, 'rechargetype' => 'wechat', 'isborrow' => $isborrow, 'borrowopenid' => $borrowopenid, 'apppay' => ($this->isapp ? 1 : 0)), array('id' => $log['id']));
	...
	public function creditShopFreight() 
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_creditshop_log') . ' WHERE `dispatchno`=:dispatchno and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':dispatchno' => $dispatchno));
		if (!(empty($log)) && ($log['dispatchstatus'] < 0)) 
		{
			pdo_update('wx_shop_creditshop_log', array('dispatchstatus' => 1), array('dispatchno' => $dispatchno));
		}
	}
	public function coupon() 
	{
		...
		$coupon = pdo_fetchcolumn('select money from ' . tablename('wx_shop_coupon') . ' where id=:id limit 1', array(':id' => $log['couponid']));
	...
	public function groups() 
	{
		...
		if ($this->is_jie) 
		{
			pdo_update('wx_shop_groups_order', array('isborrow' => '1', 'borrowopenid' => $this->get['openid']), array('orderno' => $orderno, 'uniacid' => $_W['uniacid']));
		}
		if (p('groups')) 
		{
	...
	public function threen() 
	{
		...
		if ($this->is_jie) 
		{
			pdo_update('wx_shop_threen_log', array('isborrow' => '1', 'borrowopenid' => $this->get['openid']), array('logno' => $orderno, 'uniacid' => $_W['uniacid']));
		}
		if (p('threen')) 
		{
	...
	public function grant() 
	{
		...
		if (0 < $setting['weixin']) 
		{
			...
			{
				$order = pdo_fetch('select * from ' . tablename('wx_shop_system_grant_order') . ' where logno = \'' . $this->get['out_trade_no'] . '\'');
				pdo_update('wx_shop_system_grant_order', array('paytime' => time(), 'paystatus' => 1), array('logno' => $this->get['out_trade_no']));
				...
				foreach ($plugind as $key => $value ) 
				{
					...
					$data['pluginid'] = $value;
					pdo_insert('wx_shop_system_grant_log', $data);
					$id = pdo_insertid();
					if (m('grant')) 
					{
	...
	public function plugingrant() 
	{
		...
		if (0 < $setting['weixin']) 
		{
			...
			if ($this->sign == $this->get['sign']) 
			{
				$order = pdo_fetch('select * from ' . tablename('wx_shop_system_plugingrant_order') . ' where logno = \'' . $this->get['out_trade_no'] . '\'');
				pdo_update('wx_shop_system_plugingrant_order', array('paytime' => time(), 'paystatus' => 1), array('logno' => $this->get['out_trade_no']));
				$plugind = explode(',', $order['pluginid']);
	...
	public function mr() 
	{
		...
		if (p('mr')) 
		{
			$price = pdo_fetchcolumn('select payprice from ' . tablename('wx_shop_mr_order') . ' where ordersn=:ordersn limit 1', array(':ordersn' => $ordersn));
			if ($price == $this->total_fee) 
			{
				if ($isborrow == 1) 
				{
					pdo_update('wx_shop_order', array('isborrow' => $isborrow, 'borrowopenid' => $borrowopenid), array('ordersn' => $ordersn));
				}
		...
		if (!(empty($log)) && ($log['status'] == '0') && ($log['fee'] == $this->total_fee)) 
		{
			...
			if (!(is_error($site))) 
			{
				...
				if (method_exists($site, $method)) 
				{
					...
					$ret['fee'] = $log['fee'];
					$ret['tag'] = $log['tag'];
					pdo_update('wx_shop_order', array('paytype' => 21, 'apppay' => 2), array('ordersn' => $log['tid'], 'uniacid' => $log['uniacid']));
					$result = $site->$method($ret);
					if ($result) 
					{
						$log['tag'] = iunserializer($log['tag']);
	...
	public function wxapp_recharge() 
	{
		...
		if ($OK) 
		{
			pdo_update('wx_shop_member_log', array('status' => 1, 'rechargetype' => 'wechat', 'apppay' => 2), array('id' => $log['id']));
			$shopset = m('common')->getSysset('shop');
			m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, $shopset['name'] . '会员充值:wechatnotify:credit2:' . $log['money']));
			m('member')->setRechargeCredit($log['openid'], $log['money']);
			com_run('sale::setRechargeActivity', $log);
		...
		{
			pdo_update('wx_shop_member_log', array('rechargetype' => 'wechat', 'apppay' => 2), array('id' => $log['id']));
		}
	}
