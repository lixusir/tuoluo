...
class HomeController extends Controller
{
	...
	{
		$set = pdo_fetch('select * from ' . tablename('wx_shop_system_site') . ' where `type`=:type', array(':type' => $this->type));
	...
	public function ajaxguestbook()
	{
		...
		$guestbook = pdo_fetch('SELECT * FROM' . tablename('wx_shop_system_guestbook') . ' WHERE clientip=:clientip ORDER BY createtime DESC LIMIT 1', array(':clientip' => ES_CLIENT_IP));
		...
		pdo_insert('wx_shop_system_guestbook', $guestbookform);
	...
	protected function banner($status = 'all')
	{
		...
		$result = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_system_banner') . ' WHERE 1 ' . $statusSql['sql'] . '  ORDER BY displayorder DESC', $statusSql['param']);
		return $result;
	}
	...
	{
		$statusSql = $this->statusSql($status);
		$result = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_system_case') . ' WHERE 1 ' . $statusSql['sql'] . '  ORDER BY displayorder DESC', $statusSql['param']);
		return $result;
	}
	...
	{
		$statusSql = $this->statusSql($status);
		$result = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_system_link') . ' WHERE 1 ' . $statusSql['sql'] . '  ORDER BY displayorder DESC', $statusSql['param']);
		return $result;
	}
	...
	protected function article($status = 'all', $limit = 6)
	{
		...
		$result = pdo_fetchall('SELECT a.id,a.title,a.content,a.createtime,c.name FROM ' . tablename('wx_shop_system_article') . ' AS a LEFT JOIN ' . tablename('wx_shop_system_category') . ' AS c ON a.cate = c.id WHERE 1 ' . $statusSql['sql'] . '  ORDER BY a.displayorder DESC LIMIT ' . $limit, $statusSql['param']);
		return $result;
	}
	...
	protected function companyArticle($status = 'all', $limit = 6)
	{
		$statusSql = $this->statusSql($status);
		$statusSql['sql'] = str_replace(' status ', ' a.status ', $statusSql['sql']);
		$result = pdo_fetchall('SELECT a.*,c.id as cid,c.name FROM ' . tablename('wx_shop_system_company_article') . ' AS a LEFT JOIN ' . tablename('wx_shop_system_company_category') . ' AS c ON a.cate = c.id WHERE 1 ' . $statusSql['sql'] . '  ORDER BY a.displayorder DESC LIMIT ' . $limit, $statusSql['param']);
		return $result;
	}
