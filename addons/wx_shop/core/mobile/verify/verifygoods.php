...
class Verifygoods_WxShopPage extends MobilePage 
{
	public function detail() 
	{
		...
		$saler = pdo_fetch('select * from ' . tablename('wx_shop_saler') . ' where  status=1  and openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		if (empty($saler)) 
		{
		...
		{
			$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('wx_shop_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $item['id']));
			$verifynum = 0;
	...
	public function search() 
	{
		...
		$verifygood = pdo_fetch('select *  from ' . tablename('wx_shop_verifygoods') . ' where uniacid=:uniacid and  verifycode=:verifycode  limit 1 ', array(':uniacid' => $_W['uniacid'], ':verifycode' => $verifycode));
		if (empty($verifygood)) 
		{
	...
	public function complete() 
	{
		...
		{
			$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('wx_shop_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $verifygood['id']));
			$verifynum = 0;
			foreach ($verifygoodlogs as $verifygoodlog ) 
			{
				$verifynum += intval($verifygoodlog['verifynum']);
		...
		pdo_insert('wx_shop_verifygoods_log', $data);
		$logid = pdo_insertid();
		m('notice')->sendVerifygoodMessage($logid);
		pdo_query('update ' . tablename('wx_shop_verifygoods') . ' set used=:used , verifycode=null ,codeinvalidtime=0 where id=:id', array(':id' => $verifygood['id'], ':used' => $used));
		...
		if ($isonlyverifygood) 
		{
			$status = pdo_fetchcolumn('select status  from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and id=:id  limit 1 ', array(':uniacid' => $_W['uniacid'], ':id' => $verifygood['orderid']));
			if ($status == 2) 
			{
	...
	public function finishorder($id) 
	{
		...
		pdo_update('wx_shop_order', array('status' => 3, 'sendtime' => time(), 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
