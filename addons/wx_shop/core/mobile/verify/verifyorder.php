...
class Verifyorder_WxShopPage extends MobilePage
{
	public function orderData() 
	{
		...
		if (!(empty($_GPC['searchfield'])) && !(empty($_GPC['keyword']))) 
		{
			...
			{
				$sqlcondition = ' inner join ( select DISTINCT(og.orderid) from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id';
			}
			else if ($searchfield == 'goodssn') 
			{
				$sqlcondition = ' inner join ( select DISTINCT(og.orderid) from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0))) gs on gs.orderid=o.id';
			}
		}
		$statuscondition = '';
		...
		else 
		{
			...
			$sql = 'select * from ' . tablename('wx_shop_order') . ' where uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 ' . $status_condition . ' GROUP BY id ORDER BY createtime DESC  ';
			if (empty($_GPC['export'])) 
			{
				$sql .= 'LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
			}
		...
		if ($_GPC['export'] == 1) 
		{
			...
			foreach ($list as &$row ) 
			{
				...
				if (!(empty($row['verifystoreid']))) 
				{
					if (0 < $row['merchid']) 
					{
						$row['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('wx_shop_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['verifystoreid']));
					}
					else 
					{
						$row['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('wx_shop_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['verifystoreid']));
					}
				}
				else if (!(empty($row['verifyinfo']))) 
				{
				...
				if (!(empty($row['storeid']))) 
				{
					if (0 < $row['merchid']) 
					{
						$row['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('wx_shop_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['storeid']));
					}
					else 
					{
						$row['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('wx_shop_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['storeid']));
					}
				}
				...
				if ($row['isverify']) 
				{
					if (is_array($verifyinfo)) 
					{
						if (empty($row['dispatchtype'])) 
						{
							...
							{
								$v['storename'] = pdo_fetchcolumn('select storename from ' . tablename('wx_shop_merch_store') . ' where id=:id limit 1', array(':id' => $v['verifystoreid']));
								...
								$v['salername'] = pdo_fetchcolumn('select salername from ' . tablename('wx_shop_merch_saler') . ' where openid=:openid and uniacid=:uniacid and merchid = :merchid limit 1', array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		{
			$t = pdo_fetch('SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('wx_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . ' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition, $paras);
		}
		else 
		{
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('wx_shop_order') . ' WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 ' . $status_condition, $paras);
		}
		$total = $t['count'];
		$totalmoney = $t['sumprice'];
		$pager = pagination2($total, $pindex, $psize);
		$stores = pdo_fetchall('select id,storename from ' . tablename('wx_shop_store') . ' where uniacid=:uniacid ', array(':uniacid' => $uniacid));
	...
	public function log() 
	{
		...
		{
			$openid = $_W['openid'];
			$salers = pdo_fetch('select r.perms  from ' . tablename('wx_shop_saler') . ' s inner join ' . tablename('wx_shop_newstore_perm_role') . ' r on s.roleid = r.id  where  s.openid=:openid and r.deleted =0  limit 1', array(':openid' => $openid));
	...
	protected function selectOrderPrice($day = 0) 
	{
		...
		$sql = 'SELECT o.id,o.price,o.createtime,s.openid,s.salername FROM' . tablename('wx_shop_order') . ' o inner JOIN ' . tablename('wx_shop_saler') . ' s ON o.verifyopenid=s.openid   WHERE ' . "\r\n" . '        o.uniacid=:uniacid AND o.ismr=0  AND   o.status =3  AND o.deleted=0 AND o.createtime between :createtime1 and :createtime2';
	...
	public function detail() 
	{
		...
		$refund = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order_refund') . ' WHERE orderid = :orderid and uniacid=:uniacid order by id desc', array(':orderid' => $item['id'], ':uniacid' => $_W['uniacid']));
		$diyformfields = '';
		if (p('diyform')) 
		{
