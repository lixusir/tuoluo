...
class Detail_WxShopPage extends MobilePage
{
    public function main()
    {
        ...
        if (p('task'))
        {
            if (!(empty($task_id)))
            {
                $rewarded = pdo_fetchcolumn('SELECT `rewarded` FROM ' . tablename('wx_shop_task_extension_join') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $task_id, ':uniacid' => $_W['uniacid']));
            ...
            $taskGoodsInfo = pdo_fetch('select * from ' . tablename('wx_shop_task_reward') . ' where id = :id and openid = :openid and senttime = 0 and gettime > 0', array(':id' => $taskrewardgoodsid, ':openid' => $_W['openid']));
            if (!empty($taskGoodsInfo)) {
                $reward = pdo_fetchcolumn('select reward_data from '.tablename('wx_shop_task_record').' where id=:id', array(':id' => $taskGoodsInfo['recordid']));
                $reward = json_decode($reward,true);
                foreach($reward['goods'] as $v) {
                    if($v['id'] == $_GPC['id']) {
                        $taskGoodsInfo['goodsStatus'] = true;
                        $taskGoodsInfo['price'] = $v['price'];
                        $taskGoodsInfo['reward_data'] = $v['id'];
                    }
                }
                if (!$taskGoodsInfo['goodsStatus']) {
                    $taskGoodsInfo = array();
                }
            }
        ...
        $gifts = array();
        $giftgoods = array();
        $gifts = pdo_fetchall('select id,goodsid,giftgoodsid,thumb,title from ' . tablename('wx_shop_gift') . ' where uniacid = ' . $uniacid . ' and activity = 2 and status = 1 and starttime <= ' . time() . ' and endtime >= ' . time() . '  ');
        foreach ($gifts as $key => &$value )
        {
            ...
            foreach ($gid as $ke => $val )
            {
                if ($val == $id)
                {
                    ...
                    {
                        $isgift = 1;
                        $gifts[$key]['gift'][$k] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0 and total > 0 and status = 2 and id = ' . $val . ' ');
        ...
        $goods = pdo_fetch('select * from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        if (p('offic'))
        {
        ...
        $style = pdo_fetch('SELECT id,uniacid,style FROM ' . tablename('wx_shop_goods_labelstyle') . ' WHERE uniacid=' . $uniacid);
        if ($is_openmerch == 0)
        {
            if (0 < $merchid)
            {
        ...
        {
            $address_list = pdo_fetchall('select id,datavalue from ' . tablename('wx_shop_member_address') . ' where openid=:openid and uniacid=:uniacid', array(':uniacid' => $uniacid, ':openid' => $openid));
            if (!(empty($address_list)))
            {
                ...
                foreach ($address_list as $k1 => $v1 )
                {
                    if (!(empty($v1['datavalue'])))
                    {
                        ...
                        {
                            pdo_update('wx_shop_member_address', $change_data, array('id' => $v1['id']));
                        }
                    }
                }
                pdo_update('wx_shop_member', array('updateaddress' => 1), array('id' => $member['id']));
            }
        }
        ...
        if (empty($task_goods_data['is_task_goods']))
        {
            ...
            if (p('bargain'))
            {
                $bargain = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_bargain_goods') . ' WHERE id = :id AND unix_timestamp(start_time)<' . time() . ' AND unix_timestamp(end_time)>' . time() . ' AND status = 0', array(':id' => $goods['bargain']));
                if ($bargain != false)
                {
        ...
        if ($goods['buyshow'] == 1)
        {
            $sql = 'select o.id from ' . tablename('wx_shop_order') . ' o left join ' . tablename('wx_shop_order_goods') . ' g on o.id = g.orderid';
        ...
        $package_goods = pdo_fetch('select pg.id,pg.pid,pg.goodsid,p.displayorder from ' . tablename('wx_shop_package_goods') . ' as pg' . "\r\n" . '                        left join ' . tablename('wx_shop_package') . ' as p on pg.pid = p.id' . "\r\n" . '                        where pg.uniacid = ' . $uniacid . ' and pg.goodsid = ' . $id . ' and  p.starttime <= ' . time() . ' and p.endtime >= ' . time() . ' and p.deleted = 0 and p.status = 1 ORDER BY p.displayorder desc,pg.id desc limit 1 ');
        if ($package_goods['pid'])
        {
            $packages = pdo_fetchall('SELECT id,title,thumb,packageprice FROM ' . tablename('wx_shop_package_goods') . "\r\n" . '                    WHERE uniacid = ' . $uniacid . ' and pid = ' . $package_goods['pid'] . '  ORDER BY id DESC');
        ...
        $specs = pdo_fetchall('select * from ' . tablename('wx_shop_goods_spec') . ' where goodsid=:goodsid and  uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
        $spec_titles = array();
        ...
        $params = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_goods_param') . ' WHERE uniacid=:uniacid and goodsid=:goodsid order by displayorder asc', array(':uniacid' => $uniacid, ':goodsid' => $goods['id']));
        $goods = set_medias($goods, 'thumb');
        $goods['canbuy'] = ($goods['status'] == 1) && empty($goods['deleted']);
        if (!(empty($goods['hasoption'])))
        {
            $options = pdo_fetchall('select id,stock,marketprice,islive,liveprice from ' . tablename('wx_shop_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $goods['id'], ':uniacid' => $_W['uniacid']), 'stock');
        ...
        {
            $order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $goods['id'], ':uniacid' => $uniacid, ':openid' => $openid));
        ...
        else
        {
            ...
            $goods_price = pdo_fetch('select minprice,maxprice from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        ...
        {
            $presell = pdo_fetch('select min(presellprice) as minprice,max(presellprice) as maxprice from ' . tablename('wx_shop_goods_option') . ' where goodsid = ' . $id);
        ...
        if (empty($this->merch_user))
        {
            ...
            if (($is_openmerch == 1) && (0 < $goods['merchid']))
            {
                $merch_user = pdo_fetch('select * from ' . tablename('wx_shop_merch_user') . ' where id=:id limit 1', array(':id' => intval($goods['merchid'])));
                if (!(empty($merch_user)))
                {
                    $shop = $merch_user;
                    $merch_flag = 1;
            ...
            {
                $statics = array('all' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid ' . $sqlcon . ' and status=1 and deleted=0', $param), 'new' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid ' . $sqlcon . ' and isnew=1 and status=1 and deleted=0', $param), 'discount' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid ' . $sqlcon . ' and isdiscount=1 and status=1 and deleted=0', $param));
            }
            else
            {
                $goodsids = explode(',', $shop['goodsids']);
                $statics = array('all' => count($goodsids), 'new' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid ' . $sqlcon . ' and id in( ' . $shop['goodsids'] . ' ) and isnew=1 and status=1 and deleted=0', $param), 'discount' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid ' . $sqlcon . ' and id in( ' . $shop['goodsids'] . ' ) and isdiscount=1 and status=1 and deleted=0', $param));
            }
        }
        ...
        else
        {
            ...
            if (empty($shop['selectgoods']))
            {
                $statics = array('all' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and merchid=:merchid and status=1 and deleted=0', array(':uniacid' => $_W['uniacid'], ':merchid' => $goods['merchid'])), 'new' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and merchid=:merchid and isnew=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid'], ':merchid' => $goods['merchid'])), 'discount' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and merchid=:merchid and isdiscount=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid'], ':merchid' => $goods['merchid'])));
            }
            else
            {
                $goodsids = explode(',', $shop['goodsids']);
                $statics = array('all' => count($goodsids), 'new' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and merchid=:merchid and id in( ' . $shop['goodsids'] . ' ) and isnew=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid'], ':merchid' => $goods['merchid'])), 'discount' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and merchid=:merchid and id in( ' . $shop['goodsids'] . ' ) and isdiscount=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid'], ':merchid' => $goods['merchid'])));
            }
        }
        ...
        {
            $showComments = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order_comment') . ' where goodsid=:goodsid and level>=0 and deleted=0 and checked=0 and uniacid=:uniacid', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
        }
        $goodscode = '';
        $parameter = array();
        if (com('goodscode'))
    ...
    public function querygift()
    {
        ...
        $uniacid = $_W['uniacid'];
        $giftid = $_GPC['id'];
        $gift = pdo_fetch('select * from ' . tablename('wx_shop_gift') . ' where uniacid = ' . $uniacid . ' and status = 1 and id = ' . $giftid . ' ');
        show_json(1, $gift);
    }
    ...
    public function get_comments()
    {
        ...
        $count = array('all' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order_comment') . ' where goodsid=:goodsid and level>=0 and deleted=0 and checked=0 and uniacid=:uniacid', $params), 'good' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order_comment') . ' where goodsid=:goodsid and level>=5 and deleted=0 and checked=0 and uniacid=:uniacid', $params), 'normal' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order_comment') . ' where goodsid=:goodsid and level>=2 and level<=4 and deleted=0 and checked=0 and uniacid=:uniacid', $params), 'bad' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order_comment') . ' where goodsid=:goodsid and level<=1 and deleted=0 and checked=0 and uniacid=:uniacid', $params), 'pic' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order_comment') . ' where goodsid=:goodsid and ifnull(images,\'a:0:{}\')<>\'a:0:{}\' and deleted=0 and checked=0 and uniacid=:uniacid', $params));
        $list = array();
        if (0 < $count['all'])
        {
            $percent = intval(($count['good'] / ((empty($count['all']) ? 1 : $count['all']))) * 100);
            $list = pdo_fetchall('select nickname,level,content,images,createtime from ' . tablename('wx_shop_order_comment') . ' where goodsid=:goodsid and deleted=0 and checked=0 and uniacid=:uniacid order by istop desc, createtime desc, id desc limit 2', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
            foreach ($list as &$row )
            {
    ...
    public function get_offic_list()
    {
        ...
        $total = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_offic') . ' as o where 1 ' . $condition, $params);
        $list = array();
        if (0 < $total)
        {
            $list = pdo_fetchall('SELECT o.*,m.avatar,m.nickname,g.thumb,g.title,g.minprice FROM ' . tablename('wx_shop_offic') . ' as o' . "\r\n" . '                    left join ' . tablename('wx_shop_member') . ' as m on m.openid = o.openid' . "\r\n" . '                    left join ' . tablename('wx_shop_goods') . ' as g on g.id = o.goodsid' . "\r\n" . '                    where 1 ' . $condition . ' group by o.id order by o.displayorder desc, o.createtime desc, o.id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
    ...
    public function get_comment_list()
    {
        ...
        $list = pdo_fetchall('select * from ' . tablename('wx_shop_order_comment') . ' ' . '  where goodsid=:goodsid and uniacid=:uniacid and deleted=0 and checked=0 ' . $condition . ' order by istop desc, createtime desc, id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
        foreach ($list as &$row )
        {
        ...
        unset($row);
        $total = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order_comment') . ' where goodsid=:goodsid  and uniacid=:uniacid and deleted=0 and checked=0 ' . $condition, $params);
    ...
    public function getCouponsbygood($goodid)
    {
        ...
        if (!(empty($goodid)))
        {
            ...
            $goodparam[':id'] = $goodid;
            $sql = 'select id,cates,marketprice,merchid   from ' . tablename('wx_shop_goods');
        ...
        if (!(empty($list)))
        {
            foreach ($list as $key => &$row )
            {
                ...
                if ($row['islimitlevel'] == 1)
                {
                    ...
                    {
                        $level1 = pdo_fetchall('select * from ' . tablename('wx_shop_member_level') . ' where uniacid=:uniacid and  id in (' . $row['limitmemberlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
                    ...
                    {
                        $level2 = pdo_fetchall('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid and id  in (' . $row['limitagentlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
                        if (($member['isagent'] == '1') && ($member['status'] == '1'))
                        {
                    ...
                    {
                        $level3 = pdo_fetchall('select * from ' . tablename('wx_shop_globonus_level') . ' where uniacid=:uniacid and  id in(' . $row['limitpartnerlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
                    ...
                    {
                        $level4 = pdo_fetchall('select * from ' . tablename('wx_shop_abonus_level') . ' where uniacid=:uniacid and  id in (' . $row['limitaagentlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
    ...
    public function pay($a = array(), $b = array())
    {
        ...
        $openid = $_W['openid'];
        $id = intval($_GPC['id']);
        $coupon = pdo_fetch('select * from ' . tablename('wx_shop_coupon') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
