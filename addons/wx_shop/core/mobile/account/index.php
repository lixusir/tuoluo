...
class Index_WxShopPage extends MobilePage
{
	...
	public function applogin()
	{
		...
		if ($_W['ispost']) 
		{
			$mobile = trim($_GPC['mobile']);
			$pwd = trim($_GPC['pwd']);
			$member = pdo_fetch('select id,openid,nickname,avatar,pwd,salt from ' . tablename('wx_shop_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
			if (empty($member)) 
			{
				show_json_w(-1,null,"用户不存在");
			}
			if (md5($pwd . $member['salt']) !== $member['pwd']) 
			{
				show_json_w(-1,null,"用户或密码错误");
			}
			$data['token'] = md5(serialize($member['openid']).time());
	        $data['logintime']=time();
			...
			$returnjson['nickname'] = $member['nickname'];
			$returnjson['avatar'] = $member['avatar'];
			
			$returnjson['uid']=$member['id'];
	...
	protected function rf($type) 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($mobile)) 
			{
				show_json_w(-1,null,"请输入正确的手机号");
				show_json(0, '请输入正确的手机号');
			}
			if (empty($verifycode)) 
			{
				show_json_w(-1,null,"请输入验证码");
				show_json(0, '请输入验证码');
			}
			if (empty($pwd)) 
			{
				show_json_w(-1,null,"请输入密码");
				show_json(0, '请输入密码');
			}
			...
			if (empty($type)) 
			{
				if (!(empty($member))) 
				{
					show_json_w(-1,null,"此手机号已注册, 请直接登录");
				...
				$data['token'] = md5(serialize($data['mobile']).time());
		        $data['logintime']=time();
			...
			if (empty($member)) 
			{
				...
				$s = pdo_fetch('select id from ' . tablename('wx_shop_member') . ' where yqm=:yqm limit 1', array(':yqm' => $mid));
				// pdo_fetch('select ')
				// echo '<pre>';
				// 	    print_r($mid);
				// 	echo '</pre>';
				...
				$returnjson['token'] = $data['token'];
				$returnjson['nickname'] = $data['nickname'];
				$returnjson['avatar'] = '';
				
				$returnjson['uid']= pdo_insertid();
				show_json_w(1,$returnjson,'成功');
