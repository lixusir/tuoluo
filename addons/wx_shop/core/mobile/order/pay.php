...
class Pay_WxShopPage extends MobileLoginPage
{
	public function main() 
	{
        ...
        if (!(empty($ispeerpay)))
		{
			if (pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_order_peerpay_payinfo') . ' WHERE openid = ":openid" AND pid = ":pid"', array(':openid' => $_W['openid'], ':pid' => $ispeerpay['id'])))
			{
		...
		if (empty($seckill_goods) && empty($ispeerpay)) 
		{
			...
			{
				$alipayset = pdo_fetchcolumn('select alipay_sec from '.tablename('wx_shop_payment').' where paytype=1 and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
				if (!empty($alipayset)) {
		            $options = iunserializer($alipayset);
		            $options = array(
		                'account' => $options['alipayaccount'],
		                'partner' => $options['appid'],
	...
	public function success() 
	{
		...
		if (!(empty($ispeerpay))) 
		{
			...
			$activity = com('coupon')->activity($peerprice['price']);
			if ($activity) 
			{
				$share = true;
			}
