...
class Create_WxShopPage extends MobileLoginPage
{
	...
	public function main() 
	{
		...
		if (!($packageid)) 
		{
			...
			if (p('bargain') && !(empty($bargain_id))) 
			{
				...
				$bargain_act = pdo_fetch('SELECT *  FROM ' . tablename('wx_shop_bargain_actor') . ' WHERE id = :id AND openid = :openid AND status = \'0\'', array(':id' => $bargain_id, ':openid' => $_W['openid']));
			...
			foreach ($goods as &$g ) 
			{
				...
				else 
				{
					if (p('task')) 
					{
						...
						if (!(empty($task_id))) 
						{
							$rewarded = pdo_fetchcolumn('SELECT `rewarded` FROM ' . tablename('wx_shop_task_extension_join') . ' WHERE id = :id AND openid = :openid AND  uniacid = :uniacid', array(':id' => $task_id, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
			...
			if (!(empty($saleset))) 
			{
				...
				if (!(empty($saleset['moneydeduct']))) 
				{
					...
					{
						$deductcredit2 = $taskreward['price'];
					}
				}
			}
			$goodsdata = array();
	...
	public function submit() 
	{
		...
		{
			$bargain_act = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_bargain_actor') . ' WHERE id = :id AND openid = :openid ', array(':id' => $goods[0]['bargain_id'], ':openid' => $_W['openid']));
			if (empty($bargain_act)) 
			{
				exit('没有这个商品');
			}
			...
			if (!(pdo_update('wx_shop_bargain_actor', array('status' => 1), array('id' => $goods[0]['bargain_id'], 'openid' => $_W['openid'])))) 
			{
				exit('下单失败');
			}
		...
		if ($taskgoodsprice) 
		{
			...
			pdo_update('wx_shop_task_extension_join', array('rewarded' => serialize($taskGoodsInfo0)), array('id' => $task_id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
			unset($_SESSION[$goodsid . '_task_id']);
		}
		$order = array();

        //代理分红修改begin
        $order['isweightbonus'] = $isweightbonus;
        $order['weightbonusprice'] = $weightbonusprice;
        //代理分红修改end
        ...
        //代理分红begin
        //当该订单参与分红时，异步存入分红信息
        if($order['isweightbonus']==1){
            load()->func('communication');
            ihttp_get($_W['siteroot'].'app/wx_shop_api.php?r=upgradeasyn.orderWeightbonus&i='.$_W['uniacid'].'&orderid='.$orderid.'&openid='.$order['openid']);
        }
        //代理分红end

		if (!(empty($goods[0]['bargain_id'])) && p('bargain')) 
		{
			pdo_update('wx_shop_bargain_actor', array('order' => $orderid), array('id' => $goods[0]['bargain_id'], 'openid' => $_W['openid']));
		}
		...
		if (!(is_null($_SESSION['exchangeprice'])) && !(empty($_SESSION['exchange'])) && p('exchange')) 
		{
			...
			pdo_update('wx_shop_exchange_cart', array('selected' => 0), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'selected' => 1));
		}
		...
		if (p('quick') && !(empty($_GPC['fromquick']))) 
		{
			pdo_update('wx_shop_quick_cart', array('deleted' => 1), array('quickid' => intval($_GPC['fromquick']), 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		}
		if (0 < $deductcredit) 
		{
