...
class Index_WxShopPage extends MobilePage
{
	public function detail() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('select * from ' . tablename('wx_shop_store') . ' where id =:id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if(empty($item)){
			$item = pdo_fetch('select * from ' . tablename('wx_shop_merch_store') . ' where id =:id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		}
