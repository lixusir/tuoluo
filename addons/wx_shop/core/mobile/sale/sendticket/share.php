...
class Share_WxShopPage extends MobileLoginPage
{
	...
	public function enjoy() 
	{
		...
		if (!(empty($_GPC['sid']))) 
		{
			$asql = 'SELECT * FROM ' . tablename('wx_shop_sendticket_share') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($_GPC['sid']) . ' AND status = 1 AND (expiration = 0 OR (expiration = 1 AND endtime >= ' . TIMESTAMP . '))';
		...
		$csql = 'SELECT * FROM ' . tablename('wx_shop_coupon_data') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND shareident = "' . trim($_GPC['ident']) . '" AND openid = "' . $openid . '"';
		...
		else 
		{
			...
			if (!(empty($cpid))) 
			{
				...
				{
					$couponsql = 'SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . $cv;
		...
		$storesql = 'SELECT * FROM ' . tablename('wx_shop_sysset') . ' WHERE uniacid = ' . intval($_W['uniacid']);
	...
	{
		global $_W;
		$sql = 'SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE  uniacid = ' . intval($_W['uniacid']) . ' AND id IN (' . $cpids . ')';
	...
	public function sendTicket($openid, $couponid, $gettype = 0, $num, $ident = '') 
	{
		...
		while ($i < $num) 
		{
			...
			$datas = pdo_insert('wx_shop_coupon_data', $data);
			++$i;
		}
		if ($log && $datas) 
		{
	...
	public function shareCoupon() 
	{
		...
		if ($shareOpenid == $openid) 
		{
			...
			pdo_update('wx_shop_order', $data, array('id' => intval($_GPC['orderid'])));
	...
	public function shareSuccess() 
	{
		...
		global $_GPC;
		$openid = $_W['openid'];
		$orderSql = 'SELECT isshare FROM ' . tablename('wx_shop_order') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($_GPC['orderid']);
		$orderData = pdo_fetch($orderSql);
		if ($orderData['isshare'] == 1) 
		{
			return false;
		}
		$couponDataSql = 'SELECT * FROM ' . tablename('wx_shop_coupon_data') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND shareident = "' . $_GPC['ident'] . '"';
