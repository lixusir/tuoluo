...
class Quickget_WxShopPage extends MobileLoginPage 
{
	public function main() 
	{
		...
		$time = time();
		$coupon = pdo_fetch('select * from ' . tablename('wx_shop_coupon') . ' where  (timelimit = 0  or  (timelimit =1 and timestart<=:time && timeend>=:time)) and uniacid=:uniacid  and id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $id, ':time' => $time));
