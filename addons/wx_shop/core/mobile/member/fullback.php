...
class Fullback_WxShopPage extends MobileLoginPage
{
	...
	public function get_list() 
	{
		...
		$list = array();
		$list = pdo_fetchall('select fl.*,g.thumb,g.title,og.optionname from ' . tablename('wx_shop_fullback_log') . ' as fl' . "\r\n" . '            left join ' . tablename('wx_shop_goods') . ' as g on g.id = fl.goodsid' . "\r\n" . '            left join ' . tablename('wx_shop_order_goods') . ' as og on og.orderid = fl.orderid and og.goodsid = fl.goodsid ' . "\r\n" . '            where 1 ' . $condition . ' group by fl.id order by fl.createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_fullback_log') . ' as fl' . "\r\n" . '            where 1 ' . $condition . ' order by fl.createtime desc ', $params);
		foreach ($list as &$row ) 
		{
