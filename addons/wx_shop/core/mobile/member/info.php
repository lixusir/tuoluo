...
class Info_WxShopPage extends MobileLoginPage
{
	protected $member;
	...
	public function submit() 
	{
		...
		if ($template_flag == 1) 
		{
			...
			pdo_update('wx_shop_member', $m_data, array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']));
	...
	public function face() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$res = pdo_update('wx_shop_member', array('nickname' => $nickname), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			show_json(1);
		}
		include $this->template();
	}
	...
	public function editheadpic() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			pdo_update('wx_shop_member', array('avatar' => $avatar), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			show_json(1);
		}
	}
}
