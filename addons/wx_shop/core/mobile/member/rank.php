...
class Rank_WxShopPage extends MobileLoginPage 
{
	...
	public function order_rank() 
	{
		...
		$this->orderStatus();
		$user = m('member')->getMember($_W['openid'], true);
		$result_one = pdo_fetch('select a.openid, sum(price) as price,a.uniacid,a.status' . "\r\n\t\t\t" . 'from (' . "\r\n\t\t\t" . '(select openid, sum(price) price,uniacid,status from ' . tablename('wx_shop_order') . ' where uniacid = :uniacid and status = 3 group by openid)' . "\r\n\t\t\t" . 'union  all (select openid, sum(price+freight) price,uniacid,status from ' . tablename('wx_shop_groups_order') . ' where uniacid = :uniacid and status >= 1 group by openid)' . "\r\n\t\t\t" . ') a where a.openid = :openid group by a.openid ', array(':uniacid' => $_W['uniacid'], ':openid' => $user['openid']));
	...
	protected function orderChange($openid, $day = 7) 
	{
		...
		$sql = 'select sum(price) from ' . tablename('wx_shop_order') . ' where uniacid = :uniacid and openid = :openid and status >= 1 and createtime between :createtime1 and :createtime2';
