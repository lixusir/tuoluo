...
class Coupon_WxShopComModel extends ComModel
{
	protected function getUrl($do, $query = NULL)
	{
		$url = mobileUrl($do, $query, true);

		if (strexists($url, '/addons/wx_shop/')) {
			$url = str_replace('/addons/wx_shop/', '/', $url);
		}
	...
	{
		global $_W;
		$coupon = pdo_fetch('SELECT id,total FROM ' . tablename('wx_shop_coupon') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
		...
		$gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid ', array(':couponid' => $couponid, ':uniacid' => $_W['uniacid']));
	...
	public function creditshop($logid = 0)
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_creditshop_log') . ' WHERE `id`=:id and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $logid));
		...
		if (!empty($log)) {
			...
			pdo_insert('wx_shop_coupon_log', $couponlog);
			...
			$coupon = pdo_fetch('select * from ' . tablename('wx_shop_coupon') . ' where id=:id limit 1', array(':id' => $log['couponid']));
			$coupon = $this->setCoupon($coupon, time());
			$this->sendMessage($coupon, 1, $member);
			pdo_update('wx_shop_creditshop_log', array('status' => 3), array('id' => $logid));
		}
	}
	...
	public function getAvailableCoupons($type, $money = 0, $merch_array, $goods_array = array())
	{
		...
		if (!empty($goods_array)) {
			foreach ($goods_array as $key => $value) {
				...
				$sql = 'select id,`type`,cates,marketprice,merchid   from ' . tablename('wx_shop_goods');
				...
				foreach ($goodlist as $good) {
					...
					if ((0 < $row['enough']) && ($p == 1)) {
						if ((0 < $good['optionid']) && ($good['type'] != 4)) {
							...
							$sql = 'select  marketprice  from ' . tablename('wx_shop_goods_option');
							$sql .= ' where uniacid=:uniacid and id =:id order by id desc LIMIT 1 ';
	...
	public function payResult($logno)
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_coupon_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
		...
		if (empty($log['status'])) {
			...
			$update['status'] = 1;
			pdo_update('wx_shop_coupon_log', $update, array('id' => $log['id']));
	...
	public function useRechargeCoupon($log)
	{
		...
		$data = pdo_fetch('select id,openid,couponid,used from ' . tablename('wx_shop_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['couponid'], ':uniacid' => $_W['uniacid']));
		...
		$coupon = pdo_fetch('select enough,backcredit,backmoney,backredpack,couponname from ' . tablename('wx_shop_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
		...
		pdo_update('wx_shop_coupon_data', array('used' => 1, 'usetime' => time(), 'ordersn' => $log['logno']), array('id' => $data['id']));
	...
	public function consumeCouponCount($openid, $money = 0, $merch_array, $goods_array)
	{
		...
		$param[':openid'] = $openid;
		$param[':uniacid'] = $_W['uniacid'];
		$sql = 'select d.id,d.couponid,c.enough,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodcateids,c.limitgoodids  from ' . tablename('wx_shop_coupon_data') . ' d';
		$sql .= ' left join ' . tablename('wx_shop_coupon') . ' c on d.couponid = c.id';
		...
		if (!empty($goods_array)) {
			foreach ($goods_array as $key => $value) {
				$goodparam[':uniacid'] = $_W['uniacid'];
				$goodparam[':id'] = $value['goodsid'];
				$sql = 'select id,cates,marketprice,merchid,`type`  from ' . tablename('wx_shop_goods');
				$sql .= ' where uniacid=:uniacid and id =:id order by id desc LIMIT 1 ';
				$good = pdo_fetch($sql, $goodparam);
				$good['saletotal'] = $value['total'];
				$good['optionid'] = $value['optionid'];
	...
	public function useConsumeCoupon($orderid = 0)
	{
		...
		$order = pdo_fetch('select ordersn,createtime,couponid from ' . tablename('wx_shop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
		...
		pdo_update('wx_shop_coupon_data', array('used' => 1, 'usetime' => $order['createtime'], 'ordersn' => $order['ordersn']), array('id' => $order['couponid']));
	...
	public function returnConsumeCoupon($order)
	{
		...
		if (!is_array($order)) {
			$order = pdo_fetch('select id,openid,ordersn,createtime,couponid,status,finishtime from ' . tablename('wx_shop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => intval($order), ':uniacid' => $_W['uniacid']));
		}
		...
		if (!empty($coupon['returntype'])) {
			if (!empty($coupon['used'])) {
				pdo_update('wx_shop_coupon_data', array('used' => 0, 'usetime' => 0, 'ordersn' => ''), array('id' => $order['couponid']));
	...
	public function backConsumeCoupon($orderid)
	{
		...
		if (!is_array($orderid)) {
			$order = pdo_fetch('select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid,coupongoodprice  from ' . tablename('wx_shop_order') . ' where id=:id and status>=0 and couponid >0 and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
		}
		...
		if (($isparent == 1) || ($parentid != 0)) {
			...
			if ($isparent == 1) {
				if (0 < $couponmerchid) {
					$sql = 'select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid from ' . tablename('wx_shop_order') . ' where parentid=:parentid and couponmerchid=:couponmerchid and status>=0 and uniacid=:uniacid limit 1';
		...
		if (($parentid != 0) && ($couponmerchid == 0)) {
			if ($all_done == 1) {
				$sql = 'select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid from ' . tablename('wx_shop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1';
		...
		if ($canback) {
			if (0 < $parentid) {
				$ordermoney = pdo_fetchcolumn('select coupongoodprice from ' . tablename('wx_shop_order') . ' where id=:id and status>=0 and couponid >0 and uniacid=:uniacid limit 1', array(':id' => intval($parentid), ':uniacid' => $_W['uniacid']));
			}
			else {
			...
			if ($ordermoney == 0) {
				$sql = 'select ifnull( sum(og.realprice),0) from ' . tablename('wx_shop_order_goods') . ' og ';
				$sql .= ' left join ' . tablename('wx_shop_order') . ' o on';
			...
			pdo_update('wx_shop_coupon_data', array('back' => 1, 'backtime' => time()), array('id' => $order['couponid']));
	...
	{
		global $_W;
		return pdo_fetch('select * from ' . tablename('wx_shop_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
	}

	public function getCouponByDataID($dataid = 0)
	{
		global $_W;
		$data = pdo_fetch('select id,openid,couponid,used,back,backtime from ' . tablename('wx_shop_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $dataid, ':uniacid' => $_W['uniacid']));

		if (empty($data)) {
			return false;
		}

		$coupon = pdo_fetch('select * from ' . tablename('wx_shop_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
	...
	public function setCoupon($row, $time, $withOpenid = true)
	{
		...
		if ($withOpenid) {
			...
			if (0 < $row['getmax']) {
				$gets = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and gettype=1 limit 1', array(':couponid' => $row['id'], ':openid' => $openid, ':uniacid' => $_W['uniacid']));
	...
	public function addtaskdata($orderid)
	{
		...
		$order = pdo_fetch('select id,openid,price  from ' . tablename('wx_shop_order') . ' where id=:id   and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
		...
		if ($pdata['isopengoodssendtask'] == 1) {
			...
			foreach ($goodssendtasks as $task) {
				...
				pdo_update('wx_shop_coupon_goodsendtask', array('num' => $num), array('id' => $task['taskid']));
			}
		}
	}
	...
	public function sendcouponsbytask($orderid)
	{
		...
		if (!is_array($orderid)) {
			$order = pdo_fetch('select id,openid,ordersn,createtime,status,finishtime,`virtual`,isparent,parentid  from ' . tablename('wx_shop_order') . ' where id=:id and status>=0  and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
		}
	...
	public function posterbylist($list, $openid, $gettype)
	{
		...
		foreach ($list as $taskdata) {
			...
			while ($i <= $couponnum) {
				...
				pdo_insert('wx_shop_coupon_sendshow', $data);
				++$i;
			}

			pdo_update('wx_shop_coupon_taskdata', array('status' => 1), array('id' => $taskdata['id']));
		}
	...
	public function posterbyusesendtask($couponid, $openid)
	{
		...
		foreach ($list as $taskdata) {
			...
			pdo_update('wx_shop_coupon_usesendtasks', array('num' => $num2), array('id' => $taskdata['id']));
		}
	...
	public function getCoupons()
	{
		global $_W;
		$time = time();
		return pdo_fetchall('select * from ' . tablename('wx_shop_coupon') . ' where  (timelimit = 0  or  (timelimit =1 and timestart<={$time} && timeend>={$time})) and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	}
	...
	public function getInfo()
	{
		...
		$osql = 'SELECT * FROM ' . tablename('wx_shop_order') . $condition;
		$order = pdo_fetchall($osql, $paramso);

		if (empty($order)) {
			$sql2 = 'SELECT * FROM ' . tablename('wx_shop_sendticket') . ' WHERE uniacid = ' . intval($_W['uniacid']);
			...
			if ($ticket['status'] == 1) {
				if ($ticket['expiration'] == 1) {
					if ($ticket['endtime'] < TIMESTAMP) {
						...
						pdo_update('wx_shop_sendticket', $status, array('id' => $ticket['id']));
						return false;
					}
	...
	public function getCoupon_new($cpid)
	{
		...
		if (is_array($cpids)) {
			...
			foreach ($cpids as $cpk => $cpv) {
				$cpsql = 'SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($cpv);
		...
		$cpsql = 'SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($cpid);
	...
	public function insertDraw($openid, $cpinfo)
	{
		...
		$drawsql = 'SELECT * FROM ' . tablename('wx_shop_sendticket_draw') . ' WHERE uniacid = :uniacid AND openid = :openid';
		...
		if (empty($drawdata)) {
			...
			$insert = pdo_insert('wx_shop_sendticket_draw', $data);
			return $insert;
		}
	...
	{
		global $_W;
		$sql = 'SELECT * FROM ' . tablename('wx_shop_sendticket_share') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND status = 1 AND (enough = ' . $money . ' OR enough <= ' . $money . ') AND (expiration = 0 OR (expiration = 1 AND endtime >= ' . TIMESTAMP . ')) ORDER BY enough DESC,createtime DESC LIMIT 1';
	...
	public function getCanGetCouponNum($merchid = 0)
	{
		...
		$param = array();
		$param[':uniacid'] = $_W['uniacid'];
		$sql = 'select id,timelimit,coupontype,timedays,timestart,timeend,couponname,enough,backtype,deduct,discount,backmoney,backcredit,backredpack,bgcolor,thumb,credit,money,getmax,merchid,total as t,tagtitle,settitlecolor,titlecolor  from ' . tablename('wx_shop_coupon');
