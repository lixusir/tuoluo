...
class Verify_WxShopComModel extends ComModel
{
	...
	public function allow($orderid, $times = 0, $verifycode = '', $openid = '') 
	{
		...
		if (empty($merchid)) 
		{
			$saler = pdo_fetch('select * from ' . tablename('wx_shop_saler') . ' where openid=:openid and uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		}
		else if ($merch_plugin) 
		{
			$saler = pdo_fetch('select * from ' . tablename('wx_shop_merch_saler') . ' where openid=:openid and uniacid=:uniacid and status=1 and merchid=:merchid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':merchid' => $merchid));
		}
		if (empty($saler)) 
		{
		...
		$allgoods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,o.title as optiontitle,g.isverify,g.storeids,g.status' . $sqlstr . ' from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' left join ' . tablename('wx_shop_goods_option') . ' o on o.id=og.optionid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
		if (empty($allgoods)) 
		{
			return error(-1, '订单异常!');
		}
	...
	public function verify($orderid = 0, $times = 0, $verifycode = '', $openid = '') 
	{
		...
		if ($order['isverify'] || $order['istrade']) 
		{
			if (($order['verifytype'] == 0) || ($order['verifytype'] == 3)) 
			{
				...
				pdo_update('wx_shop_order', $change_data, array('id' => $order['id']));
				if ($newstore_plugin && !(empty($order['istrade']))) 
				{
					...
					pdo_insert('wx_shop_newstore_trade_log', $insert_data);
				}
			...
			else if ($order['verifytype'] == 1) 
			{
				...
				pdo_update('wx_shop_order', array('verifyinfo' => iserializer($verifyinfo)), array('id' => $orderid));
				com_run('printer::sendOrderMessage', $orderid, array('type' => 1, 'times' => $times, 'lastverifys' => $data['lastverifys'] - $times));
				if ($order['status'] != 3) 
				{
					pdo_update('wx_shop_order', array('status' => 3, 'sendtime' => $current_time, 'finishtime' => $current_time), array('id' => $order['id']));
					$this->finish($openid, $order);
					m('order')->setGiveBalance($orderid, 1);
				}
				m('member')->upgradeLevel($order['openid'], $orderid);
			...
			else if ($order['verifytype'] == 2) 
			{
				...
				pdo_update('wx_shop_order', array('verifyinfo' => iserializer($verifyinfo)), array('id' => $order['id']));
				if ($order['status'] != 3) 
				{
					pdo_update('wx_shop_order', array('status' => 3, 'sendtime' => $current_time, 'finishtime' => $current_time, 'verifytime' => $current_time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']), array('id' => $order['id']));
					$this->finish($openid, $order);
					m('order')->setGiveBalance($orderid, 1);
				}
				m('member')->upgradeLevel($order['openid'], $orderid);
		...
		{
			pdo_update('wx_shop_order', array('status' => 3, 'fetchtime' => $current_time, 'sendtime' => $current_time, 'finishtime' => $current_time, 'verifytime' => $current_time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']), array('id' => $order['id']));
			$this->finish($openid, $order);
			m('order')->setGiveBalance($orderid, 1);
			com_run('printer::sendOrderMessage', $orderid, array('type' => 0));
			m('member')->upgradeLevel($order['openid'], $orderid);
	...
	public function getSalerInfo($openid, $merchid = 0) 
	{
		...
		if (empty($merchid)) 
		{
			$table_name = tablename('wx_shop_saler');
		}
		else 
		{
			$table_name = tablename('wx_shop_merch_saler');
			$condition .= ' and s.merchid = :merchid';
			$params['merchid'] = $merchid;
		}
		$sql = 'SELECT m.id as salerid,m.nickname as salernickname,s.salername FROM ' . $table_name . '  s ' . ' left join ' . tablename('wx_shop_member') . ' m on s.openid=m.openid and m.uniacid = s.uniacid ' . ' WHERE ' . $condition . ' Limit 1';
	...
	public function getStoreInfo($storeid, $merchid = 0) 
	{
		...
		if (empty($merchid)) 
		{
			$table_name = tablename('wx_shop_store');
		}
		else 
		{
			$table_name = tablename('wx_shop_merch_store');
