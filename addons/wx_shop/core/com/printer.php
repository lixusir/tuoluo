...
class Printer_WxShopComModel extends ComModel
{
	const PRINTRT_365 = 0;
	...
	public function printer($params, $templateid, $printerid, $type = 0) 
	{
		global $_W;
		$this->type = $type;
		$printer = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_printer') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $printerid));
		if (empty($printer)) 
		{
			return error(-1, '小票打印机未找到');
		}
		$template = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $templateid));
		if (empty($printer)) 
		{
	...
	public function getPrinterSet($merchid = 0) 
	{
		...
		{
			$data['order_printer'] = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_printer') . ' WHERE uniacid=:uniacid AND id IN (' . $data['order_printer'] . ')', array(':uniacid' => $_W['uniacid']));
		}
		else 
		{
	...
	public function getStorePrinterSet($store) 
	{
		...
		{
			$store['order_printer'] = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_printer') . ' WHERE uniacid=:uniacid AND id IN (' . $store['order_printer'] . ')', array(':uniacid' => $_W['uniacid']));
		}
		else 
		{
	...
	public function sendOrderMessage($orderid = 0, $verify = array()) 
	{
		...
		$order = pdo_fetch('select * from ' . tablename('wx_shop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
		if (empty($order)) 
		{
			return;
		}
		...
		$order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.goodssn,og.productsn from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and' . $scondition, $param);
		$goods = array();
		...
		if ($order['isverify'] && $order['verifystoreid'] && !(empty($verify))) 
		{
			...
			{
				$store = pdo_fetch('select * from ' . tablename('wx_shop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $order['verifystoreid'], ':uniacid' => $_W['uniacid']));
				$StorePrinterSet = $this->getStorePrinterSet($store);
			}
		}
	...
	public function sendCashierMessage($log, $templateid, $printerid, $operator) 
	{
		...
		{
			$goodsAll = pdo_fetchall('SELECT g.title,g.goodssn,og.price,og.total FROM ' . tablename('wx_shop_cashier_goods') . ' g LEFT JOIN ' . tablename('wx_shop_cashier_pay_log_goods') . ' og ON g.id=og.goodsid WHERE og.cashierid=:cashierid AND og.logid=:logid', array(':cashierid' => $log['cashierid'], ':logid' => $log['id']));
		...
		{
			$order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $log['uniacid'], ':orderid' => $log['orderid']));
			foreach ($order_goods as $og ) 
			{
