...
class Wxcard_WxShopComModel extends ComModel
{
	...
	public function wxCardUpdateQuantity($card_id) 
	{
		...
		if (!(empty($id))) 
		{
			$sql = 'select id,uniacid, card_id from ' . tablename('wx_shop_wxcard');
		...
		pdo_update('wx_shop_wxcard', $data, array('id' => $id));
		return true;
	}
	...
	public function consumeWxCardCount($openid, $merch_array, $goods_array) 
	{
		...
		$sql = 'select id,uniacid, card_id,least_cost,reduce_cost,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids  from ' . tablename('wx_shop_wxcard');
	...
	public function getAvailableWxcards($type, $money = 0, $merch_array, $goods_array = array()) 
	{
		...
		$sql = 'select id,uniacid,card_type,logo_url,title, card_id,least_cost,reduce_cost,discount,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids,datetype,end_timestamp,fixed_term  from ' . tablename('wx_shop_wxcard');
		$sql .= '  where uniacid=:uniacid and merchid=0 and card_id in (' . $card_id . ')  order by id desc';
		$cardlist = pdo_fetchall($sql, $param);
		if (empty($cardlist)) 
		{
	...
	{
		global $_W;
		$sql = 'select m.*  from ' . tablename('wx_shop_member') . ' m inner join ' . tablename('mc_mapping_fans') . ' f on f.openid=m.openid where m.openid=:openid and f.follow=1 limit 1';
		...
		{
			$level = pdo_fetchcolumn('select levelname from ' . tablename('wx_shop_member_level') . ' where id=:id limit 1', array(':id' => $member['level']));
		}
	...
	public function ActivateVerifygoodCard($id, $card_id, $code, $openid) 
	{
		...
		pdo_update('wx_shop_verifygoods', array('activecard' => 1, 'cardcode' => $code), array('id' => $id));
		return true;
	}
	...
	{
		global $_W;
		$sql = 'select m.*  from ' . tablename('wx_shop_member') . ' m inner join ' . tablename('mc_mapping_fans') . ' f on f.openid=m.openid where m.openid=:openid limit 1';
		$member = pdo_fetch($sql, array(':openid' => $openid));
		if (empty($member)) 
		{
			return false;
	...
	{
		global $_W;
		$sql = 'select vg.*,c.card_id,vg.cardcode  from ' . tablename('wx_shop_verifygoods') . '   vg' . "\r\n\t" . ' inner join ' . tablename('wx_shop_order_goods') . ' og on vg.ordergoodsid = og.id' . "\r\n\t" . ' inner join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n\t" . ' inner  join ' . tablename('wx_shop_goods_cards') . ' c on c.id = g.cardid' . "\r\n\t" . ' where   vg.uniacid=:uniacid  and g.cardid>0  and vg.id=:id ';
	...
	{
		global $_W;
		$sql = 'select g.id  from ' . tablename('wx_shop_goods') . ' g' . "\r\n" . '         inner join  ' . tablename('wx_shop_goods_cards') . '  c on c.id = g.cardid  where g.uniacid=:uniacid and c.card_id=:card_id';
