...
require '../../../../../addons/wx_shop/core/inc/plugin_model.php';
global $_W;
global $_GPC;
ignore_user_abort();
set_time_limit(0);
...
foreach ($sets as $set ) 
{
	...
	$p = p('commission');
	$pcoupon = com('coupon');
	$orders = pdo_fetchall('select id,couponid,openid,isparent,sendtime,price,merchid,isverify,addressid,isvirtualsend,virtual,dispatchtype from ' . tablename('wx_shop_order') . ' where uniacid=' . $_W['uniacid'] . ' and status=2 ', array(), 'id');
	if (!(empty($orders))) 
	{
		foreach ($orders as $orderid => $order ) 
		{
			...
			pdo_query('update ' . tablename('wx_shop_order') . ' set status=3,finishtime=' . time() . ' where id=' . $orderid);
			if ($order['isparent'] == 1) 
			{
				continue;
			}
...
function goodsReceive($order, $sysday = 0) 
{
	...
	if ($order['merchid'] == 0) 
	{
		$goods = pdo_fetchall('select og.goodsid, g.autoreceive from' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.orderid=' . $order['id']);
