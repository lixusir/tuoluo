...
foreach ($sets as $set ) 
{
	...
	$goods = pdo_fetchall('select * from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $_W['uniacid'] . ' and isstatustime > 0 and deleted = 0 and status < 2 ');
	foreach ($goods as $key => $value ) 
	{
		...
		pdo_update('wx_shop_goods', array('status' => $value['status']), array('id' => $value['id']));
	}
}
?>