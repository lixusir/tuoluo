...
foreach ($sets as $set ) 
{
	...
	$daytimes = 86400 * $days;
	$orders = pdo_fetchall('select id,couponid from ' . tablename('wx_shop_order') . ' where  uniacid=' . $_W['uniacid'] . ' and status=3 and isparent=0 and couponid<>0 and finishtime + ' . $daytimes . ' <=unix_timestamp() ');
	if (!(empty($orders))) 
	{
		if ($p) 
		{
