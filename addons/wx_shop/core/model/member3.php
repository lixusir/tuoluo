...
class Member_WxShopModel
{
	...
	public function getCredit($openid = '', $credittype = 'credit1')
	{
		...
		return pdo_fetchcolumn('SELECT ' . $credittype . ' FROM ' . tablename('wx_shop_member') . ' WHERE (openid=:openid or openid_wa=:openid) and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
	}
	...
	public function getCredits($openid = '', $credittypes = array('credit1', 'credit2'))
	{
		...
		return pdo_fetch('SELECT ' . $types . ' FROM ' . tablename('wx_shop_member') . ' WHERE  (openid=:openid or openid_wa=:openid) and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
	}
