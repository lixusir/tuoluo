...
class Account_WxShopModel
{
	...
	public function checkOpenid() 
	{
		...
		if (isset($_GPC[$key])) 
		{
			...
			if (is_array($session)) 
			{
				...
				if (is_array($member) && ($session['wx_shop_member_hash'] == md5($member['pwd'] . $member['salt'])))
				{
					$GLOBALS['_W']['wx_shop_member_hash'] = md5($member['pwd'] . $member['salt']);
					$GLOBALS['_W']['wx_shop_member'] = $member;
	...
	public function setLogin($member) 
	{
		...
		if (!(empty($member))) 
		{
			$member['wx_shop_member_hash'] = md5($member['pwd'] . $member['salt']);
			$key = '__wx_shop_member_session_' . $_W['uniacid'];
