...
class Excel_WxShopModel
{
	...
	public function export($list, $params = array()) 
	{
		...
		$excel = new PHPExcel();
		$excel->getProperties()->setCreator('wx商城')->setLastModifiedBy('wx商城')->setTitle('Office 2007 XLSX Test Document')->setSubject('Office 2007 XLSX Test Document')->setDescription('Test document for Office 2007 XLSX, generated using PHP classes.')->setKeywords('office 2007 openxml php')->setCategory('report file');
	...
	public function import($excefile) 
	{
		...
		$path = IA_ROOT . '/addons/wx_shop/data/tmp/';
		if (!(is_dir($path))) 
		{
			load()->func('file');
			mkdirs($path, '0777');
