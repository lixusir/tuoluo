...
class Common_WxShopModel
{
    ...
    public function updateSysset($values, $uniacid = 0)
    {
        ...
        $setdata = pdo_fetch('select id, sets from ' . tablename('wx_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));

        if (empty($setdata)) {
            pdo_insert('wx_shop_sysset', array('sets' => iserializer($values), 'uniacid' => $uniacid));
        }
        else {
            ...
            pdo_update('wx_shop_sysset', array('sets' => iserializer($sets)), array('id' => $setdata['id']));
        }

        $setdata = pdo_fetch('select * from ' . tablename('wx_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
    ...
    public function updatePluginset($values, $uniacid = 0)
    {
        ...
        $setdata = pdo_fetch('select id, plugins from ' . tablename('wx_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));


        if (empty($setdata)) {
            pdo_insert('wx_shop_sysset', array('plugins' => iserializer($values), 'uniacid' => $uniacid));
        }
        else {
            ...
            pdo_update('wx_shop_sysset', array('plugins' => iserializer($plugins)), array('id' => $setdata['id']));
        }
        $setdata = pdo_fetch('select * from ' . tablename('wx_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        m('cache')->set('sysset', $setdata, $uniacid);
    ...
    public function alipay_build($params, $alipay = array(), $type = 0, $openid = '')
    {
        ...
        if (empty($type)) {
            $set['notify_url'] = $_W['siteroot'] . 'addons/wx_shop/payment/alipay/notify.php';
    ...
    public function publicAliPay($params = array(), $return = NULL)
    {
        ...
        $url = ((WX_SHOP_DEBUG ? 'https://openapi.alipaydev.com/gateway.do' : 'https://openapi.alipay.com/gateway.do'));
    ...
    public function AliPayBarcode($params, $config)
    {
        ...
        $config['notify_url'] = $_W['siteroot'] . 'addons/wx_shop/payment/alipay/notify.php';
    ...
    public function public_build($isapp = false)
    {
        ...
        if (!(empty($set['weixin_id'])) && ($isapp == false)) {
            $payments = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_payment') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $set['weixin_id']));
...
//        $package['openid'] =str_replace('sns_wa_','',$_GPC['openid']);
    ...
    public function wechat_child_build($params, $wechat, $type = 0)
    {
        ...
        $package['nonce_str'] = random(32);
        $package['body'] = $params['title'];
        $package['device_info'] = ((isset($params['device_info']) ? 'wx_shop:' . $params['device_info'] : 'wx_shop'));
        ...
        $package['notify_url'] = ((empty($params['notify_url']) ? $_W['siteroot'] . 'addons/wx_shop/payment/wechat/notify.php' : $params['notify_url']));
        $package['trade_type'] = 'JSAPI';
    ...
    {
        global $_W;
        $payment = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_payment') . ' WHERE uniacid=:uniacid AND `type`=\'0\'', array(':uniacid' => $_W['uniacid']));
    ...
    public function getSec($uniacid = 0)
    {
        ...
        $set = pdo_fetch('select sec from ' . tablename('wx_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
    ...
    public function paylog($log = '')
    {
        ...
        if (!(empty($openpaylog))) {
            $path = IA_ROOT . '/addons/wx_shop/data/paylog/' . $_W['uniacid'] . '/' . date('Ymd');
    ...
    public function getAreas()
    {
        ...
        if (!(empty($new_area))) {
            $file = IA_ROOT . '/addons/wx_shop/static/js/dist/area/AreaNew.xml';
        }
        else {
            $file = IA_ROOT . '/addons/wx_shop/static/js/dist/area/Area.xml';
        }
    ...
    {
        return pdo_fetchall('SELECT  a.uniacid,a.name FROM ' . tablename('wx_shop_sysset') . ' s  ' . ' left join ' . tablename('account_wechats') . ' a on a.uniacid = s.uniacid' . ' left join ' . tablename('account') . ' acc on acc.uniacid = a.uniacid where acc.isdeleted=0 GROUP BY uniacid');
    }
    ...
    public function getCopyright($ismanage = false)
    {
        ...
        if (!(is_array($copyrights))) {
            $copyrights = pdo_fetchall('select *  from ' . tablename('wx_shop_system_copyright'));
        ...
        if (!(empty($merchid))) {
            $merch = pdo_fetch('select logo from ' . tablename('wx_shop_merch_user') . ' where id = :id and uniacid = :uniacid ', array(':id' => $merchid, ':uniacid' => $_W['uniacid']));
