...
class Dispatch_WxShopModel
{
	...
	{
		global $_W;
		$sql = 'select * from ' . tablename('wx_shop_dispatch') . ' where isdefault=1 and uniacid=:uniacid and merchid=:merchid and enabled=1 Limit 1';
	...
	{
		global $_W;
		$sql = 'select * from ' . tablename('wx_shop_dispatch') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by id desc Limit 1';
		$params = array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid);
		$data = pdo_fetch($sql, $params);
		return $data;
	}
	public function getOneDispatch($id) 
	{
		...
		if ($id == 0) 
		{
			$sql = 'select * from ' . tablename('wx_shop_dispatch') . ' where isdefault=1 and uniacid=:uniacid and enabled=1 Limit 1';
		}
		else 
		{
			$sql = 'select * from ' . tablename('wx_shop_dispatch') . ' where id=:id and uniacid=:uniacid and enabled=1 Limit 1';
			$params[':id'] = $id;
		}
		$data = pdo_fetch($sql, $params);
		return $data;
