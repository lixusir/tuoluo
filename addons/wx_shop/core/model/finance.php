...
class Finance_WxShopModel
{
	...
	public function wxapp_refund($openid, $out_trade_no, $out_refund_no, $totalmoney, $refundmoney = 0, $app = false, $refund_account = false) 
	{
		...
		if (is_array($certs)) 
		{
			...
			$certfile = IA_ROOT . '/addons/wx_shop/cert/' . random(64);
			file_put_contents($certfile, $certs['cert']);
			$keyfile = IA_ROOT . '/addons/wx_shop/cert/' . random(64);
			file_put_contents($keyfile, $certs['key']);
			$rootfile = IA_ROOT . '/addons/wx_shop/cert/' . random(64);
			file_put_contents($rootfile, $certs['root']);
			$extras['CURLOPT_SSLCERT'] = $certfile;
			$extras['CURLOPT_SSLKEY'] = $keyfile;
			$extras['CURLOPT_CAINFO'] = $rootfile;
	...
	public function AlipayRefund($params, $batch_no, $config) 
	{
		...
		$parameter = array('service' => 'refund_fastpay_by_platform_pwd', 'partner' => $config['partner'], '_input_charset' => 'UTF-8', 'notify_url' => (isset($params['notify_url']) ? $params['notify_url'] : $_W['siteroot'] . 'addons/wx_shop/payment/alipay/notify.php'), 'seller_user_id' => $config['partner'], 'seller_email' => $config['account'], 'refund_date' => date('Y-m-d H:i:s'), 'batch_no' => $batch_no, 'batch_num' => '1', 'detail_data' => $params['trade_no'] . '^' . $params['refund_price'] . '^' . $params['refund_reason']);
	...
	public function downloadbill($starttime, $endtime, $type = 'ALL', $datatype = 0) 
	{
		...
		foreach ($dates as $date) {
			...
			if ($datatype && !strexists($dc, 'wx_shop')) {
				continue;
			}
