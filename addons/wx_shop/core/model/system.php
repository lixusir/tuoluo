...
class System_WxShopModel
{
	...
	protected function shopMenu()
	{
		$shopmenu = array(
			...
			'game'     => array(
				...
				'icon'     => 'game',
				'items'    => array(
					array('title' => '游戏商品', 'route' => 'list', 'route_in' => true),
					array('title' => '设置', 'route' => 'set', 'route_in' => true),
					array('title' => '大转盘', 'route' => 'dzplist', 'route_in' => true),
					array('title' => '敲蛋', 'route' => 'qdlist', 'route_in' => true),
					array('title' => '敲蛋今日', 'route' => 'qdjinri', 'route_in' => true),
					array('title' => '答题', 'route' => 'dtlist', 'route_in' => true),

					array('title' => '广告等级', 'route' => 'level', 'route_in' => true),
					array('title' => '彩蛋等级', 'route' => 'blevel', 'route_in' => true),
					array('title' => '记录', 'route' => 'gamelog', 'route_in' => true),
					array('title' => '视频记录', 'route' => 'videolog', 'route_in' => true),
					// array('title' => '视频奖励记录', 'route' => 'video_jl', 'route_in' => true),
					array('title' => '昨日神鸟记录', 'route' => 'zrsn', 'route_in' => true),
					array('title' => '神鸟分红记录', 'route' => 'sn_log', 'route_in' => true),
					array('title' => '提现记录', 'route' => 'withdraw', 'route_in' => true),
					array('title' => '红包记录', 'route' => 'credit_red', 'route_in' => true),
					array('title' => '代理记录', 'route' => 'dl_log', 'route_in' => true),
					)
				),
			...
			// 			'title' => '基本功能',
			// 			'items' => array(
			// 				array('title' => '满额立减', 'route' => 'enough', 'desc' => '满额立减设置', 'keywords' => '优惠'),
			// 				array('title' => '满额包邮', 'route' => 'enoughfree', 'desc' => '满额包邮设置', 'keywords' => '优惠'),
			// 				array('title' => '抵扣设置', 'route' => 'deduct', 'desc' => '抵扣设置', 'keywords' => '优惠'),
			// 				array('title' => '充值优惠', 'route' => 'recharge', 'desc' => '充值优惠设置', 'keywords' => '优惠'),
			// 				array('title' => '积分优惠', 'route' => 'credit1', 'desc' => '积分优惠设置', 'keywords' => '优惠'),
			// 				array('title' => '套餐管理', 'route' => 'package', 'keywords' => '优惠'),
			// 				array('title' => '赠品管理', 'route' => 'gift', 'keywords' => '优惠'),
			// 				array('title' => '全返管理', 'route' => 'fullback', 'keywords' => '优惠'),
			// 				array('title' => '找人代付', 'route' => 'peerpay', 'keywords' => '优惠'),
			// 				array('title' => '绑定送积分', 'route' => 'bindmobile', 'keywords' => '优惠')
			// 				)
			// 			),
			// 		array(
			// 			'title' => '优惠券',
	...
	protected function initRightMenu($routes)
	{
		...
		if ($this->merch) {
			...
			if ($routes[0] == 'system') {
				...
				if($_W['isfounder']) $return_arr['menu_items'][] = array('text' => '切换公众号', 'href' => webUrl('sysset/account'), 'icow' => 'icow-qiehuan');
				$return_arr['menu_items'][] = array('text' => '更新缓存', 'href' => './index.php?c=system&a=updatecache', 'icow' => 'icow-qiehuan');
	...
	public function init()
	{
		...
		if (!$this->merch) {
			$arr['notice'] = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_system_copyright_notice') . ' ORDER BY displayorder DESC,createtime DESC LIMIT 5');

			if ($this->cv('commission.apply.view1')) {
				$arr['commission1'] = pdo_fetchcolumn('select count(1) from' . tablename('wx_shop_commission_apply') . ' a ' . ' left join ' . tablename('wx_shop_member') . ' m on m.uid = a.mid' . ' left join ' . tablename('wx_shop_commission_level') . ' l on l.id = m.agentlevel' . ' where a.uniacid=:uniacid and a.status=:status', array(':uniacid' => $_W['uniacid'], ':status' => 1));
			}

			if ($this->cv('commission.apply.view2')) {
				$arr['commission2'] = pdo_fetchcolumn('select count(1) from' . tablename('wx_shop_commission_apply') . ' a ' . ' left join ' . tablename('wx_shop_member') . ' m on m.uid = a.mid' . ' left join ' . tablename('wx_shop_commission_level') . ' l on l.id = m.agentlevel' . ' where a.uniacid=:uniacid and a.status=:status', array(':uniacid' => $_W['uniacid'], ':status' => 2));
			}

			if ($this->cv('shop.comment')) {
				$arr['comment'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order_comment') . ' WHERE (`checked`=1 OR replychecked=1) AND deleted=0 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
			}
		}
		else {
			$arr['notice'] = 'none';
			...
			if ($this->cv('shop.comment')) {
				$arr['comment'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order_comment') . 'c LEFT JOIN ' . tablename('wx_shop_goods') . ' g ON g.id=c.goodsid WHERE (c.checked=1 OR c.replychecked=1) AND c.deleted=0 AND c.uniacid=:uniacid AND g.merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			}
		}

		if (!empty($arr['funbar']['open'])) {
			$funbardata = pdo_fetch('select * from ' . tablename('wx_shop_funbar') . ' where uid=:uid and uniacid=:uniacid limit 1', array(':uid' => $_W['uid'], ':uniacid' => $_W['uniacid']));
	...
	protected function getOrderTotal($status = 0)
	{
		...
		if ($status == 1) {
			...
			$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . ' WHERE ' . $condition, $params);
		}
		else {
			if ($status == 4) {
	...
	public function set_version($type = 0)
	{
		...
		if (!empty($closev2)) {
			...
			$set = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_version') . ' WHERE uid=:uid AND `type`=:type limit 1', array(':type' => $type, ':uid' => $uid));
