...
class Cache_WxShopModel
{
	public function get_key($key = '', $uniacid = '') 
	{
		global $_W;
        if(empty($_W['setting']['site']['key'])) $_W['setting']['site']= $_W['config']['setting']['site'];
		static $APPID;
		static $_uniacid;
		$account_key = '';
		...
		if (function_exists('redis')) 
		{
			...
			if (!(is_error($redis))) 
			{
				...
				{
					return 'wx_shop_syscache_' . $_W['setting']['site']['key'] . '_global_' . $key;
				}
				...
				return 'wx_shop_syscache_' . $_W['setting']['site']['key'] . '_' . $uniacid . '_' . $account_key . '_' . $key;
			}
		}
		return WX_SHOP_PREFIX . md5($uniacid . '_new_' . $key);
	}
