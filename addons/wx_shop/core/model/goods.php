...
class Goods_WxShopModel
{
	...
	{
		global $_W;
		return pdo_fetch('select * from ' . tablename('wx_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid Limit 1', array(':id' => $optionid, ':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid));
	}
	...
	{
		global $_W;
		return pdo_fetchcolumn('select marketprice from ' . tablename('wx_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid', array(':id' => $optionid, ':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid));
	}
	...
	public function getList($args = array())
	{
		...
		foreach ($list as $lk => $lv ) {
			...
			if ($lv['type'] == 3) {
				$vsql = 'SELECT * FROM ' . tablename('wx_shop_virtual_type') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($lv['virtual']);
	...
	public function getListbyCoupon($args = array())
	{
		...
		$table = tablename('wx_shop_goods') . ' g';
		$distinct = '';

		if (0 < $couponid) {
			$data = pdo_fetch('select c.*  from ' . tablename('wx_shop_coupon_data') . '  cd inner join  ' . tablename('wx_shop_coupon') . ' c on cd.couponid = c.id  where cd.id=:id and cd.uniacid=:uniacid and coupontype =0  limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
				...
				if (($data['limitgoodcatetype'] == 1) && !(empty($data['limitgoodcateids']))) {
					...
					if (0 < count($limitcateids)) {
						...
						foreach ($limitcateids as $cateid ) {
							...
							$table .= 'select * from ' . tablename('wx_shop_goods') . ' where FIND_IN_SET(' . $cateid . ',cates)';
						}
	...
	{
		global $_W;
		return array('sale' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where status > 0 and checked=0 and deleted=0 and total>0 and  type !=30 and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'])), 'out' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where status > 0 and deleted=0 and total=0 and type !=30 and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'])), 'stock' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where (status=0 or checked=1) and deleted=0 and uniacid=:uniacid and type !=30', array(':uniacid' => $_W['uniacid'])), 'cycle' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where deleted=1 and uniacid=:uniacid and type !=30', array(':uniacid' => $_W['uniacid'])), 'verify' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where deleted=0  and uniacid=:uniacid and type !=30 and merchid>0 and checked=1', array(':uniacid' => $_W['uniacid'])));
	}
	...
	public function getComments($goodsid = '0', $args = array())
	{
		...
		$sql = 'SELECT id,nickname,headimgurl,content,images FROM ' . tablename('wx_shop_goods_comment') . ' where 1 ' . $condition . ' ORDER BY createtime desc LIMIT ' . (($page - 1) * $pagesize) . ',' . $pagesize;
		$list = pdo_fetchall($sql, $params);
	...
	{
		global $_W;
		$count = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member_favorite') . ' where goodsid=:goodsid and deleted=0 and openid=:openid and uniacid=:uniacid limit 1', array(':goodsid' => $id, ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		return 0 < $count;
	}
	...
	{
		global $_W;
		pdo_query('update ' . tablename('wx_shop_goods') . ' set viewcount=viewcount+1 where id=:id and uniacid=\'' . $_W[uniacid] . '\' ', array(':id' => $goodsid));
		$history = pdo_fetch('select id,times from ' . tablename('wx_shop_member_history') . ' where goodsid=:goodsid and uniacid=:uniacid and openid=:openid limit 1', array(':goodsid' => $goodsid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));

		if (empty($history)) {
			$history = array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'goodsid' => $goodsid, 'deleted' => 0, 'createtime' => time(), 'times' => 1);
			pdo_insert('wx_shop_member_history', $history);
		}
		 else {
			pdo_update('wx_shop_member_history', array('deleted' => 0, 'times' => $history['times'] + 1), array('id' => $history['id']));
		}
	}
	...
	public function getCartCount($isnewstore = 0)
	{
		...
		$count = pdo_fetchcolumn('select sum(total) from ' . tablename('wx_shop_member_cart') . ' where uniacid=:uniacid and openid=:openid ' . $sqlcondition . ' and deleted=0 limit 1', $paras);
		return $count;
	}
	...
	public function getSpecThumb($specs)
	{
		...
		if (!(empty($specid))) {
			$spec = pdo_fetch('select thumb from ' . tablename('wx_shop_goods_spec_item') . ' ' . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $specid, ':uniacid' => $_W['uniacid']));
	...
	public function getOptions($goods)
	{
		...
		if (!(empty($goods)) && $goods['hasoption']) {
			...
			foreach ($specs as &$spec ) {
				$spec['items'] = pdo_fetchall('select * from ' . tablename('wx_shop_goods_spec_item') . ' where specid=:specid order by displayorder asc', array(':specid' => $spec['id']));
			}

			unset($spec);
			$options = pdo_fetchall('select * from ' . tablename('wx_shop_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
		}
	...
	public function canBuyAgain($goods)
	{
		...
		$order_goods = pdo_fetchall('SELECT id,orderid FROM ' . tablename('wx_shop_order_goods') . ' WHERE uniacid=:uniaicd AND openid=:openid AND goodsid=:goodsid ' . $condition, array(':uniaicd' => $_W['uniacid'], ':openid' => $_W['openid'], ':goodsid' => $id), 'orderid');
		...
		$order = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_order') . ' WHERE uniacid=:uniaicd AND status>=:status AND id IN (' . implode(',', array_keys($order_goods)) . ')', array(':uniaicd' => $_W['uniacid'], ':status' => (empty($goods['buyagain_condition']) ? '1' : '3')));
		return !(empty($order));
	}
	...
	{
		global $_W;
		$order_goods = pdo_fetchall('SELECT id,goodsid FROM ' . tablename('wx_shop_order_goods') . ' WHERE uniacid=:uniaicd AND openid=:openid AND canbuyagain = 1 AND orderid <> :orderid', array(':uniaicd' => $_W['uniacid'], ':openid' => $_W['openid'], 'orderid' => $orderid), 'goodsid');
		...
		pdo_query('UPDATE ' . tablename('wx_shop_order_goods') . ' SET `canbuyagain`=\'0\' WHERE uniacid=:uniacid AND goodsid IN (' . implode(',', array_keys($order_goods)) . ')', array(':uniacid' => $_W['uniacid']));
	}
	...
	{
		global $_W;
		$path = IA_ROOT . '/addons/wx_shop/data/goodscode/' . $_W['uniacid'] . '/';
		...
		$img = $_W['siteroot'] . 'addons/wx_shop/data/goodscode/' . $_W['uniacid'] . '/' . $file;
		return $img;
	}
