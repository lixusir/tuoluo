...
class Order_WxShopModel
{
	public function fullback($orderid) 
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$order_goods = pdo_fetchall('select o.openid,og.optionid,og.goodsid,og.price,og.total from ' . tablename('wx_shop_order_goods') . ' as og' . "\r\n" . '                    left join ' . tablename('wx_shop_order') . ' as o on og.orderid = o.id' . "\r\n" . '                    where og.uniacid = ' . $uniacid . ' and og.orderid = ' . $orderid . ' ');
		foreach ($order_goods as $key => $value ) 
		{
			if (0 < $value['optionid']) 
			{
				$goods = pdo_fetch('select g.hasoption,g.id,go.goodsid,go.isfullback from ' . tablename('wx_shop_goods') . ' as g' . "\r\n" . '                left join ' . tablename('wx_shop_goods_option') . ' as go on go.goodsid = :id and go.id = ' . $value['optionid'] . "\r\n" . '                 where g.id=:id and g.uniacid=:uniacid limit 1', array(':id' => $value['goodsid'], ':uniacid' => $uniacid));
			}
			else 
			{
				$goods = pdo_fetch('select * from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $value['goodsid'], ':uniacid' => $uniacid));
			}
			if (0 < $goods['isfullback']) 
			{
				$fullbackgoods = pdo_fetch('SELECT id,minallfullbackallprice,maxallfullbackallprice,minallfullbackallratio,maxallfullbackallratio,`day`,' . "\r\n" . '                          fullbackprice,fullbackratio,status,hasoption,marketprice,`type`,startday' . "\r\n" . '                          FROM ' . tablename('wx_shop_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $value['goodsid'] . ' limit 1');
				if (!(empty($fullbackgoods)) && $goods['hasoption'] && (0 < $value['optionid'])) 
				{
					$option = pdo_fetch('select id,title,allfullbackprice,allfullbackratio,fullbackprice,fullbackratio,`day` from ' . tablename('wx_shop_goods_option') . ' ' . "\r\n" . '                        where id=:id and goodsid=:goodsid and uniacid=:uniacid and isfullback = 1 limit 1', array(':uniacid' => $uniacid, ':goodsid' => $value['goodsid'], ':id' => $value['optionid']));
					if (!(empty($option))) 
					{
				...
				if (!(empty($fullbackgoods))) 
				{
					...
					pdo_insert('wx_shop_fullback_log', $data);
				}
			}
		}
	}
	public function fullbackstop($orderid) 
	{
		...
		$uniacid = $_W['uniacid'];
		$shopset = $_S['shop'];
		$fullback_log = pdo_fetch('select * from ' . tablename('wx_shop_fullback_log') . ' where uniacid = ' . $uniacid . ' and orderid = ' . $orderid . ' ');
		pdo_update('wx_shop_fullback_log', array('isfullback' => 1), array('id' => $fullback_log['id'], 'uniacid' => $uniacid));
	}
	public function payResult($params) 
	{
		...
		$order = pdo_fetch('select id,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus from ' . tablename('wx_shop_order') . ' where  ordersn=:ordersn and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':ordersn' => $ordersn));
		if (1 <= $order['status']) 
		{
			return true;
		}
		...
		if (!(empty($ispeerpay))) 
		{
			...
			pdo_update('wx_shop_order', array('status' => 0), array('id' => $order['id']));
			$order['status'] = 0;
			pdo_update('wx_shop_order_peerpay', array('status' => 1), array('id' => $ispeerpay['id']));
			$params['type'] = 'peerpay';
		}
		if ($params['from'] == 'return') 
		{
			...
			{
				$address = pdo_fetch('select realname,mobile,address from ' . tablename('wx_shop_member_address') . ' where id=:id limit 1', array(':id' => $order['addressid']));
			}
			$carrier = false;
			...
			if ($params['type'] == 'cash') 
			{
				if ($order['isparent'] == 1) 
				{
					...
					pdo_update('wx_shop_order', $change_data, array('id' => $orderid));
			...
			if ($order['istrade'] == 0) 
			{
				if ($order['status'] == 0) 
				{
					...
					if ($order['isparent'] == 1) 
					{
						$merchSql = 'SELECT id,merchid FROM ' . tablename('wx_shop_order') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND parentid = ' . intval($order['id']);
					...
					if (p('task')) 
					{
						...
						$goodslist = pdo_fetchall('SELECT goodsid FROM ' . tablename('wx_shop_order_goods') . ' WHERE orderid = :orderid AND uniacid = :uniacid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
					...
					if (p('task')) 
					{
						...
						if (!(pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where openid = "'.$_W['openid'].'" and uniacid = ' . $_W['uniacid']))) 
						{
	...
	public function setChildOrderPayResult($order, $time, $type) 
	{
		...
		if (!(empty($list))) 
		{
			...
			foreach ($list as $k => $v ) 
			{
				if ($v['status'] == 0) 
				{
					pdo_update('wx_shop_order', $change_data, array('id' => $v['id']));
				}
			}
		}
	}
	public function setOrderPayType($orderid, $paytype, $ordersn = '') 
	{
		...
		if (!(empty($orderid))) 
		{
			pdo_update('wx_shop_order', array('paytype' => $paytype), array('parentid' => $orderid));
		}
	}
	public function getChildOrder($orderid) 
	{
		global $_W;
		$list = pdo_fetchall('select id,ordersn,status,finishtime,couponid,merchid  from ' . tablename('wx_shop_order') . ' where  parentid=:parentid and uniacid=:uniacid', array(':parentid' => $orderid, ':uniacid' => $_W['uniacid']));
		return $list;
	}
	public function payVirtualSend($orderid = 0) 
	{
		global $_W;
		global $_GPC;
		$order = pdo_fetch('select id,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid from ' . tablename('wx_shop_order') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
		$order_goods = pdo_fetch('select g.virtualsend,g.virtualsendcontent from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$time = time();
		pdo_update('wx_shop_order', array('virtualsend_info' => $order_goods['virtualsendcontent'], 'status' => '3', 'paytime' => $time, 'sendtime' => $time, 'finishtime' => $time), array('id' => $orderid));
	...
	{
		global $_W;
		$order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status from ' . tablename('wx_shop_order') . ' where id=:id limit 1', array(':id' => $orderid));
		$goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.money,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$balance = 0;
		foreach ($goods as $g ) 
		{
	...
	{
		global $_W;
		$order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status,isparent,paytype,isnewstore,storeid,istrade,status from ' . tablename('wx_shop_order') . ' where id=:id limit 1', array(':id' => $orderid));
		...
		$goods = pdo_fetchall('select og.goodsid,og.total,o.price,g.totalcnf,og.realprice,g.credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.type from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' left join '.tablename('wx_shop_order').' o on o.id=og.orderid where ' . $condition . ' and og.uniacid=:uniacid ', $param);
		$credits = 0;
		foreach ($goods as $g ) 
		{
			if (0 < $newstoreid) 
			...
			{
				$goods_item = pdo_fetch('select total as goodstotal from' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $g['goodsid'], ':uniacid' => $_W['uniacid']));
			...
			if (!(empty($stocktype))) 
			{
				...
				if (!(empty($g['optionid']))) 
				{
					...
					if (!(empty($option)) && ($option['stock'] != -1)) 
					{
						...
						if ($stock != -1) 
						{
							if (0 < $newstoreid) 
							{
								pdo_update('wx_shop_newstore_goods_option', array('stock' => $stock), array('uniacid' => $_W['uniacid'], 'goodsid' => $g['goodsid'], 'id' => $store_goods_option['id']));
							}
							else 
							{
								pdo_update('wx_shop_goods_option', array('stock' => $stock), array('uniacid' => $_W['uniacid'], 'goodsid' => $g['goodsid'], 'id' => $g['optionid']));
							}
						}
					}
				}
				if (!(empty($g['goodstotal'])) && ($g['goodstotal'] != -1)) 
				{
					...
					if ($totalstock != -1) 
					{
						if (0 < $newstoreid) 
						{
							pdo_update('wx_shop_newstore_goods', array('stotal' => $totalstock), array('uniacid' => $_W['uniacid'], 'id' => $store_goods['id']));
						}
						else 
						{
							pdo_update('wx_shop_goods', array('total' => $totalstock), array('uniacid' => $_W['uniacid'], 'id' => $g['goodsid']));
						}
					}
				}
			}
			...
			if (!(empty($isgoodspoint['isgoodspoint'])) && ($isgoodspoint['isgoodspoint'] == 1)) 
			{
				...
				//获取哪些支付方式对此功能生效
				$paytypeArr = $isgoodspoint['paytype'];
				$paytypeStr = ',';
				foreach ((array)$paytypeArr as $key => $value) {
					$paytypeStr .= $key.',';
				}
				if(strpos($paytypeStr,','.$order['paytype'].',') !== false){ 
					//按营销中的购物赠积分计算
					$goodpoint = $isgoodspoint['enough1'];
					$totalPrice = $g['price'] * $g['total'];
					foreach ($goodpoint as $k=>$v){
				        $temp[]=$v['enough1_2'];
				    }
				    $maxPrice = max($temp);
					foreach ($goodpoint as $key => $value) {

						if ($totalPrice > $value['enough1_1'] && $total <= $value['enough1_2']) {
							$credits = intval($totalPrice * $value['give1']);
						}
			...
			else if ($type == 1) 
			{
				if (1 <= $order['status']) 
				{
					$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':goodsid' => $g['goodsid'], ':uniacid' => $_W['uniacid']));
					pdo_update('wx_shop_goods', array('salesreal' => $salesreal), array('id' => $g['goodsid']));
				}
			}
		}
		...
		if (0 < $credits) 
		{
			...
			{
				//确认收货加积分（新增类别，之前没有确认收货后的积分处理 2018-08-06
				if ($credits > 0 && $order['status'] == 3) {
					m('member')->setCredit($order['openid'],'credit1',$credits,array(0,$shopset['name']. '购买成功获得积分 订单号：' .$order['ordersn']));
					m('notice')->sendMemberPointChange($order['openid'], $credits, 0);
				}
			}
		}
	...
	public function getTotals($merch = 0) 
	{
		...
		$totals['all'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and deleted=0', $paras);
		$totals['status_1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and status=-1 and refundtime=0 and deleted=0', $paras);
		$totals['status0'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and status=0 and paytype<>3 and deleted=0', $paras);
		$totals['status1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and ( status=1 or ( status=0 and paytype=3) ) and deleted=0', $paras);
		$totals['status2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and ( status=2 or (status = 1 and sendtype > 0) ) and deleted=0', $paras);
		$totals['status3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and status=3 and deleted=0', $paras);
		$totals['status4'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and refundstate>0 and refundid<>0 and deleted=0', $paras);
		$totals['status5'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and refundtime<>0 and deleted=0', $paras);
		return $totals;
	}
	...
	public function getGoodsDiscountPrice($g, $level, $type = 0) 
	{
		...
		if (!(empty($level['id']))) 
		{
			$level = pdo_fetch('select * from ' . tablename('wx_shop_member_level') . ' where id=:id and uniacid=:uniacid and enabled=1 limit 1', array(':id' => $level['id'], ':uniacid' => $_W['uniacid']));
	...
	public function getOrderDispatchPrice($goods, $member, $address, $saleset = false, $merch_array, $t, $loop = 0) 
	{
		...
		foreach ($goods as $g ) 
		{
			...
			}else if ($g['dispatchtype'] == 0){// 模板运费
				if (empty($g['dispatchid'])){//选择的运费模板
				...
				if (!(empty($dispatch_data))){
					...
					if (!(empty($user_city))){
						if (empty($isdispatcharea)){
							...
							if (!(empty($citys))) 
							{
								if (in_array($user_city_code, $citys)) //判断是否为不配送区域
								{
									$isnoarea = 1;
								}
							}
					...
					//  判断是否包邮 && 是否为不配送区域
					if (!$sendfree  && ($isnodispatch == 0)){//xiaojie 修改 2018/07/12 去掉多餘的條件
		...
		if (!(empty($dispatch_array))) 
		{
			...
			// if (!(empty($dispatch_info))) //运费信息 xiaojie 2018/07/13 运费受到影响，暂时注释掉
			// {
	...
	public function changeParentOrderPrice($parent_order) 
	{
		global $_W;
		$id = $parent_order['id'];
		$item = pdo_fetch('SELECT price,ordersn2,dispatchprice,changedispatchprice FROM ' . tablename('wx_shop_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (!(empty($item))) 
		{
			$orderupdate = array();
			...
			{
				pdo_update('wx_shop_order', $orderupdate, array('id' => $id, 'uniacid' => $_W['uniacid']));
			}
		}
	}
	public function getOrderCommission($orderid, $agentid = 0) 
	{
		...
		if (empty($agentid)) 
		{
			$item = pdo_fetch('select agentid from ' . tablename('wx_shop_order') . ' where id=:id and uniacid=:uniacid Limit 1', array('id' => $orderid, ':uniacid' => $_W['uniacid']));
			if (!(empty($item))) 
			{
	...
	public function checkOrderGoods($orderid) 
	{
		...
		$item = pdo_fetch('select * from ' . tablename('wx_shop_order') . '  where  id = :id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		...
		{
			$sql = 'select g.id,g.title,ng.gstatus,g.deleted' . ' from ' . tablename('wx_shop_order_goods') . ' og left join  ' . tablename('wx_shop_goods') . ' g  on g.id=og.goodsid and g.uniacid=og.uniacid' . ' inner join ' . tablename('wx_shop_newstore_goods') . ' ng on ng.goodsid = g.id AND ng.storeid=' . $item['storeid'] . ' where og.orderid=:orderid and og.uniacid=:uniacid';
	...
	{
		global $_W;
		$sql = 'SELECT p.*,o.openid FROM ' . tablename('wx_shop_order_peerpay') . ' AS p JOIN ' . tablename('wx_shop_order') . ' AS o ON p.orderid = o.id WHERE p.orderid = :orderid AND p.uniacid = :uniacid AND (p.status = 0 OR p.status=1) AND o.status >= 0 LIMIT 1';
	...
	public function peerStatus($param) 
	{
		...
		if (!(empty($param['tid']))) 
		{
			$sql = 'SELECT id FROM ' . tablename('wx_shop_order_peerpay_payinfo') . ' WHERE tid = :tid';
		...
		return pdo_insert('wx_shop_order_peerpay_payinfo', $param);
	}
	public function getVerifyCardNumByOrderid($orderid) 
	{
		global $_W;
		$num = pdo_fetchcolumn('select SUM(og.total)  from ' . tablename('wx_shop_order_goods') . ' og' . "\r\n\t\t" . ' inner join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n\t\t" . ' where og.uniacid=:uniacid  and og.orderid =:orderid and g.cardid>0', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		return $num;
	}
	public function checkisonlyverifygoods($orderid) 
	{
		global $_W;
		$num = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('wx_shop_order_goods') . ' og' . "\r\n\t\t" . ' inner join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n\t\t" . ' where og.uniacid=:uniacid  and og.orderid =:orderid and g.type<>5', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$num = intval($num);
		if (0 < $num) 
		{
			return false;
		}
		$num2 = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('wx_shop_order_goods') . ' og' . "\r\n" . '             inner join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n" . '             where og.uniacid=:uniacid  and og.orderid =:orderid and g.type=5', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$num2 = intval($num2);
		if (0 < $num2) 
		{
			return true;
	...
	{
		global $_W;
		$num = pdo_fetchcolumn('select COUNT(1)  from ' . tablename('wx_shop_verifygoods_log') . ' vl' . "\r\n\t\t" . ' inner join ' . tablename('wx_shop_verifygoods') . ' v on vl.verifygoodsid = v.id' . "\r\n\t\t" . ' where v.uniacid=:uniacid  and v.orderid =:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
		$num = intval($num);
		if (0 < $num) 
		{
			return true;
	...
	public function getOrderVirtual($order = array()) 
	{
		...
		$ordervirtual = array();
		$virtual_type = pdo_fetch('select fields from ' . tablename('wx_shop_virtual_type') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1 ', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
