...
class Bind_WxShopModel
{
	...
	public function update($mid = 0, $arr = array()) 
	{
		...
		pdo_update('wx_shop_member', $arr, array('id' => $mid, 'uniacid' => $_W['uniacid']));
	}
	public function merge($a = array(), $b = array()) 
	{
		...
		{
			pdo_update('wx_shop_member', $arr, array('id' => $b['id']));
		}
		pdo_update('wx_shop_commission_apply', array('mid' => $b['id']), array('uniacid' => $_W['uniacid'], 'mid' => $a['id']));
		pdo_update('wx_shop_order', array('agentid' => $b['id']), array('agentid' => $a['id']));
		pdo_update('wx_shop_member', array('agentid' => $b['id']), array('agentid' => $a['id']));
		...
		pdo_delete('wx_shop_member', array('id' => $a['id'], 'uniacid' => $_W['uniacid']));
		...
		pdo_insert('wx_shop_member_mergelog', array('uniacid' => $_W['uniacid'], 'mergetime' => time(), 'openid_a' => $a['openid'], 'openid_b' => $b['openid'], 'mid_a' => $a['id'], 'mid_b' => $b['id'], 'detail_a' => iserializer($a), 'detail_b' => iserializer($b), 'detail_c' => iserializer($c)));
		return error(1);
	}
