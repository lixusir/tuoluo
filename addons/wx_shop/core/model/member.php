...
class Member_WxShopModel
{
	public function getInfo($openid = '') 
	{
		...
		{
			$info = pdo_fetch('select * from ' . tablename('wx_shop_member') . ' where id=:id  and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
		}
		if (!(empty($info['uid']))) 
		{
			load()->model('mc');
	...
	public function getmylower($agentid = '', &$arr,$i=0){
	    ...
	    $sql = "SELECT id,agentid,nickname,avatar,agentlevel,openid FROM " . tablename('wx_shop_member') . " where agentid = :agentid AND uniacid = :uniacid ";

	    $getmyl = pdo_fetchall($sql,array(':agentid' => $agentid,':uniacid'=>$_W['uniacid']));
	    $i++;
	...
	public function getMember($openid = '') 
	{
		...
		if(!empty($info)) {
			$this->getmylower($info['id'],$arrs,1);

			$cnb = count($arrs);
			unset($arrs);


			$info['gg_levels'] = pdo_fetch('select * from ' . tablename('wx_shop_game_level') . ' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$info['gg_level']));

			$info['c_levels'] = pdo_fetch('select * from ' . tablename('wx_shop_game_blevel') . ' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$info['c_level']));

			if($cnb && $cnb != $info['xia_num']) {
				pdo_update('wx_shop_member', array('xia_num'=>$cnb), array('id' => $info['id']));

			}

			if(empty($info['token'])) {
				$info['token'] = md5(serialize($info['openid']).time());
				pdo_update('wx_shop_member', array('token'=>$info['token']), array('id' => $info['id']));
		...
		if (empty($info['uid'])) 
		{
			...
			if ($followed) 
			{
				...
				if (!(empty($uid))) 
				{
					...
					if (!(empty($upgrade))) 
					{
						pdo_update('wx_shop_member', $upgrade, array('id' => $info['id']));
					}
				}
			}
		}
	...
	{
		global $_W;
		$info = pdo_fetch('select * from ' . tablename('wx_shop_member') . ' where mobile=:mobile and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mobile' => $mobile));
		if (!(empty($info))) 
		{
			$info = $this->updateCredits($info);
		}
	...
	public function setCredit($openid = '', $credittype = 'credit1', $credits = 0, $log = array()) 
	{
		...
		$log_data = array('uid' => intval($uid), 'credittype' => $credittype, 'uniacid' => $_W['uniacid'], 'num' => $credits, 'createtime' => TIMESTAMP, 'module' => 'wx_shop', 'operator' => intval($log[0]), 'remark' => $log[1]);
		// if (!(empty($uid))) 
		// {
		...
		// else 
		// {
			$value = pdo_fetchcolumn('SELECT ' . $credittype . ' FROM ' . tablename('wx_shop_member') . ' WHERE  uniacid=:uniacid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
			$newcredit = $credits + $value;
			if ($newcredit <= 0) 
			{
				$newcredit = 0;
			}
			pdo_update('wx_shop_member', array($credittype => $newcredit), array('uniacid' => $_W['uniacid'], 'openid' => $openid));
	...
	public function getCredit($openid = '', $credittype = 'credit1') 
	{
		...
		$item = pdo_fetch('SELECT ' . $credittype . ' FROM ' . tablename('wx_shop_member') . ' WHERE openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		if (empty($item)) 
		{
			$item = pdo_fetch('SELECT ' . $credittype . ' FROM ' . tablename('wx_shop_member') . ' WHERE openid_wa=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		}
	...
	public function getCredits($openid = '', $credittypes = array('credit1', 'credit2')) 
	{
		...
		$item = pdo_fetch('SELECT ' . $types . ' FROM ' . tablename('wx_shop_member') . ' WHERE openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		if (empty($item)) 
		{
			$item = pdo_fetch('SELECT ' . $types . ' FROM ' . tablename('wx_shop_member') . ' WHERE openid_wa=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
		}
		if (empty($item)) 
		{
	...
	public function checkMember() 
	{
		...
		if (empty($member) && !(empty($openid))) 
		{
            ...
            $member['nickname'] =filter_emoji($member['nickname']);
		...
		else 
		{
			...
			if (!(empty($upgrade))) 
			{
				pdo_update('wx_shop_member', $upgrade, array('id' => $member['id']));
			}
		}
		if (p('commission')) 
		{
	...
	public function getLevels($all = true) 
	{
		...
		return pdo_fetchall('select * from ' . tablename('wx_shop_member_level') . ' where uniacid=:uniacid' . $condition . ' order by level asc', array(':uniacid' => $_W['uniacid']));
	}
	public function getLevel($openid) 
	{
		...
		{
			$level = pdo_fetch('select * from ' . tablename('wx_shop_member_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $member['level'], ':uniacid' => $_W['uniacid']));
			if (!(empty($level))) 
			{
				return $level;
			}
	...
	public function getOneGoodsLevel($openid, $goodsid) 
	{
		...
		$data = array();
		$levels = pdo_fetchall('select * from ' . tablename('wx_shop_member_level') . ' where uniacid=:uniacid and buygoods=1 and level and level > :level order by level asc', array(':uniacid' => $uniacid, ':level' => $level));
		if (!(empty($levels))) 
		{
	...
	public function getGoodsLevel($openid, $orderid) 
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$order_goods = pdo_fetchall('select goodsid from ' . tablename('wx_shop_order_goods') . ' where orderid=:orderid and uniacid=:uniacid', array(':uniacid' => $uniacid, ':orderid' => $orderid));
		$levels = array();
		$data = array();
	...
	public function upgradeLevel($openid, $orderid = 0) 
	{
		...
		if (empty($leveltype)) 
		{
			$ordermoney = pdo_fetchcolumn('select ifnull( sum(og.realprice),0) from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_order') . ' o on o.id=og.orderid ' . ' where o.openid=:openid and o.status=3 and o.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
			$level = pdo_fetch('select * from ' . tablename('wx_shop_member_level') . ' where uniacid=:uniacid  and enabled=1 and ' . $ordermoney . ' >= ordermoney and ordermoney>0  order by level desc limit 1', array(':uniacid' => $_W['uniacid']));
		}
		else if ($leveltype == 1) 
		{
			$ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where openid=:openid and status=3 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
			$level = pdo_fetch('select * from ' . tablename('wx_shop_member_level') . ' where uniacid=:uniacid and enabled=1 and ' . $ordercount . ' >= ordercount and ordercount>0  order by level desc limit 1', array(':uniacid' => $_W['uniacid']));
		}
		if (!(empty($orderid))) 
		{
		...
		if ($canupgrade) 
		{
			pdo_update('wx_shop_member', array('level' => $level['id']), array('id' => $member['id']));
			com_run('wxcard::updateMemberCardByOpenid', $openid);
	...
	public function upgradeLevelByLevelId($openid, $LevelID) 
	{
		...
		$level = pdo_fetch('select *  from ' . tablename('wx_shop_member_level') . ' where uniacid=:uniacid and enabled=1 and id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $LevelID));
		if (empty($level)) 
		{
			return;
		}
	...
	{
		global $_W;
		return pdo_fetchall('select * from ' . tablename('wx_shop_member_group') . ' where uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
	}
	...
	public function setGroups($openid, $group_ids, $reason = '') 
	{
		...
		$old_group_ids = pdo_getcolumn('wx_shop_member', $condition, 'groupid');
		...
		pdo_update('wx_shop_member', array('groupid' => $group_ids), $condition);
		foreach ($diff_ids as $groupid ) 
		{
			pdo_insert('wx_shop_member_group_log', array('add_time' => date('Y-m-d H:i:s'), 'group_id' => $groupid, 'content' => $reason, 'mid' => intval($openid), 'openid' => ($is_id ? '' : $openid)));
		}
		return true;
	}
	...
	public function checkMemberSNS($sns) 
	{
		...
		{
			pdo_update('wx_shop_member', $data, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			return $member['id'];
		}
	}
