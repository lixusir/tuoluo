...
class Notice_WxShopModel
{
	...
	public function sendTeamMessage($orderid = '0', $delRefund = false) 
	{
		...
		$order = pdo_fetch('select * from ' . tablename('wx_shop_groups_order') . ' where id=:id limit 1', array(':id' => $orderid));
		if (empty($order)) 
		{
			return;
		}
	...
	{
		global $_W;
		$order = pdo_fetch('select * from ' . tablename('wx_shop_order') . ' where id=:id limit 1', array(':id' => $orderid));
		if (empty($order)) 
		{
			return;
		}
		...
		$order_goods = pdo_fetchall('select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where ' . $scondition . ' and og.uniacid=:uniacid ', $param);
		$goods = '';
		$goodsname = '';
		$goodsnum = 0;
		...
		if (!(empty($order['storeid']))) 
		{
			if (0 < $order['merchid']) 
			{
				$store = pdo_fetch('select * from ' . tablename('wx_shop_merch_store') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $order['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
			}
			else 
			{
				$store = pdo_fetch('select * from ' . tablename('wx_shop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $order['storeid'], ':uniacid' => $_W['uniacid']));
			}
		}
		$buyerinfo = '';
		$buyerinfo_name = '';
		...
		if (!(empty($order['address']))) 
		{
			...
			if ((is_array($address) && empty($address)) || !(is_array($address))) 
			{
				...
				if ((is_array($address) && empty($address)) || !(is_array($address))) 
				{
					$address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('wx_shop_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $order['addressid'], ':uniacid' => $_W['uniacid']));
				}
			}
			if (!(empty($address))) 
			{
		...
		if ($order['status'] == 0) 
		{
			...
			pdo_update('wx_shop_order', array('willcancelmessage' => 1), array('id' => $orderid));
	...
	public function sendOrderMessage($orderid = '0', $delRefund = false) 
	{
		...
		$order_goods = pdo_fetchall('select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.sendtype,og.expresscom,og.expresssn,og.sendtime from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where ' . $scondition . ' and og.uniacid=:uniacid ', $param);
		$goods = '';
		$goodsname = '';
		$goodsnum = 0;
		foreach ($order_goods as $og ) 
		...
		if ($delRefund) 
		{
			...
			{
				$refund = pdo_fetch('select * from ' . tablename('wx_shop_order_refund') . ' where id=:id limit 1', array(':id' => $order['refundid']));
				if (empty($refund)) 
				{
					return;
				}
				...
				else if ($refund['status'] == 3) 
				{
					if (($refund['rtype'] == 2) || ($refund['rtype'] == 1)) 
					{
						...
						{
							$salerefund = pdo_fetch('select * from ' . tablename('wx_shop_refund_address') . ' where uniacid=:uniacid and isdefault=1 limit 1', array(':uniacid' => $_W['uniacid']));
		...
		{
			$item = pdo_fetch('select og.trade_time,g.title,s.storename,s.mobile,p.nickname,s.id as storeid  from ' . tablename('wx_shop_order_goods') . '  og' . "\r\n\t\t\t\t" . ' left join ' . tablename('wx_shop_order') . ' o  on  og.orderid = o.id' . "\r\n\t\t\t\t" . ' left join ' . tablename('wx_shop_goods') . ' g  on  og.goodsid = g.id' . "\r\n\t\t\t\t" . ' left join ' . tablename('wx_shop_store') . ' s  on  o.storeid = s.id' . "\r\n\t\t\t\t" . ' left join ' . tablename('wx_shop_newstore_people') . ' p  on  og.peopleid = p.id' . "\r\n\t\t\t\t" . ' where o.id =:id limit 1', array(':id' => $order['id']));
			...
			if (empty($notice['o2o_snorder_close_advanced'])) 
			{
				...
				$salers = pdo_fetchall('select *   from ' . tablename('wx_shop_saler') . ' sr  inner join ' . tablename('wx_shop_store') . ' s on s.id = sr.storeid  where  s.id=:id and sr.getnotice =1', array(':id' => $item['storeid']));
	...
	public function sendMemberLogMessage($log_id = '', $channel = 0, $isback = false) 
	{
		...
		$log_info = pdo_fetch('select * from ' . tablename('wx_shop_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log_id, ':uniacid' => $_W['uniacid']));
	...
	public function sendStockWarnMessage($goodsid, $optionid) 
	{
		...
		$goods = pdo_fetch('select  * from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid', array(':id' => $goodsid, ':uniacid' => $_W['uniacid']));
		if (empty($goods)) 
		{
			return;
		}
		...
		if (!(empty($optionid))) 
		{
			$option = pdo_fetch('select  *  from ' . tablename('wx_shop_goods_option') . ' where id=:id and uniacid=:uniacid and goodsid = :goodsid', array(':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid, ':id' => $optionid));
			if (!(empty($option))) 
			{
	...
	public function sendNotice(array $params) 
	{
		...
		if (!(empty($templateid))) 
		{
			...
			else 
			{
				...
				if (is_error($ret)) 
				{
					$templatetype = pdo_fetch('select templateid  from ' . tablename('wx_shop_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1', array(':typecode' => $tag, ':uniacid' => $_W['uniacid']));
	...
	public function settemplateidbyback($tag, $uniacid = 0) 
	{
		...
		$templatetype = pdo_fetch('select `name`,templatecode,content  from ' . tablename('wx_shop_member_message_template_type') . ' where typecode=:typecode  limit 1', array(':typecode' => $tag));
		...
		foreach ($result['template_list'] as $key => $value ) 
		{
			...
			{
				$issnoet = false;
				$defaulttemp = pdo_fetch('select 1  from ' . tablename('wx_shop_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1', array(':typecode' => $tag, ':uniacid' => $uniacid));
				if (empty($defaulttemp)) 
				{
					pdo_insert('wx_shop_member_message_template_default', array('typecode' => $tag, 'uniacid' => $uniacid, 'templateid' => $value['template_id']));
				}
				else 
				{
					pdo_update('wx_shop_member_message_template_default', array('templateid' => $value['template_id']), array('typecode' => $tag, 'uniacid' => $uniacid));
				}
				return true;
			}
		}
	...
	{
		global $_W;
		$item = pdo_fetch('select vgl.*,vg.limitnum,s.storename,sa.salername,sa.openid as saleropenid,o.openid,g.title,o.ordersn  from ' . tablename('wx_shop_verifygoods_log') . '   vgl' . "\r\n\t\t" . 'left  join ' . tablename('wx_shop_store') . ' s on s.id = vgl.storeid' . "\r\n\t\t" . 'left  join ' . tablename('wx_shop_saler') . ' sa on sa.id = vgl.salerid' . "\r\n\t\t" . 'inner  join ' . tablename('wx_shop_verifygoods') . ' vg on vg.id = vgl.verifygoodsid' . "\r\n\t\t" . 'inner  join ' . tablename('wx_shop_order_goods') . ' og on vg.ordergoodsid = og.id' . "\r\n\t\t" . 'inner  join ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . "\r\n\t\t" . 'left  join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n\t" . '  where  vgl.id =:id  and vgl.uniacid=:uniacid  limit 1', array(':id' => $verifygoodlogid, ':uniacid' => $_W['uniacid']));
		$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('wx_shop_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $item['verifygoodsid']));
		$verifynum = 0;
