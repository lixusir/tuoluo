...
class Verifygoods_WxShopModel
{
	public function createverifygoods($orderid) 
	{
		global $_W;
		$verifygoods = pdo_fetchall('select  *   from  ' . tablename('wx_shop_verifygoods') . ' where  orderid=:orderid ', array(':orderid' => $orderid));
		...
		$ordergoods = pdo_fetchall('select o.openid,o.uniacid,o.id as orderid , og.id as ordergoodsid,g.verifygoodsdays,g.verifygoodsnum,g.verifygoodslimittype,g.verifygoodslimitdate,og.total ' . $sql2 . ' from ' . tablename('wx_shop_order_goods') . '   og inner join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n" . '          inner join ' . tablename('wx_shop_order') . ' o on og.orderid = o.id' . "\r\n" . '          where   og.orderid =:orderid and g.type = 5', array(':orderid' => $orderid));
		...
		pdo_insert('wx_shop_verifygoods', $data);
		++$i;
		}
		return true;
	}
