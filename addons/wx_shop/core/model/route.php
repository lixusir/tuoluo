...
class Route_WxShopModel
{
	public function run($isweb = true) 
	{
		global $_GPC;
		global $_W;
		require_once IA_ROOT . '/addons/wx_shop/core/inc/page.php';
		if ($isweb) 
		{
			require_once WX_SHOP_CORE . 'inc/page_web.php';
			require_once WX_SHOP_CORE . 'inc/page_web_com.php';
		}
		else 
		{
			require_once WX_SHOP_CORE . 'inc/page_mobile.php';
			require_once WX_SHOP_CORE . 'inc/page_mobile_login.php';
		}
		$r = str_replace('//', '/', trim($_GPC['r'], '/'));
		$routes = explode('.', $r);
        $segs = count($routes);
		$method = 'main';
		$root = (($isweb ? WX_SHOP_CORE_WEB : WX_SHOP_CORE_MOBILE));
		$isMerch = false;
		$isNewstore = false;
		...
		{
			$isplugin = !(empty($r)) && is_dir(WX_SHOP_PLUGIN . $routes[0]);
		}
		if ($isplugin) 
		{
			if ($isweb) 
			{
				require_once WX_SHOP_CORE . 'inc/page_web_plugin.php';
			}
			else 
			{
				require_once WX_SHOP_CORE . 'inc/page_mobile_plugin.php';
				require_once WX_SHOP_CORE . 'inc/page_mobile_plugin_login.php';
				require_once WX_SHOP_CORE . 'inc/page_mobile_plugin_pf.php';
			}
			$_W['plugin'] = $routes[0];
			$root = WX_SHOP_PLUGIN . $routes[0] . '/core/' . (($isweb ? 'web' : 'mobile')) . '/';
			if ($isMerch) 
			{
				$_W['plugin'] = 'merch';
				$root = WX_SHOP_PLUGIN . 'merch/core/web/manage/';
			}
			else if ($isNewstore) 
			{
				$_W['plugin'] = 'newstore';
				$root = WX_SHOP_PLUGIN . 'newstore/core/web/manage/';
			}
			else 
			{
		...
		{
			require_once WX_SHOP_CORE . 'inc/page_system.php';
		}	
		switch ($segs) 
		{
		...
		include $file;
		$class = ucfirst($class) . '_WxShopPage';
		$instance = new $class();
