...
class Store_WxShopModel
{
	public function getStoreInfo($id) 
	{
		global $_W;
		return pdo_fetch('select * from ' . tablename('wx_shop_store') . ' where id=:id and uniacid=:uniacid Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	}
	public function getGoodsInfo($id) 
	{
		global $_W;
		$sql = 'select * from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid Limit 1';
	...
	public function getStoreGoodsInfo($goodsid, $storeid, $flag = 0) 
	{
		...
		$sql = 'select * from ' . tablename('wx_shop_newstore_goods') . ' where goodsid=:goodsid and storeid=:storeid and uniacid=:uniacid ' . $con . ' Limit 1';
	...
	{
		global $_W;
		$sql = 'select * from ' . tablename('wx_shop_newstore_goods_option') . ' where goodsid=:goodsid and storeid=:storeid and uniacid=:uniacid';
	...
	{
		global $_W;
		$sql = 'select * from ' . tablename('wx_shop_newstore_goods_option') . ' where goodsid=:goodsid and storeid=:storeid and optionid=:optionid and uniacid=:uniacid Limit 1';
	...
	public function getAllStore() 
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$sql = 'select * from ' . tablename('wx_shop_store') . ' where uniacid=:uniacid';
	...
	public function getStoreName($list, $return = 'all') 
	{
		...
		if (!(empty($store))) 
		{
			...
			$store_list = pdo_fetchall('select * from ' . tablename('wx_shop_store') . ' where uniacid=:uniacid and id in(' . implode(',', $store_ids) . ')', array(':uniacid' => $_W['uniacid']), 'id');
	...
	public function getListStoreOne($storeid) 
	{
		...
		if ($storeid) 
		{
			$store = pdo_fetch('select * from ' . tablename('wx_shop_store') . ' where uniacid=:uniacid and id=' . $storeid, array(':uniacid' => $_W['uniacid']));
			return $store;
		}
		return false;
	}
