...
class Util_WxShopModel
{
	public function getExpressList($express, $expresssn) 
	{
		...
		if (!(empty($express_set['isopen'])) && !(empty($express_set['apikey']))) 
		{
			if (!(empty($express_set['cache'])) && (0 < $express_set['cache'])) 
			{
				...
				$cache = pdo_fetch('SELECT * FROM' . tablename('wx_shop_express_cache') . 'WHERE express=:express AND expresssn=:expresssn LIMIT 1', array('express' => $express, 'expresssn' => $expresssn));
		...
		if ($useapi && (0 < $express_set['cache']) && !(empty($list))) 
		{
			if (empty($cache)) 
			{
				pdo_insert('wx_shop_express_cache', array('expresssn' => $expresssn, 'express' => $express, 'lasttime' => time(), 'datas' => iserializer($list)));
			}
			else 
			{
				pdo_update('wx_shop_express_cache', array('lasttime' => time(), 'datas' => iserializer($list)), array('id' => $cache['id']));
			}
		}
		return $list;
	}
	...
	public function get_area_config_data($uniacid = 0) 
	{
		...
		$sql = 'select * from ' . tablename('wx_shop_area_config') . ' where uniacid=:uniacid limit 1';
