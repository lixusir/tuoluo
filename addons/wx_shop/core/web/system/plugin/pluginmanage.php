...
class Pluginmanage_WxShopPage extends SystemPage
{
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT gp.*,p.identity,p.category,p.name as pname,p.version,p.author,p.status FROM ' . tablename('wx_shop_system_plugingrant_plugin') . ' as gp' . "\n" . '                left join ' . tablename('wx_shop_plugin') . ' as p on p.id = gp.pluginid' . "\n" . '                WHERE 1 ' . $condition . ' ORDER BY gp.displayorder,gp.id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
	...
	protected function post() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_system_plugingrant_plugin') . ' WHERE id =:id limit 1', array(':id' => $id));
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				pdo_update('wx_shop_system_plugingrant_plugin', $data, array('id' => $id));
			}
			else 
			{
				$data['createtime'] = time();
				pdo_insert('wx_shop_system_plugingrant_plugin', $data);
				$id = pdo_insertid();
			}
		...
		{
			$plugin = pdo_fetch('select * from ' . tablename('wx_shop_plugin') . ' where id = ' . $item['pluginid'] . ' ');
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id FROM ' . tablename('wx_shop_system_plugingrant_plugin') . ' WHERE id in( ' . $id . ' ) ');
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_system_plugingrant_plugin', array('id' => $item['id']));
	...
	public function property() 
	{
		...
		{
			pdo_update('wx_shop_system_plugingrant_plugin', array($type => $value), array('id' => $id));
			$statusstr = '';
			if ($type == 'state') 
			{
				$typestr = '启用';
	...
	public function query() 
	{
		...
		$pluginid = '';
		$grantPlugin = array();
		$grantPlugin = pdo_fetchall('select pluginid from ' . tablename('wx_shop_system_plugingrant_plugin') . ' where 1 ');
