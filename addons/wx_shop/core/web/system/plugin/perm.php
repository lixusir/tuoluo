...
class Perm_WxShopPage extends SystemPage
{
	public function main() 
	{
		...
		if ($permset) 
		{
			...
			$list = pdo_fetchall('SELECT p.*,ac.name FROM ' . tablename('wx_shop_perm_plugin') . ' p  ' . ' left join ' . tablename('account_wechats') . ' ac on p.acid = ac.acid  ' . ' WHERE 1 ' . $condition . ' ORDER BY id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
			foreach ($list as &$row ) 
			{
				...
				if (!(empty($aplugins))) 
				{
					$row['plugins'] = pdo_fetchall('select name from ' . tablename('wx_shop_plugin') . ' where identity in (' . implode(',', $aplugins) . ') and status=1');
				}
				else 
				{
					$row['plugins'] = array();
				...
				if (!(empty($acoms))) 
				{
					$row['coms'] = pdo_fetchall('select name from ' . tablename('wx_shop_plugin') . ' where identity in (' . implode(',', $acoms) . ') and status=1');
				}
				else 
				{
					$row['coms'] = array();
			...
			unset($row);
			$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_perm_plugin') . ' p  ' . ' left join ' . tablename('users') . ' u on p.uid = u.uid  ' . ' left join ' . tablename('account_wechats') . ' ac on p.acid = ac.acid  ' . ' WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post() 
	{
		global $_W;
		global $_GPC;
		$plugins = pdo_fetchall('select * from ' . tablename('wx_shop_plugin') . ' where iscom=0 and deprecated=0 and status=1 order by displayorder asc');
		$coms = pdo_fetchall('select * from ' . tablename('wx_shop_plugin') . ' where iscom=1 and deprecated=0 and status=1 order by displayorder asc');
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_perm_plugin') . ' WHERE id =:id limit 1', array(':id' => $id));
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				pdo_update('wx_shop_perm_plugin', $data, array('id' => $id));
			}
			else 
			{
				$wechatcount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $data['acid']));
				if (0 < $wechatcount) 
				{
					show_json(0, '此公众号的插件权限已经存在!', '', 'error');
				}
				pdo_insert('wx_shop_perm_plugin', $data);
				$id = pdo_insertid();
			}
	...
	public function delete() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id FROM ' . tablename('wx_shop_perm_plugin') . ' WHERE id = \'' . $id . '\'');
		if (empty($item)) 
		{
			message('抱歉，权限设置不存在或是已经被删除！', webUrl('perm/plugins', array('op' => 'display')), 'error');
		}
		pdo_delete('wx_shop_perm_plugin', array('id' => $id));
		show_json(1, array('url' => webUrl('system/plugin/perm')));
	}
