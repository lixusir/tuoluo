...
class Plugingrant_WxShopPage extends SystemPage
{
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT gl.*,w.name as wname,p.name as pname,p.thumb,p.iscom,p.identity FROM ' . tablename('wx_shop_system_plugingrant_log') . ' as gl' . "\n" . '                    left join ' . tablename('account_wechats') . ' as w on w.uniacid = gl.uniacid' . "\n" . '                    left join ' . tablename('wx_shop_plugin') . ' as p on p.id = gl.pluginid' . "\n\t\t\t" . '        WHERE 1 ' . $condition . ' ORDER BY gl.id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_system_plugingrant_log') . ' as gl' . "\n" . '                    left join ' . tablename('account_wechats') . ' as w on w.uniacid = gl.uniacid' . "\n" . '                    left join ' . tablename('wx_shop_plugin') . ' as p on p.id = gl.pluginid' . "\n\t\t\t" . '        WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_system_plugingrant_log') . ' WHERE id =:id limit 1', array(':id' => $id));
		if ($_W['ispost']) 
		{
			...
			foreach ($pluginid as $value ) 
			{
				...
				if (0 < $data['isperm']) 
				{
					$lastitem = pdo_fetch('SELECT MAX(permendtime) as permendtime,permlasttime FROM ' . tablename('wx_shop_system_plugingrant_log') . "\n" . '                            WHERE uniacid = ' . $data['uniacid'] . ' and pluginid = ' . $value . ' and isperm = 1 limit 1');
