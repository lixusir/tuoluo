...
class Pluginpackage_WxShopPage extends SystemPage
{
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT gp.*,p.identity,p.category,p.version,p.author,p.status,p.name FROM ' . tablename('wx_shop_system_plugingrant_package') . ' as gp' . "\n" . '                left join ' . tablename('wx_shop_plugin') . ' as p on p.id = gp.pluginid' . "\n" . '                WHERE 1 ' . $condition . ' ORDER BY gp.displayorder,gp.id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		foreach ($list as $key => $value ) 
		{
			$list[$key]['data'] = unserialize($value['data']);
			unset($row);
		}
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_system_plugingrant_package') . ' as gp' . "\n" . '                left join ' . tablename('wx_shop_plugin') . ' as p on p.id = gp.pluginid' . "\n" . '                WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				pdo_update('wx_shop_system_plugingrant_package', $data, array('id' => $id));
			}
			else 
			{
				pdo_insert('wx_shop_system_plugingrant_package', $data);
				$id = pdo_insertid();
			}
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,text FROM ' . tablename('wx_shop_system_plugingrant_package') . ' WHERE id in( ' . $id . ' ) ');
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_system_plugingrant_package', array('id' => $item['id']));
	...
	public function property() 
	{
		...
		{
			pdo_update('wx_shop_system_plugingrant_package', array($type => $value), array('id' => $id));
			$statusstr = '';
			if ($type == 'rec') 
			{
				$typestr = '推荐';
	...
	public function query() 
	{
		...
		$pluginid = '';
		$grantPlugin = array();
		$grantPlugin = pdo_fetchall('select pluginid from ' . tablename('wx_shop_system_plugingrant_package') . ' where 1 ');
		...
		$plugins = pdo_fetchall('select id,`name` as title,thumb,`desc` from ' . tablename('wx_shop_plugin') . ' where 1 ' . $condition . ' order by displayorder asc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_plugin') . ' WHERE 1 ' . $condition . ' ', $params);
