...
class Notice_WxShopPage extends SystemPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!(empty($id))) {
				pdo_update('wx_shop_system_copyright_notice', $data, array('id' => $id));
				plog('system.site.article.edit', '修改公告 ID: ' . $id);
			}
			 else {
				$data['createtime'] = TIMESTAMP;
				pdo_insert('wx_shop_system_copyright_notice', $data);
				$id = pdo_insertid();
		...
		$item = pdo_fetch('select * from ' . tablename('wx_shop_system_copyright_notice') . ' where id=:id limit 1', array(':id' => $id));
		include $this->template();
	}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_system_copyright_notice') . ' WHERE id in( ' . $id . ' )');

		foreach ($items as $item ) {
			pdo_delete('wx_shop_system_copyright_notice', array('id' => $item['id']));
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_system_copyright_notice') . ' WHERE id in( ' . $id . ' )');

		if (!(empty($item))) {
			pdo_update('wx_shop_system_copyright_notice', array('displayorder' => $displayorder), array('id' => $id));
	...
	public function status()
	{
		...
		foreach ($items as $item ) {
			pdo_update('wx_shop_system_copyright_notice', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
