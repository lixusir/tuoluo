...
class Index_WxShopPage extends ComWebPage
{
	...
	public function main() 
	{
		...
		$sql = 'SELECT * FROM ' . tablename('wx_shop_wxcard') . ' ' . ' where  1 and ' . $condition . ' ORDER BY id DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $data);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_wxcard') . ' where 1 and ' . $condition, $data);
	...
	protected function post() 
	{
		...
		if (!(empty($id))) 
		{
			$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_wxcard') . ' WHERE id =:id and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		}
		if ($_W['ispost']) 
		{
			$data = array();
			...
			if (!(empty($item))) 
			{
				...
				pdo_update('wx_shop_wxcard', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			else 
			{
				...
				pdo_insert('wx_shop_wxcard', $data);
				$id = pdo_insertid();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,card_id,title FROM ' . tablename('wx_shop_wxcard') . ' WHERE id in( ' . $id . ' ) and merchid=0 AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			...
			pdo_delete('wx_shop_wxcard', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function stock() 
	{
		...
		$sql = 'select id,uniacid,logo_url, card_id,title,total_quantity,quantity from ' . tablename('wx_shop_wxcard');
