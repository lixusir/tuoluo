...
class Sendtask_WxShopPage extends ComWebPage 
{
	...
	public function main() 
	{
		...
		$sendtasks = pdo_fetchall('SELECT cs.*, c.couponname, c.thumb FROM ' . tablename('wx_shop_coupon_sendtasks') . '  cs left  join  ' . tablename('wx_shop_coupon') . '  c on cs.couponid =c.id' . "\r\n" . '                    WHERE 1 ' . $condition . '  order by enough LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_coupon_sendtasks') . ' cs WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post() 
	{
		...
		if (!(empty($id))) 
		{
			$item = pdo_fetch('SELECT *  FROM ' . tablename('wx_shop_coupon_sendtasks') . ' WHERE uniacid = ' . $uniacid . ' and id =' . $id);
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				pdo_update('wx_shop_coupon_sendtasks', $data, array('id' => $id));
				plog('coupon.sendtask.edit', '修改优惠券发送任务 ID: ' . $id);
			}
			else 
			{
				pdo_insert('wx_shop_coupon_sendtasks', $data);
				$id = pdo_insertid();
	...
	public function delete() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_coupon_sendtasks') . ' WHERE id  = :id  and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (!(empty($item))) 
		{
			pdo_delete('wx_shop_coupon_sendtasks', array('id' => $id, 'uniacid' => $_W['uniacid']));
		}
		show_json(1);
	}
}
