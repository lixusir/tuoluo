...
class Setticket_WxShopPage extends WebPage 
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($id)) 
			{
				if (is_array($_GPC['couponid'])) 
				{
					...
					foreach ($_GPC['couponid'] as $cpk => $cpv ) 
					{
						...
						$sql = 'SELECT * FROM ' . tablename('wx_shop_coupon') . $where;
			...
			if (empty($id)) 
			{
				pdo_insert('wx_shop_sendticket', $data);
				plog('sendticket.set', '设置发券内容');
			}
			else 
			{
				$params = array('id' => $id);
				pdo_update('wx_shop_sendticket', $data, $params);
			}
			show_json(1);
		}
		$sql = 'SELECT * FROM ' . tablename('wx_shop_sendticket') . ' WHERE uniacid = ' . intval($_W['uniacid']);
		$item = pdo_fetch($sql);
		if (!(empty($item))) 
		{
			...
			{
				$cpsql = 'SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE uniacid = ' . intval($_W['uniacid']) . ' AND id = ' . intval($cpidv);
