...
class Index_WxShopPage extends ComWebPage
{
	...
	public function main() 
	{
		...
		$sql = 'SELECT * FROM ' . tablename('wx_shop_coupon') . ' ' . ' where  1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		foreach ($list as &$row ) 
		{
			$row['gettotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['usetotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_coupon_data') . ' where used = 1 and couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['pwdjoins'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['pwdoks'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and ok=1 limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
		...
		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_coupon') . ' where 1 and ' . $condition, $params);
		$pager = pagination2($total, $pindex, $psize);
		$category = pdo_fetchall('select * from ' . tablename('wx_shop_coupon_category') . ' where uniacid=:uniacid and merchid=0 order by id desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				{
					$pwdkey2 = pdo_fetch('SELECT pwdkey2 FROM ' . tablename('wx_shop_coupon') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				pdo_update('wx_shop_coupon', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			else 
			{
				...
				pdo_insert('wx_shop_coupon', $data);
				$id = pdo_insertid();
				plog('sale.coupon.add', '添加优惠券 ID: ' . $id . '  <br/>优惠券名称: ' . $data['couponname']);
			}
			$key = 'wx_shop:com:coupon:' . $id;
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':name' => $key));
			if (!(empty($data['pwdkey2']))) 
			{
				if (empty($rule)) 
				{
					$rule_data = array('uniacid' => $_W['uniacid'], 'name' => $key, 'module' => 'wx_shop', 'displayorder' => 0, 'status' => $data['pwdopen']);
					pdo_insert('rule', $rule_data);
					$rid = pdo_insertid();
					$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'wx_shop', 'content' => $data['pwdkey2'], 'type' => 1, 'displayorder' => 0, 'status' => $data['pwdopen']);
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE id =:id and uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if (empty($item)) 
		{
			$starttime = time();
		...
		else 
		{
			...
			if (($item['limitgoodtype'] == 1) || ($item['limitgoodtype'] == 2)) 
			{
				...
				{
					$goods = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('wx_shop_goods') . ' WHERE uniacid = :uniacid and id in (' . $item['limitgoodids'] . ') ', array(':uniacid' => $_W['uniacid']));
				}
			}
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,couponname FROM ' . tablename('wx_shop_coupon') . ' WHERE id in( ' . $id . ' ) and merchid=0 AND uniacid=' . $_W['uniacid']);
		$key = 'wx_shop:com:coupon:' . $id;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':name' => $key));
		if (!(empty($rule))) 
		{
		...
		{
			pdo_delete('wx_shop_coupon', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			pdo_delete('wx_shop_coupon_data', array('couponid' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function displayorder() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_coupon', array('displayorder' => $displayorder), array('id' => $id));
	...
	public function query() 
	{
		...
		$time = time();
		$ds = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_coupon') . '  WHERE 1 ' . $condition . ' ORDER BY id asc', $params);
		if (!(empty($ds))) 
		{
			foreach ($ds as &$d ) 
			{
	...
	public function querygoods() 
	{
		...
		$ds = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('wx_shop_goods') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
	...
	public function send() 
	{
		...
		$coupon = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
		if ($_W['ispost']) 
		{
			if (empty($coupon)) 
			{
			...
			else if ($class1 == 2) 
			{
				...
				$members = pdo_fetchall('SELECT openid FROM ' . tablename('wx_shop_member') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\'' . $where, array(), 'openid');
				if (!(empty($_GPC['send_level']))) 
				{
					$levelname = pdo_fetchcolumn('select levelname from ' . tablename('wx_shop_member_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_level']));
				}
				else 
				{
					$levelname = '全部';
			...
			else if ($class1 == 3) 
			{
				...
				if (!(empty($_GPC['send_group']))) 
				{
					$groupname = pdo_fetchcolumn('select groupname from ' . tablename('wx_shop_member_group') . ' where id=:id limit 1', array(':id' => $_GPC['send_group']));
				}
				else 
				{
					$groupname = '全部分组';
			...
			else if ($class1 == 5) 
			{
				...
				$members = pdo_fetchall('SELECT openid FROM ' . tablename('wx_shop_member') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and isagent=1 and status=1 ' . $where, array(), 'openid');
				if ($_GPC['send_agentlevel'] != '') 
				{
					$levelname = pdo_fetchcolumn('select levelname from ' . tablename('wx_shop_commission_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_agentlevel']));
				}
				else 
				{
					$levelname = '全部';
			...
			else if ($class1 == 6) 
			{
				...
				$members = pdo_fetchall('SELECT openid FROM ' . tablename('wx_shop_member') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and ispartner=1 and partnerstatus=1 ' . $where, array(), 'openid');
				if ($_GPC['send_partnerlevels'] != '') 
				{
					$levelname = pdo_fetchcolumn('select levelname from ' . tablename('wx_shop_globonus_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_partnerlevels']));
				}
				else 
				{
					$levelname = '全部';
			...
			else if ($class1 == 7) 
			{
				...
				$members = pdo_fetchall('SELECT openid FROM ' . tablename('wx_shop_member') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and isaagent=1 and aagentstatus=1 ' . $where, array(), 'openid');
				if ($_GPC['send_aagentlevels'] != '') 
				{
					$levelname = pdo_fetchcolumn('select levelname from ' . tablename('wx_shop_abonus_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_aagentlevels']));
				}
				else 
				{
					$levelname = '全部';
			...
			$members = pdo_fetchall('select id,openid,nickname from ' . tablename('wx_shop_member') . ' where openid in (' . implode(',', $mopenids) . ') and uniacid=' . $_W['uniacid']);
			if (empty($members)) 
			{
				show_json(0, '未找到发送的会员!');
			}
			...
			pdo_update('wx_shop_coupon', $upgrade, array('id' => $coupon['id']));
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY level asc');
		$list2 = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_group') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc');
		$coupons = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id asc');
		$hascommission = false;
		$plugin_com = p('commission');
		if ($plugin_com) 
		{
