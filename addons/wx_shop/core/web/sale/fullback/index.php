...
class Index_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		$gifts = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_fullback_goods') . "\r\n" . '                    WHERE 1 ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_fullback_goods') . ' WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($good_data['option']))) 
			{
				...
				pdo_update('wx_shop_goods_option', array('isfullback' => 0), array('uniacid' => $uniacid, 'goodsid' => $data['goodsid']));
				foreach ($fullbackOption as $val ) 
				{
					...
					pdo_update('wx_shop_goods_option', $optionData, array('uniacid' => $uniacid, 'id' => intval($val)));
				}
			}
			if (!(empty($id))) 
			{
				pdo_update('wx_shop_fullback_goods', $data, array('id' => $id));
				plog('sale.fullback.edit', '编辑全返 ID: ' . $id . ' <br/>全返名称: ' . $data['titles']);
			}
			else 
			{
				pdo_insert('wx_shop_fullback_goods', $data);
				$id = pdo_insertid();
				plog('sale.fullback.add', '添加全返 ID: ' . $id . '  <br/>全返名称: ' . $data['titles']);
			}
			$sql = 'update ' . tablename('wx_shop_fullback_goods') . ' g set' . "\r\n" . '                g.minallfullbackallprice = (select min(allfullbackprice) from ' . tablename('wx_shop_goods_option') . ' where goodsid = ' . $data['goodsid'] . '),' . "\r\n" . '                g.maxallfullbackallprice = (select max(allfullbackprice) from ' . tablename('wx_shop_goods_option') . ' where goodsid = ' . $data['goodsid'] . '),' . "\r\n" . '                g.minallfullbackallratio = (select min(allfullbackratio) from ' . tablename('wx_shop_goods_option') . ' where goodsid = ' . $data['goodsid'] . '),' . "\r\n" . '                g.maxallfullbackallratio = (select max(allfullbackratio) from ' . tablename('wx_shop_goods_option') . ' where goodsid = ' . $data['goodsid'] . ')' . "\r\n" . '                where g.goodsid = ' . $data['goodsid'] . ' and g.hasoption=1 and g.uniacid = ' . $uniacid . ' and g.id = ' . $id . ' ';
			pdo_query($sql);
			if (0 < $data['status']) 
			{
				pdo_update('wx_shop_goods', array('isfullback' => $id), array('id' => $data['goodsid']));
			}
			else 
			{
				pdo_update('wx_shop_goods', array('isfullback' => 0), array('id' => $data['goodsid']));
			}
			show_json(1, array('url' => webUrl('sale/fullback/edit', array('id' => $id))));
		}
		if (!(empty($id))) 
		{
			$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $id . ' ');
			$good_data = pdo_fetch('select title,thumb,hasoption' . "\r\n" . '                            from ' . tablename('wx_shop_goods') . ' where id = ' . $item['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
			if (empty($good_data)) 
			{
	...
	public function status() 
	{
		...
		$items = pdo_fetchall('SELECT id,titles FROM ' . tablename('wx_shop_fullback_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_fullback_goods', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
			pdo_update('wx_shop_goods', array('isfullback' => intval($item['id'])), array('id' => $items['goodsid']));
	...
	public function delete1() 
	{
		...
		$items = pdo_fetchall('SELECT id,titles,goodsid FROM ' . tablename('wx_shop_fullback_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_fullback_goods', array('id' => $item['id']));
			pdo_update('wx_shop_goods', array('isfullback' => 0), array('id' => $item['goodsid']));
	...
	public function change() 
	{
		...
		$gift = pdo_fetch('select id from ' . tablename('wx_shop_fullback_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if (empty($gift)) 
		{
			show_json(0, array('message' => '参数错误'));
		}
		pdo_update('wx_shop_fullback_goods', array($type => $value), array('id' => $id));
		show_json(1);
	}
	...
	public function hasoption() 
	{
		...
		$goods = pdo_fetch('select id,title,marketprice,hasoption,isfullback from ' . tablename('wx_shop_goods') . ' where uniacid = :uniacid and id = :goodsid ', $params);
		if (!(empty($id))) 
		{
			$fullback = pdo_fetch('select * from ' . tablename('wx_shop_fullback_goods') . "\r\n" . '                        where id = ' . $id . ' and uniacid = :uniacid and goodsid = :goodsid ', $params);
