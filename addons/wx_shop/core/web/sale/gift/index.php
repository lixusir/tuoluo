...
class Index_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		$gifts = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_gift') . "\r\n" . '                    WHERE 1 ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_gift') . ' WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($data['thumb'])) 
			{
				...
				{
					$giftgood = pdo_fetch('select id,title,thumb from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and deleted = 0 limit 1 ');
				}
		...
		if (!(empty($item['goodsid']))) 
		{
			...
			if ($goodsid) 
			{
				...
				{
					$goods[$key] = pdo_fetch('select id,title,thumb from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 1 and id = ' . $value . ' and deleted = 0 ');
				}
			}
		}
		if (!(empty($item['giftgoodsid']))) 
		{
			...
			if ($giftid) 
			{
				...
				{
					$gift[$key] = pdo_fetch('select id,title,thumb from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and id = ' . $value . ' and deleted = 0 and total > 0 ');
				}
			}
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_gift') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_gift', array('deleted' => 1, 'status' => 0), array('id' => $item['id']));
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_gift', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function delete1() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_gift', array('id' => $item['id']));
	...
	public function restore() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_gift', array('deleted' => 0), array('id' => $item['id']));
	...
	public function change() 
	{
		...
		$gift = pdo_fetch('select id from ' . tablename('wx_shop_gift') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if (empty($gift)) 
		{
			show_json(0, array('message' => '参数错误'));
		}
		pdo_update('wx_shop_gift', array($type => $value), array('id' => $id));
		show_json(1);
	}
