...
class Index_WxShopPage extends WebPage 
{
	public function main() 
	{
		...
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_package') . ' WHERE 1 ' . $condition . ' ', $params);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				foreach ($goodsid as $key => $value ) 
				{
					$packagenum = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_package_goods') . ' where uniacid = ' . $uniacid . ' and goodsid = ' . $value . ' ');
					$thisgoods = pdo_fetch('select id from ' . tablename('wx_shop_package_goods') . ' where uniacid = ' . $uniacid . ' and goodsid = ' . $value . ' and pid = ' . $id . ' ');
					if (!($thisgoods) && (3 <= $packagenum)) 
					{
						show_json(0, '同一件商品最多参与三个套餐活动!');
					}
				}
				$package_update = pdo_update('wx_shop_package', $data, array('id' => $id, 'uniacid' => $uniacid));
				$package_goods_del = pdo_delete('wx_shop_package_goods', array('pid' => $id, 'uniacid' => $uniacid));
				$package_goods_option_del = pdo_delete('wx_shop_package_goods_option', array('pid' => $id,  'uniacid' => $uniacid));
				foreach ($goodsid as $key => $value ) 
				{
					$good_data = pdo_fetch('select title,thumb,marketprice,goodssn,productsn,hasoption' . "\r\n" . '                            from ' . tablename('wx_shop_goods') . ' where id = ' . $value . ' and uniacid = ' . $uniacid . ' ');
					...
					$package_goods_insert = pdo_insert('wx_shop_package_goods', $good_data);
					if (!(empty($good_data['option']))) 
					{
						$packageGoodsOption = array_filter(explode(',', $good_data['option']));
						foreach ($packageGoodsOption as $k => $val ) 
						{
							$op = pdo_fetch('SELECT id,title,marketprice,goodssn,productsn FROM ' . tablename('wx_shop_goods_option') . "\r\n" . '                                WHERE uniacid = ' . $uniacid . ' and id = ' . $val . ' ');
							...
							$package_goods_option_insert = pdo_insert('wx_shop_package_goods_option', $optionData);
			...
			else 
			{
				...
				$package_insert = pdo_insert('wx_shop_package', $data);
				if (!($package_insert)) 
				{
					show_json(0, '套餐添加失败！');
				}
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_package') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_package', array('deleted' => 1, 'status' => 0), array('id' => $item['id']));
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_package', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function delete1() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_package', array('id' => $item['id']));
			pdo_delete('wx_shop_package_goods', array('pid' => $item['id']));
			pdo_delete('wx_shop_package_goods_option', array('pid' => $item['id']));
	...
	public function restore() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_package', array('deleted' => 0), array('id' => $item['id']));
	...
	public function change() 
	{
		...
		$package = pdo_fetch('select id from ' . tablename('wx_shop_package') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if (empty($package)) 
		{
			show_json(0, array('message' => '参数错误'));
		}
		pdo_update('wx_shop_package', array($type => $value), array('id' => $id));
		show_json(1);
	}
