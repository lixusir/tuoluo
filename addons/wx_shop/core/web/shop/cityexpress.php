...
class Cityexpress_WxShopPage extends WebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$cityexpress = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_city_express') . ' WHERE uniacid=:uniacid AND merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => 0));
		...
		if ($_W['ispost']) {
			...
			if (!empty($cityexpress)) {
				...
				pdo_update('wx_shop_city_express', $data, array('id' => $cityexpress['id']));
			}
			else {
				pdo_insert('wx_shop_city_express', $data);
				$id = pdo_insertid();
