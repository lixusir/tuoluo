...
class Recommand_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			//var_dump($shop['indexrecommands']);die;
		...
		if (!(empty($goodsids))) 
		{
			$goods = pdo_fetchall('select id,title,thumb from ' . tablename('wx_shop_goods') . ' where id in (' . $goodsids . ') and status=1 and deleted=0 and uniacid=' . $_W['uniacid'] . ' order by instr(\'' . $goodsids . '\',id)');
		}
