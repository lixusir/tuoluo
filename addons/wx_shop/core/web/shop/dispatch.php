...
class Dispatch_WxShopPage extends WebPage
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ($data['isdefault']) 
			{
				pdo_update('wx_shop_dispatch', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'merchid' => 0));
			}
			if (!(empty($id))) 
			{
				plog('shop.dispatch.edit', '修改配送方式 ID: ' . $id);
				pdo_update('wx_shop_dispatch', $data, array('id' => $id));
			}
			else 
			{
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_dispatch') . ' WHERE id = \'' . $id . '\' and merchid=0 and uniacid = \'' . $_W['uniacid'] . '\'');
		if (!(empty($dispatch))) 
		{
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('wx_shop_dispatch') . ' WHERE id in( ' . $id . ' ) AND merchid=0 AND uniacid=' . $_W['uniacid']);
	...
	public function displayorder() 
	{
		...
		$item = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('wx_shop_dispatch') . ' WHERE id in( ' . $id . ' ) AND merchid=0 AND uniacid=' . $_W['uniacid']);
		if (!(empty($item))) 
		{
