...
class List_WxShopPage extends WebPage
{
	protected function orderData($status, $st) 
	{
		...
		if (!(empty($_GPC['searchfield'])) && !(empty($_GPC['keyword']))) 
		{
			...
			{
				$sqlcondition = ' inner join ( select  DISTINCT(og.orderid) from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,og.optionname)>0)) gs on gs.orderid=o.id';
			}
