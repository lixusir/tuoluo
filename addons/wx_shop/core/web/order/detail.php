...
class Detail_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		if (0 < $item['sendtype']) 
		{
			...
			{
				$order_goods[$key]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.realprice from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $value['sendtype'] . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $id));
			}
		...
		if (com('verify')) 
		{
			...
			if (!(empty($item['verifyopenid']))) 
			{
				...
				{
					$saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('wx_shop_merch_saler') . ' where openid=:openid and merchid=:merchid and uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $item['verifyopenid'], ':merchid' => $item['merchid']));
				}
			}
			if (!(empty($item['verifystoreid']))) 
			{
				...
				{
					$store = pdo_fetch('select * from ' . tablename('wx_shop_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $item['verifystoreid']));
				}
			}
			if ($item['isverify']) 
			{
					...
					if (empty($item['dispatchtype'])) 
					{
						foreach ($verifyinfo as &$v ) 
						{
							if ($v['verified'] || ($item['verifytype'] == 1)) 
							{
								...
								{
									$v['salername'] = pdo_fetchcolumn('select salername from ' . tablename('wx_shop_merch_saler') . ' where openid=:openid and merchid=:merchid and uniacid=:uniacid limit 1', array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid'], ':merchid' => $item['merchid']));
								}
							}
						}
						unset($v);
