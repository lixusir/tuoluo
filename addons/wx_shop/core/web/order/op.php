...
class Op_WxShopPage extends WebPage
{
	public function delete() 
	{
		...
		pdo_update('wx_shop_order', array('deleted' => 1), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
	...
	public function changeprice() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			foreach ($changegoodsprice as $ogid => $change ) 
			{
				$og = pdo_fetch('select price,realprice from ' . tablename('wx_shop_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid']));
				if (!(empty($og))) 
				{
			...
			{
				pdo_update('wx_shop_order', $orderupdate, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
			if (0 < $item['parentid']) 
			{
			...
			foreach ($changegoodsprice as $ogid => $change ) 
			{
				$og = pdo_fetch('select price,realprice,changeprice from ' . tablename('wx_shop_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid']));
				if (!(empty($og))) 
				{
					$realprice = $og['realprice'] + $change;
					$changeprice = $og['changeprice'] + $change;
					pdo_update('wx_shop_order_goods', array('realprice' => $realprice, 'changeprice' => $changeprice), array('id' => $ogid));
				}
			}
	...
	public function close() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ((0 < $item['refundstate']) && !(empty($item['refundid']))) 
			{
				...
				pdo_update('wx_shop_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid']));
			}
			...
			pdo_update('wx_shop_order', array('status' => -1, 'refundstate' => 0, 'canceltime' => $time, 'remarkclose' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			if (!(empty($item['virtual'])) && ($item['virtual'] != 0)) 
			{
				$goodsid = pdo_fetch('SELECT goodsid FROM ' . tablename('wx_shop_order_goods') . ' WHERE uniacid = ' . $_W['uniacid'] . ' AND orderid = ' . $item['id']);
				$typeid = $item['virtual'];
				...
				$vkeynum = count($arr);
				pdo_query('update ' . tablename('wx_shop_virtual_data') . ' set openid="",usetime=0,orderid=0,ordersn="",price=0,merchid=0 where typeid=' . intval($typeid) . ' and orderid = ' . $item['id']);
				pdo_query('update ' . tablename('wx_shop_virtual_type') . ' set usedata=usedata-' . $vkeynum . ' where id=' . intval($typeid));
			}
	...
	public function paycancel() 
	{
		...
		{
			m('order')->setStocksAndCredits($item['id'], 2);
			pdo_update('wx_shop_order', array('status' => 0, 'cancelpaytime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function fetchcancel() 
	{
		...
		pdo_update('wx_shop_order', array('status' => 1, 'finishtime' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function sendcancel() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (0 < $item['sendtype']) 
			{
				...
				$data['sendtype'] = 0;
				pdo_update('wx_shop_order_goods', $data, array('orderid' => $item['id'], 'sendtype' => $sendtype, 'uniacid' => $_W['uniacid']));
		...
		else if (0 < $item['sendtype']) 
		{
			...
			{
				$bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $i . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
	...
	public function send() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ((intval($_GPC['sendtype']) == 1) || (0 < $item['sendtype'])) 
			{
				...
				{
					pdo_update('wx_shop_order_goods', $data, array('id' => $value, 'uniacid' => $_W['uniacid']));
				}
		...
		if (0 < $item['sendtype']) 
		{
			$noshipped = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = 0 and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			$i = 1;
			while ($i <= $item['sendtype']) 
			{
				$shipped[$i]['sendtype'] = $i;
				$shipped[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = ' . $i . ' and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
				++$i;
			}
		}
		$order_goods = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
	...
	public function remarksaler() 
	{
		...
		if ($_W['ispost']) 
		{
			pdo_update('wx_shop_order', array('remarksaler' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($expresssn))) 
			{
				...
				if (0 < $item['sendtype']) 
				{
					...
					pdo_update('wx_shop_order_goods', $change_data, array('orderid' => $id, 'sendtype' => $sendtype, 'uniacid' => $_W['uniacid']));
				}
				else 
				{
					pdo_update('wx_shop_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				}
	...
	public function changeaddress() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				$item = pdo_fetch('SELECT id, ordersn, address,openid FROM ' . tablename('wx_shop_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				pdo_update('wx_shop_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid']));
