...
class Export_WxShopPage extends WebPage
{
	...
	public function main() 
	{
		...
		if ($_GPC['export'] == 1) 
		{
			...
			$sql = 'select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress, d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,m.groupid,sm.id as salerid,sm.nickname as salernickname,s.salername from ' . tablename('wx_shop_order') . ' o' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid and m.uniacid =  o.uniacid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on a.id=o.addressid ' . ' left join ' . tablename('wx_shop_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('wx_shop_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('wx_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' where ' . $condition . ' ' . $statuscondition . ' ORDER BY o.createtime DESC,o.status DESC  ';
			$list = pdo_fetchall($sql, $paras);
			$list_group = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_group') . ' WHERE uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
			...
			foreach ($list as &$value ) 
			{
				...
				if (!(empty($value['verifystoreid']))) 
				{
					if (0 < $value['merchid']) 
					{
						$value['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('wx_shop_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['verifystoreid']));
					}
					else 
					{
						$value['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('wx_shop_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['verifystoreid']));
					}
				}
		...
		$stores = pdo_fetchall('select id,storename from ' . tablename('wx_shop_store') . ' where uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
