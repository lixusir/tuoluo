...
class Area_WxShopPage extends WebPage
{
	...
	public function get_area($code) 
	{
		...
		$params = array();
		$params[':code'] = $code;
		$sql = 'select * from ' . tablename('wx_shop_city') . ' where  code=:code and status=1 Limit 1';
	...
	public function count() 
	{
		...
		foreach ($list as $k => $v ) 
		{
			$params = array();
			$params[':code'] = $v['code'];
			$sql = 'select count(1) from ' . tablename('wx_shop_city') . ' where  code=:code';
	...
	public function get_area_list($level) 
	{
		...
		$params = array();
		$params[':level'] = $level;
		$sql = 'select * from ' . tablename('wx_shop_city') . ' where  level=:level and status=1';
	...
	public function get_child_list($pcode) 
	{
		...
		$params = array();
		$params[':pcode'] = $pcode;
		$sql = 'select * from ' . tablename('wx_shop_city') . ' where  pcode=:pcode and status=1 order by id';
		$list = pdo_fetchall($sql, $params);
		return $list;
	}
	...
	public function xmllist() 
	{
		...
		foreach ($list2 as $k2 => $v2 ) 
		{
			...
			$styles = array();
			$dir = IA_ROOT . '/addons/wx_shop/static/js/dist/area/list/' . $left . '/';
			if (!(is_dir($dir))) 
			{
				mkdir($dir, 511, true);
			}
