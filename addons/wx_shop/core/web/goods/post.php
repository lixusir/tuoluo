...
{
	$card = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods_cards') . ' WHERE id = :id and uniacid = :uniacid', array(':id' => $item['cardid'], ':uniacid' => $_W['uniacid']));
}
$status = $item['status'];
...
$commission_level = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_commission_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY commission1 asc');
...
if ($_W['ispost']) 
{
		...
		if ($_GPC['diyformtype'] == 2) 
		{
			...
			if ($_GPC['diysave'] == 1) 
			{
				...
				if (empty($is_save)) 
				{
					...
					pdo_insert('wx_shop_diyform_type', $insert);
					$data['diysaveid'] = pdo_insertid();
				}
				else 
				{
					pdo_update('wx_shop_diyform_type', $insert, array('id' => $diysaveid));
				}
			}
			...
			if (!(empty($opencard))) 
			{
				...
				if (!(empty($card))) 
				{
					...
					if ($change) 
					{
						...
						pdo_update('wx_shop_goods_cards', $carddata, array('id' => $card['id']));
					}
					$cardid = $card['id'];
				}
				else 
				{
					...
					pdo_insert('wx_shop_goods_cards', $carddata);
					$cardid = pdo_insertid();
				}
				$data['cardid'] = $cardid;
			}
	...
	while ($k < $len) 
	{
		...
		{
			pdo_insert('wx_shop_goods_param', $a);
			$param_id = pdo_insertid();
		}
		else 
		{
			pdo_update('wx_shop_goods_param', $a, array('id' => $get_param_id));
	...
	if (0 < count($paramids)) 
	{
		pdo_query('delete from ' . tablename('wx_shop_goods_param') . ' where goodsid=' . $id . ' and id not in ( ' . implode(',', $paramids) . ')');
	}
	else 
	{
		pdo_query('delete from ' . tablename('wx_shop_goods_param') . ' where goodsid=' . $id);
	}
	$totalstocks = 0;
	$files = $_FILES;
	...
	while ($k < $len) 
	{
		...
		{
			pdo_update('wx_shop_goods_spec', $a, array('id' => $get_spec_id));
			$spec_id = $get_spec_id;
		}
		else 
		{
			pdo_insert('wx_shop_goods_spec', $a);
		...
		while ($n < $itemlen) 
		{
			...
			{
				pdo_update('wx_shop_goods_spec_item', $d, array('id' => $get_item_id));
				$item_id = $get_item_id;
			}
			else 
			{
		...
		if (0 < count($itemids)) 
		{
			pdo_query('delete from ' . tablename('wx_shop_goods_spec_item') . ' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id . ' and id not in (' . implode(',', $itemids) . ')');
		}
		else 
		{
			pdo_query('delete from ' . tablename('wx_shop_goods_spec_item') . ' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id);
		}
		pdo_update('wx_shop_goods_spec', array('content' => serialize($itemids)), array('id' => $spec_id));
		$specids[] = $spec_id;
		++$k;
	}
	if (0 < count($specids)) 
	{
		pdo_query('delete from ' . tablename('wx_shop_goods_spec') . ' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id . ' and id not in (' . implode(',', $specids) . ')');
	}
	else 
	{
		pdo_query('delete from ' . tablename('wx_shop_goods_spec') . ' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id);
	}
	...
	while ($k < $len) 
	{
		...
		{
			pdo_insert('wx_shop_goods_option', $a);
			$option_id = pdo_insertid();
		}
		else 
		{
			pdo_update('wx_shop_goods_option', $a, array('id' => $get_option_id));
	...
	pdo_update('wx_shop_goods', array('discounts' => $discounts_json), array('id' => $id));
	$has_merch = 0;
	...
	pdo_update('wx_shop_goods', array('isdiscount_discounts' => $is_discounts_json), array('id' => $id));
	...
	pdo_update('wx_shop_goods', $commission_arr, array('id' => $id));
	...
	{
		pdo_update('wx_shop_goods', array('total' => $totalstocks), array('id' => $id));
	}
...
if (!(empty($id))) 
{
	...
	$allspecs = pdo_fetchall('select * from ' . tablename('wx_shop_goods_spec') . ' where goodsid=:id order by displayorder asc', array(':id' => $id));
	...
	unset($s);
	$params = pdo_fetchall('select * from ' . tablename('wx_shop_goods_param') . ' where goodsid=:id order by displayorder asc', array(':id' => $id));
	if (!(empty($item['thumb']))) 
	{
	...
	$options = pdo_fetchall('select * from ' . tablename('wx_shop_goods_option') . ' where goodsid=:id order by id asc', array(':id' => $id));
	$specs = array();
	if (0 < count($options)) 
	{
...
if ($com_virtual) 
{
	$virtual_types = pdo_fetchall('select * from ' . tablename('wx_shop_virtual_type') . ' where uniacid=:uniacid and merchid=:merchid and recycled = 0 order by id asc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
}
if ($merchid == 0) 
{
	$details = pdo_fetchall('select detail_logo,detail_shopname,detail_btntext1, detail_btnurl1 ,detail_btntext2,detail_btnurl2,detail_totaltitle from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and detail_shopname<>\'\' group by detail_shopname', array(':uniacid' => $_W['uniacid']));
