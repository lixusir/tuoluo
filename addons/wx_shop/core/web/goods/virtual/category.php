...
class Category_WxShopPage extends ComWebPage
{
	...
	public function main()
	{
		...
		if (!empty($_W['ispost'])) {
			...
			foreach ($_GPC['catname'] as $id => $catname) {
				...
				if (strpos($id, 'new_') !== false) {
					pdo_insert('wx_shop_virtual_category', array('name' => $catname, 'uniacid' => $_W['uniacid'], 'merchid' => 0));
					$insert_id = pdo_insertid();
					plog('goods.virtual.category.add', '添加分类 ID: ' . $insert_id);
				}
				else {
					pdo_update('wx_shop_virtual_category', array('name' => $catname), array('id' => $id, 'merchid' => 0));
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_virtual_category') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and merchid=0 ORDER BY id DESC');
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('wx_shop_virtual_category') . ' WHERE id = \'' . $id . '\' and merchid=0 AND uniacid=' . $_W['uniacid'] . '');
