...
class Index_WxShopPage extends WebPage
{
	public function main($goodsfrom = 'sale')
	{
		...
		if (!empty($_GPC['keyword'])) {
			...
			if ($merch_plugin) {
				$sqlcondition .= ' left join ' . tablename('wx_shop_merch_user') . ' merch on merch.id = g.merchid and merch.uniacid=g.uniacid';
			}
	...
	public function checked()
	{
		...
		foreach ($items as $item) {
			pdo_update('wx_shop_goods', array('checked' => intval($_GPC['checked'])), array('id' => $item['id']));
	...
	public function property()
	{
		...
		if (in_array($type, array('new', 'hot', 'recommand', 'discount', 'time', 'sendfree', 'nodiscount'))) {
			pdo_update('wx_shop_goods', array('is' . $type => $data), array('id' => $id, 'uniacid' => $_W['uniacid']));
		...
		if (in_array($type, array('status'))) {
			pdo_update('wx_shop_goods', array($type => $data), array('id' => $id, 'uniacid' => $_W['uniacid']));
