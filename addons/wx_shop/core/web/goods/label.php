...
class Label_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		$label = pdo_fetchall('SELECT id,uniacid,label,labelname,status,displayorder FROM ' . tablename('wx_shop_goods_label') . "\r\n" . '                WHERE uniacid=:uniacid ' . $condition . ' order by id limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('wx_shop_goods_label') . ' WHERE uniacid=:uniacid ' . $condition, $params);
	...
	protected function post() 
	{
		...
		if (!(empty($id))) 
		{
			$item = pdo_fetch('SELECT id,uniacid,label,labelname,status,displayorder FROM ' . tablename('wx_shop_goods_label') . "\r\n" . '                    WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $uniacid));
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($item))) 
			{
				pdo_update('wx_shop_goods_label', $data, array('id' => $item['id']));
				plog('goods.label.edit', '修改标签组 ID: ' . $id);
			}
			else 
			{
				$data['uniacid'] = $uniacid;
				pdo_insert('wx_shop_goods_label', $data);
				$id = pdo_insertid();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,label FROM ' . tablename('wx_shop_goods_label') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		if (empty($item)) 
		{
			$item = array();
		}
		...
		{
			pdo_delete('wx_shop_goods_label', array('id' => $item['id']));
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_goods_label', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function query() 
	{
		...
		$labels = pdo_fetchall('SELECT id,label,labelname FROM ' . tablename('wx_shop_goods_label') . ' WHERE 1 ' . $condition . ' order by id desc', $params);
		if (empty($labels)) 
		{
			$labels = array();
		}
	...
	public function labelfile() 
	{
		...
		$labels = pdo_fetch('SELECT id,label,labelname FROM ' . tablename('wx_shop_goods_label') . ' WHERE 1 ' . $condition . ' order by id desc', $params);
		if (empty($labels)) 
		{
			$labels = array();
			show_json(0, '您查找的标签组不存在或已删除！');
	...
	public function style() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($style))) 
			{
				pdo_update('wx_shop_goods_labelstyle', $data, array('uniacid' => $uniacid));
				plog('goods.labelstyle.edit', '修改标签组样式');
			}
			else 
			{
				$data['uniacid'] = $uniacid;
				pdo_insert('wx_shop_goods_labelstyle', $data);
				$id = pdo_insertid();
