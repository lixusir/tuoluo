...
class Goods_selector_WxShopPage extends WebPage
{
	...
	public function op() 
	{
		...
		$id = intval($_GPC['id']);
		$sql = 'select * from ' . tablename('wx_shop_goods') . ' where id = ' . $id;
		$goods = pdo_fetch($sql);
		if (empty($goods)) 
		{
		...
		if ($_GPC['nooption'] == 1) 
		{
			$sql = 'select * from ' . tablename('wx_shop_goods_option') . ' where goodsid = ' . $id;
