...
class Selecturl_WxShopPage extends WebPage
{
	...
	public function main()
	{
		...
		if (0 < $syscate['level']) {
			$categorys = pdo_fetchall('SELECT id,name,parentid FROM ' . tablename('wx_shop_category') . ' WHERE enabled=:enabled and uniacid= :uniacid  ', array(':uniacid' => $_W['uniacid'], ':enabled' => '1'));
		}
		...
		$plugins=$this->getAccountPlugins();
		...
		$seckillArr = array(
			'name' => '秒杀中心',
			'url' => '',
			'url_wxapp' => '/pages/seckill/seckill',
		);
		array_push($plugins, $seckillArr );
		$allUrls = array(
			array(
				'name' => '商城页面',
				'list' => array(
	...
	public function query()
	{
		...
		if (!empty($kw) && !empty($type)) {
			if ($type == 'good') {
				$list = pdo_fetchall('SELECT id,title,productprice,marketprice,thumb,sales,unit,minprice FROM ' . tablename('wx_shop_goods') . ' WHERE uniacid= :uniacid and status=:status and deleted=0 AND title LIKE :title ', array(':title' => '%' . $kw . '%', ':uniacid' => $_W['uniacid'], ':status' => '1'));
				$list = set_medias($list, 'thumb');
			}
			else if ($type == 'article') {
				$list = pdo_fetchall('select id,article_title from ' . tablename('wx_shop_article') . ' where article_title LIKE :title and article_state=1 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
			}
			else if ($type == 'coupon') {
				$list = pdo_fetchall('select id,couponname,coupontype from ' . tablename('wx_shop_coupon') . ' where couponname LIKE :title and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
			}
			else if ($type == 'groups') {
				$list = pdo_fetchall('select id,title from ' . tablename('wx_shop_groups_goods') . ' where title LIKE :title and status=1 and deleted=0 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
			}
			else if ($type == 'sns') {
				$list_board = pdo_fetchall('select id,title from ' . tablename('wx_shop_sns_board') . ' where title LIKE :title and status=1 and enabled=0 and uniacid=:uniacid order by id desc ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
				$list_post = pdo_fetchall('select id,title from ' . tablename('wx_shop_sns_post') . ' where title LIKE :title and checked=1 and deleted=0 and uniacid=:uniacid order by id desc ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
				$list = array();
				...
				if ($type == 'creditshop') {
					$list = pdo_fetchall('select id, thumb, title, price, credit, money from ' . tablename('wx_shop_creditshop_goods') . ' where title LIKE :title and status=1 and deleted=0 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':title' => '%' . $kw . '%'));
				}
			}
		}
    ...
    public function getAccountPlugins()
    {
        global $_W;
        global $_GPC;
        ...
        $userPlugin=array();
        $plugins=['commission','creditshop','globonus','groups','abonus','bargain','task','sns'];
        foreach ($category as $v){
            foreach ($v['plugins'] as $plugin){
                if(com_run('perm::check_plugin',$plugin['identity']) && in_array($plugin['identity'],$plugins)){
                    $userPlugin[]=['name'=>$plugin['name'],'url'=>'','url_wxapp'=>'/pages/'.$plugin['identity'].'/index/index'];
                }
            }
        }
        $userPlugin[]=['name'=>'兑换中心','url'=>'','url_wxapp'=>'/pages/exchange/common/index'];
        return $userPlugin;
    }
}
