...
class Index_WxShopPage extends ComWebPage
{
	...
	public function main() 
	{
		...
		$sql = 'SELECT * FROM ' . tablename('wx_shop_store') . ' WHERE ' . $condition . ' ORDER BY displayorder desc,id desc';
		$sql .= ' LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$sql_count = 'SELECT count(1) FROM ' . tablename('wx_shop_store') . ' WHERE ' . $condition;
		$total = pdo_fetchcolumn($sql_count, $paras);
		$pager = pagination2($total, $pindex, $psize);
		$list = pdo_fetchall($sql, $paras);
		foreach ($list as &$row ) 
		{
			$row['salercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_saler') . ' where storeid=:storeid limit 1', array(':storeid' => $row['id']));
		}
		unset($row);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				pdo_update('wx_shop_store', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				plog('shop.verify.store.edit', '编辑门店 ID: ' . $id);
			}
			else 
			{
				pdo_insert('wx_shop_store', $data);
				$id = pdo_insertid();
		...
		if (p('newstore')) 
		{
			$storegroup = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_newstore_storegroup') . ' WHERE  uniacid=:uniacid  ', array(':uniacid' => $_W['uniacid']));
			$category = pdo_fetchall('SELECT *FROM ' . tablename('wx_shop_newstore_category') . ' WHERE uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
		}
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_store') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,storename FROM ' . tablename('wx_shop_store') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_store', array('id' => $item['id']));
	...
	public function displayorder() 
	{
		...
		$item = pdo_fetchall('SELECT id,storename FROM ' . tablename('wx_shop_store') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		if (!(empty($item))) 
		{
			pdo_update('wx_shop_store', array('displayorder' => $displayorder), array('id' => $id));
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_store', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function query() 
	{
		...
		$ds = pdo_fetchall('SELECT id,storename FROM ' . tablename('wx_shop_store') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
		if ($_GPC['suggest']) 
		{
