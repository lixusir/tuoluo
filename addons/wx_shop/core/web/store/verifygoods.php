...
class Verifygoods_WxShopPage extends ComWebPage
{
	...
	public function main() 
	{
		...
		$sql = 'select vg.*,g.id as goodsid ,g.title,g.thumb,o.ordersn,o.paytime,s.storename,o.openid  from ' . tablename('wx_shop_verifygoods') . '   vg' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_store') . ' s  on s.id = vg.storeid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_order_goods') . ' og on vg.ordergoodsid = og.id' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n\t\t" . ' where  1 and  ' . $condition . ' ORDER BY vg.id DESC ';
		...
		foreach ($list as &$row ) 
		{
			...
			$verifygoodlogs = pdo_fetchall('select  *  from ' . tablename('wx_shop_verifygoods_log') . '  where  verifygoodsid =:id  ', array(':id' => $row['id']));
			$verifynum = 0;
		...
		unset($row);
		$total = pdo_fetchcolumn('select  COUNT(*)   from ' . tablename('wx_shop_verifygoods') . '   vg' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_store') . ' s  on s.id = vg.storeid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_order_goods') . ' og on vg.ordergoodsid = og.id' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n\t\t" . '  where  1 and  ' . $condition . ' ORDER BY vg.id  DESC ', $params);
	...
	public function detail() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			pdo_update('wx_shop_verifygoods', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function invalid() 
	{
		...
		$item = pdo_fetch('SELECT id FROM ' . tablename('wx_shop_verifygoods') . ' WHERE id=' . $id . '  AND uniacid=' . $_W['uniacid']);
		if (!(empty($item))) 
		{
			pdo_update('wx_shop_verifygoods', array('invalid' => $type), array('id' => $item['id']));
			com('wxcard')->updateusercardbyvarifygoodid($id);
		}
		show_json(1, array('url' => referer()));
	}
	public function verifygoodslog() 
	{
		...
		$sql = 'select vgl.id,g.id as goodsid ,g.title,g.thumb,o.ordersn,vgl.verifydate,vgl.verifynum,o.paytime,s.storename,sa.salername,vgl.remarks,o.openid  from ' . tablename('wx_shop_verifygoods_log') . '   vgl' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_verifygoods') . ' vg on vg.id = vgl.verifygoodsid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_store') . ' s  on s.id = vgl.storeid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_saler') . ' sa  on sa.id = vgl.salerid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_order_goods') . ' og on vg.ordergoodsid = og.id' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n\t\t" . ' where  1 and  ' . $condition . ' ORDER BY vgl.verifydate DESC ';
		$sql .= ' LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		foreach ($list as &$rom ) 
		{
		...
		unset($rom);
		$total = pdo_fetchcolumn('select  COUNT(*)   from ' . tablename('wx_shop_verifygoods_log') . '   vgl' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_verifygoods') . ' vg on vg.id = vgl.verifygoodsid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_store') . ' s  on s.id = vgl.storeid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_saler') . ' sa  on sa.id = vgl.salerid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_order_goods') . ' og on vg.ordergoodsid = og.id' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . "\r\n\t\t" . ' left join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n\t\t" . '  where  1 and  ' . $condition . ' ORDER BY vgl.verifydate DESC ', $params);
		$pager = pagination2($total, $pindex, $psize);
		include $this->template();
	}
	public function verifygoodslogdelete() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,verifygoodsid FROM ' . tablename('wx_shop_verifygoods_log') . ' WHERE id=' . $id . ' AND uniacid=' . $_W['uniacid']);
		if (!(empty($item))) 
		{
			pdo_delete('wx_shop_verifygoods_log', array('id' => $item['id']));
			pdo_update('wx_shop_verifygoods', array('used' => 0), array('id' => $item['verifygoodsid']));
