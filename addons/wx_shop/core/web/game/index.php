...
class Index_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		{
			header('location: ' . webUrl('game/list'));
			exit();
		}
	...
	protected function selectMemberCreate($day = 0) 
	{
		...
		$sql = 'select count(*) from ' . tablename('wx_shop_member') . ' where uniacid = :uniacid and createtime between :createtime1 and :createtime2';
	...
	public function query() 
	{
		...
		include $this->template('game/query');
	}
	protected function ajaxnewmember($day = 0) 
	{
		...
		$member_count = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where uniacid=:uniacid', $param);
	...
	protected function ajaxmembergender() 
	{
		...
		$sql_member = 'select gender,count(gender) as gender_num from ' . tablename('wx_shop_member') . ' where uniacid = :uniacid group by gender';
	...
	{
		global $_W;
		$levels = pdo_fetchall('select * from ' . tablename('wx_shop_member_level') . ' where uniacid=:uniacid order by level asc', array(':uniacid' => $_W['uniacid']), 'id');
		$levelname = array();
		foreach ($levels as $l ) 
		{
		...
		$levelname[0] = '普通等级';
		ksort($levelname);
		$sql_level = 'select level,count(level) as level_num from ' . tablename('wx_shop_member') . ' where uniacid = :uniacid group by level';
	...
	{
		global $_W;
		$province = pdo_fetchall('select province,count(province) as province_num from ' . tablename('wx_shop_member') . ' where uniacid = :uniacid group by province', array(':uniacid' => $_W['uniacid']));
		$result = array();
