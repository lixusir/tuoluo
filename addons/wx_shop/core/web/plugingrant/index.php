...
class Index_WxShopPage extends WebPage 
{
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT gp.*,p.identity,p.name as pname,p.category,p.version,p.author,p.status FROM ' . tablename('wx_shop_system_plugingrant_plugin') . ' as gp' . "\r\n" . '                left join ' . tablename('wx_shop_plugin') . ' as p on p.id = gp.pluginid' . "\r\n" . '                WHERE 1 ' . $condition . ' ORDER BY ' . $sortcondition . ' LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		foreach ($list as $key => $value ) 
		{
			$list[$key]['data'] = unserialize($value['data']);
		}
		$package = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_system_plugingrant_package') . ' ' . "\r\n" . '                WHERE state = 1 and rec = 1 ORDER BY id desc ');
		foreach ($package as $key => $value ) 
		{
			$pluginid = explode(',', $value['pluginid']);
			foreach ($pluginid as $k => $v ) 
			{
				$package[$key]['package'][$k] = pdo_fetch('select `name`,thumb,iscom from ' . tablename('wx_shop_plugin') . ' where id = ' . $v . ' ');
			}
			$package[$key]['data'] = unserialize($value['data']);
		}
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_system_plugingrant_plugin') . ' as gp' . "\r\n" . '                left join ' . tablename('wx_shop_plugin') . ' as p on p.id = gp.pluginid' . "\r\n" . '                WHERE 1 ' . $condition . ' ', $params);
		$pager = pagination($total, $pindex, $psize);
		$adv = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_system_plugingrant_adv') . ' WHERE enabled = 1  ORDER BY displayorder DESC ');
	...
	public function detail() 
	{
		...
		{
			$id = intval($_GPC['id']);
			$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_system_plugingrant_package') . ' WHERE id =:id limit 1', array(':id' => $id));
			$item['name'] = $item['text'];
			$item['data'] = unserialize($item['data']);
		}
		else if ($type == 'plugin') 
		{
			$item = pdo_fetch('SELECT gp.*,p.identity,p.name as pname,p.category,p.version,p.author,p.status,p.thumb,p.desc,p.isv2 FROM ' . tablename('wx_shop_system_plugingrant_plugin') . ' gp' . "\r\n" . '                left join ' . tablename('wx_shop_plugin') . ' as p on p.id = gp.pluginid ' . "\r\n" . '                WHERE gp.id =:id limit 1', array(':id' => $id));
			if (!(empty($item))) 
			{
				$package = pdo_fetchall('select * from ' . tablename('wx_shop_system_plugingrant_package') . ' ' . "\r\n" . '                    where find_in_set(\'' . $item['pluginid'] . '\',pluginid) and state = 1 ');
				foreach ($package as $key => $value ) 
				{
					$packplugin = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_plugin') . '  WHERE id in (' . $value['pluginid'] . ') ');
			...
			{
				$plugin = array();
				$plugin = pdo_fetchall('select * from ' . tablename('wx_shop_plugin') . ' where id in (' . trim($item['pluginid']) . ') ');
	...
	public function create() 
	{
		...
		if ($cate == 'package') 
		{
			$package = pdo_fetch('select * from ' . tablename('wx_shop_system_plugingrant_package') . ' ' . "\r\n" . '                    where id = ' . $id . ' and state = 1 ');
			$packplugin = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_plugin') . '  WHERE id in (' . $package['pluginid'] . ') ');
		...
		{
			$item = pdo_fetch('SELECT gp.*,p.identity,p.name as pname,p.category,p.version,p.author,p.status,p.thumb,p.desc FROM ' . tablename('wx_shop_system_plugingrant_plugin') . ' gp' . "\r\n" . '                left join ' . tablename('wx_shop_plugin') . ' as p on p.id = gp.pluginid ' . "\r\n" . '                WHERE gp.id =:id and state = 1 limit 1', array(':id' => $id));
			$item['data'] = unserialize($item['data']);
	...
	public function payon() 
	{
		...
		pdo_insert('wx_shop_system_plugingrant_order', $logdata);
		if ($paytype == 'wechat') 
		{
			show_json(1, $wechat);
		}
	...
	public function paystatus() 
	{
		...
		$order = pdo_fetch('select * from ' . tablename('wx_shop_system_plugingrant_order') . ' where logno = \'' . $logno . '\'');
