...
class O2o_orderusers_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		$total = pdo_fetchcolumn('select  count(1) from ' . tablename('wx_shop_store') . ' where uniacid=:uniacid  ' . $condition2, $params2);
		$sql = 'select *  from  ' . tablename('wx_shop_store') . ' where uniacid=:uniacid ' . $condition2 . ' ';
		...
		{
			$sql = 'select count(a.openid) from (SELECT  openid   from  ' . tablename('wx_shop_order') . '    where uniacid=:uniacid and storeid = :storeid  ' . $condition . '  and isnewstore=1 GROUP BY openid) a';
			$params[':storeid'] = $item['id'];
			$params[':uniacid'] = $_W['uniacid'];
			$usercount = pdo_fetchcolumn($sql, $params);
			$item['usercount'] = $usercount;
			$sql = 'SELECT  SUM(price)   from     ' . tablename('wx_shop_order') . '  where   uniacid=:uniacid and storeid = :storeid ' . $condition . '  and isnewstore=1 ';
