...
class Order_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		unset($row);
		$totalcount = $total = pdo_fetchall('select o.id from ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on a.id = o.addressid ' . ' where 1 ' . $condition, $params);
