...
class Level_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		$shopset = array();
		$shopset = $set['shop'];
		$default = array('id' => 'default', 'levelname' => (empty($set['shop']['levelname']) ? '普通等级' : $set['shop']['levelname']), 'discount' => $set['shop']['leveldiscount'], 'ordermoney' => 0, 'ordercount' => 0, 'membercount' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_member') . ' where uniacid=:uniacid and level=0 limit 1', array(':uniacid' => $_W['uniacid'])));
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_level') . ' WHERE 1 ' . $condition . ' ORDER BY level asc', $params);
		foreach ($others as &$row ) 
		{
			$row['membercount'] = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_member') . ' where uniacid=:uniacid and level=:level limit 1', array(':uniacid' => $_W['uniacid'], ':level' => $row['id']));
		}
		unset($row);
		if (empty($_GPC['keyword'])) 
		{
	...
	protected function post() 
	{
		...
		{
			$level = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_level') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => intval($id)));
			if (!(empty($level))) 
			{
				$goodsids = iunserializer($level['goodsids']);
				if (!(empty($goodsids))) 
				{
					$goods = pdo_fetchall('SELECT id,uniacid,title,thumb FROM ' . tablename('wx_shop_goods') . ' WHERE uniacid=:uniacid AND id IN (' . implode(',', $goodsids) . ')', array(':uniacid' => $_W['uniacid']));
				}
			}
		}
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				else 
				{
					...
					pdo_update('wx_shop_member_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			else 
			{
				$data['goodsids'] = $goodsids;
				$data['buygoods'] = $buygoods;
				pdo_insert('wx_shop_member_level', $data);
				$id = pdo_insertid();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('wx_shop_member_level') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_member_level', array('id' => $item['id']));
	...
	public function enabled() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_member_level', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
