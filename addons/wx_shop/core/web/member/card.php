...
class Card_WxShopPage extends WebPage
{
	...
	public function activationset() 
	{
		...
		{
			$coupon = pdo_fetch('SELECT id,couponname as title , thumb  FROM ' . tablename('wx_shop_coupon') . ' WHERE uniacid = ' . $_W['uniacid'] . ' and id =' . $item['couponid']);
		}
		$levels = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_level') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY level asc');
		if ($_W['ispost']) 
		{
