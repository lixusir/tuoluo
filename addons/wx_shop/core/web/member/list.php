...
class List_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		if ($_GPC['groupid'] != '') 
		{
			...
			$join .= ' left join (select * from ' . tablename('wx_shop_member_group_log') . ' order by log_id desc limit 1 ) glog on (glog.openid = dm.openid) and glog.group_id = ' . (int) $_GPC['groupid'];
		}
		if ($_GPC['followed'] != '') 
		{
			if ($_GPC['followed'] == 2) 
		...
		if(!empty($cz_id)) {
			...
			$member = pdo_fetch('select id,aagenttype,aagentprovinces,aagentcitys,aagentareas,address from ' . tablename('wx_shop_member') . ' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$cz_id));
		...
		$sql = 'select * ,dm.openid from ' . tablename('wx_shop_member') . ' dm ' . $join . ' where 1 ' . $condition . '  ORDER BY dm.id DESC,dm.createtime DESC ';
		...
		if (!(empty($list_group))) 
		{
			$res_group = pdo_fetchall('select id,groupname from ' . tablename('wx_shop_member_group') . ' where id in (' . implode(',', $list_group) . ')', array(), 'id');
		}
		isset($list_level) && ($list_level = array_values(array_filter($list_level)));
		if (!(empty($list_level))) 
		{
			$res_level = pdo_fetchall('select id,levelname from ' . tablename('wx_shop_member_level') . ' where id in (' . implode(',', $list_level) . ')', array(), 'id');
		}
		isset($list_agent) && ($list_agent = array_values(array_filter($list_agent)));
		if (!(empty($list_agent))) 
		{
			$res_agent = pdo_fetchall('select id,nickname as agentnickname,avatar as agentavatar from ' . tablename('wx_shop_member') . ' where id in (' . implode(',', $list_agent) . ')', array(), 'id');
		}
		...
		foreach ($list as &$row ) 
		{
			...
			$row['ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
			$row['ordermoney'] = pdo_fetchcolumn('select sum(price) from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
			$row['credit1'] = m('member')->getCredit($row['openid'], 'credit1');
			$row['credit2'] = m('member')->getCredit($row['openid'], 'credit2');

			$arrs = array();
			m('member')->getmylower($row['id'],$arrs,1);
			...
			if($row['game_level'] == 38) {
				
				$zg_level = pdo_fetchcolumn('select goodsid from ' . tablename('wx_shop_game'.substr($row['id'],-1)) . ' where uniacid=:uniacid and uid=:uid and status=1 and  goodslevel=38 order by goodsid desc',array(':uniacid'=>$_W['uniacid'],':uid'=>$row['id']));
				...
				$row['game_level_goods'] = pdo_fetch('select g.goodsname,g.img from ' . tablename('wx_shop_game_goods') . ' g left join ' . tablename('wx_shop_game'.substr($row['id'], -1)) . ' gg on gg.goodsid=g.id where g.uniacid=:uniacid and g.id=:goodsid order by g.id desc',array(':uniacid'=>$_W['uniacid'],':goodsid'=>$zg_level));

			} else {

				$row['game_level_goods'] = pdo_fetch('select goodsname,img from ' . tablename('wx_shop_game_goods') . ' where uniacid=:uniacid and level=:level',array(':uniacid'=>$_W['uniacid'],':level'=>$row['game_level']));
			...
			$row['gg_levels'] = pdo_fetch('select levelname,jy from ' . tablename('wx_shop_game_level') . ' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$row['gg_level']));

			$row['c_levels'] = pdo_fetch('select levelname,img from ' . tablename('wx_shop_game_blevel') . ' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$row['c_level']));


			// $row['game_level_goods']['img'] = toa
			...
			if($cnb && $cnb != $row['xia_num']) {
				pdo_update('wx_shop_member', array('xia_num'=>$cnb), array('id' => $row['id']));

			}


			//今日收益
			$row['jinri']  = m('game')->jgetSy($row['id']);
			// echo '<pre>';
			//     print_r($jinri);
			// echo '</pre>';
			// exit;
			//昨日收益
			$row['zuori'] = m('game')->zgetSy($row['id']);
		...
		if (($join == '') && ($condition == ' and dm.uniacid=:uniacid')) 
		{
			if ($open_redis) 
			{
				...
				if (!($total)) 
				{
					$total = pdo_fetchcolumn('select count(*) from' . tablename('wx_shop_member') . ' dm ' . $join . ' where 1 ' . $condition . ' ', $params);
	...
	public function export(){
		...
		// echo "</pre>";
		// exit;
		$sql = 'select id,nickname,realname,mobile,openid,createtime,agentid from ' . tablename('wx_shop_member') . ' where uniacid='.$_W['uniacid'].' order by id asc limit ' . $nubs . ',10000';
		...
		if(!empty($list)) {
			...
			foreach ($list as &$val) {
				...
				$shang = pdo_fetch('select mobile,agentid from ' . tablename('wx_shop_member') .' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$val['agentid']));

				$val['shang'] = $shang['mobile'];

				$shang_e = pdo_fetch('select mobile,agentid from ' . tablename('wx_shop_member') .' where uniacid=:uniacid and id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$shang['agentid']));

				$val['shang_e'] = $shang_e['mobile'];

				// // $row['groupname'] = ((empty($row['groupname']) ? '无分组' : $row['groupname']));
				// // $row['levelname'] = ((empty($row['levelname']) ? '普通会员' : $row['levelname']));
				// $row['realname'] = str_replace('=', '', $row['realname']);
				// $row['nickname'] = str_replace('=', '', $row['nickname']);
				// $row['jinbi'] = m('member')->getCredit($row['openid'], 'jinbi');
				// $row['credit_red'] = m('member')->getCredit($row['openid'], 'credit_red');
			}
			...
			m('excel')->export($list, array( 'title' => '会员数据-' . date('Y-m-d-H-i', time()), 'columns' => array( array('title' => 'id', 'field' => 'id', 'width' => 12), array('title' => '昵称', 'field' => 'nickname', 'width' => 12), array('title' => '姓名', 'field' => 'realname', 'width' => 12), array('title' => '手机号', 'field' => 'mobile', 'width' => 12), array('title' => 'openid', 'field' => 'openid', 'width' => 24),array('title' => '注册时间', 'field' => 'createtime', 'width' => 20), array('title' => '上级', 'field' => 'shang', 'width' => 20), array('title' => '上上级', 'field' => 'shang_e', 'width' => 20)  )));
	...
	public function detail() 
	{
        ...
        if ($plugin_weightbonus)
        {
            ...
            $hasweightbonus = !(empty($plugin_weightbonus_set['open']));
        }
        //代理分红修改end
        ...
        if ($hasweightbonus)
        {
            $weightlevels = $plugin_weightbonus->getLevels();
        }
        //代理分红修改end
		...
		$member['self_ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		$member['self_ordermoney'] = pdo_fetchcolumn('select sum(price) from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		if (!(empty($member['agentid']))) 
		{
			$parentagent = m('member')->getMember($member['agentid']);
		}
		$order = pdo_fetch('select finishtime from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and openid=:openid and status>=1 Limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
        ...
        if ($hasweightbonus)
        {
            $weightbonus = $plugin_weightbonus->getBonus($member['openid'], array('ok'));
            $member['weightbonusmoney'] = $weightbonus['ok'];//分红金额
        }
        //代理分红修改end
		...
		if ($hasabonus) 
		{
			...
			$oks = m('game')->getAbonus_money($member['id'],1);
			// echo "<pre>";
			// 	print_r($oks);
			// echo "</pre>";
			// exit;

			$member['abonus_ok'] = $oks['ok'];
			$member['abonus_ok1'] = 0;
			$member['abonus_ok2'] = $oks['ok2'];
			$member['abonus_ok3'] = $oks['ok3'];
		...
		if ($plugin_sns) 
		{
			...
			$sns_member = pdo_fetch('select * from ' . tablename('wx_shop_sns_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $member['openid'], ':uniacid' => $_W['uniacid']));
			$sns_member['postcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid=0 and deleted = 0 and checked=1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
			$sns_member['replycount'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid>0 and deleted = 0 and checked=1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		...
		if ($diyform_plugin) 
		{
            ...
            //代理分红修改begin
            if (!(empty($member['diyweightbonusdata'])))
            {
                $diyform_flag_weightbonus = 1;
                $wfields = iunserializer($member['diyweightbonusfields']);
            }
            //代理分红修改end
		...
		if ($_W['ispost']) 
		{
			...
			if ($openbind) 
			{
				if (!(empty($data['mobileverify']))) 
				{
					...
					$m = pdo_fetch('select id from ' . tablename('wx_shop_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniaicd limit 1 ', array(':mobile' => $data['mobile'], ':uniaicd' => $_W['uniacid']));
					if (!(empty($m)) && ($m['id'] != $id)) 
					{
			...
			pdo_update('wx_shop_member', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			if ($hascommission) 
			{
				if (cv('commission.agent.edit')) 
				{
					...
					if (!(empty($adata))) 
					{
						...
						pdo_update('wx_shop_member', $adata, array('id' => $id, 'uniacid' => $_W['uniacid']));
						if (!(empty($agent_flag))) 
						{
							if (!(empty($member['agentid']))) 
							{
                                ...
                                //代理分红修改begin
                                if (p('weightbonus')) {
                                    p('weightbonus')->upgradeLevelByAgent($member['agentid']);
                                }
                                //代理分红修改end

                                //区域代理分红修改begin
                                if (p('abonus')) {
                                    p('abonus')->upgradeLevelByAgent($member['agentid']);
                                }
                                //区域代理分红修改end
			...
			if ($hasglobonus) 
			{
				if (cv('globonus.partner.check')) 
				{
					...
					if (!(empty($gdata))) 
					{
						...
						pdo_update('wx_shop_member', $gdata, array('id' => $id, 'uniacid' => $_W['uniacid']));
					}
				}
			}
            ...
            if ($hasweightbonus)
            {
                if (cv('weightbonus.weight.check'))
                {
                    $wdata = ((is_array($_GPC['wdata']) ? $_GPC['wdata'] : array()));

                    if (!(empty($wdata)))
                    {
                        if ($_GPC['oldweightstatus']==0 && ($wdata['weightstatus'] == 1))
                        {
                            $time = time();
                            $wdata['weighttime'] = time();
                        ...
                        plog('weightbonus.weight.edit', '修改代理 <br/>代理信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
                        pdo_update('wx_shop_member', $wdata, array('id' => $id, 'uniacid' => $_W['uniacid']));
                    }
                }
            }
            //代理分红修改end
			...
			if ($hasauthor) 
			{
				if (cv('author.partner.check')) 
				{
					...
					if (!(empty($author_data))) 
					{
						...
						pdo_update('wx_shop_member', $author_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			if ($hasabonus) 
			{
				if (cv('abonus.agent.check')) 
				{
					...
					if (!(empty($aadata))) 
					{
						$aagentprovinces = ((is_array($_GPC['aagentprovinces']) ? array($_GPC['aagentprovinces'][0]) : array()));
						$aagentcitys = ((is_array($_GPC['aagentcitys']) ? array($_GPC['aagentcitys'][0]) : array()));
						$aagentareas = ((is_array($_GPC['aagentareas']) ? array($_GPC['aagentareas'][0]) : array()));
						...
						if ($aadata['aagenttype'] == 2) 
						{
							...
							//区域清空
							$aadata['aagentareas'] = iserializer(array());


							if($member['aagentcitys'][0] != $aagentcitys[0]) {
								$aadata['address'] = iserializer(array(
									'province'=>$_GPC['province'],
									'city'=>$_GPC['city'],
								));

								$aadata['province'] = $_GPC['province'];
								$aadata['city'] = $_GPC['city'];
								$aadata['area'] = ' ';
						...
						else if ($aadata['aagenttype'] == 3) 
						{
							...
							if($member['aagentareas'][0] != $aagentareas[0]) {
						...
						pdo_update('wx_shop_member', $aadata, array('id' => $id, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			...
			if ($hassns) 
			{
				if (cv('sns.member.edit')) 
				{
					...
					if (!(empty($snsdata))) 
					{
						...
						pdo_update('wx_shop_sns_member', $snsdata, array('id' => $sns_member['id'], 'uniacid' => $_W['uniacid']));
					}
				}
			}
			show_json(1);
		...
		$order = pdo_fetch('select finishtime from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3 order by id desc limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		$member['last_ordertime'] = $order['finishtime'];
		if ($hasglobonus) 
		{
			$bonus = $plugin_globonus->getBonus($member['openid'], array('ok'));
        ...
        {
            $weightbonus = $plugin_weightbonus->getBonus($member['openid'], array('ok'));
            $member['weightbonusmoney'] = $weightbonus['ok'];
        }
        //代理分红修改end
	...
	public function delete() 
	{
		...
		{
			pdo_delete('wx_shop_member', array('id' => $member['id']));
	...
	public function setblack() 
	{
		...
		foreach ($members as $member ) 
		{
			if (!(empty($black))) 
			{
				pdo_update('wx_shop_member', array('isblack' => 1), array('id' => $member['id']));
				plog('member.list.edit', '设置黑名单 <br/>用户信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else 
			{
				pdo_update('wx_shop_member', array('isblack' => 0), array('id' => $member['id']));
	...
	public function changelevel() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			else 
			{
				if (!(empty($levelid))) 
				{
					$level = pdo_fetch('select * from ' . tablename('wx_shop_member_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $levelid, ':uniacid' => $_W['uniacid']));
					if (empty($level)) 
					{
						show_json(0, '此等级不存在');
					}
			...
			$changeids = array();
			$members = pdo_fetchall('select id,openid,nickname,realname,mobile from ' . tablename('wx_shop_member') . ' WHERE id in( ' . $idsstr . ' ) AND uniacid=' . $_W['uniacid']);
			if (!(empty($members))) 
			{
				foreach ($members as $member ) 
				{
					...
					{
						pdo_update('wx_shop_member', $arr, array('id' => $member['id']));
						$changeids[] = $member['id'];
					}
				}
			}
