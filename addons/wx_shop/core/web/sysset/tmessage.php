...
class Tmessage_WxShopPage extends WebPage
{
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_message_template') . ' WHERE 1 ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_member_message_template') . ' WHERE 1 ' . $condition, $params);
	...
	protected function post() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$typegroup = pdo_fetchall('SELECT `typegroup`,groupname   FROM ' . tablename('wx_shop_member_message_template_type') . ' group by `typegroup` order by id ');
		$templatetypes = pdo_fetchall('SELECT id,`name`,`typegroup`,typecode   FROM ' . tablename('wx_shop_member_message_template_type'));
		$type_json = @json_encode($templatetypes);
		if (!(empty($_GPC['id']))) 
		{
			$list = pdo_fetch('SELECT *  FROM ' . tablename('wx_shop_member_message_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $_GPC['id'], ':uniacid' => $_W['uniacid']));
			$types = pdo_fetch('SELECT *  FROM ' . tablename('wx_shop_member_message_template_type') . ' WHERE typecode=:typecode ', array(':typecode' => $list['typecode']));
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($id)) 
			{
				pdo_insert('wx_shop_member_message_template', $insert);
				$id = pdo_insertid();
				plog('sysset.tmessage.delete', '添加群发模板 ID: ' . $id . ' 标题: ' . $insert['title'] . ' ');
			}
			else 
			{
				pdo_update('wx_shop_member_message_template', $insert, array('id' => $id));
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_message_template') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_member_message_template', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function query() 
	{
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_message_template') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
		if ($_GPC['suggest']) 
		{
