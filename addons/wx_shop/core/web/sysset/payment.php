...
class Payment_WxShopPage extends WebPage 
{
	...
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_payment') . ' WHERE 1 ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_payment') . ' WHERE 1 ' . $condition, $params);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$data['alipay_sec']['secret'] = trim($_GPC['data']['secret']);
			$data['alipay_sec']['alipayaccount'] = trim($_GPC['data']['alipayaccount']);
			...
			if (empty($id)) 
			{
				...
				pdo_insert('wx_shop_payment', $data);
				$id = pdo_insertid();
				plog('sysset.payment.add', '添加支付信息一条 ID: ' . $id . ' 标题: ' . $data['title'] . ' ');
			}
			else 
			{
				pdo_update('wx_shop_payment', $data, array('id' => $id));
		...
		if (!(empty($id))) 
		{
			$data = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_payment') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		}
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_payment') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_payment', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function query() 
	{
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
		if ($_GPC['suggest']) 
		{
