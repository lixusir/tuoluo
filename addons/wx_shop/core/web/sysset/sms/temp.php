...
class Temp_WxShopPage extends ComWebPage 
{
	...
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_sms') . ' WHERE 1 ' . $condition . '  ORDER BY id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_sms') . ' WHERE 1 ' . $condition, $params);
	...
	protected function post() 
	{
		...
		if (!(empty($_GPC['id']))) 
		{
			$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_sms') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
...
//			}
			...
			if (empty($id)) 
			{
				...
				pdo_insert('wx_shop_sms', $arr);
				$id = pdo_insertid();
				plog('sysset.sms.temp.add', '添加短信模板 ID: ' . $id . ' 标题: ' . $arr['title'] . ' ');
			}
			else 
			{
				pdo_update('wx_shop_sms', $arr, array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id, name FROM ' . tablename('wx_shop_sms') . ' WHERE id in( ' . $id . ' ) and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
		if (!(empty($items))) 
		{
			foreach ($items as $item ) 
			{
				pdo_delete('wx_shop_sms', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function status() 
	{
		...
		if (!(empty($items))) 
		{
			foreach ($items as $item ) 
			{
				pdo_update('wx_shop_sms', array('status' => $status), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
