...
class Index_WxShopPage extends WebPage
{
	...
	public function settemplateid() 
	{
		...
		foreach ($result['template_list'] as $key => $value ) 
		{
			...
			{
				$issnoet = false;
				$defaulttemp = pdo_fetch('select 1  from ' . tablename('wx_shop_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1', array(':typecode' => $tag, ':uniacid' => $_W['uniacid']));
				if (empty($defaulttemp)) 
				{
					pdo_insert('wx_shop_member_message_template_default', array('typecode' => $tag, 'uniacid' => $_W['uniacid'], 'templateid' => $value['template_id']));
				}
				else 
				{
					pdo_update('wx_shop_member_message_template_default', array('templateid' => $value['template_id']), array('typecode' => $tag, 'uniacid' => $_W['uniacid']));
				}
	...
	public function notice() 
	{
		...
		if (isset($data['openid2'])) 
		{
			if (!(empty($data['openid2']))) 
			{
				...
				$salers2 = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('wx_shop_member') . ' where openid in (' . implode(',', $openids2) . ') and uniacid=' . $_W['uniacid']);
			}
		}
		$salers3 = array();
		if (isset($data['openid3'])) 
		{
			if (!(empty($data['openid3']))) 
			{
				...
				$salers3 = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('wx_shop_member') . ' where openid in (' . implode(',', $openids3) . ') and uniacid=' . $_W['uniacid']);
			}
		}
		$salers4 = array();
		if (isset($data['openid4'])) 
		{
			if (!(empty($data['openid4']))) 
			{
				...
				$salers4 = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('wx_shop_member') . ' where openid in (' . implode(',', $openids4) . ') and uniacid=' . $_W['uniacid']);
			}
		}
		$opensms = com('sms');
		if ($_W['ispost']) 
	...
	{
		global $_W;
		$path = IA_ROOT . '/addons/wx_shop/cert';
		load()->func('file');
		mkdirs($path);
	...
	public function payset() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			pdo_update('wx_shop_sysset', array('sec' => iserializer($sec)), array('uniacid' => $_W['uniacid']));
		...
		$payments = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_payment') . ' WHERE uniacid=:uniacid and paytype = 0 ', array(':uniacid' => $_W['uniacid']));
		$paymentalis = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_payment') . ' WHERE uniacid=:uniacid and paytype = 1 ', array(':uniacid' => $_W['uniacid']));
		if (empty($payments)) 
		{
			$payments = array();
			...
			if (!(empty($payment['wechat']['mchid']))) 
			{
				...
				$payments[] = $default;
				pdo_insert('wx_shop_payment', $default);
		...
		$url = $_W['siteroot'] . 'addons/wx_shop/payment/wechat/notify.php';
		load()->func('communication');
		$resp = ihttp_get($url);
		include $this->template();
		exit();
	...
	public function templat() 
	{
		...
		$styles = array();
		$dir = IA_ROOT . '/addons/wx_shop/template/mobile/';
	...
	public function wap() 
	{
		...
		$styles = array();
		$dir = IA_ROOT . '/addons/wx_shop/template/account/';
		if ($handle = opendir($dir)) 
		{
			while (($file = readdir($handle)) !== false) 
			{
	...
	public function funbar() 
	{
		...
		if ($_W['ispost']) 
		{
			$data = pdo_fetch('select * from ' . tablename('wx_shop_funbar') . ' where uid=:uid and uniacid=:uniacid limit 1', array(':uid' => $_W['uid'], ':uniacid' => $_W['uniacid']));
			$funbardata = ((is_array($_GPC['funbardata']) ? $_GPC['funbardata'] : array()));
			$funbardata = serialize($funbardata);
			if (empty($data)) 
			{
				pdo_insert('wx_shop_funbar', array('uid' => $_W['uid'], 'datas' => $funbardata, 'uniacid' => $_W['uniacid']));
			}
			else 
			{
				pdo_update('wx_shop_funbar', array('datas' => $funbardata), array('uid' => $data['uid'], 'uniacid' => $_W['uniacid']));
			}
			show_json(1);
		}
	}
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($data) || empty($data['new_area'])) 
			{
				...
				if (!(empty($array['new_area']))) 
				{
					...
					$change_data['city'] = '';
					$change_data['area'] = '';
					pdo_update('wx_shop_member', $change_data, array('uniacid' => $uniacid));
					pdo_update('wx_shop_member_address', $change_data, array('uniacid' => $uniacid));
				}
				else 
				{
			...
			if (empty($data)) 
			{
				...
				pdo_insert('wx_shop_area_config', $array);
			}
			else if (!(empty($array))) 
			{
				pdo_update('wx_shop_area_config', $array, array('id' => $data['id'], 'uniacid' => $uniacid));
			}
			$data = m('util')->get_area_config_data();
    ...
    {
        header('location: ' . webUrl('sysset/printer/printer_list'));die;
    }
}
?>