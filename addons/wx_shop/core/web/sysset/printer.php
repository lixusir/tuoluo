...
class Printer_WxShopPage extends WebPage
{
	...
	protected function post() 
	{
		...
		if (!(empty($id))) 
		{
			$list = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' AND merchid=0');
		if (!(empty($items))) 
		{
	...
	protected function printer_post() 
	{
		...
		if (!(empty($id))) 
		{
			$list = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_printer') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			$print_data = json_decode($list['print_data'], true);
	...
	public function printer_delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_printer') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . '  and merchid=0');
		foreach ($items as $item ) 
		{
	...
	public function set() 
	{
		...
		$order_template = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
		include $this->template();
	}
}
?>