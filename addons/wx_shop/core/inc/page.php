...

{
...

	{
		...
		if (($lasttime + $interval) <= $current) {
			...
			ihttp_request(WX_SHOP_TASK_URL . 'order/receive.php', NULL, NULL, 1);
		...
		if (($lasttime + $interval) <= $current) {
			...
			ihttp_request(WX_SHOP_TASK_URL . 'order/close.php', NULL, NULL, 1);
		...
		if (($lasttime + $interval) <= $current) {
			...
			ihttp_request(WX_SHOP_TASK_URL . 'order/fullback.php', NULL, NULL, 1);
		...
		if (($lasttime + $interval) <= $current) {
			...
			ihttp_request(WX_SHOP_TASK_URL . 'goods/status.php', NULL, NULL, 1);
		...
		if (com('coupon')) {
			...
			if (($lasttime + $interval) <= $current) {
				...
				ihttp_request(WX_SHOP_TASK_URL . 'order/willclose.php', NULL, NULL, 1);
		...
		if (com('coupon')) {
			...
			if (($lasttime + $interval) <= $current) {
				...
				ihttp_request(WX_SHOP_TASK_URL . 'coupon/back.php', NULL, NULL, 1);
		...
		if (p('groups')) {
			...
			if (($groups_order_lasttime + $groups_order_interval) <= $groups_order_current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/wx_shop/plugin/groups/task/order.php', NULL, NULL, 1);
			...
			if (($groups_team_lasttime + $groups_team_interval) <= $groups_team_current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/wx_shop/plugin/groups/task/refund.php', NULL, NULL, 1);
			...
			if (($groups_receive_lasttime + $groups_receive_interval) <= $groups_receive_current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/wx_shop/plugin/groups/task/receive.php', NULL, NULL, 1);
		...
		if (p('seckill')) {
			...
			if (($lasttime + $interval) <= $current) {
				...
				ihttp_request($_W['siteroot'] . 'addons/wx_shop/plugin/seckill/task/receive.php', NULL, NULL, 1);
		...
		if($data['k_set'] == 1) {
			...
			ihttp_request(WX_SHOP_TASK_URL . 'commission/index.php', NULL, NULL, 1);
		}

		// echo '<pre>';
		//     print_r(WX_SHOP_TASK_URL);
		// echo '</pre>';
...
/*		$isv3 = $set['style_v3'];
		...
		$moduleroot = IA_ROOT . '/addons/wx_shop';
