...
class Receiver extends WeModuleReceiver
{
	...
	public function saleVirtual($obj = NULL)
	{
		...
		$totalagent = pdo_fetchcolumn('select count(*) from' . tablename('wx_shop_member') . ' where uniacid =' . (int) $account['uniacid'] . ' and isagent =1');
		$totalmember = pdo_fetchcolumn('select count(*) from' . tablename('wx_shop_member') . ' where uniacid =' . (int) $account['uniacid']);
