...
class MobilePage extends Page
{
    ...
    public function __construct()
    {
        ...
        if (!(empty($_GPC['merchid'])) && $merch_plugin && $merch_data['is_openmerch']) {
            $this->merch_user = pdo_fetch('select * from ' . tablename('wx_shop_merch_user') . ' where id=:id limit 1', array(':id' => intval($_GPC['merchid'])));
        }
    }
    ...
    public function followBar($diypage = false, $merch = false)
    {
        ...
        if ((!($followed) && ($memberid != $mid)) || (!(empty($diyfollowbar['params']['showtype'])) && !(empty($diyfollowbar['params']['isopen'])))) {
            ...
            if (!(empty($mid)) && ($memberid != $mid)) {
                if (!(empty($_SESSION[WX_SHOP_PREFIX . '_shareid'])) && ($_SESSION[WX_SHOP_PREFIX . '_shareid'] == $mid)) {
                    $mid = $_SESSION[WX_SHOP_PREFIX . '_shareid'];
                }
                $member = m('member')->getMember($mid);
                if (!(empty($member))) {
                    $_SESSION[WX_SHOP_PREFIX . '_shareid'] = $mid;
                    $friend = true;
    ...
    public function MemberBar($diypage = false, $merch = false)
    {
        ...
        if (!(empty($data['become_goodsid']))) {
            $goods = pdo_fetch('select id,title,thumb from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $data['become_goodsid'], ':uniacid' => $_W['uniacid']));
        } else {
            return;
        }
    ...
    public function diyPage($type)
    {
        ...
        if (!empty($typeMaping[$type])&&empty($merch)) {
            $pageID = pdo_fetchcolumn('SELECT * FROM ' . tablename('wx_shop_diypage') . ' WHERE uniacid=:uniacid AND type=:type AND is_use=:is_use ORDER BY id DESC', [':uniacid' => $_W['uniacid'], ':is_use' => 1, ':type' => $typeMaping[$type]]);
            if (!empty($pageID)) {

                $diyPage = p('diypage');
                if ($diyPage) {
                    header('location:'.mobileUrl('diypage',['id'=>$pageID]));
                    exit;
                    ...
                    if (!(empty($page))) {
                        ...
                        if ($type == 'home') {
                            ...
                            if (empty($trade['shop_strengthen'])) {
                                $order = pdo_fetch('select id,price  from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and status = 0 and openid=:openid order by createtime desc limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
                                if (!(empty($order))) {
                                    $goods = pdo_fetchall('select g.*,og.total as totals  from ' . tablename('wx_shop_order_goods') . ' og inner join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id   where og.uniacid=:uniacid    and og.orderid=:orderid  limit 3', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
                                    $goodstotal = pdo_fetchcolumn('select COUNT(*)  from ' . tablename('wx_shop_order_goods') . ' og inner join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id   where og.uniacid=:uniacid    and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
                                }
                            }
                        }
    ...
    public function diyDanmu($diy = false)
    {
        ...
        if (empty($danmu['params']['datatype'])) {
            ...
            $danmu['data'] = pdo_fetchall('SELECT nickname, avatar as imgurl FROM' . tablename('wx_shop_member') . ' WHERE uniacid=:uniacid AND nickname!=\'\' AND avatar!=\'\' ' . $condition . ' ORDER BY rand() LIMIT 10', array(':uniacid' => $_W['uniacid']));
            ...
            $danmu['data'] = pdo_fetchall('SELECT m.nickname, m.avatar as imgurl, o.createtime as time FROM' . tablename('wx_shop_order') . ' o LEFT JOIN ' . tablename('wx_shop_member') . ' m ON m.openid=o.openid WHERE o.uniacid=:uniacid AND m.nickname!=\'\' AND m.avatar!=\'\' ORDER BY o.createtime DESC LIMIT 10', array(':uniacid' => $_W['uniacid']));
