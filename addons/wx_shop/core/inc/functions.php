...
if (!function_exists('m')) {
...

	{
		...
		$model = WX_SHOP_CORE . 'model/' . strtolower($name) . '.php';
		...
		$class_name = ucfirst($name) . '_WxShopModel';
...
if (!function_exists('d')) {
...

	{
		...
		$model = WX_SHOP_CORE . 'data/' . strtolower($name) . '.php';
		...
		require_once WX_SHOP_INC . 'data_model.php';

		require_once $model;

		$class_name = ucfirst($name) . '_WxShopDataModel';
...
if (!function_exists('socket')) {
...

	{
		...
		$socket = WX_SHOP_PLUGIN . strtolower($name) . '/core/socket.php';
...
if (!function_exists('p')) {
...

	{
		...
		$model = WX_SHOP_PLUGIN . strtolower($name) . '/core/model.php';
		...
		if (com_run('perm::check_plugin', $name) || ($name == 'grant') || ($name == 'qpay') || ($name == 'sign')) {
...
if (!function_exists('com')) {
...

	{
		...
		$model = WX_SHOP_CORE . 'com/' . strtolower($name) . '.php';
		...
		require_once WX_SHOP_CORE . 'inc/com_model.php';

		require_once $model;

		$class_name = ucfirst($name) . '_WxShopComModel';
...
if (!function_exists('show_json_w')) {

	function show_json_w($status = 1, $return = NULL,$msg)
...
if (!function_exists('get_auth')) {
...

	{
		...
		$set = pdo_fetch('select sets from ' . tablename('wx_shop_sysset') . ' order by id asc limit 1');
...
if (!function_exists('rc')) {
...

	{
		...
		$resp = ihttp_request(WX_SHOP_AUTH_URL, array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain, 'plugin' => $plugin), NULL, 1);
...

	{
		...
		foreach ($options['items'] as $item) {
			...
			if ($options['type'] == 'image') {
				...
				$html .= "\r\n                    <tr class='multi-product-item' data-" . $options['key'] . '=\'' . $item['goodsid'] . "' >\r\n                        <input type='hidden' class='form-control img-textname' readonly='' value='" . $item[$options['text']] . "'>\r\n                       <input type='hidden'  value='" . $item['goodsid'] . '\' name=\'' . $id . "'>\r\n                        <td style='width:80px;'>\r\n                            <img src='" . tomedia($item[$options['thumb']]) . "' style='width:70px;border:1px solid #ccc;padding:1px' onerror=\"this.src='../addons/wx_shop/static/images/nopic.png'\">\r\n                        </td>\r\n                        <td style='width:220px;'>" . $item[$options['text']] . "</td>\r\n                        <td>";
...
if (!function_exists('mobileUrl')) {
...

	{
		...
		$query = array_merge(array('m' => 'wx_shop'), $query);
...
if (!function_exists('tpl_form_field_image2')) {
...

	{
		...
		if (empty($default)) {
			...
			$default = '../addons/wx_shop/static/images/nopic.png';
		...
		if (!empty($value)) {
			...
			$val = '../addons/wx_shop/static/images/default-pic.jpg';
...
if (!function_exists('tpl_form_field_multi_image2')) {
...

	{
		...
		if (!defined('TPL_INIT_MULTI_IMAGE')) {
			...
			$s = "\r\n<script type=\"text/javascript\">\r\n\tfunction uploadMultiImage(elm) {\r\n\t\tvar name = \$(elm).next().val();\r\n\t\tutil.image( \"\", function(urls){\r\n\t\t\t\$.each(urls, function(idx, url){\r\n\t\t\t\t\$(elm).parent().parent().next().append('<div class=\"multi-item\"><img onerror=\"this.src=\\'../addons/wx_shop/static/images/nopic.png\\'; this.title=\\'图片未找到.\\'\" src=\"'+url.url+'\" class=\"img-responsive img-thumbnail\"><input type=\"hidden\" name=\"'+name+'[]\" value=\"'+url.attachment+'\"><em class=\"close\" title=\"删除这张图片\" onclick=\"deleteMultiImage(this)\">×</em></div>');\r\n\t\t\t});\r\n\t\t}, " . json_encode($options) . ");\r\n\t}\r\n\tfunction deleteMultiImage(elm){\r\n\t\trequire([\"jquery\"], function(\$){\r\n\t\t\t\$(elm).parent().remove();\r\n\t\t});\r\n\t}\r\n</script>";
		...
		if (is_array($value) && (0 < count($value))) {
			...
			foreach ($value as $row) {
				...
				$s .= "\r\n<div class=\"multi-item\">\r\n\t<img src=\"" . tomedia($row) . "\" onerror=\"this.src='../addons/wx_shop/static/images/nopic.png'; this.title='图片未找到.'\" class=\"img-responsive img-thumbnail\">\r\n\t<input type=\"hidden\" name=\"" . $name . '[]" value="' . $row . "\" >\r\n\t<em class=\"close\" title=\"删除这张图片\" onclick=\"deleteMultiImage(this)\">×</em>\r\n</div>";
...
if (!(function_exists('tpl_form_field_wxshop_daterange'))) {

	function tpl_form_field_wxshop_daterange($name, $value = array(), $time = false)
...
if (!(function_exists('tpl_form_field_wxshop_date'))) {

	function tpl_form_field_wxshop_date($name, $value = '', $withtime = false)
...
if (!(function_exists('tpl_goods_selector'))) {
...

	{
		...
		$path = IA_ROOT . '/addons/wx_shop/template/web_v3/util/tpl_goods_selector.html';
