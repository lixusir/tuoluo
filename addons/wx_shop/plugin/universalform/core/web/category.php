...
class Category_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		if (!empty($_GPC['catname'])) {
			...
			foreach ($_GPC['catname'] as $id => $catname ) {
				...
				if ($id == 'new') {
					pdo_insert('wx_shop_universalform_category', array('name' => $catname, 'uniacid' => $_W['uniacid']));
					$insert_id = pdo_insertid();
					plog('universalform.category.add', '添加万能表单分类 ID: ' . $insert_id);
				}
				 else {
					pdo_update('wx_shop_universalform_category', array('name' => $catname), array('id' => $id));
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_universalform_category') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id DESC');
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('wx_shop_universalform_category') . ' WHERE id = \'' . $id . '\' AND uniacid=' . $_W['uniacid'] . '');
		...
		pdo_delete('wx_shop_universalform_category', array('id' => $id));
