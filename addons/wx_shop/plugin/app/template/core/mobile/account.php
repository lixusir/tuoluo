...
class Account_WxShopPage extends AppMobilePage
{
	protected $key = '';
	protected $expire = 0;
	...
	public function login()
	{
		...
		$member = pdo_fetch('select id,openid,mobile,pwd,salt,avatar,nickname from ' . tablename('wx_shop_member') . ' where mobile=:mobile and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
	...
	public function register()
	{
		...
		$member = pdo_fetch('select id,openid,mobile,pwd,salt from ' . tablename('wx_shop_member') . ' where mobile=:mobile and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':uniacid' => $_W['uniacid']));
		...
		if (empty($salt)) {
			...
			while (1) {
				$count = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where salt=:salt limit 1', array(':salt' => $salt));
		...
		if (empty($member)) {
			...
			pdo_update('wx_shop_member', array('openid' => $openid, 'salt' => $salt, 'pwd' => md5($pwd . $salt), 'mobileverify' => 1), array('id' => $member['id']));
		}
	...
	public function changepwd()
	{
		...
		pdo_update('wx_shop_member', array('salt' => $salt, 'pwd' => md5($pwd . $salt)), array('id' => $member['id']));
	...
	public function changemobile()
	{
		...
		$newmember = pdo_fetch('select id,openid,mobile,pwd,salt from ' . tablename('wx_shop_member') . ' where mobile=:mobile and uniacid=:uniacid limit 1', array(':mobile' => $newmobile, ':uniacid' => $_W['uniacid']));
		...
		pdo_update('wx_shop_member', array('mobile' => $newmobile), array('id' => $member['id']));
