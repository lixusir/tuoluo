...
class Refund_WxShopPage extends AppMobilePage
{
	protected function globalData()
	{
		...
		if ($order['status'] == 2) {
			$goods = pdo_fetchall('select og.goodsid, og.price, og.total, og.optionname, g.cannotrefund, g.thumb, g.title from' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.orderid=' . $order['id']);
