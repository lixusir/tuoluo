...
class My_WxShopPage extends AppMobilePage
{
	public function getlist()
	{
		...
		$psize = 10;
		$time = time();
		$sql = 'select d.id,d.couponid,d.gettime,c.timelimit,c.coupontype,c.timedays,c.timestart,c.timeend,c.thumb,c.couponname,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.bgcolor,c.thumb,c.merchid,c.tagtitle,c.settitlecolor,c.titlecolor from ' . tablename('wx_shop_coupon_data') . ' d';
		...
		pdo_update('wx_shop_coupon_data', array('isnew' => 0), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		...
		foreach ($coupons as $i => &$row) {
			...
			$row['check'] = $check;
			$row['title2'] = $title2;
			$row['iconurl'] = $_W['siteroot'] . 'addons/wx_shop/template/mobile/default/static/images/coupon/' . $row['imgname'] . '.png';
			unset($row['timelimit']);
	...
	public function getdetail()
	{
		...
		$data = pdo_fetch('select * from ' . tablename('wx_shop_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		$num = pdo_fetchcolumn('select ifnull(count(*),0) from ' . tablename('wx_shop_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and used=0 ', array(':couponid' => $coupon['id'], ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
	...
	public function showcoupon2()
	{
		...
		$data = pdo_fetch('select c.*  from ' . tablename('wx_shop_coupon_data') . '  cd inner join  ' . tablename('wx_shop_coupon') . ' c on cd.couponid = c.id  where cd.id=:id and cd.uniacid=:uniacid and coupontype =0  limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
