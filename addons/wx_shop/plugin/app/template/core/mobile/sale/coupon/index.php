...
class Index_WxShopPage extends AppMobilePage
{
			...
			foreach ($list as $key => &$row) {
				...
				if ($row['islimitlevel'] == 1) {
					...
					if (!empty($row['limitmemberlevels']) || ($row['limitmemberlevels'] == '0')) {
						$level1 = pdo_fetchall('select * from ' . tablename('wx_shop_member_level') . ' where uniacid=:uniacid and  id in (' . $coupon['limitmemberlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
					...
					if ((!empty($coupon['limitagentlevels']) || ($coupon['limitagentlevels'] == '0')) && $hascommission) {
						$level2 = pdo_fetchall('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid and id  in (' . $coupon['limitagentlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
					...
					if ((!empty($coupon['limitpartnerlevels']) || ($coupon['limitpartnerlevels'] == '0')) && $hasglobonus) {
						$level3 = pdo_fetchall('select * from ' . tablename('wx_shop_globonus_level') . ' where uniacid=:uniacid and  id in(' . $coupon['limitpartnerlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
					...
					if ((!empty($coupon['limitaagentlevels']) || ($coupon['limitaagentlevels'] == '0')) && $hasabonus) {
						$level4 = pdo_fetchall('select * from ' . tablename('wx_shop_abonus_level') . ' where uniacid=:uniacid and  id in (' . $coupon['limitaagentlevels'] . ') ', array(':uniacid' => $_W['uniacid']));
	...
	public function pay($a = array(), $b = array())
	{
		...
		if ($goodsdetail) {
			...
			if (0 < $coupon['money']) {
				pdo_delete('wx_shop_coupon_log', array('couponid' => $id, 'openid' => $openid, 'status' => 0, 'paystatus' => 0));
				$needpay = true;
				$lastlog = pdo_fetch('select * from ' . tablename('wx_shop_coupon_log') . ' where couponid=:couponid and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(':couponid' => $id, ':openid' => $openid, ':uniacid' => $_W['uniacid']));
				...
				pdo_delete('wx_shop_coupon_log', array('couponid' => $id, 'openid' => $openid, 'status' => 0));
			}
		}
		...
		if ($goodsdetail) {
			...
			if ($needpay) {
				...
				pdo_update('wx_shop_coupon_log', array('paytype' => $useweixin ? 1 : 0), array('id' => $logid));
	...
	public function payresult($a = array())
	{
		...
		$log = pdo_fetch('select id,logno,status,paystatus,couponid from ' . tablename('wx_shop_coupon_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid']));
	...
	public function getCouponCate()
	{
		...
		$param = array();
		$param[':uniacid'] = $_W['uniacid'];
		$sql = 'select * from ' . tablename('wx_shop_coupon_category') . ' where uniacid=:uniacid';
	...
	public function getlist()
	{
		...
		$param = array();
		$param[':uniacid'] = $_W['uniacid'];
		$sql = 'select id,timelimit,coupontype,timedays,timestart,timeend,thumb,couponname,enough,backtype,deduct,discount,backmoney,backcredit,backredpack,bgcolor,thumb,credit,money,getmax,merchid,total as t,tagtitle,settitlecolor,titlecolor from ' . tablename('wx_shop_coupon') . ' c ';
	...
	public function getdetail()
	{
		...
		if ($coupon['limitgoodtype'] != 0) {
			...
			$goods = pdo_fetchall('select `title` from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid ' . $where, array(':uniacid' => $_W['uniacid']), 'id');
		}
		...
		if ($coupon['limitgoodcatetype'] != 0) {
			...
			$category = pdo_fetchall('select `name`  from ' . tablename('wx_shop_category') . ' where uniacid=:uniacid   ' . $where, array(':uniacid' => $_W['uniacid']), 'id');
		}
