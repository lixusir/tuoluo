...
class Recharge_WxShopPage extends AppMobilePage
{
	...
	public function submit()
	{
		...
		pdo_delete('wx_shop_member_log', array('openid' => $_W['openid'], 'status' => 0, 'type' => 0, 'uniacid' => $_W['uniacid']));
		...
		pdo_insert('wx_shop_member_log', $log);
		$logid = pdo_insertid();
		$type = $_GPC['type'];
	...
	public function wechat_complete()
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_log') . ' WHERE `id`=:id and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $logid));

		if (empty($log)) {
			$logno = intval($_GPC['logno']);
			$log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
		}
		...
		if (!empty($log)) {
			...
			if (!is_error($payquery)) {
				if (empty($log['status'])) {
					pdo_update('wx_shop_member_log', array('status' => 1, 'rechargetype' => 'wechat', 'apppay' => is_h5app() ? 1 : 0), array('id' => $logid));
	...
	public function alipay_complete()
	{
		...
		if (!empty($log)) {
			...
			if ($alisign) {
				if (empty($log['status'])) {
					pdo_update('wx_shop_member_log', array('status' => 1, 'rechargetype' => 'alipay', 'apppay' => 2), array('id' => $logid));
					m('member')->setCredit($log['openid'], 'credit2', $log['money'], array(0, $_W['shopset']['shop']['name'] . '会员充值:wechatcomplete:credit2:' . $log['money']));
					m('member')->setRechargeCredit($log['openid'], $log['money']);
					com_run('sale::setRechargeActivity', $log);
					com_run('coupon::useRechargeCoupon', $log);
