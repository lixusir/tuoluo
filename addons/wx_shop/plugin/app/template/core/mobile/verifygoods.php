...
class verifygoods_WxShopPage extends AppMobilePage
{
	public function get_list()
	{
		...
		$psize = 10;
		$time = time();
		$sql = 'select vg.*,g.title,g.subtitle,g.thumb,c.card_id  from ' . tablename('wx_shop_verifygoods') . '   vg' . "\r\n\t\t" . ' inner join ' . tablename('wx_shop_order_goods') . ' og on vg.ordergoodsid = og.id' . "\r\n\t\t" . ' left  join ' . tablename('wx_shop_order') . ' o on vg.orderid = o.id' . "\r\n\t\t" . ' left  join ' . tablename('wx_shop_order_refund') . ' orf on o.refundid = orf.id' . "\r\n\t\t" . ' inner join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id' . "\r\n\t\t" . ' left  join ' . tablename('wx_shop_goods_cards') . ' c on c.id = g.cardid' . "\r\n\t\t" . ' where   vg.uniacid=:uniacid and vg.openid=:openid and vg.invalid =0 and (o.refundid=0 or orf.status<0) and o.status>0';
	...
	public function get_detail()
	{
		...
		if (empty($verifycode) || ($item['codeinvalidtime'] < time())) {
			...
			while (1) {
				$count = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_verifygoods') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
			...
			pdo_update('wx_shop_verifygoods', $data, array('id' => $item['id']));
		}
