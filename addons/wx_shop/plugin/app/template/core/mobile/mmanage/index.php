...
class Index_WxShopPage extends AppMobileAuthPage
{
	...
	{
		global $_W;
		$notices = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_system_copyright_notice') . ' WHERE status=1 ORDER BY displayorder ASC,createtime DESC LIMIT 10');
		$order = $this->order(0);
		$member = $this->newmember(0);
		$goods = m('goods')->getTotals();
		$goodscount = $goods['sale'] + $goods['out'] + $goods['stock'] + $goods['cycle'];
		$member_count = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$result = array(
	...
	private function selectOrderPrice($day = 0)
	{
		...
		$sql = 'select id,price,createtime from ' . tablename('wx_shop_order') . ' where uniacid = :uniacid and ismr=0 and isparent=0 and (status > 0 or ( status=0 and paytype=3)) and deleted=0 and createtime between :createtime1 and :createtime2';
