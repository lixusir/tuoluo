...
class Order_WxShopPage extends AppMobileAuthPage
{
	...
	public function get_list()
	{
		...
		if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
			...
			if ($searchfield == 'ordersn') {
				...
				$sqlcondition = ' left join ' . tablename('wx_shop_store') . ' store on store.id = o.verifystoreid and store.uniacid=o.uniacid';
			}
			else if ($searchfield == 'goodstitle') {
				$sqlcondition = ' inner join ( select  og.orderid from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id';
			}
			else if ($searchfield == 'goodssn') {
				$sqlcondition = ' inner join ( select og.orderid from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0))) gs on gs.orderid=o.id';
			}
			else if ($searchfield == 'goodsoptiontitle') {
				$sqlcondition = ' inner join ( select  og.orderid from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and (locate(:keyword,og.optionname)>0)) gs on gs.orderid=o.id';
			}
			else {
				if ($searchfield == 'merch') {
					if ($merch_plugin) {
						$condition .= ' AND (locate(:keyword,merch.merchname)>0)';
						$sqlcondition = ' left join ' . tablename('wx_shop_merch_user') . ' merch on merch.id = o.merchid and merch.uniacid=o.uniacid';
					}
				}
			}
		}
		...
		if (($condition != ' o.uniacid = :uniacid and and o.deleted=0 and o.isparent=0') || !empty($sqlcondition)) {
			...
			$sql = 'select * from ' . tablename('wx_shop_order') . ' where uniacid = :uniacid and deleted=0 and isparent=0 ' . $status_condition . ' GROUP BY id ORDER BY createtime DESC  ';
			...
			if (!empty($list)) {
				...
				$refundid_array = pdo_fetchall('SELECT id,rtype,status as rstatus FROM ' . tablename('wx_shop_order_refund') . ' WHERE id IN (' . $refundid . ')', array(), 'id');
				$openid_array = pdo_fetchall('SELECT openid,nickname,id as mid,realname as mrealname,mobile as mmobile FROM ' . tablename('wx_shop_member') . ' WHERE openid IN (' . $openid . ') AND uniacid=' . $_W['uniacid'], array(), 'openid');
				$addressid_array = pdo_fetchall('SELECT id,realname as arealname,mobile as amobile,province as aprovince ,city as acity , area as aarea,address as aaddress FROM ' . tablename('wx_shop_member_address') . ' WHERE id IN (' . $addressid . ')', array(), 'id');
				$dispatchid_array = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('wx_shop_dispatch') . ' WHERE id IN (' . $dispatchid . ')', array(), 'id');
				$verifyopenid_array = pdo_fetchall('SELECT sm.id as salerid,sm.nickname as salernickname,sm.openid,s.salername FROM ' . tablename('wx_shop_saler') . ' s LEFT JOIN ' . tablename('wx_shop_member') . ' sm ON sm.openid = s.openid and sm.uniacid=s.uniacid WHERE s.openid IN (' . $verifyopenid . ')', array(), 'openid');
			...
			foreach ($list as &$value) {
				...
				if (!empty($agentid) && (0 < $level)) {
					...
					if ($value['agentid'] == $agentid) {
						$value['level'] = 1;
						$level1_commissions = pdo_fetchall('select commission1,commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where og.orderid=:orderid and o.agentid= ' . $agentid . '  and o.uniacid=:uniacid', array(':orderid' => $value['id'], ':uniacid' => $uniacid));
						...
						if (0 < $agent['level2']) {
							$level2_commissions = pdo_fetchall('select commission2,commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where og.orderid=:orderid and  o.agentid in ( ' . implode(',', array_keys($agent['level1_agentids'])) . ')  and o.uniacid=:uniacid', array(':orderid' => $value['id'], ':uniacid' => $uniacid));
						...
						if (in_array($value['agentid'], array_keys($agent['level2_agentids']))) {
							...
							if (0 < $agent['level3']) {
								$level3_commissions = pdo_fetchall('select commission3,commissions from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where og.orderid=:orderid and  o.agentid in ( ' . implode(',', array_keys($agent['level2_agentids'])) . ')  and o.uniacid=:uniacid', array(':orderid' => $value['id'], ':uniacid' => $uniacid));
		...
		unset($value);
		if (($condition != ' o.uniacid = :uniacid and and o.deleted=0 and o.isparent=0') || !empty($sqlcondition)) {
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('wx_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . ' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition, $paras);
		}
		else {
			$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('wx_shop_order') . ' WHERE uniacid = :uniacid and deleted=0 and isparent=0 ' . $status_condition, $paras);
		}
	...
	public function get_detail()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if (0 < $item['sendtype']) {
			$order_goods = pdo_fetchall('SELECT orderid,goodsid,sendtype,expresssn,expresscom,express,sendtime FROM ' . tablename('wx_shop_order_goods') . "\r\n            WHERE orderid = " . $id . ' and sendtime > 0 and uniacid=' . $_W['uniacid'] . ' and sendtype > 0 group by sendtype ');

			foreach ($order_goods as $key => $value) {
				$order_goods[$key]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $value['sendtype'] . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $id));
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_dispatch') . ' WHERE id = :id and uniacid=:uniacid and merchid=0', array(':id' => $item['dispatchid'], ':uniacid' => $_W['uniacid']));
		...
		if (empty($item['addressid'])) {
			...
			if (!is_array($user_address)) {
				$user_address = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
			}
		...
		if (com('verify')) {
			...
			if (!empty($item['verifyopenid'])) {
				...
				$saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('wx_shop_saler') . ' where openid=:openid and uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $item['verifyopenid']));
			}

			if (!empty($item['verifystoreid'])) {
				$store = pdo_fetch('select * from ' . tablename('wx_shop_store') . ' where id=:storeid limit 1 ', array(':storeid' => $item['verifystoreid']));
			}
					...
					if (empty($item['dispatchtype'])) {
						foreach ($verifyinfo as &$v) {
							if ($v['verified'] || ($item['verifytype'] == 1)) {
								$v['storename'] = pdo_fetchcolumn('select storename from ' . tablename('wx_shop_store') . ' where id=:id limit 1', array(':id' => $v['verifystoreid']));

								if (empty($v['storename'])) {
									$v['storename'] = '总店';
								}

								$v['nickname'] = pdo_fetchcolumn('select nickname from ' . tablename('wx_shop_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid']));
								$v['salername'] = pdo_fetchcolumn('select salername from ' . tablename('wx_shop_saler') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid']));
							}
						}
	...
	public function remarksaler()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('wx_shop_order', array('remarksaler' => $remarksaler), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
	...
	public function payorder()
	{
		...
		if (!empty($item['virtual']) && com('virtual')) {
			...
			pdo_update('wx_shop_order', array('status' => 1, 'paytype' => 11, 'paytime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function send()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($sendtype)) {
				...
				$ogoods = array();
				$ogoods = pdo_fetchall('select sendtype from ' . tablename('wx_shop_order_goods') . "\r\n                    where orderid = " . $item['id'] . ' and uniacid = ' . $_W['uniacid'] . ' order by sendtype desc ');
				...
				if ($goodsids) {
					foreach ($goodsids as $key => $value) {
						pdo_update('wx_shop_order_goods', $data, array('orderid' => $item['id'], 'goodsid' => $value, 'uniacid' => $_W['uniacid']));
					}
				}

				$send_goods = pdo_fetch('select * from ' . tablename('wx_shop_order_goods') . "\r\n                    where orderid = " . $item['id'] . ' and sendtype = 0 and uniacid = ' . $_W['uniacid'] . ' limit 1 ');

				if (empty($send_goods)) {
					$senddata['status'] = 2;
				}

				pdo_update('wx_shop_order', $senddata, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
			else {
				$data['status'] = 2;
				pdo_update('wx_shop_order', $data, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			}
			...
			if (!empty($item['refundid'])) {
				...
				if (!empty($refund)) {
					pdo_update('wx_shop_order_refund', array('status' => -1, 'endtime' => $time), array('id' => $item['refundid']));
		...
		if (0 < $item['sendtype']) {
			$noshipped = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = 0 and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			$i = 1;

			while ($i <= $item['sendtype']) {
				$row = array('sendtype' => $i, 'goods' => pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.sendtype = ' . $i . ' and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id'])));
		...
		$order_goods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
		$order_goods = set_medias($order_goods, 'thumb');
	...
	public function sendcancel()
	{
			...
			if (0 < $item['sendtype']) {
				...
				if (is_array($sendtype)) {
					foreach ($sendtype as $key => $value) {
						$data['sendtype'] = 0;
						pdo_update('wx_shop_order_goods', $data, array('orderid' => $item['id'], 'sendtype' => $value, 'uniacid' => $_W['uniacid']));
						$order = pdo_fetch('select sendtype from ' . tablename('wx_shop_order') . ' where id = ' . $item['id'] . ' and uniacid = ' . $_W['uniacid'] . ' ');
						pdo_update('wx_shop_order', array('sendtype' => $order['sendtype'] - 1, 'status' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
					}
				}
			}
			else {
		...
		if (0 < $sendtype) {
			$sendgoods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $sendtype . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			...
			if (0 < $item['sendtype']) {
				...
				while ($i <= intval($item['sendtype'])) {
					$bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.optionname,og.total from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $i . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
	...
	public function changeaddress()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('wx_shop_order', array('address' => $address_array), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
	...
	public function changeexpress()
	{
		...
		if ($_W['ispost']) {
			...
			if (0 < $item['sendtype']) {
				...
				if (is_array($sendtype)) {
					foreach ($sendtype as $key => $value) {
						pdo_update('wx_shop_order_goods', $change_data, array('orderid' => $orderid, 'sendtype' => $value, 'uniacid' => $_W['uniacid']));
					}
				}
			}
			else {
				pdo_update('wx_shop_order', $change_data, array('id' => $orderid, 'uniacid' => $_W['uniacid']));
			}
		...
		if (0 < $sendtype) {
			$sendgoods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $sendtype . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
			$sendgoods = set_medias($sendgoods, 'thumb');
		}
		else {
			if (0 < $item['sendtype']) {
				...
				while ($i <= intval($item['sendtype'])) {
					$bundle = array('goods' => pdo_fetchall('select g.id,g.title,og.optionname,og.total,g.thumb,og.sendtype,g.ispresell from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid and og.sendtype=' . $i . ' ', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id'])), 'sendtype' => $i);
	...
	public function finish()
	{
		...
		$orderid = intval($_GPC['id']);
		$item = $this->getOrder($orderid);
		pdo_update('wx_shop_order', array('status' => 3, 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
