...
class Login_WxShopPage extends AppMobilePage
{
	protected $appid;
	protected $appsecret;
	...
	public function submit()
	{
		...
		if ($type == 'wechat') {
			...
			$openid = $arr['openid'];
			$roleuser = pdo_fetch('SELECT id, uid, uniacid, username, status, openid_wa FROM' . tablename('wx_shop_perm_user') . 'WHERE openid_wa=:openid LIMIT 1', array(':openid' => $openid));
			...
			if ($role == 'operator') {
				$roleuser = pdo_fetch('SELECT id, uid, uniacid FROM' . tablename('wx_shop_perm_user') . 'WHERE uid=:uid LIMIT 1', array(':uid' => $account['uid']));
