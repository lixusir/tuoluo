...
class Goods_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'SELECT g.id FROM ' . tablename('wx_shop_goods') . 'g' . $sqlcondition . $condition;
		$total_all = pdo_fetchall($sql, $params);
		$total = count($total_all);
		unset($total_all);

		if (!empty($total)) {
			$sql = 'SELECT g.id, g.title, g.createtime, g.marketprice, g.thumb FROM ' . tablename('wx_shop_goods') . 'g' . $sqlcondition . $condition . " ORDER BY g.`status` DESC, g.`displayorder` DESC,\r\n                g.`id` DESC LIMIT " . (($pindex - 1) * $psize) . ',' . $psize;
