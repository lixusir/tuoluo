...
class Page_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT id, `name`, `type`, createtime, lasttime, status, isdefault FROM ' . tablename('wx_shop_wxapp_page') . ' WHERE ' . $condition . ' ORDER BY isdefault DESC, id DESC' . $limit, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_wxapp_page') . ' WHERE ' . $condition, $params);
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			if (!(empty($isdefault))) {
				$arr['status'] = 1;
				$arr['isdefault'] = 1;
				pdo_update('wx_shop_wxapp_page', array('isdefault' => 0), array('uniacid' => $_W['uniacid']));
			}


			if (!(empty($id))) {
				pdo_update('wx_shop_wxapp_page', $arr, array('id' => $id, 'uniacid' => $_W['uniacid']));
			}
			 else {
				$arr['uniacid'] = $_W['uniacid'];
				$arr['createtime'] = time();
				pdo_insert('wx_shop_wxapp_page', $arr);
				$id = pdo_insertid();
			}
		...
		$diyadvs = pdo_fetchall('select id, `name` from ' . tablename('wx_shop_wxapp_startadv') . ' where status=1 and uniacid=:uniacid  order by id desc', array(':uniacid' => $_W['uniacid']));
	...
	public function status()
	{
		...
		$list = pdo_fetchall('SELECT id, `name`, isdefault FROM ' . tablename('wx_shop_wxapp_page') . ' WHERE id in( ' . $id . ' ) AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		...
		if (!(empty($list))) {
			foreach ($list as $row ) {
				...
				pdo_update('wx_shop_wxapp_page', array('status' => $status), array('id' => $row['id'], 'uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		if (!(empty($list))) {
			foreach ($list as $row ) {
				...
				pdo_delete('wx_shop_wxapp_page', array('id' => $row['id'], 'uniacid' => $_W['uniacid']));
	...
	public function setdefault()
	{
		...
		$item = pdo_fetch('SELECT id, `name`, `status`, isdefault FROM ' . tablename('wx_shop_wxapp_page') . ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		pdo_update('wx_shop_wxapp_page', array('isdefault' => 0), array('uniacid' => $_W['uniacid']));
		pdo_update('wx_shop_wxapp_page', array('isdefault' => 1), array('id' => $id, 'uniacid' => $_W['uniacid']));
