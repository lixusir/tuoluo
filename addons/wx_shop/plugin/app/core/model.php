...
if (!(class_exists('AppModel'))) {
	class AppModel extends PluginModel
	{
		...
		public function wxpay($params, $type = 0)
		{
			...
			$package['notify_url'] = $_W['siteroot'] . 'addons/wx_shop/payment/wechat/notify.php';
		...
		public function getAccessToken()
		{
			...
			$cacheKey = 'wxshop:wxapp:accesstoken:' . $_W['uniacid'];
		...
		{
			global $_W;
			$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_wxapp_tmessage') . ' WHERE id=:id AND uniacid=:uniacid AND status=1 LIMIT 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		public function getGlobal()
		{
			...
			$path = IA_ROOT . '/addons/wx_shop/data/global';
		...
		public function getPage($id = 0, $mobile = false)
		{
			...
			$page = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_wxapp_page') . $where . ' LIMIT 1 ', $params);
								...
								if (!(empty($item['data']))) {
									...
									if (($item['params']['saleout'] == 0) && !(empty($_W['shopset']['shop']['saleout']))) {
										...
										if (empty($item['params']['saleout'])) {
											$item['params']['saleout'] = tomedia('../addons/wx_shop/plugin/diypage/static/images/default/saleout-2.png');
										}

									}
									 else if (($item['params']['saleout'] == 1) && empty($item['params']['saleout'])) {
										$item['params']['saleout'] = tomedia('../addons/wx_shop/plugin/diypage/static/images/default/saleout-' . $item['style']['saleoutstyle'] . '.png');
									}
			...
			if ($item['id'] == 'goods') {
				if (empty($item['params']['goodsdata']) && !(empty($item['data'])) && is_array($item['data'])) {
					...
					if (!(empty($goodsids)) && is_array($goodsids)) {
						...
						$goods = pdo_fetchall('select id, title, subtitle, thumb, minprice, sales, salesreal, total, showlevels, showgroups, bargain, productprice, video from ' . tablename('wx_shop_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
					}
					...
					$group = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods_group') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['groupid'], ':uniacid' => $_W['uniacid']));
					...
					if (!(empty($group)) && !(empty($group['goodsids']))) {
						...
						$item['params']['goodsnum'] = ((!(empty($item['params']['goodsnum'])) ? $item['params']['goodsnum'] : 20));
						$goods = pdo_fetchall('select id, title, subtitle, thumb, minprice, sales, salesreal, total, showlevels, showgroups, bargain, productprice, video from ' . tablename('wx_shop_goods') . ' where id in( ' . $group['goodsids'] . ' ) and status=1 and `deleted`=0 and `status`=1 and uniacid=:uniacid ' . $orderby . ' limit ' . $item['params']['goodsnum'], array(':uniacid' => $_W['uniacid']));
					}
		...
		public function getAuth()
		{
			...
			//cc_zhong
			load()->func('communication');
			$request = ihttp_get(WX_SHOP_AUTH_WXAPP . 'index/index/getAuth.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].'&title='.$_W['account']['name'].$uri);

			if ($request['code'] != 200) {
				// return error(-1, '信息查询失败！稍后重试');
				return error(-1, '发布与审核请联系产品经理!');
			}
		...
		global $_W;
			...
			load()->func('communication');
			$request = ihttp_get(WX_SHOP_AUTH_WXAPP . 'index/index/getRelease.html?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid'].$uri);
		...
		public function getReleaseList()
		{
			...
			load()->func('communication');
			$request = ihttp_get(WX_SHOP_AUTH_WXAPP . 'index/index/getReleasList?site_id=' . SITE_ID . '&uniacid=' . $_W['uniacid']);
		...
		{
			load()->func('communication');
			$request = ihttp_get(WX_SHOP_AUTH_URL . 'wxapp_log?pindex=' . $page . '&psize=' . $psize);
			$content = json_decode($request['content'], true);
			return $content;
		}
		...
		{
			$path1 = __DIR__ . '/mobile/wx_shop_api.php';
			$path2 = IA_ROOT . '/app/wx_shop_api.php';
		...
		public function createAuth()
		{
			...
			$auth = get_auth();
			load()->func('communication');
			$resp = ihttp_request(WX_SHOP_AUTH_URL . 'app', array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain, 'appid' => $_W['account']['key']), NULL, 1);
		...
		public function diyMenu($menuid)
		{
			...
			$menu = pdo_fetch('select * from ' . tablename('wx_shop_diypage_menu') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	'items'    => $menu
		...
		public function getUrl($url)
		{
			...
			if (strexists($url, './index.php?') && strexists($url, 'wx_shop') && strexists($url, 'mobile')) {
				$parse = parse_url($url);
				...
				if (($newVars['m'] != 'wx_shop') || ($newVars['do'] != 'mobile')) {
		...
		{
			$file = WX_SHOP_PLUGIN . 'app/static/iconfont.json';
		...
		{
			global $_W;
			$arr = array('app_id' => $config['appid'], 'method' => 'alipay.trade.app.pay', 'format' => 'JSON', 'charset' => 'utf-8', 'sign_type' => 'RSA2', 'timestamp' => date('Y-m-d H:i:s', time()), 'version' => '1.0', 'notify_url' => $_W['siteroot'] . 'addons/wx_shop/payment/alipay/notify.php', 'biz_content' => json_encode(array('timeout_express' => '90m', 'product_code' => 'QUICK_MSECURITY_PAY', 'total_amount' => $params['total_amount'], 'subject' => $params['subject'], 'body' => $params['body'], 'out_trade_no' => $params['out_trade_no'])));
			ksort($arr);
			$string1 = '';
