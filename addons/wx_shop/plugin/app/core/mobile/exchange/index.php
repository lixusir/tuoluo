...
class Index_WxShopPage extends PluginMobileLoginPage
{
	...
	public function main() 
	{
		...
		// $exchangeGroupId = trim($_GPC['exchangeGroupId']);
		// @session_start(); //开启session
		if (!(empty($all)) && !(empty($codetype))) 
		{
			// include $this->template('exchange/common');
			// exit();
			show_json(1, array('url'=>'exchange/common'));
		}
		else if (!(empty($all)) && empty($codetype)) 
		{
			$keyresult = pdo_fetch('SELECT groupid FROM ' . tablename('wx_shop_exchange_code') . ' WHERE `key` = :code AND uniacid = :uniacid', array(':code' => $key, ':uniacid' => $_W['uniacid']));
			if (empty($keyresult)) 
			{
		...
		if (!(empty($codetype))) 
		{
			$res = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE id = :id AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':id' => $id));
			...
			if ($plugin_diypage) 
			{
				...
				if ($diypage) 
				{
					...
					// exit();
					show_json(1, 'diypage/exchange');
				}
			}
			show_json(1, 'exchange/common');
			// include $this->template('exchange/common');
			// exit();
		}
		...
		// }
		$is_exchange = $this->is_exchange($key, $exchangeGroupId);
		if ($is_exchange[0] === '1') 
		{
	...
	/**
	* exchangeGroupId  缓存的组id
	*/
	private function is_exchange($key, $exchangeGroupId=NULL) 
	{
		global $_W;
		$set = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_setting') . ' WHERE uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
		//统计错误
		$this->counterror($set);
		$time = strtotime('now');
		$time = date('Y-m-d');
		...
		{
			$exchangelimit = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_record') . ' WHERE openid =:openid AND uniacid = :uniacid AND `time` > :timea AND `time` <= :timeb', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':timea' => $time1, ':timeb' => $time2));
		...
		{
			// $exchangelimit2 = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_record') . ' WHERE openid =:openid AND uniacid = :uniacid AND `time` > :timea AND `time` <= :timeb AND groupid = :groupid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':timea' => $time1, ':timeb' => $time2, ':groupid' => $_SESSION['exchangeGroupId']));
			$exchangelimit2 = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_record') . ' WHERE openid =:openid AND uniacid = :uniacid AND `time` > :timea AND `time` <= :timeb AND groupid = :groupid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':timea' => $time1, ':timeb' => $time2, ':groupid' => $exchangeGroupId));
		...
		$return = array();
		$table1 = tablename('wx_shop_exchange_group');
		$table2 = tablename('wx_shop_exchange_code');
		...
		if ($exchangeGroupId != $codeResult['groupid']) 
		{
			$return = array('0', '兑换码不存在或已失效');
			pdo_query('UPDATE ' . tablename('wx_shop_exchange_query') . ' SET `errorcount` = `errorcount` + 1 WHERE openid = :openid', array(':openid' => $_W['openid']));
			return $return;
		}
		if ($codeResult === false) 
		{
		...
		pdo_query('UPDATE ' . tablename('wx_shop_exchange_query') . ' SET `errorcount` = 0 AND `unfreeze`=0 WHERE openid = :openid', array(':openid' => $_W['openid']));
		...
		{
			if (p('exchange')) {
				p('exchange')->checkRepeatExchange($key);
			}
		}
		...
		pdo_query('UPDATE ' . tablename('wx_shop_exchange_code') . ' SET openid = :openid , `count` = `count` + 1 WHERE openid != :openid AND uniacid = :uniacid AND `key`=:key', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid'], ':key' => $key));
	...
	public function balance() 
	{
		global $_W;
		global $_GPC;
		// $key = $_SESSION['exchange_key'] ? $_SESSION['key'] : trim($_GPC['key']);
		$key = trim($_GPC['key']);
		$exchangeGroupId = trim($_GPC['exchangeGroupId']);
		if (empty($key)) 
		{
		...
		if ($exchange == '1') 
		{
			...
			if ($balance_res === 1) 
			{
				...
				{
					pdo_update('wx_shop_exchange_code', array('status' => 2), array('key' => $key, 'uniacid' => $_W['uniacid'], 'status' => 1, 'repeatcount' => 1));
				}
				...
				pdo_insert('wx_shop_exchange_record', $record);
		...
		else 
		{
			...
			show_json(1, 'exchange.balance&exchange=1');
		}
	}
	public function score() 
	{
		...
		else 
		{
			...
			show_json(1, 'exchange.score&exchange=1');
		}
	}
	public function redpacket() 
	{
		...
		else 
		{
			...
			show_json(1, 'exchange.redpacket&exchange=1');
		}
	}
	public function coupon() 
	{
		...
		if ($exchange == '1') 
		{
			...
			$allCoupon = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE ' . $condition . ' and uniacid=:uniacid and merchid=0', array(':uniacid' => $_W['uniacid']));
		...
		else 
		{
			...
			show_json(1, 'exchange.coupon&exchange=1');
		}
	}
	public function goods() 
	{
		...
		else 
		{
			...
			if ($goods_arr['goods'] != false) 
			{
				...
				{
					$goodsList[$k] = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $v, ':uniacid' => $_W['uniacid']));
				}
			}
			if ($goods_arr['option'] != false) 
			{
				...
				{
					$optionList[$k] = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $k, ':uniacid' => $_W['uniacid']));
					$optionstr = implode('-', $v);
					$optionList[$k]['optionstr'] = $optionstr;
					$optionList[$k]['is_select'] = false;
					$optionList[$k]['thumb'] =  tomedia($optionList[$k]['thumb']);
				}
			}
			...
			if(!empty($groupResult['img'])){
				$groupResult['img'] = tomedia($groupResult['img']);
			}
			if(count($goodsList) > 0 ){
				foreach ($goodsList as $k => $v) {
					if(!empty($v['thumb'])){
						$goodsList[$k]['thumb'] = tomedia($v['thumb']);
						$goodsList[$k]['is_select'] = false;
					}
				}	
			}
			show_json(1, array('banner'=>$banner, 'key'=>$key, 'groupResult'=>$groupResult, 'codeResult'=>$codeResult, 'goodsList'=>$goodsList, 'optionList'=>$optionList));
		}
	}
	public function group() 
	{
		...
		if ($exchange == '1') 
		{
			...
			if ($balance_res === 1) 
			{
				pdo_update('wx_shop_exchange_code', array('balancestatus' => 2), array('key' => $key, 'uniacid' => $_W['uniacid']));
				$info = m('member')->getInfo($_W['openid']);
				$record_exist = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], 'key' => $key));
				...
				else 
				{
					...
					pdo_update('wx_shop_exchange_record', $record, array('key' => $key, 'uniacid' => $_W['uniacid']));
				}
				...
				{
					pdo_update('wx_shop_exchange_code', array('status' => 2), array('key' => $key, 'uniacid' => $_W['uniacid']));
					pdo_query('UPDATE ' . tablename('wx_shop_exchange_group') . ' SET `use` = `use` + 1 WHERE uniacid = :uniacid AND `id` = :id', array(':uniacid' => $_W['uniacid'], ':id' => $codeResult['groupid']));
				}
		...
		else if ($exchange == '2') 
		{
			...
			if (!(is_error($result))) 
			{
				pdo_update('wx_shop_exchange_code', array('redstatus' => 2), array('key' => $key, 'uniacid' => $_W['uniacid']));
				$info = m('member')->getInfo($_W['openid']);
				$record_exist = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], 'key' => $key));
				if (empty($record_exist)) 
				{
		...
		else if ($exchange == '3') 
		{
			...
			if ($balance_res === 1) 
			{
				pdo_update('wx_shop_exchange_code', array('scorestatus' => 2), array('key' => $key, 'uniacid' => $_W['uniacid']));
				$info = m('member')->getInfo($_W['openid']);
				$record_exist = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], 'key' => $key));
				if (empty($record_exist)) 
				{
		...
		else if ($exchange == '4') 
		{
			...
			$allCoupon = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE ' . $condition . ' and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
			if (empty($allCoupon[0])) 
			{
				show_json(0, '没有可兑换的优惠券');
				// $this->message('没有可兑换的优惠券');
			...
			$this->model->sendMessage($resp, 1, $m);
			pdo_update('wx_shop_exchange_code', array('couponstatus' => 2), array('key' => $key, 'uniacid' => $_W['uniacid']));
			$info = m('member')->getInfo($_W['openid']);
			$record_exist = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], 'key' => $key));
			if (empty($record_exist)) 
			{
		...
		else if ($exchange == '5') 
		{
			...
			show_json(1, array(
				...
				'key' => $key ? $key : '',
				'exchangeGroupId' => $exchangeGroupId ? $exchangeGroupId : '',
	...
	public function modal() 
	{
		...
		$goods = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (!empty($goods['thumb'])) {
			if (!preg_match('/^http/', $goods['thumb'])) {
				$goods['thumb'] = $_W['siteroot'].'attachment/'.$goods['thumb'];
			}
		}
		...
		foreach ($optionid as $k => $v ) 
		{
			...
			$option[$k] = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods_option') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $v, ':uniacid' => $_W['uniacid']));
			$option[$k]['is_select'] = false;
		}
		show_json(1, array(
			'goods' =>  $goods,
			// 'optionstr' =>  $optionstr,
	...
	public function calculate() 
	{
		...
		$dada = '';
		// @session_start();
		pdo_delete('wx_shop_exchange_cart', array('openid' => $_W['openid'], 'selected' => 1));
		// $key = $_SESSION['exchange_key'];
		$key = $_GPC['key'];
		$exchange = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_code') . ' WHERE `key` = :key AND uniacid = :uniacid', array(':key' => $key, ':uniacid' => $_W['uniacid']));
		$group = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $exchange['groupid'], ':uniacid' => $_W['uniacid']));
		...
		{
			$price = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE id = :id AND uniacid=:uniacid', array(':id' => $gv, ':uniacid' => $_W['uniacid']));
			
			$insert_cart = pdo_insert('wx_shop_exchange_cart', array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'goodsid' => $price['id'], 'marketprice' => $price['marketprice'], 'optionid' => 0, 'merchid' => $price['merchid'], 'title' => $price['title'], 'groupid' => $exchange['groupid'], 'serial' => $exchange['serial']));
		...
		if(is_array($_GPC['option'])){
			...
			foreach ($option_str as $k => $v ) 
			{
				...
				{
					$price = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods_option') . ' WHERE id = :id AND uniacid=:uniacid', array(':id' => $v2, ':uniacid' => $_W['uniacid']));
					pdo_insert('wx_shop_exchange_cart', array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'goodsid' => $price['goodsid'], 'marketprice' => $price['marketprice'], 'optionid' => $price['id'], 'merchid' => $price['merchid'], 'title' => $price['title'], 'groupid' => $exchange['groupid'], 'serial' => $exchange['serial']));
		...
		$dada['exchangegoods'] = json_encode($goods);
		unset($price);
	...
	public function groupexchange() 
	{
		...
		global $_W;
		$key = trim($_GPC['key']);
		$code = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_code') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':key' => $key, ':uniacid' => $_W['uniacid']));
		if ($code == false) 
		{
			show_json(0, '没有这个兑换码');
		}
		$group = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE id =:id AND uniacid = :uniacid', array(':id' => $code['groupid'], ':uniacid' => $_W['uniacid']));
		if ($group == false) 
		{
			show_json(0, '没有这个兑换组');
		}
	...
	public function counterror($set) 
	{
		...
		$query = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_query') . ' WHERE openid = :openid AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		if (empty($query)) 
		{
			pdo_insert('wx_shop_exchange_query', array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'querytime' => time()));
		}
		if ($query['querytime'] < strtotime(date('Y-m-d', time()) . ' 00:00:00')) 
		{
			pdo_update('wx_shop_exchange_query', array('errorcount' => 0, 'querytime' => time()));
		}
		...
		{
			pdo_update('wx_shop_exchange_query', array('unfreeze' => time() + ($set['freeze'] * 86400)), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
			pdo_update('wx_shop_exchange_query', array('errorcount' => 0, 'unfreeze' => time() + ($set['freeze'] * 86400)), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
	...
	*/
	public function findgroupid()
	{
		...
		$table2 = tablename('wx_shop_exchange_code');
		$result = pdo_fetch('SELECT groupid FROM ' . $table2 . ' WHERE uniacid = :uniacid AND `key`=:key ', array(':key' => $key, ':uniacid' => $_W['uniacid']));
