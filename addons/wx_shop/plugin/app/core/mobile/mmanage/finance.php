...
class Finance_WxShopPage extends AppMobileAuthPage
{
	...
	protected function log($type)
	{
		...
		$sql = 'select log.id,m.id as mid, m.realname,m.avatar,log.logno,log.type,log.status,log.rechargetype,m.nickname,m.mobile,log.money,log.createtime,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,log.apppay from ' . tablename('wx_shop_member_log') . ' log ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('wx_shop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('wx_shop_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
