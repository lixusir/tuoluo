...
class Set_WxShopPage extends AppMobileAuthPage
{
	public function main()
	{
		...
		$roleuser = pdo_fetch('SELECT id, uid, username, status, openid, realname, mobile, openid_wa, member_nick FROM' . tablename('wx_shop_perm_user') . 'WHERE uid=:uid AND uniacid=:uniacid', array(':uid' => $account['uid'], ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			pdo_update('wx_shop_perm_user', $data, array('id' => $roleuser['id'], 'uniacid' => $_W['uniacid']));
	...
	public function bindwx()
	{
		...
		load()->model('user');
		$roleuser = pdo_fetch('SELECT id, uid, username, status, openid, realname, mobile, openid_wa, member_nick FROM' . tablename('wx_shop_perm_user') . 'WHERE openid_wa=:openid AND uniacid=:uniacid', array(':openid' => $openid, ':uniacid' => $_W['uniacid']));
		...
		pdo_update('wx_shop_perm_user', $data, array('uid' => $account['uid'], 'uniacid' => $_W['uniacid']));

		if (!empty($roleuser)) {
			pdo_update('wx_shop_perm_user', array('openid_wa' => '', 'member_nick' => ''), array('id' => $roleuser['id']));
		}
