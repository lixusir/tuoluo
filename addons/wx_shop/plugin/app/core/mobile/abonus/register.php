...
class Register_WxShopPage //extends AbonusMobileLoginPage
{
	public function main() 
	{
		...
		global $_GPC;
		$openid = $_GPC['openid'];
		$set = set_medias(p('abonus')->getSet(), 'regbg');
		// var_dump($set);die;
		$member = m('member')->getMember($openid);
		// show_json(1, array('member'=>$member, 'set'=>$set));
		if (($member['isaagent'] == 1) && ($member['aagentstatus'] == 1)) 
		{
			// header('location: ' . mobileUrl('abonus'));
			// exit();
			show_json(0, mobileUrl('abonus'));
		}
		...
		// include $this->template();
		show_json(1, array('member'=>$member,'shopname' => $_W['shopset']['shop']['name'], 'set'=>$set, 'apply_set'=>$apply_set));
	}
}
?>