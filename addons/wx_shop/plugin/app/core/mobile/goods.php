...
class Goods_WxShopPage extends AppMobilePage
{
	/**
     * 商品列表
     */
	...
	public function get_detail()
	{
		...
		$goods['params'] = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_goods_param') . ' WHERE uniacid=:uniacid and goodsid=:goodsid order by displayorder asc', array(':uniacid' => $uniacid, ':goodsid' => $goods['id']));
		...
		$grftarray = array();
		$i = 0;
		$gifts = pdo_fetchall('select id,goodsid,giftgoodsid,thumb,title from ' . tablename('wx_shop_gift') . ' where uniacid = :uniacid and activity = 2 and status = 1 and starttime <= :starttime and endtime >= :endtime ', array(':uniacid' => $uniacid, ':starttime' => time(), ':endtime' => time()));
			...
			foreach ($gid as $ke => $val ) {
				if ($val == $id) {
					...
					foreach ($giftgoods as $k => $val ) {
						$isgift = 1;
						$gifts[$key]['gift'][$k] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('wx_shop_goods') . ' where uniacid = :uniacid and deleted = 0 and total > 0 and status = 2 and id = :id ', array(':uniacid' => $uniacid, ':id' => $val));
		...
		$labelstyle = pdo_fetch('SELECT id,uniacid,style FROM ' . tablename('wx_shop_goods_labelstyle') . ' WHERE uniacid=:uniacid LIMIT 1', array(':uniacid' => $uniacid));
		...
		// $goods['packagegoods'] = $package_goods; xiaojie 修改 2018/07/13 因业务需要
		$goods['packagegoods'] = $packages;
		// 查询商品的套餐 end
		...
		$city_express = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_city_express') . ' WHERE uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $_W['uniacid']));
        ...
        //秒杀限购数量  及个人剩余购买数量
        $seckillinfo = plugin_run('seckill::getSeckill', $id, '0', true, $openid);

        $seckillselfcount = 0;
        $g = array();
        if ($seckillinfo && ($seckillinfo['status'] == 0)) 
        {
             $order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $id, ':uniacid' => $uniacid, ':openid' => $openid));
            
            if( (int)$order_goodscount <=  $seckillinfo['maxbuy']){
                $g['seckillselfcount'] = $seckillinfo['maxbuy'] - $order_goodscount;
            }else{
                $g['seckillselfcount'] = 0;
            }
            $g['seckillmaxbuy'] = $seckillinfo['maxbuy'];
            $g['seckillprice'] = $seckillinfo['price'];
        }
		//当前时间
		$goods['serverNowTime'] = time();
		app_json(array('goods' => $goods, 'g'=>$g));
	}
	...
	public function get_content()
	{
		...
		$uniacid = $_W['uniacid'];
		$id = intval($_GPC['id']);
		$goods = pdo_fetch('select content from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function get_picker()
	{
		...
		$goods = pdo_fetch('select id,thumb,title,marketprice,total,maxbuy,minbuy,unit,hasoption,showtotal,diyformid,diyformtype,diyfields, `type`, isverify, maxprice, minprice, merchsale from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if (!(empty($goods)) && $goods['hasoption']) {
			$specs = pdo_fetchall('select id, goodsid, title from ' . tablename('wx_shop_goods_spec') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));

			foreach ($specs as &$spec ) {
				$spec['items'] = pdo_fetchall('select id, specid, title, thumb, `virtual` from ' . tablename('wx_shop_goods_spec_item') . ' where specid=:specid and `show`=1 order by displayorder asc', array(':specid' => $spec['id']));
			}

			unset($spec);
			$options = pdo_fetchall('select id, goodsid, title, thumb, productprice, marketprice, stock, weight, specs, goodssn, productsn, `virtual`  from ' . tablename('wx_shop_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
		}
