...
class Poster_WxShopPage extends Base_WxShopPage
{
	public function main()
	{
		global $_W;
		$list = pdo_fetchall('SELECT id FROM ' . tablename('wx_shop_wxapp_poster') . ' WHERE uniacid=:uniacid AND status=1 ORDER BY displayorder ASC, id DESC', array(':uniacid' => $_W['uniacid']));
	...
	public function getimage()
	{
		...
		$poster = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_wxapp_poster') . ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		$path = IA_ROOT . '/addons/wx_shop/data/poster_wxapp/commission/' . $_W['uniacid'] . '/';
	...
	{
		global $_W;
		return $_W['siteroot'] . 'addons/wx_shop/data/poster_wxapp/commission/' . $_W['uniacid'] . '/' . $filename . '?v=' . time();
	}
