...
class Index_WxShopPage extends Base_WxShopPage
{
	public function main()
	{
		...
		$member['applycount'] = pdo_fetchcolumn('select count(id) from ' . tablename('wx_shop_commission_apply') . ' where mid=:mid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
		$openselect = false;
		...
		$posercount = pdo_fetchcolumn('SELECT COUNT(id) FROM ' . tablename('wx_shop_wxapp_poster') . ' WHERE uniacid=:uniacid AND status=1', array(':uniacid' => $_W['uniacid']));
        ...
        $app=p('app');
		$data = array('set' => $return_set, 'member' => $return_member, 'levelname' => (empty($level) ? ((empty($this->set['levelname']) ? '普通等级' : $this->set['levelname'])) : $level['levelname']), 'agentname' => (empty($up) ? '总店' : $up['nickname']), 'cansettle' => $cansettle, 'menu' => ($app?$app->diyMenu('commission'):''));
		app_json($data);
	}
	...
	public function withdraw()
	{
		...
		if (!(empty($agentid))) {
			$data = pdo_fetch('select sum(deductionmoney) as sumcharge from ' . tablename('wx_shop_commission_log') . ' where mid=:mid and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $agentid));
		...
		$data = array('set' => $set, 'member' => $member, 'cansettle' => $cansettle, 'menu' => $app?$app->diyMenu('commission'):'');
		app_json($data);
	}
	...
	public function apply()
	{
		...
		if (1 <= $level) {
			$level1_orders = pdo_fetchall('select distinct o.id from ' . tablename('wx_shop_order') . ' o ' . ' left join  ' . tablename('wx_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=3  and og.status1=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.uniacid=:uniacid  group by o.id', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
		...
		if (2 <= $level) {
			if (0 < $member['level1']) {
				$level2_orders = pdo_fetchall('select distinct o.id from ' . tablename('wx_shop_order') . ' o ' . ' left join  ' . tablename('wx_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ')  and o.status>=3  and og.status2=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.uniacid=:uniacid  group by o.id', array(':uniacid' => $_W['uniacid']));
		...
		if (3 <= $level) {
			if (0 < $member['level2']) {
				$level3_orders = pdo_fetchall('select distinct o.id from ' . tablename('wx_shop_order') . ' o ' . ' left join  ' . tablename('wx_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ')  and o.status>=3  and  og.status3=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ')   and o.uniacid=:uniacid  group by o.id', array(':uniacid' => $_W['uniacid']));
		...
		if (($this->set['cashweixin'] == 1) && !(is_h5app())) {
            $type_array[] = array('type' => 1, 'title' => $this->set['texts']['withdraw'] . '到' . '微信钱包');
            $type_array_new[] = 1;
		}
		...
		if ($_W['ispost']) {
			...
			foreach ($orderids as $o ) {
				pdo_update('wx_shop_order_goods', array('status' . $o['level'] => 1, 'applytime' . $o['level'] => $time), array('orderid' => $o['orderid'], 'uniacid' => $_W['uniacid']));
			}
			...
			pdo_insert('wx_shop_commission_apply', $apply);
	...
	public function register()
	{
			...
			if ($template_flag == 1) {
				...
				if ($become_check == 1) {
					...
					if (!(empty($mid))) {
                        ...
                        //代理分红修改begin
                        if (p('weightbonus')) {
                            p('weightbonus')->upgradeLevelByAgent($mid);
                        }
                        //代理分红修改end

                        //区域代理分红修改begin
                        if (p('abonus')) {
                            p('abonus')->upgradeLevelByAgent($mid);
                        }
                        //区域代理分红修改end
		...
		if (empty($set['become'])) {
			if (empty($member['status']) || empty($member['isagent'])) {
				...
				if ($become_check == 1) {
                    ...
                    //代理分红修改begin
                    if (p('weightbonus')) {
                        p('weightbonus')->upgradeLevelByAgent($member['id']);
                    }
                    //代理分红修改end

                    //区域代理分红修改begin
                    if (p('abonus')) {
                        p('abonus')->upgradeLevelByAgent($member['id']);
                    }
                    //区域代理分红修改end
		...
		$res = array(
			...
			'menu'             => $app?$app->diyMenu('commission'):''
			);
		app_json($res);
	}
}
