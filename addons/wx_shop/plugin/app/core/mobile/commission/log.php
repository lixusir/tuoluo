...
class Log_WxShopPage extends Base_WxShopPage
{
	...
	public function detail_list()
	{
		...
		$list = pdo_fetchall('select o.id,o.agentid, o.ordersn,o.price,o.goodsprice, o.dispatchprice,o.createtime, o.paytype from ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_member') . ' m on o.openid = m.openid and o.uniacid = m.uniacid' . ' where  o.uniacid=:uniacid and o.id in ( ' . implode(',', $ids) . ' ) LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$total = (int) pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_member') . ' m on o.openid = m.openid and o.uniacid = m.uniacid' . ' where  o.uniacid=:uniacid and o.id in ( ' . implode(',', $ids) . ' ) ', array(':uniacid' => $_W['uniacid']));
		$totalcommission = 0;
		$totalpay = 0;
