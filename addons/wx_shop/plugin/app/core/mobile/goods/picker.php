...
class Picker_WxShopPage extends MobilePage
{
	public function main()
	{
		...
		if (!(empty($goods)) && $goods['hasoption']) {
			...
			foreach ($specs as &$spec ) {
				$spec['items'] = pdo_fetchall('select * from ' . tablename('wx_shop_goods_spec_item') . ' where specid=:specid and `show`=1 order by displayorder asc', array(':specid' => $spec['id']));
			}
