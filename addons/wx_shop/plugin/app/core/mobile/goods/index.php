...
class Index_WxShopPage extends MobilePage
{
	public function main() 
	{
		...
		show_json(1, array('catlevel'=>$catlevel, 'opencategory'=>$opencategory, 'allcategory'=>$allcategory));
	}
	public function gift() 
	{
		...
		$gift = pdo_fetch('select * from ' . tablename('wx_shop_gift') . ' where uniacid = ' . $uniacid . ' and id = ' . $giftid . ' and starttime <= ' . time() . ' and endtime >= ' . time() . ' and status = 1 ');
		...
		if (!(empty($giftgoodsid))) 
		{
			...
			{
				$giftgoods[$key] = pdo_fetch('select id,status,title,thumb,marketprice from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0 and total > 0 and id = ' . $value . ' and status = 2 ');
			}
