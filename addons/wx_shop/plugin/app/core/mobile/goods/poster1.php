...
class Poster_WxShopPage extends AppMobilePage
{
	...
	private function createPoster($goods = array(), $member = array())
	{
		...
		$path = IA_ROOT . '/addons/wx_shop/data/poster_wxapp/goods/' . $_W['uniacid'] . '/';
		...
		$pricebgstr = file_get_contents(IA_ROOT . '/addons/wx_shop/plugin/app/static/images/poster/goodsprice.png');
		$pricebg = imagecreatefromstring($pricebgstr);
		imagecopyresized($target, $pricebg, 32, 632, 0, 0, 686, 86, 686, 86);
		$font = IA_ROOT . '/addons/wx_shop/static/fonts/pingfang.ttf';
		...
		$boxstr = file_get_contents(IA_ROOT . '/addons/wx_shop/plugin/app/static/images/poster/goodsbox.png');
	...
	{
		global $_W;
		return $_W['siteroot'] . 'addons/wx_shop/data/poster_wxapp/goods/' . $_W['uniacid'] . '/' . $filename . '?v=1.0';
	}
