...
class Wholesalepicker_WxShopPage extends MobilePage
{
	public function main() 
	{
		...
		{
			$specs1 = pdo_fetch('select *  from ' . tablename('wx_shop_goods_spec') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc limit 1', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
			if (!(empty($specs1))) 
			{
				$hasoption = 1;
				$spec1items = pdo_fetchall('select *  from ' . tablename('wx_shop_goods_spec_item') . ' where specid=:specid and `show`=1 order by displayorder asc', array(':specid' => $specs1['id']));
				if (!(empty($spec1items))) 
				{
			...
			$specs2 = pdo_fetch('select *  from ' . tablename('wx_shop_goods_spec') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc limit 1,1', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
			if (!(empty($specs2))) 
			{
				$hasoption = 2;
				$spec2items = pdo_fetchall('select *  from ' . tablename('wx_shop_goods_spec_item') . ' where specid=:specid and `show`=1 order by displayorder asc', array(':specid' => $specs2['id']));
				if (!(empty($spec2items))) 
				{
			...
			$optionlist = pdo_fetchall('select *  from ' . tablename('wx_shop_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
			$options = array();
