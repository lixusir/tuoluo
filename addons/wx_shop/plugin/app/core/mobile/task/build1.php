...
class build_WxShopPage extends PluginPfMobilePage
{
	public function main() 
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('wx_shop_task_poster') . ' where keyword=:keyword and uniacid=:uniacid and `status`=1 and `is_delete`=0 limit 1', array(':keyword' => $content, ':uniacid' => $_W['uniacid']));
		if (empty($poster)) 
		{
		...
		$img = '';
		$is_waiting = false;
		$task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('wx_shop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $poster['poster_type'] . ' and is_reward=0 and failtime>' . time(), array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid']));
		if ($task_count) 
		{
			$task_info = pdo_fetch('select `needcount`,`completecount`,`is_reward`,`failtime` from ' . tablename('wx_shop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_id=:task_id and task_type=:task_type and  failtime>' . time() . ' order by `addtime` DESC limit 1', array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid'], ':task_id' => $poster['id'], ':task_type' => $poster['poster_type']));
			if ($task_info) 
			{
				$is_waiting = true;
		...
		else 
		{
			...
			$other_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('wx_shop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $poster_type . ' and failtime>' . time(), array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid']));
			if ($other_task_count) 
			{
				...
				if ($default) 
				{
					...
					{
						$end_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('wx_shop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $poster['poster_type'] . ' and failtime<' . time(), array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid']));
						if ($end_task_count) 
						{
							$end_task_info = pdo_fetch('select `needcount`,`completecount`,`failtime` from ' . tablename('wx_shop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_id=:task_id and task_type=:task_type and failtime<' . time() . ' order by `addtime` DESC limit 1', array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid'], ':task_id' => $poster['id'], ':task_type' => $poster['poster_type']));
							if ($end_task_info) 
							{
								if ($poster['is_repeat']) 
								{
					...
					else if ($default['is_posterall'] == 0) 
					{
						...
						m('message')->sendCustomNotice($openid, '您已经有另外类型的海报任务正在进行，不能同时参加');
						return;
						$end_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('wx_shop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $poster['poster_type'] . ' and (is_reward=1 or failtime<' . time() . ')', array(':uniacid' => $_W['uniacid'], ':join_user' => $member['openid']));
						if ($end_task_count) 
						{
	...
	private function join_task($member, $poster) 
	{
		...
		pdo_insert('wx_shop_task_join', $task_join);
		$id = pdo_insertid();
		$img = '';
		if ($id) 
		{
