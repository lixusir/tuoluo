...
class index_WxShopPage extends PluginMobilePage
{
	public function main() 
	{
		...
		$now_time = time();
		$task_sql = 'SELECT * FROM ' . tablename('wx_shop_task_poster') . ' WHERE timestart<=' . $now_time . ' AND timeend>' . $now_time . ' AND uniacid=' . $_W['uniacid'] . ' AND `status`=1 AND `is_delete`=0 ORDER BY `createtime` DESC LIMIT 0,15';
		...
		$complete_sql = 'SELECT `join`.*,`task`.title,`task`.titleicon,`task`.poster_type FROM ' . tablename('wx_shop_task_join') . ' AS `join` LEFT JOIN ' . tablename('wx_shop_task_poster') . ' AS `task` ON `join`.task_id=`task`.`id` WHERE `join`.uniacid=' . $_W['uniacid'] . ' AND `join`.`join_user`="' . $openid . '" AND `join`.`is_reward`=1 ORDER BY `join`.`addtime` DESC LIMIT 0,15';
		...
		$advs = pdo_fetchall('select id,advname,link,thumb from ' . tablename('wx_shop_task_adv') . ' where uniacid=:uniacid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid']));
	...
	public function gettaskinfo() 
	{
		...
		if (intval($_GPC['id'])) 
		{
			...
			$now_time = time();
			$task_sql = 'SELECT * FROM ' . tablename('wx_shop_task_poster') . ' WHERE timestart<=' . $now_time . ' AND timeend>' . $now_time . ' AND uniacid=:uniacid AND id=:id AND `status`=1  ';
			...
			{
				$task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('wx_shop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $taskinfo['poster_type'] . ' and is_reward=0 and failtime>' . time(), array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
				if (empty($task_count)) 
				{
					$end_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('wx_shop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_id=' . $taskinfo['id'] . ' and (is_reward=1 or failtime<' . time() . ')', array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
					if ($end_task_count) 
					{
			...
			else 
			{
				...
				$other_task_count = pdo_fetchcolumn('select COUNT(*) from ' . tablename('wx_shop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $poster_type . ' and failtime>' . time(), array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
				$task_count = pdo_fetchcolumn('select task_id from ' . tablename('wx_shop_task_join') . ' where uniacid=:uniacid and join_user=:join_user and task_type=' . $taskinfo['poster_type'] . ' and failtime>' . time(), array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
	...
	public function getcompleteinfo() 
	{
		...
		if (intval($_GPC['id'])) 
		{
			...
			$task_sql = 'SELECT `join`.*,`task`.title,`task`.titleicon,`task`.poster_banner FROM ' . tablename('wx_shop_task_join') . ' AS `join` LEFT JOIN ' . tablename('wx_shop_task_poster') . ' AS `task` ON `join`.task_id=`task`.`id` WHERE `join`.uniacid=' . $_W['uniacid'] . ' AND `join`.join_user=:join_user AND `join`.`is_reward`=1 AND `join`.`join_id`=:join_id ';
			$taskinfo = pdo_fetch($task_sql, $param);
	...
	public function goodslist() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$tasksql = 'SELECT * FROM ' . tablename('wx_shop_task_join') . ' WHERE `join_id`=' . $id . ' AND `join_user`=:join_user';
		...
		if ($taskinfo['task_type'] == 1) 
		{
			...
			{
				$searchsql = 'SELECT thumb,marketprice FROM ' . tablename('wx_shop_goods') . ' WHERE uniacid= ' . $_W['uniacid'] . ' and id=' . $k . ' and status=1 and deleted=0';
