...
class Create_WxShopPage extends Base_WxShopPage
{
	public function main() 
	{
		...
		show_json(1,array('goods'=>$goods,'stores'=>$stores, 'shop_name'=>$_W['shopset']['shop']['name'], 'set_realname'=>$set['set_realname'], 'set_mobile'=>$set['set_mobile'], 'realname'=>$member['realname'], 'mobile'=>$member['mobile']));
//		include $this->template();
	...
	public function dispatch() 
	{
		...
		$address = pdo_fetch('select id,realname,mobile,address,province,city,area,datavalue from ' . tablename('wx_shop_member_address') . "\r\n" . '        where id=:id and uniacid=:uniacid limit 1', array(':id' => $addressid, ':uniacid' => $_W['uniacid']));
		$user_city = '';
		$user_city_code = '';
		if (empty($new_area)) 
		{
