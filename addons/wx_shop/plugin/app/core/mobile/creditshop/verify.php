...
class Verify_WxShopPage extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		show_json(1, array('qrcode'=>$qrcode));
	}
	...
	public function check() 
	{
		...
		$uniacid = $_W['uniacid'];
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select id,status,isverify,verifytype,verifynum from ' . tablename('wx_shop_groups_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		if (empty($order)) 
		{
			show_json(0);
		}
