...
class Log_WxShopPage extends Base_WxShopPage
{
	...
	public function getlist() 
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('wx_shop_creditshop_log') . ' log' . "\r\n" . '                left join ' . tablename('wx_shop_creditshop_goods') . ' g on log.goodsid = g.id' . "\r\n" . '                where 1 ' . $condition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();
		if (!(empty($total))) 
		{
			$sql = 'SELECT log.id,log.logno,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money,g.isverify,g.goodstype,log.addressid,log.storeid,' . 'g.goodstype,log.time_send,log.time_finish,log.iscomment,op.title as optiontitleg,g.merchid ' . ' FROM ' . tablename('wx_shop_creditshop_log') . ' log ' . ' left join ' . tablename('wx_shop_creditshop_goods') . ' g on log.goodsid = g.id ' . ' left join ' . tablename('wx_shop_creditshop_option') . ' op on op.id = log.optionid ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
	...
	public function detail() 
	{
		...
		$id = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id=:id and openid=:openid ' . $condition . ' limit 1', array(':id' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
		if (empty($log)) 
		{
			show_json(-1, '兑换记录不存在!');
		}
		...
		{
            $address = pdo_fetch('select * from ' . tablename('wx_shop_member_address') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':id' => $log['addressid'], ':uniacid' => $uniacid, ':openid' => $openid));
		...
		{
			$verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_creditshop_verify') . ' where logid = :id and openid=:openid ' . $condition . ' and verifycode = :verifycode ', array(':id' => $id, ':openid' => $log['openid'], ':uniacid' => $log['uniacid'], ':verifycode' => $log['eno']));
			if ($goods['verifytype'] == 0) 
			{
				$verify = pdo_fetch('select isverify from ' . tablename('wx_shop_creditshop_verify') . ' where logid = :id and openid=:openid ' . $condition . ' and verifycode = :verifycode ', array(':id' => $log['id'], ':openid' => $log['openid'], ':uniacid' => $log['uniacid'], ':verifycode' => $log['eno']));
			}
        ...
        $log['time_finish']=date('Y-m-d H:i:s', $log['time_finish']);
        $log['time_send']=date('Y-m-d H:i:s', $log['time_send']);
        $log['is_time_send']=$log['time_send'];
		show_json(1,array(
			'log'=>$log,
			'goods'=>$goods,
			'address'=>$address,
			...
			'stores'=>$stores,
			'isreply'=>$set['isreply'],
			'shopset_shop_name'=>$_W['shopset']['shop']['name'],
		));
	...
	public function Receivepacket() 
	{
		...
		$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id = ' . $logid . ' ' . $condition . ' ');
		if (!($log)) 
		{
		...
		$goods = pdo_fetch('select surplusmoney from ' . tablename('wx_shop_creditshop_goods') . ' where id = ' . $log['goodsid'] . ' ' . $condition . ' ');
		...
		else 
		{
			...
			pdo_update('wx_shop_creditshop_goods', $updategoods, array('id' => $log['goodsid']));
		}
		show_json(1,array('log'=>$log,'packet'=>$packet,'goods'=>$goods));
	}
	...
	public function express() 
	{
		...
		if (empty($logid)) 
		{
			 // show_json(-1,array('message'=>"订单号为空"));
			 // 
		...
		$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id=:logid ' . $condition . ' and openid=:openid limit 1', array(':logid' => $logid, ':uniacid' => $uniacid, ':openid' => $openid));
		if (empty($log)) 
		{
			app_error(AppError::$CommissionReg,'操作非法');
			// header('location: ' . mobileUrl('creditshop/log'));
		...
		$goods = pdo_fetch('select *  from ' . tablename('wx_shop_creditshop_goods') . '  where id=:id ' . $condition . ' ', array(':uniacid' => $uniacid, ':id' => $log['goodsid']));
		$expresslist = m('util')->getExpressList($log['express'], $log['expresssn']);

		show_json(1,array('log'=>$log,'expresslist'=>$expresslist,'goods'=>$goods));
	}
	...
	public function finish() 
	{
		...
		// $log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id=:id ' . $condition . ' and openid=:openid limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id=:id ' . $condition . ' and openid=:openid limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid'], ':openid' => $_GPC['openid']));
		if (empty($log)) 
		{
			show_json(0, '订单未找到');
		}
		...
		pdo_update('wx_shop_creditshop_log', array('time_finish' => time()), array('id' => $logid, 'uniacid' => $_W['uniacid']));
		show_json(1,'订单完成');
	}
	...
	public function paydispatch() 
	{
		...
		if ($goods['dispatch'] <= 0) 
		{
			pdo_update('wx_shop_creditshop_log', array('dispatchstatus' => 1, 'addressid' => $addressid), array('id' => $log['id']));
		...
		if ($paytype == 'wechat') 
		{
			...
			if (empty($dispatchno)) 
			{
				...
				pdo_update('wx_shop_creditshop_log', array('dispatchno' => $dispatchno, 'addressid' => $addressid), array('id' => $log['id']));
			}
			$params = array();
		...
		else if ($paytype == 'alipay') 
		{
			...
			if (isset($set['pay']) && ($set['pay']['alipay'] == 1)) 
			{
				...
				if (is_array($setting['payment'])) 
				{
					...
					$alipay = m('common')->alipay_build($params, $options, 21, $_GPC['openid']);
					if (!(empty($alipay['url']))) 
					{
	...
	public function dispatch_complete() 
	{
		...
		if ($lastlog['dispatchstatus'] < 1) 
		{
			...
			pdo_update('wx_shop_creditshop_log', $record, array('dispatchno' => $tid));
			...
			$creditlog = pdo_fetch('select id from ' . tablename('wx_shop_creditshop_log') . "\r\n" . '                    where dispatchno=:dispatchno and openid=:openid and dispatchstatus=1 and uniacid=:uniacid limit 1', array(':dispatchno' => $tid, ':openid' => $_GPC['openid'], ':uniacid' => $_W['uniacid']));
			if (is_h5app()) 
			{
	...
	public function wechat_dispatch_complete() 
	{
		...
		if (empty($log)) 
		{
			...
			$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where dispatchno=:logno and uniacid=:uniacid limit 1', array(':logno' => $logno, ':uniacid' => $_W['uniacid']));
		}
		$member = m('member')->getMember($openid);
		$goods = $this->model->getGoods($log['goodsid'], $member, $log['optionid']);
		$goods['dispatch'] = $this->model->dispatchPrice($log['goodsid'], $log['addressid'], $log['optionid']);
		if (!(empty($log))) 
		{
			...
			if (!(is_error($payquery)) || !(is_error($payqueryborrow))) 
			{
				if ($log['dispatchstatus'] < 1) 
				{
					$record = array();
					$record['dispatchstatus'] = '1';
					pdo_update('wx_shop_creditshop_log', $record, array('dispatchno' => $log['dispatchno']));
					...
					$creditlog = pdo_fetch('select id from ' . tablename('wx_shop_creditshop_log') . "\r\n" . '                    where dispatchno=:logno and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(':logno' => $log['dispatchno'], ':openid' => $_GPC['openid'], ':uniacid' => $_W['uniacid']));
					if (is_h5app()) 
					{
						pdo_update('wx_shop_creditshop_log', array('apppay' => 1), array('dispatchno' => $log['dispatchno']));
					}
				}
				if (is_h5app()) 
				{
	...
	public function setstore() 
	{
		...
		if (!(empty($upgrade))) 
		{
			pdo_update('wx_shop_creditshop_log', $upgrade, array('id' => $log['id']));
		}
		if (!(empty($upgrade1))) 
		{
			pdo_update('wx_shop_member', $upgrade1, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			if (!(empty($member['uid']))) 
			{
