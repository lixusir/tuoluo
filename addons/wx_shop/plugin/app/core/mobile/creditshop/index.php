...
//		var_dump($this->diyPage('creditshop'));
		...
		$advs = pdo_fetchall('select id,advname,link,thumb from ' . tablename('wx_shop_creditshop_adv') . ' where ' . $contation . ' and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
		$advs = set_medias($advs, 'thumb');
		$credit = m('member')->getCredit($openid, 'credit1');
//        var_dump(m('member')->getMember($openid));exit;
        $category = array();
		if (0 < intval($_GPC['merchid'])) 
		{
			...
			if (!(empty($merch_category))) 
			{
				...
				foreach ($merch_category as $index => $row ) 
				{
					if (0 < $row) 
					{
						$list = pdo_fetch('select id,name,thumb,isrecommand from ' . tablename('wx_shop_creditshop_category') . "\r\n\t\t\t\t\t\t" . 'where id = ' . $index . ' and uniacid=:uniacid and  enabled=1 ', array(':uniacid' => $uniacid));
		...
		{
			$category = pdo_fetchall('select id,name,thumb,isrecommand from ' . tablename('wx_shop_creditshop_category') . ' where uniacid=:uniacid and  enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
			$category = set_medias($category, 'thumb');
		}
		array_values($category);
		$lotterydraws = pdo_fetchall('select id, title,goodstype, subtitle, credit, money, thumb,`type`,price from ' . tablename('wx_shop_creditshop_goods') . "\r\n\t\t\t\t" . 'where ' . $contation . ' and isrecommand = 1 and `type` = 1 and  status=1 and deleted=0 order by displayorder,id desc limit 4', array(':uniacid' => $uniacid));
...
//		$_W['shopshare'] = array('title' => $this->set['share_title'], 'imgUrl' => tomedia($this->set['share_icon']), 'link' => mobileUrl('creditshop', array(), true), 'desc' => $this->set['share_desc']);
		...
		$result=array('advs'=>$advs,'credit'=>$credit,'category'=>$category,'lotterydraws'=>$lotterydraws,'exchanges'=>$exchanges,'redbags'=>$redbags,'balances'=>$balances,'coupons'=>$coupons);
		app_json($result);
