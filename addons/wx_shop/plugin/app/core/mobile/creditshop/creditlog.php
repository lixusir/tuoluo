...
class Creditlog_WxShopPage extends Base_WxShopPage
{
	public function main() 
	{
		...
		show_json(1,array('credit'=>$credit));
	}
	public function getlist() 
	{
		...
		if (!(empty($total))) 
		{
			$sql = 'SELECT log.id,log.goodsid,g.title,g.thumb,g.credit,g.type,g.money,log.createtime, log.status, g.thumb FROM ' . tablename('wx_shop_creditshop_log') . ' log ' . ' left join ' . tablename('wx_shop_creditshop_goods') . ' g on log.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
