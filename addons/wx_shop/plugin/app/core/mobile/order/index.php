...
class Index_WxShopPage extends AppMobilePage
{
    ...
    public function get_list()
    {
        ...
        if ($show_status != '') {
            ...
            case 2:
                ...
                break;
                 // and (status=2 or status=0 and paytype=3)
            case 4:
        ...
        $list = pdo_fetchall('select id,ordersn,price,userdeleted,isparent,refundstate,paytype,status,addressid,refundid,isverify,dispatchtype,verifytype,verifyinfo,verifycode,iscomment from ' . tablename('wx_shop_order') . ' where 1 ' . $condition . ' order by createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
        ...
        foreach ($list as &$row ) {
            ...
            $sql = 'SELECT og.goodsid,og.total,g.title,g.thumb,og.price,og.optionname as optiontitle,og.optionid,og.seckill,og.seckill_taskid,op.specs,g.merchid,g.status FROM ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on og.goodsid = g.id ' . ' left join ' . tablename('wx_shop_goods_option') . ' op on og.optionid = op.id ' . ' where ' . $scondition . ' order by og.id asc';
    ...
    public function detail()
    {
        ...
        $gn = 0;
        $nog = 0;
        $goods = pdo_fetchall('select g.id,og.goodsid,og.price,g.title,g.thumb,g.status,og.total,g.credit,og.optionid,og.optionname as optiontitle,og.seckill,
og.seckill_taskid,g.isverify,g.isfullback,g.storeids' . $diyformfields . '  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where ' . $scondition . ' and og.uniacid=:uniacid ', $param);
        $goods = set_medias($goods, array('thumb'));

        if (!(empty($goods))) {
            foreach ($goods as &$g ) {
                if ($g['isfullback']) {
                    $fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_fullback_goods') . ' WHERE goodsid = :goodsid and uniacid = :uniacid  limit 1 ', array(':goodsid' => $g['goodsid'], ':uniacid' => $uniacid));

                    if ($g['optionid']) {
                        $option = pdo_fetch('select `day`,allfullbackprice,fullbackprice,allfullbackratio,fullbackratio,isfullback' . "\r\n" . '                      from ' . tablename('wx_shop_goods_option') . ' where id = :id and uniacid = :uniacid ', array(':id' => $g['optionid'], ':uniacid' => $uniacid));
    ...
    public function express()
    {
        ...
        $order = pdo_fetch('select expresscom,expresssn,addressid,status,express from ' . tablename('wx_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
