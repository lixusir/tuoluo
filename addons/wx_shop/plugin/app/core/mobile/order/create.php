...
class Create_WxShopPage extends AppMobilePage
{
	...
	public function main()
	{
        ...
        if (!($packageid)) {
            ...
            if (p('bargain') && !(empty($bargain_id))) {
                ...
                if(empty($bargain_act) && $_GPC['openid']){
            ...
            $gifts = array();
            if (empty($id)) {
                if(!empty($_GPC['exchange'])){
                    $exchangeOrder = trim($_GPC['exchange']);
                    // var_dump($openid);die;
                    //兑换中心购物车查询  传入exchange参数
                ...
                foreach ($goods as $k => $v ) {
                    ...
                    if (!(empty($goods[$k]['seckillinfo']['maxbuy'])) && (($goods[$k]['seckillinfo']['maxbuy'] - $goods[$k]['seckillinfo']['selfcount']) < $goods[$k]['total'])) 
                    {
                        ...
                        app_error('您已购买了' . $goods[$k]['seckillinfo']['selfcount'] . '最多购买' . $goods[$k]['seckillinfo']['maxbuy'] . '件');
                    }
                ...
                if (!(empty($bargain_act))) {
         		...
         		// var_dump($data['seckillinfo']);die;

                $merch_id = $data['merchid'];
                $fullbackgoods = array();

                if ($data['isfullback']) {
                    $fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_fullback_goods') . ' WHERE goodsid = :goodsid and uniacid = :uniacid and status = 1 limit 1 ', array(':goodsid' => $data['goodsid'], ':uniacid' => $uniacid));
                }
                ...
                if (!(empty($optionid))) {
                    $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,`virtual`,stock,weight,specs,isfullback,`day`,allfullbackprice,fullbackprice,allfullbackratio,fullbackratio from ' . tablename('wx_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $optionid));
            ...
            if ($isverify) {
                ...
                if (!($isvirtual) && !($isonlyverifygoods)) {
                    ...
                    $dispatch_price = $dispatch_array['dispatch_price'];
                    $dispatch_price2 = $dispatch_array['dispatch_price'] - $dispatch_array['seckill_dispatch_price'];
                    // 秒杀
            ...
            //var_dump($dispatch_price);die;
            // $realprice += $dispatch_price;
            //var_dump($seckill_price);die;

            // 秒杀

        	$realprice += $dispatch_price2 + $seckill_dispatchprice;
            ...
            if ($giftid) {
                ...
                if ($giftdata['giftgoodsid']) {
                    ...
                    foreach ($giftgoodsid as $key => $value ) {
                        $gift[$key] = pdo_fetch('select id as goodsid,title,thumb from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and id = ' . $value . ' and deleted = 0 ');
                ...
                foreach ($gifts as $key => $value ) {
                    ...
                    foreach ($giftgoods as $k => $val ) {
                        $gifts[$key]['gift'][$k] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and status = 2 and id = ' . $val . ' ');
                    }
	...
	public function caculate()
	{
		...
		if (0 < $packageid) {
			...
			foreach ($goodsarr as &$g ) {
				...
				if (!(empty($optionid))) {
					$option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual`,weight from ' . tablename('wx_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
	...
	public function submit()
	{
		...
		$goods[0]['bargain_id'] = $_GPC['bargainid'];
		// var_dump($_GPC['bargainid']);die;
		$_SESSION['bargain_id'] = NULL;
                            ...
                            if ($stock_num != -1) 
                            {
                                ...
                                if (!(empty($data['unite_total']))) 
                                {
                                    if (($stock_num - intval($total_array[$goodsid]['total'])) < 0) 
                                    {
                                        ...
                                        app_error(AppError::$OrderCreateStockError, $data['title'] . '<br/>总库存不足!当前总库存为' . $stock_num);
                                    }
                                }
                            }
                        }
		...
		//如果是兑换  
		if ($_GPC['exchange']=='1' && p('exchange')) 
		{
			if((int)$_GPC['exchangepostage'] > 0 ){
				//价格等于 （（价格 + 运费） -  市场价  单个）
				$order['price'] = ($_GPC['exchangeprice'] + $_GPC['exchangepostage']);
			}
		...
		if ($multiple_order == 0) {
			foreach ($allgoods as $goods ) {
				...
				if ($_GPC['exchange']=='1' && p('exchange')) 
				{
					if((int)$_GPC['exchangepostage'] > 0 ){
						//价格等于 （（价格 + 运费） -  市场价  单个）
						$order_goods['price'] = ($_GPC['exchangeprice'] + $_GPC['exchangepostage']);
					}
				...
				if ($goods['seckillinfo'] && ($goods['seckillinfo']['status'] == 0)) 
				{
					...
					plugin_run('seckill::setSeckill', $goods['seckillinfo'], $goods, $openid, $orderid, 0, $order['createtime']);
				}
		...
		else 
		{
			...
			foreach ($merch_array as $key => $value ) {
				...
				if ($_GPC['exchange']=='1' && p('exchange')) 
				{
					if((int)$_GPC['exchangepostage'] > 0 ){
						//价格等于 （（价格 + 运费） -  市场价  单个）
						$order['price'] = ($ch_order_data[$merchid]['price'] + (int)$_GPC['exchangepostage']) - $ch_order_data[$merchid]['price'];
					}
