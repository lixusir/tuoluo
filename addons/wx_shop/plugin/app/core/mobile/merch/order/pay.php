...
class Pay_WxShopPage //extends MobileLoginPage
{
	public function main() 
	{
		...
		if (!(empty($ispeerpay))) 
		{
			if (pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_order_peerpay_payinfo') . ' WHERE openid = :openid AND pid = :pid', array(':openid' => $_GPC['openid'], ':pid' => $ispeerpay['id']))) 
			{
				// $this->message('每人只能代付一次');
				show_json(0, '每人只能代付一次');
			}
		...
		if (empty($order['istrade'])) 
		{
			if ($order['status'] == -1) 
			{
				...
				// exit();
				show_json(0,  mobileUrl('order/detail', array('id' => $order['id'])));
			}
		...
		else if (($order['status'] == 1) && ($order['tradestatus'] == 2)) 
		{
			...
			// exit();
			show_json(0,  mobileUrl('newstore/norder/detail', array('id' => $order['id'])));
		}
		...
		if (empty($order['price']) && !($credit['success'])) 
		{
			...
			// exit();
			show_json(0, mobileUrl('order/pay/complete', array('id' => $order['id'], 'type' => 'credit', 'ordersn' => $order['ordersn'])));
		}
		load()->model('payment');
			...
			if (isset($set['pay']) && ($set['pay']['alipay'] == 1)) 
			{
				if (is_array($setting['payment']['alipay']) && $setting['payment']['alipay']['switch']) 
				{
					...
					if (is_array($setting['payment'])) 
					{
						...
						$alipay = m('common')->alipay_build($params, $options, 0, $_GPC['openid']);
						if (!(empty($alipay['url']))) 
						{
		...
		if (p('seckill')) 
		{
			...
			{
				plugin_run('seckill::getSeckill', $data['goodsid'], $data['optionid'], true, $_GPC['openid']);
			}
		}
		show_json(1, array(
			'payinfo'=>$payinfo
	...
	public function orderstatus() 
	{
		...
		$uniacid = $_W['uniacid'];
		$orderid = intval($_GPC['id']);
		$order = pdo_fetch('select status from ' . tablename('wx_shop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		if (1 <= $order['status']) 
		{
			@session_start();
	...
	public function complete() 
	{
		...
		if (is_h5app() && empty($orderid)) 
		{
			...
			$orderid = pdo_fetchcolumn('select id from ' . tablename('wx_shop_order') . ' where ordersn=:ordersn and uniacid=:uniacid and openid=:openid limit 1', array(':ordersn' => $ordersn, ':uniacid' => $uniacid, ':openid' => $openid));
		}
		if (empty($orderid)) 
		{
			if ($_W['ispost']) 
		...
		{
			$package = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_package') . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $order['packageid'] . ' ');
			if (empty($package)) 
			{
				show_json(0, '未找到套餐！');
			}
		...
		if ((empty($order['isnewstore']) || empty($order['storeid'])) && empty($order['istrade'])) 
		{
			...
			foreach ($order_goods as $data ) 
			{
				...
				$seckillinfo = plugin_run('seckill::getSeckill', $data['goodsid'], $data['optionid'], true, $_GPC['openid']);
				if ($data['seckill']) 
				{
		...
		{
			$sql = 'select og.id,g.title, og.goodsid,og.optionid,ng.stotal as stock,og.total as buycount,ng.gstatus as status,ng.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups,g.totalcnf,og.seckill,og.storeid ' . ' from ' . tablename('wx_shop_order_goods') . ' og left join  ' . tablename('wx_shop_goods') . ' g  on g.id=og.goodsid and g.uniacid=og.uniacid' . ' inner join ' . tablename('wx_shop_newstore_goods') . ' ng on g.id=ng.goodsid   ' . ' where og.orderid=:orderid and og.uniacid=:uniacid and ng.storeid=:storeid';
			...
			foreach ($order_goods as $data ) 
			{
				...
				if ($data['totalcnf'] == 1) 
				{
					if (!(empty($data['optionid']))) 
					{
						$option = pdo_fetch('select id,marketprice,stock from ' . tablename('wx_shop_newstore_goods_option') . ' where optionid=:optionid and goodsid=:goodsid and uniacid=:uniacid and storeid=:storeid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $data['goodsid'], ':optionid' => $data['optionid'], ':storeid' => $data['storeid']));
						if (!(empty($option))) 
						{
							if ($option['stock'] != -1) 
							{
		...
		else if ($type == 'wechat') 
		{
			...
			if (!(is_error($payquery)) || !(is_error($payquery_jie)) || !(empty($ispeerpay))) 
			{
				...
				if (is_h5app()) 
				{
					pdo_update('wx_shop_order', array('apppay' => 1), array('id' => $order['id']));
				}
				$ret = array();
				$ret['result'] = 'success';
				$ret['type'] = 'wechat';
	...
	public function success() 
	{
		...
		$order = pdo_fetch('select * from ' . tablename('wx_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		@session_start();
		if (!(isset($_SESSION[WX_SHOP_PREFIX . '_order_pay_complete'])))
		{
			if (empty($order['istrade'])) 
			{
				// header('location: ' . mobileUrl('order'));
			...
			else 
			{
				...
				show_json(0, mobileUrl('newstore/norder'));
			}
			// exit();
		}
		unset($_SESSION[WX_SHOP_PREFIX . '_order_pay_complete']);
		...
		if (!(empty($ispeerpay))) 
		{
			...
			$peerpayuid = m('member')->getInfo($_GPC['openid']);
			$peerprice = pdo_fetch('SELECT `price` FROM ' . tablename('wx_shop_order_peerpay_payinfo') . ' WHERE uid = :uid ORDER BY id DESC LIMIT 1', array(':uid' => $peerpayuid['id']));
		...
		else 
		{
			...
			if ($order['isverify']) 
			{
				...
				{
					$stores = pdo_fetchall('select * from ' . tablename('wx_shop_store') . ' where id = :id  and uniacid=:uniacid and status=1  order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':id' => $order['storeid']));
				}
				else if (empty($storeids)) 
				{
					if (0 < $merchid) 
					{
						$stores = pdo_fetchall('select * from ' . tablename('wx_shop_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
					}
					else 
					{
						$stores = pdo_fetchall('select * from ' . tablename('wx_shop_store') . ' where  uniacid=:uniacid and status=1 order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
					}
				}
				else if (0 < $merchid) 
				{
					$stores = pdo_fetchall('select * from ' . tablename('wx_shop_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else 
				{
					$stores = pdo_fetchall('select * from ' . tablename('wx_shop_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
				}
			}
			if (p('lottery')) 
			{
		...
		if (!(empty($order['virtual'])) && !(empty($order['virtual_str']))) 
		{
			...
			$virtualtemp = pdo_fetch('SELECT linktext, linkurl FROM ' . tablename('wx_shop_virtual_type') . ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid']));
		}
		$order['price'] = number_format($order['price'],2);
		show_json(1, array(
			'address'=>$address,
			'isonlyverifygoods'=>$isonlyverifygoods,
			'lottery_changes'=>$lottery_changes,
			'order'=>$order,
			'stores'=>$stores,
			'hasverifygood'=>$hasverifygood,
			'carrier'=>$carrier,
	...
	public function peerpayshare() 
	{
		...
		else if ($peerpay['openid'] !== $_GPC['openid']) 
		{
