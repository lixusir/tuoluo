...
class Refund_WxShopPage //extends MobileLoginPage
{
	protected function globalData() 
	{
		...
		$order = pdo_fetch('select id,status,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,finishtime,isverify,`virtual`,refundstate,merchid from ' . tablename('wx_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
		...
		if ($order['status'] == 2) 
		{
			$goods = pdo_fetchall('select og.goodsid, og.price, og.total, og.optionname, g.cannotrefund, g.thumb, g.title,g.isfullback from' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.orderid=' . $order['id']);
			if (!(empty($goods))) 
			{
				foreach ($goods as $g ) 
				{
		...
		$fullback_log = pdo_fetch('select * from ' . tablename('wx_shop_fullback_log') . ' where orderid = ' . $orderid . ' and uniacid = ' . $uniacid . ' ');
		if ($fullback_log) 
		{
			$fullbackgoods = pdo_fetch('select refund from ' . tablename('wx_shop_fullback_goods') . ' where goodsid = ' . $fullback_log['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
		...
		show_json(1, array('uniacid' => $uniacid, 'openid' => $_W['openid'], 'orderid' => $orderid, 'order' => $order, 'refundid' => $order['refundid'], 'fullback_log' => $fullback_log, 'fullbackgoods' => $fullbackgoods, 'orderprice' => $orderprice));
	}
	public function main() 
	{
		...
		if (0 < $order['refundstate']) 
		{
			if (!(empty($refundid))) 
			{
				$refund = pdo_fetch('select * from ' . tablename('wx_shop_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1', array(':id' => $refundid, ':uniacid' => $uniacid, ':orderid' => $orderid));
	...
	public function submit() 
	{
		...
		if ($order['refundstate'] == 0) 
		{
			...
			pdo_insert('wx_shop_order_refund', $refund);
			$refundid = pdo_insertid();
			pdo_update('wx_shop_order', array('refundid' => $refundid, 'refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
		}
		else 
		{
			pdo_update('wx_shop_order', array('refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
			pdo_update('wx_shop_order_refund', $refund, array('id' => $refundid, 'uniacid' => $uniacid));
		}
	...
	public function cancel() 
	{
		...
		pdo_update('wx_shop_order_refund', $change_refund, array('id' => $refundid, 'uniacid' => $uniacid));
		pdo_update('wx_shop_order', array('refundstate' => 0), array('id' => $orderid, 'uniacid' => $uniacid));
		show_json(1);
	}
	...
	public function receive() 
	{
		...
		pdo_update('wx_shop_order_refund', $refund_data, array('id' => $refundid, 'uniacid' => $uniacid));
		$order_data = array();
		$order_data['refundstate'] = 0;
		$order_data['status'] = -1;
		$order_data['refundtime'] = $time;
		pdo_update('wx_shop_order', $order_data, array('id' => $orderid, 'uniacid' => $uniacid));
		show_json(1);
	}
