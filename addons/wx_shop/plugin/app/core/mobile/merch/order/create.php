...
class Create_WxShopPage //extends MobileLoginPage
{
	...
	public function main() 
	{
		...
		if (p('threen')) 
		{
			$threenvip = p('threen')->getMember($_GPC['openid']);
			if (!(empty($threenvip))) 
			{
				$threenprice = true;
			}
		...
		if (!($packageid)) 
		{
			...
			if (p('bargain') && !(empty($bargain_id))) 
			{
				...
				$bargain_act = pdo_fetch('SELECT *  FROM ' . tablename('wx_shop_bargain_actor') . ' WHERE id = :id AND openid = :openid AND status = \'0\'', array(':id' => $bargain_id, ':openid' => $_GPC['openid']));
				if (empty($bargain_act)) 
				{
					// exit('没有这个商品!');
					show_json(0, '没有这个商品');
				}
				$bargain_act_id = pdo_fetch('SELECT *  FROM ' . tablename('wx_shop_bargain_goods') . ' WHERE id = \'' . $bargain_act['goods_id'] . '\'');
				if (empty($bargain_act_id)) 
				{
					// exit('没有这个商品!');
					show_json(0, '没有这个商品');
				}
				$if_bargain = pdo_fetch('SELECT bargain FROM ' . tablename('wx_shop_goods') . ' WHERE id = :id AND uniacid = :uniacid ', array(':id' => $bargain_act_id['goods_id'], ':uniacid' => $_W['uniacid']));
			...
			if (empty($id)) 
			{
				if (!(empty($quickid))) 
				{
					$sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype' . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,' . ' g.storeids,g.isverify,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts,' . ' g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy ' . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates,g.isfullback, ' . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,g.unite_total,' . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption, g.threen' . ' FROM ' . tablename('wx_shop_quick_cart') . ' c ' . ' left join ' . tablename('wx_shop_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('wx_shop_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid and c.quickid=' . $quickid . '  order by c.id desc';
					$goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
				}
				else if (empty($exchangeOrder)) 
				{
					$sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype' . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,' . ' g.storeids,g.isverify,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts,' . ' g.deduct2,g.ednum,g.edmoney,g.edareas,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy ' . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates, ' . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,' . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption' . ' FROM ' . tablename('wx_shop_member_cart') . ' c ' . ' left join ' . tablename('wx_shop_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('wx_shop_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc';
					$goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
				}
				else if (p('exchange')) 
				{
					$sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype' . ',g.weight,o.weight as optionweight,g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice,o.title as optiontitle,c.optionid,' . ' g.storeids,g.isverify,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts,' . ' g.deduct2,g.ednum,g.edmoney,g.edareas,g.edareas_code,g.diyformtype,g.diyformid,diymode,g.dispatchtype,g.dispatchid,g.dispatchprice,g.minbuy ' . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates,g.isfullback, ' . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,g.unite_total,' . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption' . ' FROM ' . tablename('wx_shop_exchange_cart') . ' c ' . ' left join ' . tablename('wx_shop_goods') . ' g on c.goodsid = g.id ' . ' left join ' . tablename('wx_shop_goods_option') . ' o on c.optionid = o.id ' . ' where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc';
					$goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
				}
				if (empty($goods)) 
				{
					...
					foreach ($goods as $k => $v ) 
					{
						...
						if (0 < $v['hasoption']) 
						{
							...
							if (empty($opdata) || empty($v['optionid'])) 
							{
								...
								show_json(0, '商品' . $v['title'] . '的规格不存在,请到购物车删除该商品重新选择规格!');
							}
						...
						if ($v['isfullback']) 
						{
							$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $v['goodsid'] . ' limit 1 ');
						}
						if ($is_openmerch == 0) 
						{
							if (0 < $v['merchid']) 
						...
						$goods[$k]['seckillinfo'] = plugin_run('seckill::getSeckill', $v['goodsid'], $v['optionid'], true, $_GPC['openid']);
						if (!(empty($goods[$k]['seckillinfo']['maxbuy'])) && (($goods[$k]['seckillinfo']['maxbuy'] - $goods[$k]['seckillinfo']['selfcount']) < $goods[$k]['total'])) 
						{
							...
							show_json(0, '您已购买了' . $goods[$k]['seckillinfo']['selfcount'] . '最多购买' . $goods[$k]['seckillinfo']['maxbuy'] . '件');
						}
					}
			...
			else if (!(empty($id)) && !(empty($iswholesale))) 
			{
				...
				if (empty($optionsdata) || !(is_array($optionsdata))) 
				{
					...
					show_json(0, '商品' . $data['title'] . '的规格不存在,请重新选择规格!');
				}
				...
				foreach ($optionsdata as $option ) 
				{
					...
					{
						$option = pdo_fetch('select id,title,marketprice,presellprice,goodssn,productsn,`virtual`,stock,weight,specs from ' . tablename('wx_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $option['optionid']));
						if (!(empty($option))) 
						{
							...
							if (empty($data['unite_total'])) 
							{
								...
								if ($option['stock'] < $num) 
								{
									...
									show_json(0, '商品' . $data['title'] . '的购买数量超过库存剩余数量,请重新选择规格!');
								}
							}
			...
			else 
			{
				...
				$sql = 'SELECT id as goodsid,type,title,weight,issendfree,isnodiscount,ispresell,presellprice,' . ' thumb,marketprice,liveprice,islive,storeids,isverify,deduct,hasoption,preselltimeend,presellsendstatrttime,presellsendtime,presellsendtype,' . ' manydeduct,`virtual`,maxbuy,usermaxbuy,discounts,total as stock,deduct2,showlevels,' . ' ednum,edmoney,edareas,edareas_code,unite_total,' . ' diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,cates,minbuy, ' . ' isdiscount,isdiscount_time,isdiscount_discounts,isfullback, ' . ' virtualsend,invoice,needfollow,followtip,followurl,merchid,checked,merchsale, ' . ' buyagain,buyagain_islong,buyagain_condition, buyagain_sale' . $threensql . $ishotelsql . ' FROM ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
				$data = pdo_fetch($sql, array(':uniacid' => $uniacid, ':id' => $id));
				...
				$data['seckillinfo'] = plugin_run('seckill::getSeckill', $data['goodsid'], $optionid, true, $_GPC['openid']);
				if ($data['seckillinfo'] && ($data['seckillinfo']['status'] == 0)) 
				{
				}
				else if ($giftid) 
				{
					$gift = pdo_fetch('select id,title,thumb,activity,giftgoodsid,goodsid from ' . tablename('wx_shop_gift') . "\r\n" . '                where uniacid = ' . $uniacid . ' and id = ' . $giftid . ' and status = 1 and starttime <= ' . time() . ' and endtime >= ' . time() . ' ');
					if (!(strstr($gift['goodsid'], (string) $goodsid))) 
					{
						// $this->message('赠品与商品不匹配或者商品没有赠品!', '', 'error');
						show_json(0, '赠品与商品不匹配或者商品没有赠品!');
					}
					$giftGood = array();
					if (!(empty($gift['giftgoodsid']))) 
					{
						...
						if ($giftGoodsid) 
						{
							...
							{
								$giftGood[$key] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and total > 0 and status = 2 and id = ' . $value . ' and deleted = 0 ');
							}
				...
				if ($data['isfullback']) 
				{
					$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $data['goodsid'] . ' limit 1 ');
				}
			...
			foreach ($goods as &$g ) 
			{
				...
				else 
				{
					if (p('task')) 
					{
						...
						if (!(empty($task_id))) 
						{
							$rewarded = pdo_fetchcolumn('SELECT `rewarded` FROM ' . tablename('wx_shop_task_extension_join') . ' WHERE id = :id AND openid = :openid AND  uniacid = :uniacid', array(':id' => $task_id, ':uniacid' => $_W['uniacid'], ':openid' => $_GPC['openid']));
				...
				else 
				{
					...
					if (0 < $g['usermaxbuy']) 
					{
						$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=0 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $g['goodsid'], ':uniacid' => $uniacid, ':openid' => $openid));
			...
			if ($hasinvoice) 
			{
				$invoicename = pdo_fetchcolumn('select invoicename from ' . tablename('wx_shop_order') . ' where openid=:openid and uniacid=:uniacid and ifnull(invoicename,\'\')<>\'\'', array(':openid' => $openid, ':uniacid' => $uniacid));
			...
			if (!($isverify) && !($isvirtual) && !($ismerch)) 
			{
				if (0 < $merch_id) 
				{
					$carrier_list = pdo_fetchall('select * from ' . tablename('wx_shop_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(1,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merch_id));
				}
				else 
				{
					$carrier_list = pdo_fetchall('select * from ' . tablename('wx_shop_store') . ' where  uniacid=:uniacid and status=1 and type in(1,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
				}
			}
			...
			if ($isverify) 
			{
				...
				if (empty($storeids)) 
				{
					if (0 < $merchid) 
					{
						$stores = pdo_fetchall('select * from ' . tablename('wx_shop_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
					}
					else 
					{
						$stores = pdo_fetchall('select * from ' . tablename('wx_shop_store') . ' where  uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
					}
				}
				else if (0 < $merchid) 
				{
					$stores = pdo_fetchall('select * from ' . tablename('wx_shop_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else 
				{
					$stores = pdo_fetchall('select * from ' . tablename('wx_shop_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
				}
			}
			else 
			{
				$address = pdo_fetch('select * from ' . tablename('wx_shop_member_address') . ' where openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid limit 1', array(':uniacid' => $uniacid, ':openid' => $openid));
			...
			foreach ($goods as $g ) 
			{
				...
				{
					$gift = array();
					$giftdata = pdo_fetch('select giftgoodsid from ' . tablename('wx_shop_gift') . ' where uniacid = ' . $uniacid . ' and id = ' . $giftid . ' and status = 1 and starttime <= ' . time() . ' and endtime >= ' . time() . ' ');
					if ($giftdata['giftgoodsid']) 
					{
						$giftgoodsid = explode(',', $giftdata['giftgoodsid']);
						foreach ($giftgoodsid as $key => $value ) 
						{
							$gift[$key] = pdo_fetch('select id as goodsid,title,thumb from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and total > 0 and status = 2 and id = ' . $value . ' and deleted = 0 ');
							if ($gift[$key]) 
							{
				...
				else 
				{
					...
					foreach ($gifts as $key => $value ) 
					{
						...
						{
							$giftgoodsdetail = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0 and total > 0 and status = 2 and id = ' . $val . ' ');
							if ($giftgoodsdetail) 
							{
		...
		else 
		{
			...
			{
				$goods[$key] = pdo_fetch('select id,title,thumb,marketprice from ' . tablename('wx_shop_goods') . "\r\n" . '                            where id = ' . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
				$option = array();
				$packagegoods = array();
				if (0 < $value['optionid']) 
				{
					$option = pdo_fetch('select title,packageprice,marketprice from ' . tablename('wx_shop_package_goods_option') . "\r\n" . '                            where optionid = ' . $value['optionid'] . ' and goodsid=' . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' and pid = ' . $packageid . ' ');
					$goods[$key]['packageprice'] = $option['packageprice'];
				}
				else 
				{
					$packagegoods = pdo_fetch('select title,packageprice,marketprice from ' . tablename('wx_shop_package_goods') . "\r\n" . '                            where goodsid=' . $value['goodsid'] . ' and uniacid = ' . $uniacid . ' and pid = ' . $packageid . ' ');
		...
		show_json(1, array(
			...
			'ordervirtual'=>$ordervirtual,
			'isonlyverifygood'=>$isonlyverifygood,
		));
	}
	...
	public function caculatecoupon($contype, $couponid, $wxid, $wxcardid, $wxcode, $goodsarr, $totalprice, $discountprice, $isdiscountprice, $isSubmit = 0, $discountprice_array = array(), $merchisdiscountprice = 0) 
	{
		...
		if ($contype == 1) 
		{
			$sql = 'select id,uniacid,card_type,logo_url,title, card_id,least_cost,reduce_cost,discount,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids,merchid,limitdiscounttype  from ' . tablename('wx_shop_wxcard');
		...
		{
			$sql = 'SELECT d.id,d.couponid,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodids,c.limitgoodcateids,c.limitdiscounttype  FROM ' . tablename('wx_shop_coupon_data') . ' d';
	...
	public function caculate() 
	{
		...
		if (is_array($goodsarr)) 
		{
			...
			foreach ($goodsarr as &$g ) 
			{
				...
				if (!(empty($optionid))) 
				{
					$option = pdo_fetch('select id,title,marketprice,presellprice,goodssn,productsn,stock,`virtual`,weight,liveprice,islive from ' . tablename('wx_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
					if (!(empty($option))) 
					{
						$data['optionid'] = $optionid;
						$data['optiontitle'] = $option['title'];
	...
	public function submit() 
	{
		...
		if (!(empty($packageid))) 
		{
			$package = pdo_fetch('SELECT id,title,price,freight,cash,starttime,endtime FROM ' . tablename('wx_shop_package') . "\r\n" . '                    WHERE uniacid = ' . $uniacid . ' and id = ' . $packageid . ' and deleted = 0 and status = 1  ORDER BY id DESC');
			if (empty($package)) 
			{
				show_json(0, '未找到套餐！');
			}
			...
			$packgoods = pdo_fetchall('SELECT id,title,thumb,packageprice,`option`,goodsid FROM ' . tablename('wx_shop_package_goods') . "\r\n" . '                    WHERE uniacid = ' . $uniacid . ' and pid = ' . $packageid . '  ORDER BY id DESC');
			if (empty($packgoods)) 
			{
				show_json(0, '未找到套餐商品！');
			}
		...
		if ($giftid) 
		{
			...
			if ($giftdata['giftgoodsid']) 
			{
				...
				foreach ($giftgoodsid as $key => $value ) 
				{
					$gift[$key] = pdo_fetch('select id as goodsid,title,thumb from ' . tablename('wx_shop_goods') . ' where uniacid = ' . $uniacid . ' and deleted = 0 and status = 2 and id = ' . $value . ' and deleted = 0 ');
				}
				$gift = array_filter($gift);
		...
		foreach ($goods as $g ) 
		{
			...
			else 
			{
				...
				if (p('lottery')) 
				{
					$lottery_id = pdo_get('wx_shop_lottery_log', array('log_id' => $log_id), array('lottery_id'));
					if ($lottery_id['lottery_id']) 
					{
						$is_goods = pdo_get('wx_shop_lottery', array('lottery_id' => $lottery_id['lottery_id']), array('is_goods'));
			...
			else 
			{
				...
				{
					$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=0 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $data['goodsid'], ':uniacid' => $uniacid, ':openid' => $openid));
			...
			if (!(empty($optionid))) 
			{
				$option = pdo_fetch('select id,title,marketprice,liveprice,presellprice,goodssn,productsn,stock,`virtual`,weight' . $sql_condition . ' from ' . tablename('wx_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
				if (!(empty($option))) 
				{
					if (!(empty($_SESSION['exchange'])) && p('exchange')) 
					{
						...
						{
							pdo_query('UPDATE ' . tablename('wx_shop_goods_option') . ' SET exchange_stock = exchange_stock - 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $optionid, ':uniacid' => $_W['uniacid']));
						}
					}
					else 
					{
					...
					if ($data['type'] != 4) 
					{
						...
						if ($packageid) 
						{
							$packageoption = pdo_fetch('select packageprice from ' . tablename('wx_shop_package_goods_option') . "\r\n" . '                                where uniacid = ' . $uniacid . ' and goodsid = ' . $goodsid . ' and optionid = ' . $optionid . ' and pid = ' . $packageid . ' ');
			...
			if ($packageid) 
			{
				$pg = pdo_fetch('select packageprice from ' . tablename('wx_shop_package_goods') . "\r\n" . '                                where uniacid = ' . $uniacid . ' and goodsid = ' . $goodsid . ' and pid = ' . $packageid . ' ');
			...
			else if (!(empty($_SESSION['exchange'])) && p('exchange')) 
			{
				...
				{
					pdo_query('UPDATE ' . tablename('wx_shop_goods') . ' SET exchange_stock = exchange_stock - 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $data['goodsid'], ':uniacid' => $_W['uniacid']));
				}
				else 
				{
			...
			if (intval($_GPC['fromcart']) == 1) 
			{
				if ($diyform_plugin) 
				{
					$cartdata = pdo_fetch('select id,diyformdataid,diyformfields,diyformdata from ' . tablename('wx_shop_member_cart') . ' ' . ' where goodsid=:goodsid and optionid=:optionid and openid=:openid and deleted=0 order by id desc limit 1', array(':goodsid' => $data['goodsid'], ':optionid' => intval($data['optionid']), ':openid' => $openid));
					if (!(empty($cartdata))) 
					{
		...
		if ($isverify || $dispatchtype) 
		{
			if ($isverify) 
			{
				if (($verifytype == 0) || ($verifytype == 1)) 
				{
					...
					while (1) 
					{
						$count = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
						if ($count <= 0) 
						{
							break;
						}
				...
				else if ($verifytype == 2) 
				{
					...
					while ($i <= $totaltimes) 
					{
						...
						while (1) 
						{
							$count = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where concat(verifycodes,\'|\' + verifycode +\'|\' ) like :verifycodes and uniacid=:uniacid limit 1', array(':verifycodes' => '%' . $verifycode . '%', ':uniacid' => $_W['uniacid']));
							if ($count <= 0) 
							{
								break;
							}
		...
		{
			$bargain_act = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_bargain_actor') . ' WHERE id = :id AND openid = :openid ', array(':id' => $goods[0]['bargain_id'], ':openid' => $_GPC['openid']));
			if (empty($bargain_act)) 
			{
				exit('没有这个商品');
			}
			...
			if (!(pdo_update('wx_shop_bargain_actor', array('status' => 1), array('id' => $goods[0]['bargain_id'], 'openid' => $_GPC['openid']))))
			{
				exit('下单失败');
			}
		...
		if ($taskgoodsprice) 
		{
			...
			pdo_update('wx_shop_task_extension_join', array('rewarded' => serialize($taskGoodsInfo0)), array('id' => $task_id, 'uniacid' => $_W['uniacid'], 'openid' => $_GPC['openid']));
		...
		pdo_insert('wx_shop_order', $order);
		$orderid = pdo_insertid();
		if (!(empty($goods[0]['bargain_id'])) && p('bargain')) 
		{
			pdo_update('wx_shop_bargain_actor', array('order' => $orderid), array('id' => $goods[0]['bargain_id'], 'openid' => $_GPC['openid']));
		}
		if ($multiple_order == 0) 
		{
			...
			foreach ($allgoods as $goods ) 
			{
				...
				if (!(empty($bargain_act)) && p('bargain')) 
				{
					...
					pdo_query('UPDATE ' . tablename('wx_shop_goods') . ' SET sales = sales + 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $goods['goodsid'], ':uniacid' => $uniacid));
				}
				...
				pdo_insert('wx_shop_order_goods', $order_goods);
				if ($goods['seckillinfo'] && ($goods['seckillinfo']['status'] == 0)) 
				{
					plugin_run('seckill::setSeckill', $goods['seckillinfo'], $goods, $_GPC['openid'], $orderid, 0, $order['createtime']);
				}
			}
		}
		else 
		...
		if ($data['type'] == 3) 
		{
			$order_v = pdo_fetch('select id,ordersn, price,openid,dispatchtype,addressid,carrier,status,isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,createtime,buyagainprice,istrade,tradestatus from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and id = :id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
			com('virtual')->pay_befo($order_v);
		}
		if (!(empty($_SESSION['exchange'])) && p('exchange')) 
		{
			$codeResult = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_code') . ' WHERE `key` = :key AND uniacid =:uniacid', array(':key' => $_SESSION['exchange_key'], ':uniacid' => $_W['uniacid']));
			if ($codeResult['status'] == 2) 
			{
				show_json(0, '兑换失败:此兑换码已兑换');
			}
			$groupResult = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE id = :id AND uniacid = :uniacid ', array(':uniacid' => $_W['uniacid'], ':id' => $codeResult['groupid']));
			$record_exsit = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_record') . ' WHERE `key`=:key AND uniacid = :uniacid', array(':key' => $_SESSION['exchange_key'], ':uniacid' => $_W['uniacid']));
			if (empty($record_exsit)) 
			{
				...
				pdo_insert('wx_shop_exchange_record', $data);
			}
			...
			else 
			{
				...
				{
					$exchange_res = pdo_update('wx_shop_exchange_code', array('status' => 2), array('key' => $_SESSION['exchange_key'], 'status' => 1, 'uniacid' => $_W['uniacid']));
					pdo_query('UPDATE ' . tablename('wx_shop_exchange_group') . ' SET `use` = `use` + 1 WHERE id = :id AND uniacid = :uniacid', array(':id' => $_SESSION['groupid'], ':uniacid' => $_W['uniacid']));
				}
				else 
				{
					pdo_update('wx_shop_exchange_code', array('goodsstatus' => 2), array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid']));
					...
					{
						pdo_update('wx_shop_exchange_code', array('status' => 2), array('key' => $_SESSION['exchange_key'], 'status' => 1, 'uniacid' => $_W['uniacid']));
					}
				}
			}
		}
		if (!(is_null($_SESSION['exchangeprice'])) && !(empty($_SESSION['exchange'])) && p('exchange')) 
		{
			$exchangeinfo = m('member')->getInfo($_GPC['openid']);
			if ($groupResult['mode'] != 6) 
			{
				$exchangedata = array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid'], 'goods' => $_SESSION['exchangegoods'], 'orderid' => $orderid, 'time' => time(), 'openid' => $_GPC['openid'], 'mode' => 1, 'nickname' => $exchangeinfo['nickname'], 'groupid' => $_SESSION['groupid'], 'title' => $_SESSION['exchangetitle'], 'serial' => $_SESSION['exchangeserial'], 'ordersn' => $order['ordersn'], 'goods_title' => $exchangetitle);
				pdo_update('wx_shop_exchange_record', $exchangedata, array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid']));
			}
			else 
			{
				$exchangedata = array('goods' => $_SESSION['exchangegoods'], 'orderid' => $orderid, 'time' => time(), 'openid' => $_GPC['openid'], 'mode' => 6, 'nickname' => $exchangeinfo['nickname'], 'ordersn' => $order['ordersn'], 'goods_title' => $exchangetitle);
				pdo_update('wx_shop_exchange_record', $exchangedata, array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid']));
			}
			pdo_update('wx_shop_exchange_cart', array('selected' => 0), array('uniacid' => $_W['uniacid'], 'openid' => $_GPC['openid'], 'selected' => 1));
		}
		...
		if (is_array($carrier)) 
		{
			...
			pdo_update('wx_shop_member', $up, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			if (!(empty($member['uid']))) 
			{
				load()->model('mc');
		...
		if ($_GPC['fromcart'] == 1) 
		{
			pdo_query('update ' . tablename('wx_shop_member_cart') . ' set deleted=1 where  openid=:openid and uniacid=:uniacid and selected=1 ', array(':uniacid' => $uniacid, ':openid' => $openid));
		}
		if (p('quick') && !(empty($_GPC['fromquick']))) 
		{
			pdo_update('wx_shop_quick_cart', array('deleted' => 1), array('quickid' => intval($_GPC['fromquick']), 'uniacid' => $_W['uniacid'], 'openid' => $_GPC['openid']));
		}
		if (0 < $deductcredit) 
		{
		...
		{
			$vgoods = $allgoods[0];
			pdo_update('wx_shop_goods', array('sales' => $vgoods['sales'] + $vgoods['total']), array('id' => $vgoods['goodsid']));
		}
	...
	protected function singleDiyformData($id = 0) 
	{
		...
		{
			$sql = 'SELECT id as goodsid,type,diyformtype,diyformid,diymode,diyfields FROM ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
	...
	public function diyform() 
	{
		...
		if (empty($goods_temp)) 
		{
			pdo_insert('wx_shop_diyform_temp', $insert);
			$gdid = pdo_insertid();
		}
		else 
		{
			pdo_update('wx_shop_diyform_temp', $insert, array('id' => $goods_temp['id']));
			$gdid = $goods_temp['id'];
		}
		if (!(empty($cartid))) 
		{
			$cart_data = array('diyformid' => $insert['diyformid'], 'diyformfields' => $insert['diyformfields'], 'diyformdata' => $insert['diyformdata']);
			pdo_update('wx_shop_member_cart', $cart_data, array('id' => $cartid));
		}
