...
class Index_WxShopPage //extends MobileLoginPage
{
	...
	public function get_list() 
	{
		...
		foreach ($list as &$row ) 
		{
			...
			{
				$refund = pdo_fetch('select * from ' . tablename('wx_shop_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1', array(':id' => $row['refundid'], ':uniacid' => $uniacid, ':orderid' => $row['id']));
				if (!(empty($refund))) 
				{
	...
	public function detail() 
	{
		...
		if (!(empty($order['istrade']))) 
		{
			...
			// exit();
			show_json(0, mobileUrl('newstore/norder/detail', array('id' => $orderid)));	
		}
		$isonlyverifygood = m('order')->checkisonlyverifygoods($order['id']);
		if ($order['refundid'] != 0) 
		{
			$refund = pdo_fetch('SELECT *  FROM ' . tablename('wx_shop_order_refund') . ' WHERE orderid = :orderid and uniacid=:uniacid order by id desc', array(':orderid' => $order['id'], ':uniacid' => $_W['uniacid']));
		}
		...
		$goodsid_array = array();
		$goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,g.status, g.cannotrefund, og.total,g.credit,og.optionid,' . "\r\n" . '            og.optionname as optiontitle,g.isverify,g.storeids,og.seckill,g.isfullback,' . "\r\n" . '            og.seckill_taskid' . $diyformfields . $condition1 . ',og.prohibitrefund  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where ' . $scondition . ' and og.uniacid=:uniacid ', $param);
		$prohibitrefund = false;
		foreach ($goods as &$g ) 
		{
			if ($g['isfullback']) 
			{
				$fullbackgoods = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_fullback_goods') . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $g['goodsid'] . ' limit 1 ');
				if ($g['optionid']) 
				{
					$option = pdo_fetch('select `day`,allfullbackprice,fullbackprice,allfullbackratio,fullbackratio,isfullback' . "\r\n" . '                      from ' . tablename('wx_shop_goods_option') . ' where id = ' . $g['optionid'] . ' and uniacid = ' . $uniacid . ' ');
		...
		{
			$order_goods = pdo_fetchall('select orderid,goodsid,sendtype,expresscom,expresssn,express,sendtime from ' . tablename('wx_shop_order_goods') . "\r\n" . '            where orderid = ' . $orderid . ' and uniacid = ' . $uniacid . ' and sendtype > 0 group by sendtype order by sendtime asc ');
			$expresslist = m('util')->getExpressList($order['express'], $order['expresssn']);
			if (0 < count($expresslist)) 
			{
				$express = $expresslist[0];
	...
	public function express() 
	{
		...
		if ((0 < $order['sendtype']) && ($sendtype == 0)) 
		{
			...
			while ($i <= intval($order['sendtype'])) 
			{
				...
				$bundlelist[$i]['goods'] = pdo_fetchall('select g.title,g.thumb,og.total,og.optionname as optiontitle,og.expresssn,og.express,' . "\r\n" . '                    og.sendtype,og.expresscom,og.sendtime from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.orderid=:orderid and og.sendtype = ' . $i . ' and og.uniacid=:uniacid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
