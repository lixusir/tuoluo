...
class Cart_WxShopPage //extends MobileLoginPage
{
	...
	public function select() 
	{
		...
		if (!(empty($id))) 
		{
			$data = pdo_fetch('select id,goodsid,optionid, total from ' . tablename('wx_shop_member_cart') . ' ' . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
			if (!(empty($data))) 
			{
				pdo_update('wx_shop_member_cart', array('selected' => $select), array('id' => $id, 'uniacid' => $_W['uniacid']));
			}
		}
		else 
		{
			pdo_update('wx_shop_member_cart', array('selected' => $select), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
		}
		show_json(1);
	}
	public function update() 
	{
		...
		$goods = pdo_fetch('select id,maxbuy,minbuy,total,unit from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid and status=1 and deleted=0', array(':id' => $data['goodsid'], ':uniacid' => $_W['uniacid']));
		if (empty($goods)) 
		{
			show_json(0, '商品未找到');
		}
		pdo_update('wx_shop_member_cart', array('total' => $goodstotal, 'optionid' => $optionid), array('id' => $id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
	...
	public function add() 
	{
		...
		($total <= 0) && ($total = 1);
		$optionid = intval($_GPC['optionid']);
		$goods = pdo_fetch('select id,marketprice,diyformid,diyformtype,diyfields, isverify, `type`,merchid, cannotrefund from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (empty($goods)) 
		{
			show_json(0, '商品未找到');
		}
		...
		$gift = pdo_fetch('select * from ' . tablename('wx_shop_gift') . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $giftid . ' and starttime >= ' . time() . ' and endtime <= ' . time() . ' and status = 1 ');
		...
		$data = pdo_fetch('select id,total,diyformid from ' . tablename('wx_shop_member_cart') . ' where goodsid=:id and openid=:openid and   optionid=:optionid  and deleted=0 and  uniacid=:uniacid   limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':optionid' => $optionid, ':id' => $id));
		if (empty($data)) 
		{
			...
			pdo_insert('wx_shop_member_cart', $data);
		}
		else 
		{
			...
			$data['total'] += $total;
			pdo_update('wx_shop_member_cart', $data, array('id' => $data['id']));
		}
		$cartcount = pdo_fetchcolumn('select sum(total) from ' . tablename('wx_shop_member_cart') . ' where openid=:openid and deleted=0 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
	...
	public function addwholesale() 
	{
		...
		foreach ($optionsdata as $option ) 
		{
			...
			$data = pdo_fetch('select id,total,diyformid from ' . tablename('wx_shop_member_cart') . ' where goodsid=:id and openid=:openid and   optionid=:optionid  and deleted=0 and  uniacid=:uniacid   limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':optionid' => $option['optionid'], ':id' => $id));
			if (empty($data)) 
			{
	...
	public function remove() 
	{
		...
		$sql = 'update ' . tablename('wx_shop_member_cart') . ' set deleted=1 where uniacid=:uniacid and openid=:openid and id in (' . implode(',', $ids) . ')';
	...
	public function tofavorite() 
	{
		...
		foreach ($ids as $id ) 
		{
			$goodsid = pdo_fetchcolumn('select goodsid from ' . tablename('wx_shop_member_cart') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1 ', array(':id' => $id, ':uniacid' => $uniacid, ':openid' => $openid));
			if (!(empty($goodsid))) 
			{
				$fav = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member_favorite') . ' where goodsid=:goodsid and uniacid=:uniacid and openid=:openid and deleted=0 limit 1 ', array(':goodsid' => $goodsid, ':uniacid' => $uniacid, ':openid' => $openid));
				if ($fav <= 0) 
				{
					$fav = array('uniacid' => $uniacid, 'goodsid' => $goodsid, 'openid' => $openid, 'deleted' => 0, 'createtime' => time());
					pdo_insert('wx_shop_member_favorite', $fav);
				}
			}
		}
		$sql = 'update ' . tablename('wx_shop_member_cart') . ' set deleted=1 where uniacid=:uniacid and openid=:openid and id in (' . implode(',', $ids) . ')';
