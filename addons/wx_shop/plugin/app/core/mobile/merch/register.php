<?php
class Register_WxShopPage //extends PluginMobileLoginPage 
{
	public function main() 
	{
		...
		$reg = pdo_fetch('select * from ' . tablename('wx_shop_merch_reg') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':uniacid' => $_W['uniacid']));
		$user = false;
		if (!(empty($reg['status']))) 
		{
			$user = pdo_fetch('select * from ' . tablename('wx_shop_merch_user') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':uniacid' => $_W['uniacid']));
		}
		...
		if ($_W['ispost']) 
		{
			...
			$usercount1 = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_merch_reg') . ' where ' . $where1 . ' limit 1', $params1);
			$where2 = ' username=:username';
			$params2 = array(':username' => $uname);
			$usercount2 = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_merch_account') . ' where ' . $where2 . ' limit 1', $params2);
			if ((0 < $usercount1) || (0 < $usercount2)) 
			{
				show_json(0, '帐号 ' . $uname . ' 已经存在,请更改!');
			}
			$upass = m('util')->pwd_encrypt($upass, 'E');
			$data = array('uniacid' => $_W['uniacid'], 'openid' => $openid, 'status' => 0, 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']), 'uname' => $uname, 'upass' => $upass, 'merchname' => trim($_GPC['merchname']), 'salecate' => trim($_GPC['salecate']), 'desc' => trim($_GPC['desc']));
			...
			{
				$data['applytime'] = time();
				pdo_insert('wx_shop_merch_reg', $data);
			}
			else 
			{
				pdo_update('wx_shop_merch_reg', $data, array('id' => $reg['id']));
			}
			...
			show_json(1,'您的申请已经提交，请等待我们联系您!');
		}
		show_json(1, array('user'=>$user, 'reg'=>$reg, 'template_flag'=>$template_flag, 'set'=>$set, 'apply_set'=>$apply_set));
