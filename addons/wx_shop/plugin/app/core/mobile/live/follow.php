...
class Follow_WxShopPage //extends PluginMobileLoginPage
{
	public function main()
	{
        ...
        // print_r($shop);
         ...
         $shop['saleout']=tomedia($shop['saleout']);
         $shop['loading']=tomedia($shop['loading']);
         foreach($shop["cubes"] as &$cubes){
         	$cubes['img']=tomedia($cubes['img']);
         	$cubes['url']=tomedia($cubes['url']);
         }
        // foreach($shop as )
		show_json(1,array('shop'=>$shop,'set'=>$set));
	...
	public function get_list()
	{
		...
		$sql = 'SELECT COUNT(1) FROM ' . tablename('wx_shop_live_favorite') . " as lf\r\n                    right join " . tablename('wx_shop_live') . ' as l on l.id = lf.roomid and l.living = ' . $living . "\r\n            \t\twhere 1 " . $condition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT lf.id,lf.roomid,l.title,l.thumb,l.livetime,l.subscribe,l.living,l.covertype,l.cover FROM ' . tablename('wx_shop_live_favorite') . " as lf\r\n                    right join " . tablename('wx_shop_live') . ' as l on l.id = lf.roomid and l.living = ' . $living . "\r\n            \t\twhere 1 " . $condition . ' ORDER BY lf.id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
