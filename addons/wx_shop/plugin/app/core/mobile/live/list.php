...
class List_WxShopPage //extends PluginMobileLoginPage
{
	public function main()
	{
     ...
     foreach($shop["cubes"] as &$v){
     	$v["img"]=tomedia($v["img"]);
     	$v["url"]=tomedia($v["url"]);
     }

	show_json(1,array('categorys'=>$categorys,'shop'=>$shop,'cate'=>$cate,'keywords'=>$keywords));
	...
	public function get_list()
	{
		...
		if (!empty($total)) {
			$sql = 'SELECT id,title,thumb,livetime,covertype,cover,subscribe,living FROM ' . tablename('wx_shop_live') . "\r\n            \t\twhere 1 " . $condition . ' ORDER BY displayorder desc,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
			...
			foreach ($list as $key => &$row) {
				...
				$favorite = pdo_fetch('select deleted from ' . tablename('wx_shop_live_favorite') . ' where uniacid = ' . $uniacid . ' and openid = \'' . $openid . '\' and roomid = ' . $row['id'] . ' and deleted = 0  ');
