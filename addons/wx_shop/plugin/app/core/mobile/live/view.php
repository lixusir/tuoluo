...
class View_WxShopPage extends PluginMobileLoginPage
{
	public function main()
	{
		...
		$follows = pdo_fetchall('select * from ' . tablename('wx_shop_live_view') . ' where uniacid = ' . $uniacid . ' and openid = \'' . $openid . '\' ');
	...
	public function get_view()
	{
		...
		$sql = 'SELECT COUNT(1) FROM ' . tablename('wx_shop_live_view') . " as lv\r\n                    right join " . tablename('wx_shop_live') . " as l on l.id = lv.roomid and l.uniacid = lv.uniacid\r\n            \t\twhere 1 " . $condition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();

		if (!empty($total)) {
			$sql = 'SELECT l.id,l.title,l.thumb,l.livetime,l.subscribe,l.living FROM ' . tablename('wx_shop_live_view') . " as lv\r\n                    right join " . tablename('wx_shop_live') . " as l on l.id = lv.roomid\r\n            \t\twhere 1 " . $condition . ' ORDER BY lv.viewing DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
