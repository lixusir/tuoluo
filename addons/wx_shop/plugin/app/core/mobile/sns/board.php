...
class Board_WxShopPage extends Base_WxShopPage//extends SnsMobilePage
{
	...
	public function main()
	{
		...
		$isafollow = m('user')->followed($openid);
		...
		if (!$board['banner']) {
			$board['banner'] = $_W['siteroot'].'addons/wx_shop/plugin/sns/static/images/banner.png';
		} else {
			if (!preg_match('/^http/', $board['banner'])) {
				$board['banner'] = $_W['siteroot'].'attachment/'.$board['banner'];
			}
		}
		show_json(1, array(
			'board' => $board,
	 ...
	 * 获取首页或精华区
	 * @author lucky
	 * @DateTime 2018-08-04T17:08:09+0800
	...
	public function getlist()
	{
		...
		foreach ($list as &$row) {
			// $row['avatar'] = $this->getAvatar($row['avatar']);
			// $row['createtime'] = $this->timeBefore($row['createtime']);
			...
			$row['postcount'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_sns_post') . ' where pid=:pid and deleted = 0 limit 1', array(':pid' => $row['id']));
			...
			$isgood = pdo_fetch('select id from ' . tablename('wx_shop_sns_like') . ' where pid=:pid and openid=:openid limit 1', array(':pid' => $row['id'], ':openid' => $openid ));
			//增加的语句
			if ((empty($isgood))) {
				$row['isgood']  = 0;
			} else {
			...
			//增加一个字段用来显示表情路径
			$row['content_new'] = str_replace('../', 'https://xcxvip.iiio.top/', $row['content']);
			foreach ($row['images'] as $key => &$value) {
				$row['content_new'] =str_replace('../', 'https://xcxvip.iiio.top/', $row['content_new'] . '<p><img src="'.$value.'"></p>');
				// var_dump($value);
			}
	 ...
	 * 获取相关板块
	 * @author lucky
	 * @DateTime 2018-08-04T16:10:23+0800
	 * @return   [type]                   [description]
	 */
	public function relate()
	{
		...
		$sql = 'select id,title,logo,`desc`  from ' . tablename('wx_shop_sns_board') . ' where 1 ' . $condition . ' order by displayorder desc';
		$list = pdo_fetchall($sql, $params);
		// echo '<pre>';
		// print_r($list);
		foreach ($list as &$row) {
	 ...
	 * 获取板块的全部分类
	 * @author lucky
	 * @DateTime 2018-08-06T10:47:13+0800
	 * @return   [type]                   [description]
	 */
	 ...
	 * 获取
	 * @author lucky
	 * @DateTime 2018-08-04T16:02:31+0800
	 * @return   [type]                   [description]
	 */
	public function get_boardlist()
	{
		...
		unset($row);
		$total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('wx_shop_sns_board') . ' as sb WHERE 1 ' . $condition, $params);
	 ...
	 * 关注板块
	 * @author lucky
	 * @DateTime 2018-08-04T15:52:23+0800
	 * @return   [type]                   [description]
	 */
	public function follow()
	{
		...
		$follow = pdo_fetch('select id from ' . tablename('wx_shop_sns_board_follow') . ' where bid=:bid and openid=:openid limit 1', array(':bid' => $bid, ':openid' => $openid ));
		// dd($follow);die;

		if (!empty($follow)) {
			pdo_delete('wx_shop_sns_board_follow', array('id' => $follow['id']));
			show_json(1, array('isfollow' => false));
		}
		else {
			$follow = array('uniacid' => $_W['uniacid'], 'bid' => $bid, 'openid' => $openid, 'createtime' => time());
			pdo_insert('wx_shop_sns_board_follow', $follow);
     ...
     * @return bool
     */
	private function isFollow($bid)
	{
		global $_W;
		global $_GPC;
		$openid = empty($_W['openid']) ? $_GPC['openid']: $_W['openid'];
		$count = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_sns_board_follow') . ' where uniacid=:uniacid and bid=:bid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':bid' => $bid, ':openid' => $openid));
		return 0 < $count;
	}
}
