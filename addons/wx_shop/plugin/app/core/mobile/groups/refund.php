...
// class Refund_WxShopPage extends PluginMobileLoginPage
class Refund_WxShopPage extends AppMobilePage
{
	protected function globalData()
	{
		...
		$goodRefund = false;
		$groupsSet = pdo_fetch('select goodsid,refundday from ' . tablename('wx_shop_groups_set') . 'where uniacid = :uniacid ', array(':uniacid' => $uniacid));
		...
		return array('uniacid' => $uniacid, 'openid' => $openid, 'orderid' => $orderid, 'order' => $order, 'refundid' => $order['refundid']);
	...
	public function main()
	{
		...
		try {
			...
			if (0 < $order['refundstate']) {
				if (!empty($refundid)) {
					$refund = pdo_fetch('select * from ' . tablename('wx_shop_groups_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1', array(':id' => $refundid, ':uniacid' => $uniacid, ':orderid' => $orderid));

					if (!empty($refund['refundaddress'])) {
						$refund['refundaddress'] = iunserializer($refund['refundaddress']);
					}
					else if ($refund['refundaddressid']) {
						$refund['refundaddress'] = pdo_fetch('select * from ' . tablename('wx_shop_member_address') . ' where id = :id and uniacid=:uniacid', array(':id' => $refund['refundaddressid'], ':uniacid' => $_W['uniacid']));
					}
					else {
						$refund['refundaddress'] = pdo_fetch('select * from ' . tablename('wx_shop_refund_address') . ' where uniacid=:uniacid order by id desc limit 1', array(':uniacid' => $uniacid));
					}
				}

				if (!empty($refund['images'])) {
					$refund['images'] = iunserializer($refund['images']);
					if(count($refund['images'])){
						foreach ($refund['images'] as $k => $v) {
							$refund['images'][$k] = tomedia($v);
						}
					}
				}
			}
			...
			p('groups')->groupsShare();
			$express_list = m('express')->getExpressList();
			// include $this->template();
			
			
			
			$refund['count_images']  = count($refund['images']);
			$order['number_format_refundprice'] = number_format($order['refundprice'],2);
			$refund['applytime'] = date('Y-m-d H:i',$refund['applytime']);
			...
			show_json(1, array(
				...
				'refundid'=>$refundid,
				'refund'=>$refund,
				'show_price'=>number_format($show_price,2),
			...
			show_json(0, array('message'=>$content));
		}
	}
	...
	public function submit()
	{
		...
		if ($order['refundstate'] == 0) {
			...
			pdo_insert('wx_shop_groups_order_refund', $refund);
			$refundid = pdo_insertid();
			pdo_update('wx_shop_groups_order', array('refundid' => $refundid, 'refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
		}
		else {
			pdo_update('wx_shop_groups_order', array('refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
			pdo_update('wx_shop_groups_order_refund', $refund, array('id' => $refundid, 'uniacid' => $uniacid));
		}
	...
	public function cancel()
	{
		...
		pdo_update('wx_shop_groups_order_refund', $change_refund, array('id' => $refundid, 'uniacid' => $uniacid));
		pdo_update('wx_shop_groups_order', array('refundstate' => 0), array('id' => $orderid, 'uniacid' => $uniacid));
		show_json(1);
	}
	...
	public function receive()
	{
		...
		pdo_update('wx_shop_groups_order_refund', $refund_data, array('id' => $refundid, 'uniacid' => $uniacid));
		$order_data = array();
		$order_data['refundstate'] = 0;
		$order_data['refundstatus'] = -1;
		$order_data['refundtime'] = $time;
		pdo_update('wx_shop_groups_order', $order_data, array('id' => $orderid, 'uniacid' => $uniacid));
		show_json(1);
	}
