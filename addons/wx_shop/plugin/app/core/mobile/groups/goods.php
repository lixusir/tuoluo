...
require "base.php";
class Goods_WxShopPage //extends PluginMobileLoginPage
{
	public function main()
	{
		...
		if (empty($id) || empty($goods)) {
			...
			show_json(0, array('message' => '你访问的商品不存在或已删除!'));
		}

		if (!empty(iunserializer($goods['thumb_url']))) {
			
			$new_thumb_url = '';
			foreach (iunserializer($goods['thumb_url']) as $k => $v) {
				$new_thumb_url[$k] = tomedia($v);
			}
			$goods['thumb_url'] = array_merge($new_thumb_url);
		}else{
			$goods['thumb_url'] = iunserializer($goods['thumb_url']);
		}
		$goods['fightnum'] = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_groups_order') . ' where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ', array(':goodid' => $goods['id'], ':uniacid' => $uniacid));
		...
		if (empty($goods)) {
			...
			show_json(0, array('message' => '商品已下架或被删除!'));
		}
		...
		if(isset($goods['headsdiscount']) && !empty($goods['headsdiscount'])){
			$goods['headsdiscount'] = number_format($goods['headsdiscount'] / 10,1);
		}
		show_json(1, array('goods' => $goods, 'groupsset' => $groupsset));
	}
	...
	public function openGroups()
	{
		...
		$teams = pdo_fetchall('select * from ' . tablename('wx_shop_groups_goods') . ' where deleted = 0 and status = 1 and uniacid = :uniacid order by sales desc limit 4', array(':uniacid' => $uniacid));

		foreach ($teams as $key => $value) {
			$value['fightnum'] = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_groups_order') . ' where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ', array(':goodid' => $value['id'], ':uniacid' => $uniacid));
		...
		show_json(1, array('teams' => $teams, 'goods' => $goods));
		// include $this->template();
	}
	...
	public function fightGroups()
	{
		...
		$goods['fightnum'] = $goods['teamnum'] + $goods['fightnum'];
		$goods = set_medias($goods, 'thumb');
		$teams = pdo_fetchall('select o.paytime,o.id,o.goodid,o.teamid,m.nickname,m.realname,m.mobile,m.avatar,g.endtime,g.groupnum from ' . tablename('wx_shop_groups_order') . " as o\r\n\t\t\t\tleft join " . tablename('wx_shop_member') . " as m on m.openid=o.openid and m.uniacid =  o.uniacid\r\n\t\t\t\tleft join " . tablename('wx_shop_groups_goods') . " as g on g.id = o.goodid\r\n\t\t\t\twhere o.goodid = :goodid and o.uniacid = :uniacid and o.openid != :openid and o.deleted = 0 and o.heads = 1 and o.paytime > 0 and o.success = 0 limit 10 ", array(':goodid' => $goods['id'], ':uniacid' => $uniacid, ':openid' => $openid));

		foreach ($teams as $key => $value) {
			$num = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_groups_order') . ' where uniacid = :uniacid and deleted = 0 and teamid = :teamid and status > 0 ', array(':teamid' => $value['teamid'], ':uniacid' => $uniacid));
