...
class GrantModel extends PluginModel
{
	public function checkplugin($identity)
	{
		...
		if (!strstr($setting['plugin'], $identity) && !strstr($setting['com'], $identity)) {
			$plugin = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_system_plugingrant_log') . ' WHERE uniacid = ' . $uniacid . ' and `identity` = \'' . $identity . '\' ');
	...
	{
		$item = pdo_fetch('SELECT uniacid,pluginid,`month` FROM ' . tablename('wx_shop_system_plugingrant_log') . ' WHERE id = ' . $id . ' and isperm = 0  ');
		...
		pdo_update('wx_shop_system_plugingrant_log', $data, array('id' => $id));
	}
	...
	public function aliconfig()
	{
		...
		$config = array('partner' => $setting['partner'], 'seller_id' => $setting['partner'], 'key' => $setting['secret'], 'notify_url' => $_W['siteroot'] . 'addons/wx_shop/payment/alipay/pluginverify.php', 'return_url' => webUrl('plugingrant/success', array(), true), 'sign_type' => 'MD5', 'input_charset' => 'UTF-8', 'transport' => 'https', 'payment_type' => '1', 'service' => 'create_direct_pay_by_user', 'anti_phishing_key' => '', 'exter_invoke_ip' => '');
