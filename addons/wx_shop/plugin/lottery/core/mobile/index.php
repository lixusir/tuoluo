...
class index_WxShopPage extends PluginMobilePage
{
	public function main()
	{
        ...
        $lottery=pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_lottery') . ' WHERE 1 ' . $condition . ' ORDER BY addtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$task_sql = 'SELECT COUNT(*) FROM ' . tablename('wx_shop_lottery_join') . '  WHERE lottery_num>0 and uniacid=:uniacid AND `join_user`=:join_user ';
//		$lottery = pdo_fetchcolumn($task_sql, array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
	...
	public function getlotterylist()
	{
		...
		$limit = ($page - 1) * 15;
		$task_sql = 'SELECT j.*,l.lottery_title,l.lottery_type,l.lottery_days FROM ' . tablename('wx_shop_lottery_join') . ' as j left join ' . tablename('wx_shop_lottery') . ' as l on j.lottery_id=l.lottery_id WHERE j.lottery_num>0 and j.uniacid=:uniacid AND j.`join_user`=:join_user AND l.`is_delete`=0 ORDER BY j.addtime DESC LIMIT ' . $limit . ',15';
		...
		foreach ($lottery as $key => $value) {
			...
			if ($value['lottery_type'] == 1) {
				$lottery[$key]['icon'] = '../addons/wx_shop/plugin/lottery/static/images/panicon.png';
			}
			else if ($value['lottery_type'] == 2) {
				$lottery[$key]['icon'] = '../addons/wx_shop/plugin/lottery/static/images/guaicon.png';
			}
			else {
				if ($value['lottery_type'] == 3) {
					$lottery[$key]['icon'] = '../addons/wx_shop/plugin/lottery/static/images/gridicon.png';
				}
			}
		}
	...
	public function lottery_info()
	{
		...
		if ($id) {
			$task_sql = 'SELECT * FROM ' . tablename('wx_shop_lottery') . ' WHERE uniacid=:uniacid AND lottery_id=:id AND `is_delete`=0  ';
			$lottery = pdo_fetch($task_sql, array(':uniacid' => $_W['uniacid'], ':id' => $id));
			$reward = unserialize($lottery['lottery_data']);
			$set = pdo_fetchcolumn('SELECT `data` FROM ' . tablename('wx_shop_lottery_default') . ' WHERE uniacid =:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));

			if (!empty($set)) {
				$set = unserialize($set);
			}

			$log = pdo_fetchall('SELECT l.*,m.`nickname`,m.`avatar` FROM ' . tablename('wx_shop_lottery_log') . ' AS l LEFT JOIN ' . tablename('wx_shop_member') . ' AS m ON m.openid=l.join_user WHERE l.uniacid=:uniacid AND l.lottery_id=:lottery_id AND l.is_reward=1 order by id desc LIMIT 5', array(':uniacid' => $_W['uniacid'], ':lottery_id' => $id));
			...
			$has_changes = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_lottery_join') . 'where uniacid=:uniacid AND lottery_id=:lottery_id  AND join_user=:join_user and lottery_num>0 ' . $conditon, array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid'], ':lottery_id' => $id));
	...
	public function myreward()
	{
		...
		global $_GPC;
		$member = m('member')->getMember($_W['openid'], true);
		$mylog = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_lottery_log') . ' WHERE uniacid=:uniacid  AND join_user=:join_user AND is_reward=1 ', array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
	...
	public function myrewardpage()
	{
		...
		$limit = ($page - 1) * 15;
		$mylog = pdo_fetchall('SELECT l.*,m.`nickname`,m.`avatar` FROM ' . tablename('wx_shop_lottery_log') . ' AS l LEFT JOIN ' . tablename('wx_shop_member') . ' AS m ON m.openid=l.join_user WHERE l.uniacid=:uniacid  AND l.join_user=:join_user AND l.is_reward=1 order by addtime desc LIMIT ' . $limit . ',15', array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
		$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_lottery_log') . ' WHERE uniacid=:uniacid  AND join_user=:join_user AND is_reward=1 ', array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
		...
		foreach ($mylog as $key => $value) {
			...
			if (isset($lottery_data['credit'])) {
				...
				$mylog[$key]['icon'] = '../addons/wx_shop/plugin/lottery/static/images/jifen.png';
			}
			else if (isset($lottery_data['money'])) {
				$mylog[$key]['title'] = '奖金:' . $lottery_data['money']['num'];
				$mylog[$key]['rewarded'] = 1;
				$mylog[$key]['icon'] = '../addons/wx_shop/plugin/lottery/static/images/jiangjin.png';
			}
			else if (isset($lottery_data['bribery'])) {
				$mylog[$key]['title'] = '红包:' . $lottery_data['bribery']['num'];
				$mylog[$key]['rewarded'] = 1;
				$mylog[$key]['icon'] = '../addons/wx_shop/plugin/lottery/static/images/hongbao.png';
			}
			else if (isset($lottery_data['goods'])) {
				$goods = array_shift($lottery_data['goods']);
				$mylog[$key]['title'] = '特惠商品:' . $goods['title'];
				$mylog[$key]['rewarded'] = 0;
				$mylog[$key]['icon'] = '../addons/wx_shop/plugin/lottery/static/images/shangpin.png';
			}
			else {
				if (isset($lottery_data['coupon'])) {
					...
					$mylog[$key]['rewarded'] = 1;
					$mylog[$key]['icon'] = '../addons/wx_shop/plugin/lottery/static/images/quan.png';
				}
			}
	...
	public function getlottery()
	{
		global $_W;
		global $_GPC;
		$set_info = pdo_fetchcolumn('SELECT `data` FROM ' . tablename('wx_shop_lottery_default') . ' WHERE uniacid =:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
	...
	public function getreward()
	{
		...
		$has_changes = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_lottery_join') . ' WHERE uniacid=:uniacid AND lottery_id=:id AND join_user=:join_user AND lottery_num>0', array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid'], ':id' => $id));
		...
		if (empty($has_changes)) {
			...
			$join_id = pdo_fetchcolumn('SELECT `id` FROM ' . tablename('wx_shop_lottery_join') . ' WHERE uniacid=:uniacid AND lottery_id=:id AND join_user=:join_user AND lottery_num>0 order by addtime desc limit 1', array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid'], ':id' => $id));
		}
		...
		pdo_insert('wx_shop_lottery_log', $log_data);
		$res = pdo_query('UPDATE ' . tablename('wx_shop_lottery_join') . ' SET lottery_num=lottery_num-1 WHERE uniacid=:uniacid AND lottery_id=:id AND join_user=:join_user and id=:join_id', array(':id' => $id, ':join_id' => $join_id, ':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
	...
	public function reward()
	{
		...
		$res = pdo_update('wx_shop_lottery', $lottery_data, array('uniacid' => $_W['uniacid'], 'lottery_id' => $id));
	...
	public function mygoods()
	{
		global $_W;
		global $_GPC;
		$loglist = pdo_fetchall('SELECT l.* FROM ' . tablename('wx_shop_lottery_log') . ' AS l WHERE l.uniacid=:uniacid  AND l.join_user=:join_user AND l.is_reward=1', array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
		$goodslist = array();
		...
		foreach ($loglist as $key => $value) {
			...
			if (isset($value['goods']) && !empty($value['goods'])) {
				...
				if (!empty($goods)) {
					$searchsql = 'SELECT thumb,marketprice FROM ' . tablename('wx_shop_goods') . ' WHERE uniacid= ' . $_W['uniacid'] . ' and id=' . $goods['id'] . ' and status=1 and deleted=0';
