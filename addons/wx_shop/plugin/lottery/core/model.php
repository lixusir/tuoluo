...
class LotteryModel extends PluginModel
{
	public function getGoods($param = '')
	{
		...
		$log = pdo_fetch('select * from ' . tablename('wx_shop_lottery_log') . ' where log_id=:log_id and join_user=:join_user and is_reward=1', array(':log_id' => $param['log_id'], ':join_user' => $param['openid']));
		...
		if (isset($param['goods_num']) && !empty($param['goods_num'])) {
			...
			if (!empty($goods_info['spec'])) {
				...
				pdo_update('wx_shop_lottery_log', array('lottery_data' => serialize($lottery_data)), array('log_id' => $param['log_id']));
				return true;
			}
		...
		$lottery = pdo_fetch('select lottery_days,is_goods from ' . tablename('wx_shop_lottery') . ' where lottery_id=:lottery_id', array(':lottery_id' => $log['lottery_id']));
	...
	{
		global $_W;
		$lotterylist = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_lottery') . ' WHERE uniacid=' . $_W['uniacid'] . ' AND start_time<' . time() . ' AND  end_time>' . time() . ' AND is_delete=0 AND task_type=' . $type . ' ORDER BY addtime DESC');
			...
			if ($type == 1) {
				...
				if (!empty($join_info['lottery_id'])) {
					...
					while ($i <= $join_info['num']) {
						...
						pdo_insert('wx_shop_lottery_join', $join_data);
						++$i;
					}
	...
	public function getLotteryList($openid, $param = array())
	{
		...
		$lottery_list = pdo_fetch('select j.*,l.lottery_title from ' . tablename('wx_shop_lottery_join') . ' as j left join ' . tablename('wx_shop_lottery') . ' as l on j.lottery_id=l.lottery_id where j.lottery_num>0 and j.uniacid=:uniacid and j.join_user=:join_user and j.lottery_id=:lottery_id and l.is_delete=0', array(':uniacid' => $_W['uniacid'], ':join_user' => $openid, ':lottery_id' => intval($param['lottery_id'])));

		if (!empty($lottery_list)) {
			$datas = array(
				array('name' => '活动名称', 'value' => $lottery_list['title'])
				);
			$url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=wx_shop&do=mobile&r=lottery.index.lottery_info&id=' . $param['lottery_id'];
	...
	public function reward($poster, $openid, $title, $lottery_id)
	{
		...
		if (isset($poster['bribery']) && (0 < $poster['bribery'])) {
			...
			if (!is_error($err)) {
				...
				$log_id = pdo_fetchcolumn('SELECT log_id FROM ' . tablename('wx_shop_lottery_log') . ' WHERE uniacid=:uniacid AND join_user=:join_user AND lottery_id=:lottery_id AND is_reward=1 ORDER BY addtime DESC LIMIT 1', array(':uniacid' => $_W['uniacid'], ':join_user' => $openid, ':lottery_id' => $lottery_id));
				pdo_update('wx_shop_lottery_log', $upgrade, array('log_id' => $log_id));
			}
			else {
	...
	public function check_isreward()
	{
		...
		$changes = pdo_fetch('select * from ' . tablename('wx_shop_lottery_join') . ' where uniacid=:uniacid and join_user=:join_user and addtime>' . $start_time . ' and addtime<=' . $end_time . ' limit 1', array(':uniacid' => $_W['uniacid'], ':join_user' => $_W['openid']));
