...
class Pages_WxShopPage extends PluginWebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			...
			show_json(1, array('url' => webUrl('lottery/quick/pages/edit', array('id' => $id, 'tab' => $tab))));
		}
	...
	public function delete()
	{
		...
		if (!empty($items)) {
			foreach ($items as $item) {
				...
				plog('lottery.quick.pages.delete', '删除购买页面 ID: ' . $item['id'] . ' 标题: ' . $item['title'] . ' ');
	...
	public function status()
	{
		...
		if (!empty($items)) {
			foreach ($items as $item) {
				...
				plog('lottery.quick.pages.status', ('修改页面状态<br/>ID: ' . $item['id'] . '<br/>标题: ' . $item['title'] . '<br/>状态: ' . $_GPC['status']) == 1 ? '显示' : '隐藏');
			}
		}
