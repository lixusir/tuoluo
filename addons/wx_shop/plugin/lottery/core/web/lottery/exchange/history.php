...
class History_WxShopPage extends PluginWebPage 
{
	public function main() 
	{
		...
		$sql = 'SELECT * FROM ' . tablename('wx_shop_exchange_record') . ' WHERE uniacid = :uniacid ' . $keyword_condition . $group_condition . $time_condition . $limit;
		$record = pdo_fetchall($sql, array(':uniacid' => $_W['uniacid']));
		$countsql = 'SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_record') . ' WHERE uniacid = :uniacid ' . $keyword_condition . $group_condition . $time_condition;
	...
	public function detail() 
	{
		...
		if ($status == 1) 
		{
			$t1 = tablename('wx_shop_exchange_group');
			$t2 = tablename('wx_shop_exchange_code');
			$res = pdo_fetch('SELECT * FROM ' . $t1 . ' INNER JOIN ' . $t2 . ' ON ' . $t1 . '.id = ' . $t2 . '.groupid WHERE ' . $t2 . '.key = :key AND ' . $t2 . '.id = :id', array('key' => $key, ':id' => $id));
			$res_swi = 1;
		}
		else 
		{
			$res = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_record') . ' WHERE uniacid = :uniacid AND `key` = :key', array(':uniacid' => $_W['uniacid'], ':key' => $key));
		}
		switch ($res['mode']) 
		{
			case 1: $type = '商品兑换';
			...
			{
				$allcoupon[$k] = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE id=:id AND uniacid = :uniacid', array(':id' => $v, ':uniacid' => $_W['uniacid']));
			}
			break;
			case 6: $type = '组合兑换';
			$couponList = json_decode($res['coupon'], 1);
	...
	public function statistics() 
	{
		...
		while (0 <= $i) 
		{
			...
			$all = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_record') . ' WHERE uniacid = ' . $_W['uniacid'] . ' AND `time` >= ' . $compare_time1 . ' AND `time` <=' . $compare_time2);
			$all = intval($all);
			array_push($count_value, $all);
			$res = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_record') . ' WHERE uniacid = ' . $_W['uniacid'] . ' AND `time` >= ' . $compare_time1 . ' AND `time` <=' . $compare_time2);
			$sum = 0;
			if ($res != false) 
			{
