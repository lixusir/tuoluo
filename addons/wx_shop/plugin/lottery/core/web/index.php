...
class Index_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_lottery') . ' WHERE 1 ' . $condition . ' ORDER BY addtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_lottery') . ' where 1 ' . $condition . ' ', $params);
		$pager = pagination2($total, $pindex, $psize);

		foreach ($list as $key => $value) {
			$member_total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_lottery_join') . ' where lottery_id=:lottery_id ', array(':lottery_id' => $value['lottery_id']));
	...
	protected function post()
	{
		...
		if ($id) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_lottery') . ' WHERE lottery_id =:lottery_id and uniacid=:uniacid limit 1', array(':lottery_id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if ($id) {
				$res = pdo_update('wx_shop_lottery', $data, array('lottery_id' => $id, 'uniacid' => $_W['uniacid']));
				...
				$res = pdo_insert('wx_shop_lottery', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		$posters = pdo_fetchall('SELECT lottery_id,lottery_title FROM ' . tablename('wx_shop_lottery') . ' WHERE lottery_id in ( ' . $id . ' ) and uniacid=' . $_W['uniacid']);

		foreach ($posters as $poster) {
			pdo_update('wx_shop_lottery', array('is_delete' => 1), array('lottery_id' => $poster['lottery_id'], 'uniacid' => $_W['uniacid']));
	...
	{
		global $_W;
		$sql = 'SELECT COUNT(1) FROM ' . tablename('wx_shop_lottery') . ' WHERE uniacid = :uniacid AND task_type = :task_type AND is_delete = 0';
		$res = pdo_fetchcolumn($sql, array(':uniacid' => $_W['uniacid'], ':task_type' => $task_type));
		$res = intval($res);

		if ($res === 1) {
			$sql = 'SELECT COUNT(1) FROM ' . tablename('wx_shop_lottery') . ' WHERE uniacid = :uniacid AND task_type = :task_type AND lottery_id = :id AND is_delete = 0';
