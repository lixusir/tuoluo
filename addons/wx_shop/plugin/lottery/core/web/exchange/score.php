...
class Score_WxShopPage extends PluginWebPage
{
	public function main() 
	{
		...
		$psize = 10;
		$pstart = $psize * ($page - 1);
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 4 AND uniacid=:uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 4 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 4 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 4 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	public function nostart() 
	{
		...
		$psize = 10;
		$pstart = $psize * ($page - 1);
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(starttime) >' . time() . ' AND mode = 4 AND uniacid=:uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 4 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 4 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 4 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	public function creat() 
	{
		...
		if ((0 < intval($_GPC['ajax'])) && (0 < intval($_GPC['loop']))) 
		{
			...
			while (0 < $num) 
			{
				...
				$insert = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:exchange:score', 'module' => 'reply', 'displayorder' => $rand_id, 'status' => 1);
				if (IMS_VERSION == '1.0') 
				{
	...
	public function search() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE mode = 4 AND uniacid=:uniacid  ' . $like . ' ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE mode = 4 AND uniacid=:uniacid  ' . $like, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 4 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 4 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 4 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
