...
class Setting_WxShopPage extends PluginWebPage
{
	...
	public function download() 
	{
		global $_GPC;
		global $_W;
		$dir = IA_ROOT . '/addons/wx_shop/data/qrcode/' . $_W['uniacid'] . '/exchange';
	...
	public function other() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (pdo_get('wx_shop_exchange_setting', array('uniacid' => $_W)) == false) 
			{
				pdo_insert('wx_shop_exchange_setting', array('uniacid' => $_W['uniacid'], 'mistake' => $mistake, 'freeze' => $freeze, 'grouplimit' => $grouplimit, 'alllimit' => $alllimit, 'no_qrimg' => (int) $_GPC['no_qrimg'], 'rule' => $_GPC['rule']));
			}
			else 
			{
				pdo_update('wx_shop_exchange_setting', array('mistake' => $mistake, 'freeze' => $freeze, 'grouplimit' => $grouplimit, 'alllimit' => $alllimit, 'no_qrimg' => (int) $_GPC['no_qrimg'], 'rule' => $_GPC['rule']), array('uniacid' => $_W['uniacid']));
			}
			show_json(1, '保存成功');
		}
		else 
		{
			$res = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_setting') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
		}
		include $this->template();
	}
}
