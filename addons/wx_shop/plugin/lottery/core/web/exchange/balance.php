...
class Balance_WxShopPage extends PluginWebPage
{
	public function main() 
	{
		...
		$psize = 10;
		$pstart = $psize * ($page - 1);
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 2 AND uniacid=:uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
	...
	public function nostart() 
	{
		...
		$psize = 10;
		$pstart = $psize * ($page - 1);
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(starttime) >' . time() . ' AND mode = 2 AND uniacid=:uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 2 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 2 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 2 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	public function creat() 
	{
		...
		global $_W;
		$id = (int) $_GPC['id'];
		$res = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' ' . "\r" . '            WHERE id = :id AND uniacid = :uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		@session_start();
		if ((0 < intval($_GPC['ajax'])) && (0 < intval($_GPC['loop']))) 
		{
			...
			while (0 < $num) 
			{
				...
				$insert = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:exchange:balance', 'module' => 'news', 'displayorder' => $rand_id, 'status' => 1);
				if (IMS_VERSION == '1.0') 
				{
	...
	public function goodspost() 
	{
		...
		global $_GPC;
		$id = $_GPC['id'];
		$all = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE uniacid = :uniacid AND deleted = 0 ' . "\r" . '        AND hasoption = 0', array(':uniacid' => $_W['uniacid']));
		$hasoption = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE uniacid = :uniacid AND deleted = 0 ' . "\r" . '        AND hasoption = 1', array(':uniacid' => $_W['uniacid']));
		$i = 0;
	...
	public function search() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE mode = 2 AND uniacid=:uniacid  ' . $like . ' ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE mode = 2 AND uniacid=:uniacid  ' . $like, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 2 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 2 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 2 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
