...
class Extension_WxShopPage extends PluginWebPage
{
	public $action = '';
	public $taskType = '';
	...
	public function add() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			else if (pdo_update('wx_shop_task', $data, array('id' => $id, 'uniacid' => $_W['uniacid']))) 
			{
		...
		{
			$data = pdo_get('wx_shop_task', array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function goods() 
	{
		...
		if (!(empty($type))) 
		{
			...
			else if ($type == 'coupon') 
			{
				...
				$totalsql = 'select count(*) from ' . tablename('wx_shop_coupon') . ' where couponname LIKE :title and uniacid=:uniacid ';
				$searchsql = 'select id,couponname,coupontype,enough,thumb,backtype,deduct,backmoney,backcredit,`total`,backredpack,discount,displayorder from ' . tablename('wx_shop_coupon') . ' where couponname LIKE :title and uniacid=:uniacid ORDER BY `displayorder` DESC,`id` DESC LIMIT ' . (($page - 1) * $psize) . ',' . $psize;
				$total = pdo_fetchcolumn($totalsql, $params);
				$pagelast = intval(($total - 1) / $psize) + 1;
				$list = pdo_fetchall($searchsql, $params);
				foreach ($list as &$d ) 
	...
	public function delete() 
	{
		...
		foreach ($ids as $v ) 
		{
			pdo_delete('wx_shop_task', array('id' => intval($v), 'uniacid' => $_W['uniacid']));
		}
		show_json(1);
	}
}
