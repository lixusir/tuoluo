...
class Setting_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_lottery_task') . ' WHERE 1 ' . $condition . ' ORDER BY addtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_lottery_task') . ' where 1 ' . $condition . ' ', $params);
	...
	protected function post()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_lottery_task') . ' WHERE uniacid =:uniacid AND `id`=:id LIMIT 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		$tasklist = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_task_poster') . ' WHERE 1 ' . $condition . ' ORDER BY createtime desc LIMIT 15');
		$condition = ' AND `is_delete`=0 AND uniacid= ' . $_W['uniacid'];
		$lotterylist = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_lottery') . ' WHERE 1 ' . $condition . ' ORDER BY addtime desc LIMIT 15');
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				$param = array('id' => $id);
				$res = pdo_update('wx_shop_lottery_task', $data, $param);
				...
				$data['addtime'] = time();
				pdo_insert('wx_shop_lottery_task', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		$posters = pdo_fetchall('SELECT `id`,`title` FROM ' . tablename('wx_shop_lottery_task') . ' WHERE `id` in ( ' . $id . ' ) and `uniacid`=' . $_W['uniacid']);

		foreach ($posters as $poster) {
			pdo_update('wx_shop_lottery_task', array('is_delete' => 1), array('id' => $poster['id'], 'uniacid' => $_W['uniacid']));
	...
	public function setstart()
	{
		global $_W;
		global $_GPC;
		$defaultcount = pdo_fetchcolumn('select COUNT(*) from ' . tablename('wx_shop_lottery_default') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		$default = pdo_fetchcolumn('select `data` from ' . tablename('wx_shop_lottery_default') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (0 < $defaultcount) {
				...
				if (!empty($keyword)) {
					if ($keyword['name'] != 'wx_shop:lottery') {
						show_json(0, '关键字已存在!');
					}
				}

				$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'wx_shop:lottery'));
				...
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:lottery', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
				...
				$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'wx_shop', 'title' => $default['title'], 'description' => $default['desc'], 'thumb' => $default['thumb'], 'url' => mobileUrl('lottery'));
				...
				pdo_update('wx_shop_lottery_default', $update_data, array('uniacid' => $_W['uniacid']));
				...
				pdo_insert('wx_shop_lottery_default', $insert_data);
			}
		}
	...
	public function setlottery()
	{
		...
		if ($_W['ispost']) {
			...
			$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_lottery_default') . ' WHERE uniacid =:uniacid', array(':uniacid' => $_W['uniacid']));

			if ($count) {
				pdo_update('wx_shop_lottery_default', $data, array('uniacid' => $_W['uniacid']));
			}
			else {
				$data['uniacid'] = $_W['uniacid'];
				pdo_insert('wx_shop_lottery_default', $data);
			}
