...
class PolyapiModel extends PluginModel
{
	public function get_key_info($array)
	{
		...
		$item = pdo_fetch('select * from ' . tablename('wx_shop_polyapi_key') . ' where appkey=:appkey and token=:token limit 1', $params);
		return $item;
	}
	...
	public function GetOrder($array)
	{
		...
		$t = pdo_fetch('SELECT COUNT(1) as count FROM ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('wx_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . ' where ' . $condition, $paras);
		$total = $t['count'];
		...
		if (!(empty($list))) {
			foreach ($list as $k => $v ) {
				...
				if (0 < $v['parentid']) {
					$parent_item = pdo_fetch('SELECT id,ordersn,ordersn2,price FROM ' . tablename('wx_shop_order') . ' WHERE id = :id and uniacid=:uniacid Limit 1', array(':id' => $v['parentid'], ':uniacid' => $uniacid));
				...
				if (empty($v['addressid'])) {
					...
					if (!(is_array($user))) {
						$user = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_address') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $v['addressid'], ':uniacid' => $uniacid));
					}
				...
				if (!(empty($refundid))) {
					$refund = pdo_fetch('select status from ' . tablename('wx_shop_order_refund') . ' where id=:id limit 1', array(':id' => $refundid));
				}
	...
	public function Send($array)
	{
		...
		if (empty($IsSplit)) {
			...
			pdo_update('wx_shop_order', $change_data, array('id' => $item['id'], 'uniacid' => $uniacid));
		}
		 else {
			...
			foreach ($goodsid as $key => $value ) {
				pdo_update('wx_shop_order_goods', $change_data, array('orderid' => $item['id'], 'goodsid' => $value, 'uniacid' => $uniacid));
			}
			...
			pdo_update('wx_shop_order', $senddata, array('id' => $item['id'], 'uniacid' => $uniacid));
		}
		...
		if (!(empty($Status))) {
			...
			if (!(empty($total))) {
				...
				foreach ($list as $k => &$v ) {
					...
					if ($v['hasoption'] == 1) {
						$options = pdo_fetchall('select * from ' . tablename('wx_shop_goods_option') . ' where goodsid=:id order by id asc', array(':id' => $v['id']));
	...
	public function SyncStock($array)
	{
		...
		$item = pdo_fetch('select * from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid Limit 1', array(':id' => $PlatProductID, ':uniacid' => $uniacid));
		...
		if ($item['hasoption'] == 1) {
			...
			pdo_update('wx_shop_goods_option', array('stock' => $Quantity), array('id' => $SkuID, 'goodsid' => $PlatProductID));
			pdo_update('wx_shop_goods', array('total' => $goods_total), array('id' => $PlatProductID));
		}
		 else {
			pdo_update('wx_shop_goods', array('total' => $Quantity), array('id' => $PlatProductID));
		}
	...
	{
		global $_W;
		$express = pdo_fetch('select * from ' . tablename('wx_shop_express') . ' where code=:code limit 1', array(':code' => $LogisticType));
		return $express;
	}
	...
	{
		global $_W;
		$item = pdo_fetch('select * from ' . tablename('wx_shop_order') . ' where  ordersn=:ordersn and uniacid=:uniacid Limit 1', array(':ordersn' => $ordersn, ':uniacid' => $uniacid));
		return $item;
	}
	...
	public function insert_data($data, $uniacid)
	{
		...
		pdo_insert('wx_shop_polyapi_data', $insert_data);
	}
	...
	public function get_data($uniacid)
	{
		...
		$params = array();
		$params[':uniacid'] = $uniacid;
		$sql = 'SELECT * FROM ' . tablename('wx_shop_polyapi_data') . ' WHERE uniacid=:uniacid order by id desc';
		$list = pdo_fetchall($sql, $params);
