...
class Set_WxShopPage extends PluginWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$item = pdo_fetch('select * from ' . tablename('wx_shop_polyapi_key') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (empty($item)) {
				...
				pdo_insert('wx_shop_polyapi_key', $data);
			}
			 else {
				$data['updatetime'] = $time;
				pdo_update('wx_shop_polyapi_key', $data, array('uniacid' => $_W['uniacid']));
			}
	...
	public function update()
	{
		global $_W;
		global $_GPC;
		$item = pdo_fetch('select code from ' . tablename('wx_shop_express') . ' where express=:express limit 1', array(':express' => 'shunfeng'));
		...
		if (empty($item['code'])) {
			...
			foreach ($data as $k => $v ) {
				pdo_update('wx_shop_express', array('code' => $v), array('express' => $k));
			}
		}
