...
class List_WxShopPage extends PluginWebPage
{
	protected function orderData($status, $st)
	{
		...
		if (($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0') || !(empty($sqlcondition))) {
			$t = pdo_fetch('SELECT COUNT(1) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_dispatch') . ' d on d.id = o.dispatchid ' . ' left join ' . tablename('wx_shop_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . ' left join ' . tablename('wx_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . ' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition, $paras);
		}
		 else {
			$t = pdo_fetch('SELECT COUNT(1) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('wx_shop_order') . ' WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 ' . $status_condition, $paras);
		}
