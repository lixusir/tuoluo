...
class MerchModel extends PluginModel 
{
	...
	{
		global $_W;
		return pdo_fetchall('select id,groupname from ' . tablename('wx_shop_merch_group') . ' where uniacid=:uniacid and status=1 order by isdefault desc , id asc', array(':uniacid' => $_W['uniacid']), 'id');
	}
	public function getCategory($data = array()) 
	{
		...
		return pdo_fetchall('select ' . $res['field'] . ' from ' . tablename('wx_shop_merch_category') . $res['condition'], $res['params'], $res['column']);
	}
	public function getCategorySwipe($data = array()) 
	{
		...
		$params = array(':uniacid' => $_W['uniacid']);
		$res = $this->build($condition, $params, $data);
		return pdo_fetchall('select ' . $res['field'] . ' from ' . tablename('wx_shop_merch_category_swipe') . $res['condition'], $res['params'], $res['column']);
	}
	public function getMerch($data = array()) 
	{
		...
		return pdo_fetchall('select ' . $res['field'] . ' from ' . tablename('wx_shop_merch_user') . $res['condition'], $res['params'], $res['column']);
	}
	public function updateSet($values = array(), $merchid = 0) 
	{
		...
		pdo_update('wx_shop_merch_user', array('sets' => iserializer($sets)), array('id' => $merchid));
	...
	{
		global $_W;
		return array('sale' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where checked=0 and status=1 and deleted=0 and total>0 and uniacid=:uniacid and merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])), 'out' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where checked=0 and  status=1 and deleted=0 and total=0 and uniacid=:uniacid and merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])), 'check' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where checked=1 and deleted=0 and uniacid=:uniacid and merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])), 'stock' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where checked=0 and status=0 and deleted=0 and uniacid=:uniacid  and merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])), 'cycle' => pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_goods') . ' where deleted=1 and uniacid=:uniacid and merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'])));
	}
	public function getOrderTotals() 
	{
		...
		$totals['all'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and deleted=0', $paras);
		$totals['status_1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and status=-1 and refundtime=0 and deleted=0', $paras);
		$totals['status0'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and status=0 and paytype<>3 and deleted=0', $paras);
		$totals['status1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and ( status=1 or ( status=0 and paytype=3) ) and deleted=0', $paras);
		$totals['status2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and status=2 and deleted=0', $paras);
		$totals['status3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and status=3 and deleted=0', $paras);
		$totals['status4'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and refundstate>0 and refundid<>0 and deleted=0', $paras);
		$totals['status5'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_order') . '' . ' WHERE uniacid = :uniacid ' . $condition . ' and merchid = :merchid and refundtime<>0 and deleted=0', $paras);
		return $totals;
	}
	public function getListUser($list, $return = 'all') 
	{
		...
		if (!(empty($merch))) 
		{
			...
			$merch_user = pdo_fetchall('select * from ' . tablename('wx_shop_merch_user') . ' where uniacid=:uniacid and id in(' . implode(',', $merch_ids) . ')', array(':uniacid' => $_W['uniacid']), 'id');
	...
	public function getListUserOne($merchid) 
	{
		...
		if ($merchid) 
		{
			$merch_user = pdo_fetch('select * from ' . tablename('wx_shop_merch_user') . ' where uniacid=:uniacid and id=' . $merchid, array(':uniacid' => $_W['uniacid']));
			return $merch_user;
		}
		return false;
	}
	...
	private function check($permtype = '') 
	{
		...
		$user = pdo_fetch('select u.status as userstatus,r.status as rolestatus,r.perms as roleperms,u.roleid from ' . tablename('wx_shop_merch_account') . ' u ' . ' left join ' . tablename('wx_shop_merch_perm_role') . ' r on u.roleid = r.id ' . ' where u.id=:merchuid limit 1 ', array(':merchuid' => $uid));
	...
	public function log($type = '', $op = '') 
	{
		...
		pdo_insert('wx_shop_merch_perm_log', $log);
	}
	...
	public function select_operator($merchid = 0) 
	{
		global $_W;
		$merchid = ((empty($merchid) ? $_W['merchid'] : intval($merchid)));
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_merch_account') . ' WHERE 1  and uniacid = :uniacid and merchid = :merchid and isfounder<>1', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		return $total;
	}
	...
	public function getUserTotals() 
	{
		global $_W;
		global $_GPC;
		$totals = array('reg0' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_reg') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])), 'reg_1' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_reg') . ' where uniacid=:uniacid and status=-1 limit 1', array(':uniacid' => $_W['uniacid'])), 'user0' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_user') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])), 'user1' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_user') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])), 'user2' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_user') . ' where uniacid=:uniacid and status=2 limit 1', array(':uniacid' => $_W['uniacid'])), 'user3' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_user') . ' where uniacid=:uniacid and status=1 and TIMESTAMPDIFF(DAY,now(),FROM_UNIXTIME(accounttime))<=30  limit 1', array(':uniacid' => $_W['uniacid'])));
		return $totals;
	}
	public function getClearTotals() 
	{
		global $_W;
		global $_GPC;
		$totals = array('status0' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_clearing') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])), 'status1' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_clearing') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])), 'status2' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_clearing') . ' where uniacid=:uniacid and status=2 limit 1', array(':uniacid' => $_W['uniacid'])));
		return $totals;
	}
	public function getMerchOrderTotals($type = 0) 
	{
		...
		$sql = 'select sum(o.price) as totalmoney from ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_merch_user') . ' u on u.id = o.merchid ' . ' where 1 ' . $condition . ' ';
		$price = pdo_fetch($sql, $params);
		$totalmoney = round($price['totalmoney'], 2);
		$sql = 'select count(o.id) as totalcount from ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_merch_user') . ' u on u.id = o.merchid ' . ' where 1 ' . $condition . ' ';
	...
	public function getMerchOrderTotalPrice($merchid) 
	{
		...
		$sql = 'select *  from ' . tablename('wx_shop_merch_bill') . ' where 1 ' . $condition . ' and status=1';
		...
		{
			$status1order = pdo_fetchall('SELECT id FROM ' . tablename('wx_shop_order') . ' WHERE id in(' . join(',', $status1orderids) . ') AND uniacid=' . $_W['uniacid']);
			$commission1 = 0;
		...
		$sql = 'select sum(realprice) as totalmoney from ' . tablename('wx_shop_merch_bill') . ' where 1 ' . $condition . ' and status=2';
		...
		{
			$status2order = pdo_fetchall('SELECT id FROM ' . tablename('wx_shop_order') . ' WHERE id in(' . join(',', $status2orderids) . ') AND uniacid=' . $_W['uniacid']);
			$commission2 = 0;
		...
		$sql = 'select *  from ' . tablename('wx_shop_merch_bill') . ' where 1 ' . $condition . ' and status=3';
		...
		{
			$status3order = pdo_fetchall('SELECT id FROM ' . tablename('wx_shop_order') . ' WHERE id in(' . join(',', $status3orderids) . ') AND uniacid=' . $_W['uniacid']);
			$commission3 = 0;
	...
	public function getMerchPrice($merchid, $flag = 0) 
	{
		...
		$sql = 'select ' . $con . ' from ' . tablename('wx_shop_merch_user') . ' u ' . ' left join ' . tablename('wx_shop_order') . ' o on u.id=o.merchid' . ' where 1 ' . $condition . ' limit 1';
		$list = pdo_fetch($sql, $params);
		$merchcouponprice = pdo_fetchcolumn('select sum(o.couponprice) from ' . tablename('wx_shop_merch_user') . ' u ' . ' left join ' . tablename('wx_shop_order') . ' o on u.id=o.merchid' . ' where o.couponmerchid>0 ' . $condition . ' limit 1', $params);
		if (0 < $flag) 
		{
			$sql = 'select o.id,o.agentid from ' . tablename('wx_shop_merch_user') . ' u ' . ' left join ' . tablename('wx_shop_order') . ' o on u.id=o.merchid' . ' where 1 ' . $condition;
	...
	public function getMerchPriceList($merchid, $orderid = 0, $flag = 0) 
	{
		...
		$sql = 'select ' . $con . ' from ' . tablename('wx_shop_merch_user') . ' u ' . ' left join ' . tablename('wx_shop_order') . ' o on u.id=o.merchid' . ' where 1 ' . $condition;
		$order = pdo_fetchall($sql, $params);
	...
	public function getOneApply($id) 
	{
		...
		$sql = 'select b.*,u.merchname,u.realname,u.mobile from ' . tablename('wx_shop_merch_bill') . ' b ' . ' left join ' . tablename('wx_shop_merch_user') . ' u on b.merchid = u.id' . ' where 1 ' . $condition . ' Limit 1';
	...
	public function getMerchApplyTotals($merchid = 0) 
	{
		...
		$totals['status1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_merch_bill') . ' WHERE 1 ' . $condition . ' and status=1', $params);
		$totals['status2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_merch_bill') . ' WHERE 1 ' . $condition . ' and status=2', $params);
		$totals['status3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_merch_bill') . ' WHERE 1 ' . $condition . ' and status=3', $params);
		$totals['status_1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_merch_bill') . ' WHERE 1 ' . $condition . ' and status=-1', $params);
		return $totals;
	}
	public function getAllUserTotals() 
	{
		global $_W;
		$totals = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_merch_user') . ' where uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		return $totals;
	}
	...
	{
		global $_W;
		$data = pdo_fetch('select datas from ' . tablename('wx_shop_perm_plugin') . ' where acid=:acid Limit 1', array(':acid' => $_W['uniacid']));
		$max_merch = 0;
		if (!(empty($data['datas']))) 
		{
	...
	public function tempData($type, $merchid = 0) 
	{
		...
		$sql = 'SELECT id,expressname,expresscom,isdefault FROM ' . tablename('wx_shop_exhelper_express') . ' where  1 and ' . $condition . ' ORDER BY isdefault desc, id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exhelper_express') . ' where 1 and ' . $condition, $params);
	...
	public function setDefault($id, $type, $merchid = 0) 
	{
		global $_W;
		$merchid = ((empty($merchid) ? $_W['merchid'] : $merchid));
		$item = pdo_fetch('SELECT id,expressname,type FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE id=:id and type=:type AND uniacid=:uniacid and merchid=:merchid', array(':id' => $id, ':type' => $type, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
		if (!(empty($item))) 
		{
			pdo_update('wx_shop_exhelper_express', array('isdefault' => 0), array('type' => $type, 'uniacid' => $_W['uniacid'], 'merchid' => $merchid));
			pdo_update('wx_shop_exhelper_express', array('isdefault' => 1), array('id' => $id, 'merchid' => $merchid));
			if ($type == 1) 
			{
	...
	public function tempDelete($id, $type, $merchid = 0) 
	{
		global $_W;
		$merchid = ((empty($merchid) ? $_W['merchid'] : $merchid));
		$items = pdo_fetchall('SELECT id,expressname FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE id in( ' . $id . ' ) and type=:type and uniacid=:uniacid and merchid=:merchid', array(':type' => $type, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_exhelper_express', array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $merchid));
			if ($type == 1) 
			{
	...
	public function getTemp($merchid = 0) 
	{
		...
		global $_GPC;
		$merchid = ((empty($merchid) ? $_W['merchid'] : $merchid));
		$temp_sender = pdo_fetchall('SELECT id,isdefault,sendername,sendertel FROM ' . tablename('wx_shop_exhelper_senduser') . ' WHERE uniacid=:uniacid and merchid=:merchid order by isdefault desc ', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
		$temp_express = pdo_fetchall('SELECT id,type,isdefault,expressname FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE type=1 and uniacid=:uniacid and merchid=:merchid order by isdefault desc ', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
		$temp_invoice = pdo_fetchall('SELECT id,type,isdefault,expressname FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE type=2 and uniacid=:uniacid and merchid=:merchid order by isdefault desc ', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
	...
	public function updateOrderPay() 
	{
		...
		$params = array();
		$params[':uniacid'] = $_W['uniacid'];
		$sql = 'select id,parentid from ' . tablename('wx_shop_order') . ' where parentid>0 and status>0 and paytype=0 and uniacid=:uniacid';
		$list = pdo_fetchall($sql, $params);
		if (!(empty($list))) 
		{
			foreach ($list as $k => $v ) 
			{
				...
				$sql1 = 'select paytype from ' . tablename('wx_shop_order') . ' where id=:orderid and status>0 and paytype>0 and uniacid=:uniacid';
				$item = pdo_fetch($sql1, $params);
				if (0 < $item['paytype']) 
				{
					pdo_update('wx_shop_order', array('paytype' => $item['paytype']), array('id' => $v['id']));
				}
			}
		}
	}
	public function getPluginsetByMerch($key = '') 
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$set = pdo_fetch('select * from ' . tablename('wx_shop_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
		if (empty($set)) 
		{
			$set = array();
		}
