...
if ($_W['routes'] != 'merch.manage.login') 
{
	...
	if (is_array($session)) 
	{
		$account = pdo_fetch('select * from ' . tablename('wx_shop_merch_account') . ' where id=:id limit 1', array(':id' => $session['id']));
...
$merch_user = pdo_fetch('select u.*,g.groupname,g.goodschecked,g.commissionchecked,g.changepricechecked,g.finishchecked from ' . tablename('wx_shop_merch_user') . ' u left join ' . tablename('wx_shop_merch_group') . ' g on u.groupid=g.id where u.id=:id limit 1', array(':id' => $session['merchid']));
