...
class Check_WxShopPage extends PluginWebPage
{
	protected function main($status = 1) 
	{
		...
		$sql = 'select b.*,u.merchname,u.realname,u.mobile from ' . tablename('wx_shop_merch_bill') . ' b ' . ' left join ' . tablename('wx_shop_merch_user') . ' u on b.merchid = u.id' . ' where 1 ' . $condition . ' ORDER BY b.id desc ';
		...
		$total = pdo_fetchcolumn('select COUNT(1) from ' . tablename('wx_shop_merch_bill') . ' b ' . ' left join ' . tablename('wx_shop_merch_user') . ' u on b.merchid = u.id' . ' where 1 ' . $condition, $params);
	...
	public function detail() 
	{
		...
		{
			$merch_user = pdo_fetch('select * from ' . tablename('wx_shop_merch_user') . ' where uniacid=:uniacid and id=' . $item['merchid'], array(':uniacid' => $_W['uniacid']));
	...
	public function merchpay() 
	{
		...
		pdo_update('wx_shop_merch_bill', $change_data, array('id' => $id));
		$orderids = iunserializer($item['passorderids']);
		foreach ($orderids as $key => $orderid ) 
		{
			pdo_update('wx_shop_order', array('merchapply' => 3), array('id' => $orderid));
		}
		show_json(1);
	}
	public function confirm() 
	{
		...
		if ($type == 1) 
		{
			...
			foreach ($orderids as $key => $orderid ) 
			{
				...
				{
					pdo_update('wx_shop_order', array('merchapply' => 2), array('id' => $orderid));
				}
				else 
				{
					pdo_update('wx_shop_order', array('merchapply' => -1), array('id' => $orderid));
				}
			}
		}
		else if ($type == -1) 
