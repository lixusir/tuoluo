...
class Reg_WxShopPage extends PluginWebPage
{
	public function main() 
	{
		...
		$sql = 'select  *  from ' . tablename('wx_shop_merch_reg') . '   where uniacid=:uniacid ' . $condition . ' ORDER BY  applytime desc';
		if (empty($_GPC['export'])) 
		{
			$sql .= ' limit ' . (($pindex - 1) * $psize) . ',' . $psize;
		}
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(*) from' . tablename('wx_shop_merch_reg') . '  where uniacid = :uniacid ' . $condition, $params);
		if ($_GPC['export'] == '1') 
		{
	...
	public function detail() 
	{
		...
		$id = intval($_GPC['id']);
		$item = pdo_fetch('select * from ' . tablename('wx_shop_merch_reg') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (empty($item)) 
		{
			if ($_W['ispost']) 
			{
		...
		if ($_W['ispost']) 
		{
			...
			pdo_update('wx_shop_merch_reg', $item, array('id' => $item['id']));
			if ($status == 1) 
			{
				$usercount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_user') . ' where regid=:regid and uniacid=:uniacid limit 1', array(':regid' => $item['id'], ':uniacid' => $_W['uniacid']));
				if (empty($usercount)) 
				{
					$user = $item;
					unset($user['id']);
					...
					$user['regid'] = $item['id'];
					$user['status'] = 0;
					pdo_insert('wx_shop_merch_user', $user);
					$userid = pdo_insertid();
					pdo_update('wx_shop_merch_reg', $item, array('id' => $item['id']));
	...
	public function delete() 
	{
		...
		$regs = pdo_fetchall('SELECT id,merchname FROM ' . tablename('wx_shop_merch_reg') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($regs as $reg ) 
		{
			pdo_delete('wx_shop_merch_reg', array('id' => $reg['id']));
