...
class User_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select  u.*,g.groupname  from ' . tablename('wx_shop_merch_user') . '  u ' . ' left join  ' . tablename('wx_shop_merch_group') . ' g on u.groupid = g.id ' . ' where u.uniacid=:uniacid ' . $condition . ' ORDER BY ' . $sortfield . ' desc';
		...
		$total = pdo_fetchcolumn('select count(*) from' . tablename('wx_shop_merch_user') . ' u  ' . ' left join  ' . tablename('wx_shop_merch_group') . ' g on u.groupid = g.id ' . ' where u.uniacid = :uniacid ' . $condition, $params);
    ...
    protected function post()
    {
        ...
        $item = pdo_fetch('select * from ' . tablename('wx_shop_merch_user') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        ...
        if (!(empty($item['accountid']))) {
            ...
            $account = pdo_fetch('select * from ' . tablename('wx_shop_merch_account') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['accountid'], ':uniacid' => $_W['uniacid']));
        }

        if ($_GPC['status'] == 0) {
            $regitem = pdo_fetch('select * from '.tablename('wx_shop_merch_reg').' where uniacid=:uniacid and id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $item['regid']));
        }
        ...
        if ($_W['ispost']) {
            ...
            if ($checkUser) {
                ...
                if (empty($account) && empty($_GPC['pwd']) && empty($show_pass)) {
                ...
                $usercount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_account') . ' where ' . $where . ' limit 1', $params);
                ...
                if (!(empty($account))) {
                    ...
                    if (empty($account['pwd']) && empty($_GPC['pwd']) && empty($show_pass)) {
            ...
            if (empty($account) || empty($account['salt']) || !(empty($_GPC['pwd']))) {
                ...
                while (1) {
                    ...
                    $saltcount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_account') . ' where salt=:salt limit 1', array(':salt' => $salt));
            ...
            if (empty($item)) {
                ...
                $item['applytime'] = time();
                pdo_insert('wx_shop_merch_user', $data);
                $id = pdo_insertid();
                $account['merchid'] = $id;
                pdo_insert('wx_shop_merch_account', $account);
                $accountid = pdo_insertid();
                pdo_update('wx_shop_merch_user', array('accountid' => $accountid), array('id' => $id));
                ...
                pdo_update('wx_shop_merch_user', $data, array('id' => $id));


                if (!(empty($item['accountid']))) {

                    pdo_update('wx_shop_merch_account', $account, array('id' => $item['accountid']));
                }
	...
	public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,merchname FROM ' . tablename('wx_shop_merch_user') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);


		foreach ($items as $item ) {

			pdo_update('wx_shop_merch_user', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_merch_user') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);


		foreach ($items as $item ) {

			pdo_update('wx_shop_goods', $change_data, array('merchid' => $item['id'], 'uniacid' => $uniacid));
			pdo_delete('wx_shop_merch_account', array('merchid' => $item['id'], 'uniacid' => $uniacid));
			pdo_delete('wx_shop_merch_user', array('id' => $item['id'], 'uniacid' => $uniacid));
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT id,merchname FROM ' . tablename('wx_shop_merch_user') . ' WHERE ' . $condition . ' order by id asc', $params);
		include $this->template();
		exit();
	}
	...
	public function querymerchs()
	{
		...
		$ds = pdo_fetchall('SELECT id,merchname as title ,logo as thumb FROM ' . tablename('wx_shop_merch_user') . ' WHERE 1 ' . $condition . ' order by id desc', $params);
