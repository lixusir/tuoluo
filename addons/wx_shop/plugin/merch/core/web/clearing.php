...
class Clearing_WxShopPage extends PluginWebPage
{
	protected function main($status = 0) 
	{
		...
		$sql = 'select u.merchname,u.mobile,u.realname,u.logo,o.* from ' . tablename('wx_shop_merch_clearing') . ' o ' . ' left join ' . tablename('wx_shop_merch_user') . ' u on u.id=o.merchid' . ' where 1 ' . $condition . ' ORDER BY o.id DESC';
		...
		$total = pdo_fetchcolumn('select COUNT(u.id) from ' . tablename('wx_shop_merch_user') . ' u ' . ' left join ' . tablename('wx_shop_order') . ' o on u.id=o.merchid' . ' where 1 ' . $condition . ' GROUP BY u.id', $params);
		$total = count($total);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ($id) 
			{
				...
				pdo_update('wx_shop_merch_clearing', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			}
			else 
			{
				pdo_insert('wx_shop_merch_clearing', $data);
				$id = pdo_insertid();
			}
			show_json(1);
		}
		if ($id) 
		{
			$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_merch_clearing') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			empty($item) && $this->message('未找到该账单', '', 'error');
			$merch = pdo_fetch('select id,merchname,logo,realname,mobile from ' . tablename('wx_shop_merch_user') . ' where id=:id and uniacid=:uniacid', array(':id' => $item['merchid'], ':uniacid' => $_W['uniacid']));
		}
		else if ($merchid) 
		{
			$merch = pdo_fetch('select id,merchname,logo,realname,mobile from ' . tablename('wx_shop_merch_user') . ' where id=:id and uniacid=:uniacid', array(':id' => $merchid, ':uniacid' => $_W['uniacid']));
		}
	...
	public function query() 
	{
		...
		$sql = 'select id,merchname,logo,realname,mobile from ' . tablename('wx_shop_merch_user') . ' where 1 ' . $condition . ' ORDER BY id ASC';
	...
	public function status() 
	{
		...
		if ($id) 
		{
			pdo_update('wx_shop_merch_clearing', array('status' => $status), array('id' => $id, 'uniacid' => $_W['uniacid']));
			show_json(1);
		}
		show_json(0);
	}
	public function delete() 
	{
		...
		if ($id) 
		{
			pdo_query('DELETE FROM ' . tablename('wx_shop_merch_clearing') . ' WHERE id=:id AND status<>2 AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
			show_json(1);
		}
		show_json(0);
	}
