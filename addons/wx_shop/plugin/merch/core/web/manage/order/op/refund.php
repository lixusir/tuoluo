...
class Refund_WxShopPage extends MerchWebPage
{
	protected function opData() 
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order') . ' WHERE id = :id and uniacid=:uniacid and merchid = :merchid  Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
		if (empty($item)) 
		{
			if ($_W['isajax']) 
			{
		...
		if (!(empty($refundid))) 
		{
			$refund = pdo_fetch('select * from ' . tablename('wx_shop_order_refund') . ' where id=:id limit 1', array(':id' => $refundid));
	...
	public function submit() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			{
				pdo_update('wx_shop_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
				show_json(0, '未找需要处理的维权申请，不需处理！');
			}
			if (empty($refund['refundno'])) 
			{
				$refund['refundno'] = m('common')->createNO('order_refund', 'refundno', 'SR');
				pdo_update('wx_shop_order_refund', array('refundno' => $refund['refundno']), array('id' => $refund['id']));
			}
			...
			else if ($refundstatus == 3) 
			{
				...
				if ($raid == 0) 
				{
					$raddress = pdo_fetch('select * from ' . tablename('wx_shop_refund_address') . ' where isdefault=1 and uniacid=:uniacid and merchid = :merchid limit 1', array(':uniacid' => $uniacid, ':merchid' => $merchid));
				}
				else 
				{
					$raddress = pdo_fetch('select * from ' . tablename('wx_shop_refund_address') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $raid, ':uniacid' => $uniacid, ':merchid' => $merchid));
				}
				if (empty($raddress)) 
				{
					$raddress = pdo_fetch('select * from ' . tablename('wx_shop_refund_address') . ' where uniacid=:uniacid and merchid = :merchid order by id desc limit 1', array(':uniacid' => $uniacid, ':merchid' => $merchid));
				}
				...
				pdo_update('wx_shop_order_refund', $change_refund, array('id' => $item['refundid']));
			...
			else if ($refundstatus == 10) 
			{
				...
				pdo_update('wx_shop_order_refund', $refund_data, array('id' => $item['refundid'], 'uniacid' => $uniacid));
				$order_data = array();
				$order_data['refundstate'] = 0;
				$order_data['status'] = 3;
				$order_data['refundtime'] = $time;
				pdo_update('wx_shop_order', $order_data, array('id' => $item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));
				m('notice')->sendOrderMessage($item['id'], true);
			}
			else if ($refundstatus == 1) 
			{
				if (0 < $item['parentid']) 
				{
					$parent_item = pdo_fetch('SELECT id,ordersn,ordersn2,price,transid,paytype,apppay FROM ' . tablename('wx_shop_order') . ' WHERE id = :id and uniacid=:uniacid Limit 1', array(':id' => $item['parentid'], ':uniacid' => $_W['uniacid']));
				...
				$goods = pdo_fetchall('SELECT g.id,g.credit, o.total,o.realprice,g.isfullback FROM ' . tablename('wx_shop_order_goods') . ' o left join ' . tablename('wx_shop_goods') . ' g on o.goodsid=g.id ' . ' WHERE o.orderid=:orderid and o.uniacid=:uniacid', array(':orderid' => $item['id'], ':uniacid' => $uniacid));
				$refundtype = 0;
				...
				else if ($item['paytype'] == 21) 
				{
					...
					else 
					{
						...
						{
							$pid = $ispeerpay['id'];
							$peerpaysql = 'SELECT * FROM ' . tablename('wx_shop_order_peerpay_payinfo') . ' WHERE pid = :pid';
				...
				m('notice')->sendOrderMessage($item['id'], true);
				pdo_update('wx_shop_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));
				foreach ($goods as $g ) 
				{
					$salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid and o.merchid=:merchid limit 1', array(':goodsid' => $g['id'], ':uniacid' => $uniacid, 'merchid' => $merchid));
					pdo_update('wx_shop_goods', array('salesreal' => $salesreal), array('id' => $g['id']));
				}
			...
			{
				pdo_update('wx_shop_order_refund', array('reply' => $refundcontent, 'status' => -1, 'endtime' => $time), array('id' => $item['refundid']));
				m('notice')->sendOrderMessage($item['id'], true);
				plog('order.op.refund', '订单退款拒绝 ID: ' . $item['id'] . ' 订单号: ' . $item['ordersn'] . ' 原因: ' . $refundcontent);
				pdo_update('wx_shop_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));
			}
			else if ($refundstatus == 2) 
			{
				...
				m('notice')->sendOrderMessage($item['id'], true);
				pdo_update('wx_shop_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid, 'merchid' => $merchid));
				$goods = pdo_fetchall('SELECT g.id,g.credit, o.total,o.realprice FROM ' . tablename('wx_shop_order_goods') . ' o left join ' . tablename('wx_shop_goods') . ' g on o.goodsid=g.id ' . ' WHERE o.orderid=:orderid and o.uniacid=:uniacid and o.merchid = :merchid', array(':orderid' => $item['id'], ':uniacid' => $uniacid, ':merchid' => $merchid));
				$credits = m('order')->getGoodsCredit($goods);
				if (0 < $credits) 
				{
					m('member')->setCredit($item['openid'], 'credit1', -$credits, array(0, $shopset['name'] . '退款扣除购物赠送积分: ' . $credits . ' 订单号: ' . $item['ordersn']));
		...
		$refund_address = pdo_fetchall('select * from ' . tablename('wx_shop_refund_address') . ' where uniacid=:uniacid and merchid = :merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
	...
	{
		pdo_update('wx_shop_order_peerpay_payinfo', array('refundstatus' => 1, 'refundprice' => $refundprice), array('id' => $id));
	}
}
?>