...
class List_WxShopPage extends MerchWebPage
{
	protected function orderData($status, $st) 
	{
		...
		if (!(empty($_GPC['searchfield'])) && !(empty($_GPC['keyword']))) 
		{
			...
			else if ($searchfield == 'store') 
			{
				...
				$sqlcondition = ' left join ' . tablename('wx_shop_merch_store') . ' store on store.id = o.verifystoreid and store.uniacid=o.uniacid';
			}
			...
			foreach ($list as &$row ) 
			{
				...
				if (com('verify')) 
				{
					...
					if (!(empty($row['verifyopenid']))) 
					{
						...
						$merch_saler = pdo_fetch('select id,salername from ' . tablename('wx_shop_merch_saler') . ' where openid=:openid and uniacid=:uniacid and merchid = :merchid limit 1 ', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'], ':openid' => $row['verifyopenid']));
		...
		$t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.id =o.refundid ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_merch_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid and s.merchid=o.merchid' . ' left join ' . tablename('wx_shop_member') . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid' . ' ' . $sqlcondition . ' WHERE ' . $condition . ' ' . $statuscondition, $paras);
		$total = $t['count'];
		$totalmoney = $t['sumprice'];
		$pager = pagination2($total, $pindex, $psize);
		$stores = pdo_fetchall('select id,storename from ' . tablename('wx_shop_merch_store') . ' where uniacid=:uniacid and merchid = :merchid', array(':uniacid' => $uniacid, ':merchid' => $_W['merchid']));
