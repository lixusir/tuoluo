...
class Batchsend_WxShopPage extends MerchWebPage
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			foreach ($rows as $rownum => $col ) 
			{
				...
				$sql = 'select id,status,refundid from ' . tablename('wx_shop_order') . ' where ordersn=:ordersn and uniacid=:uniacid and isparent=0 and merchid = :merchid';
				...
				if (!(empty($order))) 
				{
					...
					{
						$refund = pdo_fetch('select id from ' . tablename('wx_shop_order_refund') . ' where id=:id limit 1', array(':id' => $order['refundid']));
						if (!(empty($refund))) 
						{
							$refund_flag = 1;
							pdo_update('wx_shop_order_refund', array('status' => -1, 'endtime' => $time), array('id' => $order['refundid']));
						}
					}
					$data = array();
					$data['status'] = 2;
					...
					pdo_update('wx_shop_order', $data, array('id' => $order['id']));
					if ($status == 1) 
					{
