...
class Op_WxShopPage extends MerchWebPage
{
	public function delete() 
	{
		...
		pdo_update('wx_shop_order', array('deleted' => 1), array('id' => $orderid, 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
	...
	protected function opData() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order') . ' WHERE id = :id and uniacid=:uniacid and merchid = :merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		if (empty($item)) 
		{
			if ($_W['isajax']) 
			{
	...
	public function changeprice() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			foreach ($changegoodsprice as $ogid => $change ) 
			{
				$og = pdo_fetch('select price,realprice from ' . tablename('wx_shop_order_goods') . ' where id=:ogid and uniacid=:uniacid and merchid = :merchid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
				if (!(empty($og))) 
				{
			...
			{
				pdo_update('wx_shop_order', $orderupdate, array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
			}
			if (0 < $item['parentid']) 
			{
			...
			foreach ($changegoodsprice as $ogid => $change ) 
			{
				$og = pdo_fetch('select price,realprice,changeprice from ' . tablename('wx_shop_order_goods') . ' where id=:ogid and uniacid=:uniacid and merchid = :merchid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
				if (!(empty($og))) 
				{
					$realprice = $og['realprice'] + $change;
	...
	public function pay($a = array(), $b = array()) 
	{
		...
		{
			pdo_update('wx_shop_order', array('status' => 1, 'paytype' => 11, 'paytime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
	...
	public function close() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ((0 < $item['refundstate']) && !(empty($item['refundid']))) 
			{
				...
				pdo_update('wx_shop_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
			}
	...
	public function paycancel() 
	{
		...
		{
			m('order')->setStocksAndCredits($item['id'], 2);
			pdo_update('wx_shop_order', array('status' => 0, 'cancelpaytime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
	...
	public function finish() 
	{
		...
		pdo_update('wx_shop_order', array('status' => 3, 'finishtime' => time()), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
	...
	public function fetchcancel() 
	{
		...
		pdo_update('wx_shop_order', array('status' => 1, 'finishtime' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
	...
	public function sendcancel() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			pdo_update('wx_shop_order', array('status' => 1, 'sendtime' => 0, 'remarksend' => $remark), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
			if ($item['paytype'] == 3) 
			{
				m('order')->setStocksAndCredits($item['id'], 2);
			}
	...
	public function fetch() 
	{
		...
		pdo_update('wx_shop_order', $d, array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
	...
	public function send() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$time = time();
			pdo_update('wx_shop_order', array('status' => 2, 'express' => trim($_GPC['express']), 'expresscom' => trim($_GPC['expresscom']), 'expresssn' => trim($_GPC['expresssn']), 'sendtime' => $time), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
			if (!(empty($item['refundid']))) 
			{
				$refund = pdo_fetch('select * from ' . tablename('wx_shop_order_refund') . ' where id=:id limit 1', array(':id' => $item['refundid']));
				if (!(empty($refund))) 
	...
	public function remarksaler() 
	{
		...
		if ($_W['ispost']) 
		{
			pdo_update('wx_shop_order', array('remarksaler' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
	...
	public function changeexpress() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($expresssn))) 
			{
				...
				pdo_update('wx_shop_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
	...
	public function changeaddress() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				$item = pdo_fetch('SELECT id, ordersn, address FROM ' . tablename('wx_shop_order') . ' WHERE id = :id and uniacid=:uniacid and merchid = :merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
				...
				pdo_update('wx_shop_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
