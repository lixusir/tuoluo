...
class Detail_WxShopPage extends MerchWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			pdo_update('wx_shop_order', array('remark' => trim($_GPC['remark'])), array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_dispatch') . ' WHERE id = :id and uniacid=:uniacid and merchid = :merchid', array(':id' => $item['dispatchid'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		$refund = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order_refund') . ' WHERE orderid = :orderid and uniacid=:uniacid and merchid = :merchid order by id desc', array(':orderid' => $item['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		$diyformfields = '';
		...
		if (com('verify')) {
			...
			if (!(empty($item['verifyopenid']))) {
				...
				$saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('wx_shop_merch_saler') . ' where openid=:openid and uniacid=:uniacid and merchid = :merchid limit 1 ', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid'], ':openid' => $item['verifyopenid']));
			}
