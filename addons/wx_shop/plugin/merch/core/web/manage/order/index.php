...
class Index_WxShopPage extends MerchWebPage
{
	...
	protected function selectOrderPrice($day = 0) 
	{
		...
		$sql = 'select id,price,createtime from ' . tablename('wx_shop_order') . ' where uniacid = :uniacid and merchid = :merchid and ismr=0 and isparent=0 and (status > 0 or ( status=0 and paytype=3)) and deleted=0 and createtime between :createtime1 and :createtime2';
