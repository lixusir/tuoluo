...
class Pages_WxShopPage extends MerchWebPage
{
	...
	protected function post() 
	{
		...
		if (!(empty($id))) 
		{
			$item = pdo_fetch('SELECT * FROM' . tablename('wx_shop_quick') . ' WHERE id=:id AND uniacid=:uniacid AND merchid=:merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			if (!(empty($item['datas']))) 
			{
		...
		if ($_W['ispost']) 
		{
			...
			{
				pdo_update('wx_shop_quick', $arr, array('uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid'], 'id' => $id));
				plog('quick.pages.add', '添加购买页面 ID: ' . $id . ' 标题: ' . $arr['title']);
			}
			if (!(empty($arr['keyword']))) 
			{
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title,cart,keyword FROM ' . tablename('wx_shop_quick') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		if (!(empty($items))) 
		{
