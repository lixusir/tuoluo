...
class Index_WxShopPage extends MerchWebPage
{
	public function main() 
	{
		...
		if (p('cashier')) 
		{
			$sql = 'SELECT * FROM ' . tablename('wx_shop_cashier_user') . ' WHERE uniacid=:uniacid AND merchid=:merchid AND deleted=0 AND status=1';
			$res = pdo_fetch($sql, array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			if (!(empty($res)) && (time() < $res['lifetimeend'])) 
			{
				$cashier = $res;
				$auth_code = base64_encode(authcode($cashier['username'] . '|' . $cashier['password'] . '|' . $cashier['salt'], 'ENCODE', 'wx_shop_cashier'));
