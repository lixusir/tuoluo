...
class List_WxShopPage extends MerchWebPage
{
	protected function applyData($status, $st) 
	{
		...
		$sql = 'select b.* from ' . tablename('wx_shop_merch_bill') . ' b ' . ' left join ' . tablename('wx_shop_merch_user') . ' u on b.merchid = u.id' . ' where 1 ' . $condition . ' ORDER BY ' . $orderby . ' desc ';
		...
		$total = pdo_fetchcolumn('select count(b.id) from' . tablename('wx_shop_merch_bill') . ' b ' . ' left join ' . tablename('wx_shop_merch_user') . ' u on b.merchid = u.id' . ' where 1 ' . $condition, $params);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			pdo_insert('wx_shop_merch_bill', $insert);
			$billid = pdo_insertid();
			foreach ($list as $k => $v ) 
			{
				$orderid = $v['id'];
				...
				pdo_insert('wx_shop_merch_billo', $insert_data);
				$change_order_data = array();
				$change_order_data['merchapply'] = 1;
				pdo_update('wx_shop_order', $change_order_data, array('id' => $orderid));
			}
	...
	public function getLastApply($merchid, $applytype = -1) 
	{
		...
		$sql = 'select applytype,alipay,bankname,bankcard,applyrealname from ' . tablename('wx_shop_merch_bill') . ' where merchid=:merchid and uniacid=:uniacid';
		if (-1 < $applytype) 
		{
