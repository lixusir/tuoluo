...
class Index_WxShopPage extends MerchWebPage
{
	...
	public function main() 
	{
		...
		foreach ($list as &$row ) 
		{
			...
			$row['gettotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid and merchid=:merchid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			$row['usetotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_coupon_data') . ' where used = 1 and couponid=:couponid and uniacid=:uniacid and merchid=:merchid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			$row['pwdjoins'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and merchid=:merchid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			$row['pwdoks'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and merchid=:merchid and ok=1 limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		}
		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_coupon') . ' where 1 and ' . $condition, $params);
		$pager = pagination($total, $pindex, $psize);
		$category = pdo_fetchall('select * from ' . tablename('wx_shop_coupon_category') . ' where uniacid=:uniacid and merchid=:merchid order by id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']), 'id');
	...
	protected function post() 
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE id =:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id, ':merchid' => $_W['merchid']));
		if (empty($item)) 
		{
			$starttime = time();
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,couponname FROM ' . tablename('wx_shop_coupon') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' AND merchid=' . $_W['merchid']);
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_coupon', array('id' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
			pdo_delete('wx_shop_coupon_data', array('couponid' => $item['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
