...
class Log_WxShopPage extends MerchWebPage
{
	...
	public function main() 
	{
		...
		if (!(empty($couponid))) 
		{
			$coupon = pdo_fetch('select * from ' . tablename('wx_shop_coupon') . ' where id=:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		$sql = 'SELECT d.*, c.coupontype,c.couponname,m.nickname,m.avatar,m.realname,m.mobile FROM ' . tablename('wx_shop_coupon_data') . ' d ' . ' left join ' . tablename('wx_shop_coupon') . ' c on d.couponid = c.id ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid = d.openid and m.uniacid = d.uniacid ' . ' where  1 and ' . $condition . ' ORDER BY gettime DESC';
		...
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_coupon_data') . ' d ' . ' left join ' . tablename('wx_shop_coupon') . ' c on d.couponid = c.id ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid = d.openid and m.uniacid = d.uniacid ' . 'where 1 and ' . $condition, $params);
