...
class Dispatch_WxShopPage extends MerchWebPage
{
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_dispatch') . ' WHERE 1 ' . $condition . '  ORDER BY displayorder DESC limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_dispatch') . ' WHERE 1 ' . $condition, $params);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ($data['isdefault']) 
			{
				pdo_update('wx_shop_dispatch', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
			}
			if (!(empty($id))) 
			{
				mplog('shop.dispatch.edit', '修改配送方式 ID: ' . $id);
				pdo_update('wx_shop_dispatch', $data, array('id' => $id, 'merchid' => $_W['merchid']));
			}
			else 
			{
				pdo_insert('wx_shop_dispatch', $data);
				$id = pdo_insertid();
				mplog('shop.dispatch.add', '添加配送方式 ID: ' . $id);
			}
			show_json(1, array('url' => merchUrl('shop/dispatch', array('op' => 'display'))));
		}
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_dispatch') . ' WHERE id = \'' . $id . '\' and uniacid = \'' . $_W['uniacid'] . '\' and merchid=\'' . $_W['merchid'] . '\'');
		if (!(empty($dispatch))) 
		{
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('wx_shop_dispatch') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' AND merchid=' . $_W['merchid']);
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_dispatch', array('id' => $item['id']));
	...
	public function enabled() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_dispatch', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
	...
	public function setdefault() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_dispatch', array('isdefault' => intval($_GPC['isdefault'])), array('id' => $item['id']));
	...
	public function displayorder() 
	{
		...
		$item = pdo_fetchall('SELECT id,dispatchname FROM ' . tablename('wx_shop_dispatch') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' and merchid=' . $_W['merchid']);
		if (!(empty($item))) 
		{
			pdo_update('wx_shop_dispatch', array('displayorder' => $displayorder), array('id' => $id));
