...
class Comment_WxShopPage extends MerchWebPage
{
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT  c.*, o.ordersn,g.title,g.thumb FROM ' . tablename('wx_shop_order_comment') . ' c  ' . ' left join ' . tablename('wx_shop_goods') . ' g on c.goodsid = g.id  ' . ' left join ' . tablename('wx_shop_order') . ' o on c.orderid = o.id  ' . ' WHERE 1 ' . $condition . ' ORDER BY createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_order_comment') . ' c  ' . ' left join ' . tablename('wx_shop_goods') . ' g on c.goodsid = g.id  ' . ' left join ' . tablename('wx_shop_order') . ' o on c.orderid = o.id  ' . ' WHERE 1 ' . $condition . ' ', $params);
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id FROM ' . tablename('wx_shop_order_comment') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_order_comment', array('deleted' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			$goods = pdo_fetch('select id,thumb,title from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
	...
	protected function virtual() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order_comment') . ' WHERE id =:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) 
		{
			...
			$goods = set_medias(pdo_fetch('select id,thumb,title from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsid, ':uniacid' => $_W['uniacid'])), 'thumb');
			if (empty($goods)) 
			{
				show_json(0, array('message' => '请选择要评价的商品'));
			}
			...
			if (!(empty($id))) 
			{
				pdo_update('wx_shop_order_comment', $data, array('id' => $id));
				plog('shop.comment.edit', '编辑商品虚拟评价 ID: ' . $id . ' 商品ID: ' . $goods['id'] . ' 商品标题: ' . $goods['title']);
			}
			else 
			{
				pdo_insert('wx_shop_order_comment', $data);
				$id = pdo_insertid();
		...
		$goods = pdo_fetch('select id,thumb,title from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsid, ':uniacid' => $_W['uniacid']));
	...
	public function post() 
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order_comment') . ' WHERE id =:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		$goods = pdo_fetch('select id,thumb,title from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
		$order = pdo_fetch('select id,ordersn from ' . tablename('wx_shop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['orderid'], ':uniacid' => $_W['uniacid']));
		if ($_W['ispost']) 
		{
			if ($type == 0) 
			{
			...
			else 
			{
				...
				pdo_update('wx_shop_order_comment', $change_data, array('id' => $id));
