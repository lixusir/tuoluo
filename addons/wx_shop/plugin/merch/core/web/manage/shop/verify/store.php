...
class Store_WxShopPage extends MerchWebPage
{
	...
	public function main() 
	{
		...
		$sql = 'SELECT * FROM ' . tablename('wx_shop_merch_store') . ' WHERE ' . $condition . ' ORDER BY displayorder desc,id desc';
		$sql .= ' LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$sql_count = 'SELECT count(1) FROM ' . tablename('wx_shop_merch_store') . ' WHERE ' . $condition;
		$total = pdo_fetchcolumn($sql_count, $paras);
		$pager = pagination($total, $pindex, $psize);
		$list = pdo_fetchall($sql, $paras);
		foreach ($list as &$row ) 
		{
			$row['salercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_merch_saler') . ' where storeid=:storeid limit 1', array(':storeid' => $row['id']));
		}
		unset($row);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				pdo_update('wx_shop_merch_store', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				mplog('shop.verify.store.edit', '编辑门店 ID: ' . $id);
			}
			else 
			{
				pdo_insert('wx_shop_merch_store', $data);
				$id = pdo_insertid();
				mplog('shop.verify.store.add', '添加门店 ID: ' . $id);
			}
			show_json(1, array('url' => merchUrl('shop/verify/store')));
		}
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_merch_store') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		include $this->template();
	}
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,storename FROM ' . tablename('wx_shop_merch_store') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' and merchid=' . $_W['merchid']);
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_merch_store', array('id' => $item['id']));
	...
	public function displayorder() 
	{
		...
		$item = pdo_fetchall('SELECT id,storename FROM ' . tablename('wx_shop_merch_store') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' and merchid=' . $_W['merchid']);
		if (!(empty($item))) 
		{
			pdo_update('wx_shop_merch_store', array('displayorder' => $displayorder), array('id' => $id));
	...
	public function status() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_merch_store', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function query() 
	{
		...
		$ds = pdo_fetchall('SELECT id,storename FROM ' . tablename('wx_shop_merch_store') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
		if ($_GPC['suggest']) 
		{
