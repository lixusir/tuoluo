...
class Goods_WxShopPage extends MerchWebPage
{
	public function main()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('wx_shop_creditshop_goods') . ' where  1 and ' . $condition . ' ORDER BY displayorder DESC,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
	...
	protected function post()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_creditshop_goods') . ' WHERE id =:id and merchid = ' . $merchid . ' and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		if (!(empty($item['storeids']))) {
			...
			if (0 < $merchid) {
				...
				$stores = pdo_fetchall('select * from ' . tablename('wx_shop_merch_store') . ' where id in (' . $item['storeids'] . ') and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
			}

			 else {

				$stores = pdo_fetchall('select * from ' . tablename('wx_shop_store') . ' where id in (' . $item['storeids'] . ') and uniacid=:uniacid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid']));
			}
