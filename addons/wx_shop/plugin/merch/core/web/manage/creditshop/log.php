...
class Log_WxShopPage extends MerchWebPage
{
	...
	protected function getData($type) 
	{
		...
		$sql = 'select log.*, m.nickname,m.avatar,m.realname as mrealname,m.mobile as mmobile, g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype,g.isverify,' . "\n\t\t\t" . 'g.goodstype as iscoupon,s.storename,s.address as storeaddress,g.dispatch,g.goodstype,g.type,g.merchid as gmerchid' . "\n\t\t\t" . 'from ' . tablename('wx_shop_creditshop_log') . ' log ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('wx_shop_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('wx_shop_store') . ' s on s.id = log.storeid' . ' left join ' . tablename('wx_shop_creditshop_goods') . ' g on g.id = log.goodsid' . ' where 1 ' . $condition . ' ORDER BY log.createtime desc ';
		if (empty($_GPC['export'])) 
		{
			$sql .= ' limit ' . (($pindex - 1) * $psize) . ',' . $psize;
		}
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(log.id) from' . tablename('wx_shop_creditshop_log') . ' log ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('wx_shop_store') . ' s on s.id = log.storeid' . ' left join ' . tablename('wx_shop_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('wx_shop_creditshop_goods') . ' g on g.id = log.goodsid' . ' where 1 ' . $condition, $params);
		foreach ($list as &$row ) 
		{
			$row['address'] = array();
			if (!(empty($row['addressid']))) 
			{
				$row['address'] = pdo_fetch('select realname,mobile,address,province,city,area from ' . tablename('wx_shop_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $row['addressid'], ':uniacid' => $_W['uniacid']));
			}
			else 
			{
				if (0 < intval($row['gmerchid'])) 
				{
					$stores = pdo_fetch('select * from ' . tablename('wx_shop_merch_store') . ' where id=:id and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':id' => $row['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $row['gmerchid']));
				}
				else 
				{
					$stores = pdo_fetch('select * from ' . tablename('wx_shop_store') . ' where id=:id and uniacid=:uniacid and status=1 and type in(2,3)', array(':id' => $row['storeid'], ':uniacid' => $_W['uniacid']));
				}
			...
			if ($row['optionid']) 
			{
				$option = pdo_fetch('select title from ' . tablename('wx_shop_creditshop_option') . ' where id = ' . $row['optionid'] . ' and uniacid = ' . $_W['uniacid'] . ' ');
			...
			$canexchange = true;
			$verifynum = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_creditshop_verify') . ' where logid = ' . $row['id'] . ' and uniacid = ' . $_W['uniacid'] . ' ');
			if ($row['status'] == 2) 
			{
	...
	public function detail() 
	{
		...
		$id = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		if (empty($log)) 
		{
		...
		if ($goods['isverify']) 
		{
			if (!(empty($log['storeid']))) 
			{
				if (0 < $merchid) 
				{
					$stores = pdo_fetch('select * from ' . tablename('wx_shop_merch_store') . ' where id=:id and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':id' => $log['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else 
				{
					$stores = pdo_fetch('select * from ' . tablename('wx_shop_store') . ' where id=:id and uniacid=:uniacid and status=1 and type in(2,3)', array(':id' => $log['storeid'], ':uniacid' => $_W['uniacid']));
				}
			}
		}
		else 
		{
			...
			if (!(is_array($address))) 
			{
				$address = pdo_fetch('select realname,mobile,address,province,city,area from ' . tablename('wx_shop_member_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['addressid'], ':uniacid' => $_W['uniacid']));
			}
		}
	...
	public function doexchange() 
	{
		...
		$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		$verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_creditshop_verify') . ' where logid=:logid and uniacid=:uniacid', array(':logid' => $id, ':uniacid' => $_W['uniacid']));
		if (empty($log)) 
		{
			show_json(0, '兑换记录不存在!');
		}
		...
		if (!(empty($goods['isverify']))) 
		{
			if (!(empty($log['storeid']))) 
			{
				$store = pdo_fetch('select id,storename,address  from ' . tablename('wx_shop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['storeid'], ':uniacid' => $_W['uniacid']));
			}
		}
		else 
		{
		...
		if ($_W['ispost']) 
		{
			if ($goods['goodstype'] == 0) 
			{
				...
				pdo_update('wx_shop_creditshop_log', array('status' => 2, 'usetime' => time(), 'time_send' => time(), 'expresscom' => $_GPC['expresscom'], 'expresssn' => $_GPC['expresssn'], 'express' => $_GPC['express']), array('id' => $id));
				if ($goods['verifytype'] == 0) 
				{
					pdo_update('wx_shop_creditshop_log', array('status' => 3, 'usetime' => time(), 'verifyopenid' => 'system', 'time_finish' => time()), array('id' => $id));
