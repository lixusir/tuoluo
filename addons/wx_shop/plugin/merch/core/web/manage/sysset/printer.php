...
class Printer_WxShopPage extends MerchWebPage
{
	public function main() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE ' . $condition, $params);
	...
	protected function post() 
	{
		...
		if (!(empty($id))) 
		{
			$list = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($id)) 
			{
				...
				pdo_insert('wx_shop_member_printer_template', $data);
				$id = pdo_insertid();
				mplog('sysset.printer.add', '添加打印机模板 ID: ' . $id . ' 标题: ' . $data['title'] . ' ');
			}
			else 
			{
				pdo_update('wx_shop_member_printer_template', $data, array('id' => $id));
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE id in( ' . $id . ' ) AND uniacid=:uniacid AND merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_member_printer_template', array('id' => $id));
	...
	public function query() 
	{
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE ' . $condition . ' order by id asc', $params);
		if ($_GPC['suggest']) 
		{
	...
	public function printer_list() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_printer') . ' WHERE ' . $condition . '  ORDER BY id asc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_member_printer') . ' WHERE ' . $condition, $params);
		$pager = pagination($total, $pindex, $psize);
	...
	protected function printer_post() 
	{
		...
		if (!(empty($id))) 
		{
			$list = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_printer') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			$print_data = json_decode($list['print_data'], true);
		...
		if ($_W['ispost']) 
		{
			...
			{
				$data['createtime'] = time();
				pdo_insert('wx_shop_member_printer', $data);
				$id = pdo_insertid();
				mplog('sysset.printer.printer_add', '添加打印机 ID: ' . $id . ' 标题: ' . $data['title'] . ' ');
			}
			else 
			{
				pdo_update('wx_shop_member_printer', $data, array('id' => $id));
	...
	public function printer_delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_printer') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' AND merchid=' . $_W['merchid']);
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_member_printer', array('id' => $id));
	...
	public function printer_query() 
	{
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_printer') . ' WHERE ' . $condition . ' order by id asc', $params);
		if ($_GPC['suggest']) 
		{
			exit(json_encode(array('value' => $ds)));
		}
	...
	public function set() 
	{
		...
		$order_template = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE uniacid=:uniacid AND merchid=:merchid', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
		include $this->template();
	}
}
?>