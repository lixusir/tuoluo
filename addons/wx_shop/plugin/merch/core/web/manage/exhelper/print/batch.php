...
class Batch_WxShopPage extends MerchWebPage
{
	...
	public function getdata() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			foreach ($orders as $i => $order ) 
			{
				...
				$order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,g.thumb,g.unit,g.goodssn,og.optionid,og.goodssn as option_goodssn, g.productsn, g.weight, og.productsn as option_productsn, og.total,og.price,og.optionname as optiontitle, og.realprice,og.printstate,og.printstate2,og.id as ordergoodid from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
				...
				if ($order['dispatchtype'] == 1) 
				{
					...
					$store = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_merch_store') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $order['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
	...
	public function dosend() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (($orderinfo['status'] == 1) || (($orderinfo['status'] == 0) && ($orderinfo['paytype'] == 3))) 
			{
				...
				{
					$refund = pdo_fetch('select * from ' . tablename('wx_shop_order_refund') . ' where id=:id limit 1', array(':id' => $orderinfo['refundid'], 'merchid' => $merchid));
					if (!(empty($refund))) 
					{
