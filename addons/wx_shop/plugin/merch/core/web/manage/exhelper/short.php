...
class Short_WxShopPage extends MerchWebPage
{
	public function main() 
	{
		...
		$sql = 'SELECT id,title,shorttitle,status FROM ' . tablename('wx_shop_goods') . ' where 1 and ' . $condition . ' ORDER BY  shorttitle desc, status desc, id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_goods') . ' where 1 and ' . $condition, $params);
	...
	public function edit() 
	{
		...
		$goods = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid'] . ' AND merchid=' . $merchid);
		foreach ($goods as $good ) 
		{
			pdo_update('wx_shop_goods', array('shorttitle' => $shorttitle), array('id' => $good['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $merchid));
			if (empty($shorttitle)) 
			{
