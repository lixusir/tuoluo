...
class Single_WxShopPage extends MerchWebPage
{
	public function main() 
	{
		...
		$printset = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exhelper_sys') . ' WHERE uniacid=:uniacid and merchid=:merchid limit 1', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
	...
	public function getdata() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($_GPC['searchfield'])) && !(empty($_GPC['keyword']))) 
			{
				...
				{
					$sqlcondition = ' inner join ( select distinct og.orderid from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and og.merchid = \'' . $merchid . '\' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id';
					$paras[':keyword'] = trim($_GPC['keyword']);
				}
				else if ($searchfield == 'goodssn') 
				{
					$sqlcondition = ' inner join ( select distinct og.orderid from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and og.merchid = \'' . $merchid . '\' and (locate(:keyword,g.goodssn)>0)) gs on gs.orderid=o.id';
					$paras[':keyword'] = trim($_GPC['keyword']);
				}
			}
	...
	public function getorder() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			foreach ($list as &$value ) 
			{
				...
				$order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,g.thumb,g.goodssn,og.optionid,g.unit,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, g.weight, og.total,og.price,og.optionname as optiontitle, og.realprice,og.id as ordergoodid,og.printstate,og.printstate2 from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $value['id']));
			...
			unset($value);
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' WHERE o.id in ( ' . implode(',', $arr) . ') and o.uniacid=' . $_W['uniacid'] . ' and o.isparent=0 and o.merchid=' . $merchid, $paras);
			$totalmoney = pdo_fetchcolumn('SELECT sum(o.price) FROM ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' WHERE o.id in ( ' . implode(',', $arr) . ') and o.uniacid=' . $_W['uniacid'] . ' and o.isparent=0 and o.merchid=' . $merchid, $paras);
			$address = false;
			if (!(empty($list))) 
			{
	...
	public function saveuser() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (is_array($ordersns)) 
			{
				...
				{
					pdo_update('wx_shop_order', array('address_send' => $address_send), array('ordersn' => $ordersn, 'merchid' => $merchid));
				}
				exit();
			}
		}
	...
	public function getprintTemp() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$tempSender = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exhelper_senduser') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $printUserId, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
			$expTemp = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $printTempId, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
	...
	public function changestate() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			foreach ($arr as $i => $data ) 
			{
				...
				$ordergood = pdo_fetch('SELECT id,goodsid,printstate,printstate2 FROM ' . tablename('wx_shop_order_goods') . ' WHERE goodsid=:goodsid and orderid=:orderid and uniacid=:uniacid and merchid=:merchid limit 1', array(':orderid' => $orderid, ':goodsid' => $ordergoodid, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				if ($type == 1) 
				{
					pdo_update('wx_shop_order_goods', array('printstate' => $ordergood['printstate'] + 1), array('id' => $ordergood['id']));
					$orderprint = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_order_goods') . ' WHERE orderid=:orderid and printstate=0 and uniacid= :uniacid and merchid=:merchid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else if ($type == 2) 
				{
					pdo_update('wx_shop_order_goods', array('printstate2' => $ordergood['printstate2'] + 1), array('id' => $ordergood['id']));
					$orderprint = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_order_goods') . ' WHERE orderid=:orderid and printstate2=0 and uniacid= :uniacid and merchid=:merchid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				if ($orderprint == 0) 
				{
					$printstatenum = 2;
	...
	public function getorderinfo() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$printTemp = pdo_fetch('SELECT id,type,expressname,express,expresscom FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE id=:id and type=:type and uniacid=:uniacid and merchid=:merchid limit 1', array(':id' => $temp_express, ':type' => 1, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
	...
	public function dosend() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$orderinfo = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order') . ' WHERE id=:orderid and status>-1 and uniacid=:uniacid and merchid=:merchid limit 1', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
			if (empty($orderinfo)) 
			{
			...
			{
				pdo_update('wx_shop_order', array('express' => trim($express), 'expresssn' => trim($expresssn), 'expresscom' => trim($expresscom), 'sendtime' => time(), 'status' => 2), array('id' => $orderid));
				if (!(empty($orderinfo['refundid']))) 
				{
					$refund = pdo_fetch('select * from ' . tablename('wx_shop_order_refund') . ' where id=:id and merchid=:merchid limit 1', array(':id' => $orderinfo['refundid'], ':merchid' => $merchid));
					if (!(empty($refund))) 
					{
