...
class Order_WxShopPage extends MerchWebPage
{
	public function main()
	{
		...
		$sql = 'select o.*, a.realname as addressname,m.realname from ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on a.id = o.addressid ' . ' where 1 ' . $condition . ' ';
		...
		$count = pdo_fetch('select count(o.id) as count from ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on a.id = o.addressid ' . ' where 1 ' . $condition, $params);
