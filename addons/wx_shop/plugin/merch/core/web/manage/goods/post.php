...
if (0 < $maxgoods) {
	...
	$sql = 'SELECT COUNT(1) FROM ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and merchid=:merchid';
...
$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE id = :id and uniacid=:uniacid and merchid=:merchid', array(':id' => $id, ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
...
if (!(empty($id))) {
	...
	if (!(empty($item['storeids']))) {
		...
		$stores = pdo_fetchall('select id,storename from ' . tablename('wx_shop_merch_store') . ' where id in (' . $item['storeids'] . ' ) and uniacid=' . $_W['uniacid'] . ' and merchid=' . $_W['merchid']);
	}
...
$dispatch_data = pdo_fetchall('select * from ' . tablename('wx_shop_dispatch') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));


if ($com_virtual) {

	$virtual_types = pdo_fetchall('select * from ' . tablename('wx_shop_virtual_type') . ' where uniacid=:uniacid and merchid=:merchid order by id asc', array(':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
}
