...
class Index_WxShopPage extends MerchWebPage
{
	public function main($goodsfrom = '') 
	{
		...
		$sql = 'SELECT COUNT(g.`id`) FROM ' . tablename('wx_shop_goods') . 'g' . $sqlcondition . $condition . $groupcondition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();
		if (!(empty($total))) 
		{
			$sql = 'SELECT g.* FROM ' . tablename('wx_shop_goods') . 'g' . $sqlcondition . $condition . $groupcondition . ' ORDER BY g.`status` DESC, g.`merchdisplayorder` DESC,' . "\r\n" . '                g.`id` DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
	...
	public function change() 
	{
		...
		$goods = pdo_fetch('select id,hasoption from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if (empty($goods)) 
		{
			show_json(0, array('message' => '参数错误'));
		}
	...
	public function query() 
	{
		...
		$ds = pdo_fetchall('SELECT id,title,thumb,marketprice,productprice,share_title,share_icon,description,minprice FROM ' . tablename('wx_shop_goods') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
