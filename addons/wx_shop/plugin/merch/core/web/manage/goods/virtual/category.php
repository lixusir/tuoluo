...
class Category_WxShopPage extends MerchWebPage
{
	...
	public function main() 
	{
		...
		if (!(empty($_GPC['catname']))) 
		{
			...
			foreach ($_GPC['catname'] as $id => $catname ) 
			{
				...
				if ($id == 'new') 
				{
					pdo_insert('wx_shop_virtual_category', array('name' => $catname, 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
					$insert_id = pdo_insertid();
					mplog('goods.virtual.category.add', '添加分类 ID: ' . $insert_id);
				}
				else 
				{
					pdo_update('wx_shop_virtual_category', array('name' => $catname), array('id' => $id, 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_virtual_category') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' and merchid = \'' . $_W['merchid'] . '\' ORDER BY id DESC');
	...
	public function delete() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('wx_shop_virtual_category') . ' WHERE id = \'' . $id . '\' AND uniacid=' . $_W['uniacid'] . ' AND merchid=' . $_W['merchid']);
		if (empty($item)) 
		{
			$this->message('抱歉，分类不存在或是已经被删除！', merchUrl('goods/virtual/category', array('op' => 'display')), 'error');
		}
		pdo_delete('wx_shop_virtual_category', array('id' => $id));
