...
class Index_WxShopPage extends MerchWebPage
{
	...
	public function menu() 
	{
		global $_W;
		global $_GPC;
		$list = pdo_fetchall('select id, name from ' . tablename('wx_shop_diypage_menu') . ' where merch=:merch and uniacid=:uniacid order by id desc', array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid']));
		$data = $this->model->getSet('diypage');
		if ($_W['ispost']) 
		{
			$diymenu = $_GPC['menu'];
