...
class Index_WxShopPage extends MerchWebPage
{
	public function main() 
	{
		global $_W;
		global $_GPC;
		$user = pdo_fetch('select `id`,`logo`,`merchname`,`desc` from ' . tablename('wx_shop_merch_user') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $_W['uniaccount']['merchid'], ':uniacid' => $_W['uniacid']));
		$order_sql = 'select id,ordersn,createtime,address,price,invoicename from ' . tablename('wx_shop_order') . ' where uniacid = :uniacid and merchid=:merchid and isparent=0 and deleted=0 AND ( status = 1 or (status=0 and paytype=3) ) ORDER BY createtime ASC LIMIT 20';
	...
	public function updatepassword() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_merch_account') . ' WHERE id=:id AND uniacid=:uniacid AND merchid=:merchid', array(':id' => $_W['uniaccount']['id'], ':uniacid' => $_W['uniacid'], ':merchid' => $_W['merchid']));
			...
			pdo_update('wx_shop_merch_account', $date, array('id' => $_W['uniaccount']['id'], 'uniacid' => $_W['uniacid'], 'merchid' => $_W['merchid']));
			show_json(1);
		}
	...
	public function switchversion() 
	{
		...
		$id = intval($_GPC['id']);
		$set = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_version') . ' WHERE uid=:uid AND `type`=1', array(':uid' => $_W['merchuid']));
		// $data = array('type' => 1, 'version' => (!(empty($_W['shopversion'])) ? 0 : 1));
		$data = array('type' => 1, 'version' =>  1);
