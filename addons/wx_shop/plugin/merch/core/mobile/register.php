...
class Register_WxShopPage extends PluginMobileLoginPage
{
	public function main() 
	{
		...
		$reg = pdo_fetch('select * from ' . tablename('wx_shop_merch_reg') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		$user = false;
		if (!(empty($reg['status']))) 
		{
			$user = pdo_fetch('select * from ' . tablename('wx_shop_merch_user') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
		}
		if (!(empty($user)) && (1 <= $user['status'])) 
		{
			$this->message('您已成功入驻多商户，请前往PC端装修店铺！', '', 'error');
		}
		$apply_set = array();
