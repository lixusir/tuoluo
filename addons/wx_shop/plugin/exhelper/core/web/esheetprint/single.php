...
class Single_WxShopPage extends PluginWebPage
{
	public function main() 
	{
		...
		$printset = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exhelper_sys') . ' WHERE uniacid=:uniacid and merchid=0 limit 1', array(':uniacid' => $_W['uniacid']));
	...
	public function getdata() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($_GPC['searchfield'])) && !(empty($_GPC['keyword']))) 
			{
				...
				{
					$sqlcondition = ' inner join ( select distinct og.orderid from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and og.merchid=0 and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id';
					$paras[':keyword'] = trim($_GPC['keyword']);
				}
				else if ($searchfield == 'goodssn') 
				{
					$sqlcondition = ' inner join ( select distinct og.orderid from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid where og.uniacid = \'' . $uniacid . '\' and og.merchid=0 and (locate(:keyword,g.goodssn)>0)) gs on gs.orderid=o.id';
					$paras[':keyword'] = trim($_GPC['keyword']);
				}
			}
			$sql = 'select o.* ,a.realname ,m.nickname, d.dispatchname,m.nickname,r.status as refundstatus from ' . tablename('wx_shop_order') . ' o' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_dispatch') . ' d on d.id = o.dispatchid ' . $sqlcondition . ' where ' . $condition . ' ' . $statuscondition . ' ORDER BY o.createtime DESC,o.status DESC  ';
	...
	public function getorder() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$sql = 'select o.* , a.realname,a.mobile,a.province,a.city,a.area, d.dispatchname,m.nickname,r.status as refundstatus from ' . tablename('wx_shop_order') . ' o' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_dispatch') . ' d on d.id = o.dispatchid ' . ' where o.id in ( ' . implode(',', $arr) . ') and o.uniacid=' . $_W['uniacid'] . ' and m.uniacid=' . $_W['uniacid'] . ' and o.merchid=0 and o.isparent=0 ORDER BY o.createtime DESC,o.status DESC  ';
			...
			unset($value);
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' WHERE o.id in ( ' . implode(',', $arr) . ') and o.merchid=0 and o.isparent=0 and o.uniacid=' . $_W['uniacid'], $paras);
			$totalmoney = pdo_fetchcolumn('SELECT sum(o.price) FROM ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' WHERE o.id in ( ' . implode(',', $arr) . ') and o.merchid=0 and o.isparent=0 and o.uniacid=' . $_W['uniacid'], $paras);
			$address = false;
			if (!(empty($list))) 
			{
	...
	public function changestate() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			foreach ($arr as $i => $data ) 
			{
				...
				$ordergood = pdo_fetch('SELECT id,goodsid,printstate,printstate2 FROM ' . tablename('wx_shop_order_goods') . ' WHERE goodsid=:goodsid and orderid=:orderid and uniacid=:uniacid and merchid=0 limit 1', array(':orderid' => $orderid, ':goodsid' => $ordergoodid, ':uniacid' => $_W['uniacid']));
				if ($type == 1) 
				{
					pdo_update('wx_shop_order_goods', array('printstate' => $ordergood['printstate'] + 1), array('id' => $ordergood['id']));
					$orderprint = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_order_goods') . ' WHERE orderid=:orderid and printstate=0 and uniacid= :uniacid and merchid=0', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
				}
				else if ($type == 2) 
				{
					pdo_update('wx_shop_order_goods', array('printstate2' => $ordergood['printstate2'] + 1), array('id' => $ordergood['id']));
					$orderprint = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_order_goods') . ' WHERE orderid=:orderid and printstate2=0 and uniacid= :uniacid and merchid=0', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
				}
				if ($orderprint == 0) 
				{
					$printstatenum = 2;
				...
				if ($type == 1) 
				{
					pdo_update('wx_shop_order', array('printstate' => $printstatenum), array('id' => $orderid));
				}
				else if ($type == 2) 
				{
					pdo_update('wx_shop_order', array('printstate2' => $printstatenum), array('id' => $orderid));
				}
			}
	...
	public function getorderinfo() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$sql = 'SELECT temp.id,temp.esheetname,esheet.express,esheet.name as expresscom  FROM ' . tablename('wx_shop_exhelper_esheet_temp') . ' temp ' . ' left join ' . tablename('wx_shop_exhelper_esheet') . ' esheet on temp.esheetid = esheet.id ' . ' where temp.id=:id and temp.uniacid=:uniacid and temp.merchid=0 limit 1';
			$printTemp = pdo_fetch($sql, $params);
			if (empty($printTemp) || !(is_array($printTemp))) 
			{
				exit();
			}
			$orders = pdo_fetchall('SELECT id,ordersn,address,address_send,status,paytype,expresscom,expresssn,express,dispatchtype FROM ' . tablename('wx_shop_order') . ' WHERE id in( ' . $in . ' ) and (status=1 or (paytype=3 and status=0)) and uniacid=:uniacid and merchid=0 and isparent=0 order by ordersn desc ', array(':uniacid' => $_W['uniacid']));
			if (empty($orders)) 
			{
				exit();
			}
	...
	public function dosend() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$orderinfo = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_order') . ' WHERE id=:orderid and status>-1 and uniacid=:uniacid and merchid=0 limit 1', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
			if (empty($orderinfo)) 
			{
			...
			{
				pdo_update('wx_shop_order', array('express' => trim($express), 'expresssn' => trim($expresssn), 'expresscom' => trim($expresscom), 'sendtime' => time(), 'status' => 2, 'refundstate' => 0), array('id' => $orderid));
				if (!(empty($orderinfo['refundid']))) 
				{
					$refund = pdo_fetch('select * from ' . tablename('wx_shop_order_refund') . ' where id=:id limit 1', array(':id' => $orderinfo['refundid']));
					if (!(empty($refund))) 
					{
						pdo_update('wx_shop_order_refund', array('status' => -1), array('id' => $orderinfo['refundid']));
						pdo_update('wx_shop_order', array('refundid' => 0), array('id' => $orderinfo['id']));
					}
				}
	...
	public function getesheetTemp() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$params = array(':uniacid' => $_W['uniacid'], ':id' => $temp_esheetid);
			$sql = 'SELECT *  FROM ' . tablename('wx_shop_exhelper_esheet_temp') . ' temp ' . ' left join ' . tablename('wx_shop_exhelper_esheet') . ' esheet on temp.esheetid = esheet.id ' . ' where temp.id=:id and temp.uniacid=:uniacid and temp.merchid=0 limit 1';
			$esheet_item = pdo_fetch($sql, $params);
			$sender_item = pdo_fetch('SELECT *  FROM ' . tablename('wx_shop_exhelper_senduser') . ' WHERE id=:id and uniacid=:uniacid and merchid=0 limit 1', array(':id' => $temp_senderid, ':uniacid' => $_W['uniacid']));
			$eorder = array();
			...
			{
				pdo_query('UPDATE ' . tablename('wx_shop_order_goods') . ' SET `esheetprintnum` = `esheetprintnum` + 1 WHERE id in (' . $_GPC['printarr']['ordergoodid'] . ')');
				pdo_update('wx_shop_order_goods', array('ordercode' => $eorder['OrderCode']), array('id' => explode(',', $_GPC['printarr']['ordergoodid'])));
				...
				pdo_update('wx_shop_order', $data, array('id' => explode(',', $_GPC['printarr']['orderid'])));
