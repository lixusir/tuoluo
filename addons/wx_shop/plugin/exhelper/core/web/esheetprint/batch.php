...
class Batch_WxShopPage extends PluginWebPage
{
	...
	public function getdata() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$sql = 'select o.* ,a.realname ,m.nickname, d.dispatchname,m.nickname,r.status as refundstatus from ' . tablename('wx_shop_order') . ' o' . ' left join ' . tablename('wx_shop_order_refund') . ' r on r.orderid=o.id and ifnull(r.status,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid and m.uniacid = o.uniacid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_dispatch') . ' d on d.id = o.dispatchid ' . $sqlcondition . ' where ' . $condition . ' ' . $statuscondition . '  ORDER BY o.createtime DESC,o.status DESC  ';
	...
	public function saveuser() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (is_array($ordersns)) 
			{
				...
				{
					pdo_update('wx_shop_order', array('address_send' => $address_send), array('ordersn' => $ordersn, 'merchid' => 0));
				}
				exit();
			}
		}
		...
		if ($_W['ispost']) 
		{
			...
			if (($orderinfo['status'] == 1) || (($orderinfo['status'] == 0) && ($orderinfo['paytype'] == 3))) 
			{
				...
				if (!(empty($orderinfo['refundid']))) 
				{
					...
					if (!(empty($refund))) 
					{
						pdo_update('wx_shop_order_refund', array('status' => -1, 'endtime' => time()), array('id' => $orderinfo['refundid']));
