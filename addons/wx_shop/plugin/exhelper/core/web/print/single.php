...
class Single_WxShopPage extends PluginWebPage
{
	...
	public function getprintTemp() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$tempSender = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exhelper_senduser') . ' WHERE id=:id and uniacid=:uniacid and merchid=0 limit 1', array(':id' => $printUserId, ':uniacid' => $_W['uniacid']));
			if (!(empty($tempSender['province']))) 
			{
				$tempSender['sendercity'] = $tempSender['province'] . $tempSender['city'] . $tempSender['area'];
			}
			$expTemp = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE id=:id and uniacid=:uniacid and merchid=0 limit 1', array(':id' => $printTempId, ':uniacid' => $_W['uniacid']));
	...
	public function getorderinfo() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$printTemp = pdo_fetch('SELECT id,type,expressname,express,expresscom FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE id=:id and type=:type and uniacid=:uniacid and merchid=0 limit 1', array(':id' => $temp_express, ':type' => 1, ':uniacid' => $_W['uniacid']));
			...
			$orders = pdo_fetchall('SELECT id,ordersn,address,address_send,status,paytype,expresscom,expresssn,dispatchtype FROM ' . tablename('wx_shop_order') . ' WHERE id in( ' . $in . ' ) and (status=1 or (paytype=3 and status=0)) and uniacid=:uniacid and merchid=0 and isparent=0 order by ordersn desc ', array(':uniacid' => $_W['uniacid']));
			if (empty($orders)) 
			{
				exit();
			}
