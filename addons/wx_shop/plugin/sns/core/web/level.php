...
class Level_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_sns_level') . ' WHERE 1 ' . $condition . ' ORDER BY ' . $orderby . ' asc', $params);
	...
	protected function post()
	{
		...
		if ($id == 'default') {
			...
			$level = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_sns_level') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => intval($id)));
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if ($id == 'default') {
					...
					pdo_update('wx_shop_sns_level', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
					plog('sns.level.edit', '修改会员等级 ID: ' . $id . $updatecontent);
				}
			}
			else {
				pdo_insert('wx_shop_sns_level', $data);
				$id = pdo_insertid();
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,levelname FROM ' . tablename('wx_shop_sns_level') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('wx_shop_sns_level', array('id' => $item['id']));
	...
	public function enabled()
	{
		...
		foreach ($items as $item) {
			pdo_update('wx_shop_sns_level', array('enabled' => intval($_GPC['enabled'])), array('id' => $item['id']));
