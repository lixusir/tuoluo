...
class Board_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_sns_board') . ' WHERE 1 ' . $condition . '  ORDER BY displayorder DESC limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('wx_shop_sns_board') . ' WHERE 1 ' . $condition, $params);
	...
	protected function post()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('select * from ' . tablename('wx_shop_sns_board') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if (!empty($data['keyword'])) {
					$keyword = pdo_fetchcolumn('SELECT keyword FROM ' . tablename('wx_shop_sns_board') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
				...
				pdo_update('wx_shop_sns_board', $data, array('id' => $id));
				...
				pdo_insert('wx_shop_sns_board', $data);
				$id = pdo_insertid();
				plog('sns.board.add', '添加版块 ID: ' . $id);
			}

			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':name' => 'wx_shop:sns:board:' . $id));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:sns:board:' . $id, 'module' => 'wx_shop', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'wx_shop', 'content' => trim($data['keyword']), 'type' => 1, 'displayorder' => 0, 'status' => 1);
		...
		if (!empty($item)) {
			$managers = pdo_fetchall('select m.id,m.openid,m.avatar,m.nickname from ' . tablename('wx_shop_sns_manage') . ' sm ' . ' left join ' . tablename('wx_shop_member') . ' m on sm.openid = m.openid and sm.uniacid = m.uniacid' . ' where sm.uniacid=:uniacid and sm.bid=:bid ', array(':uniacid' => $_W['uniacid'], ':bid' => $item['id']));
		}
		include $this->template();
	}
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title,keyword FROM ' . tablename('wx_shop_sns_board') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('wx_shop_sns_board', array('id' => $item['id']));
			pdo_delete('wx_shop_sns_board_follow', array('bid' => $item['id']));
			pdo_delete('wx_shop_sns_post', array('bid' => $item['id']));
	...
	public function displayorder()
	{
		...
		$item = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_sns_board') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		if (!empty($item)) {
			pdo_update('wx_shop_sns_board', array('displayorder' => $displayorder), array('id' => $id));
	...
	public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_sns_board') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_update('wx_shop_sns_board', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
