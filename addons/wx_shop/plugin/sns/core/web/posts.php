...
class Posts_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = "select p.id,p.title,p.createtime,p.content,p.images ,p.replytime, p.openid, p.nickname,p.avatar,p.isbest,p.isboardbest,p.istop,p.isboardtop,\r\n        p.checked,p.deleted,p.isadmin,b.title as boardtitle,b.logo as boardlogo " . '  from ' . tablename('wx_shop_sns_post') . ' p ' . ' left join ' . tablename('wx_shop_sns_board') . ' b on p.bid = b.id  and b.uniacid = p.uniacid' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid = p.openid and m.uniacid = p.uniacid ' . '  where 1 ' . $condition . ' ORDER BY p.replytime DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_sns_post') . ' p ' . ' left join ' . tablename('wx_shop_sns_board') . ' b on p.bid = b.id  and b.uniacid = p.uniacid' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid = p.openid and m.uniacid = p.uniacid ' . ' where 1 ' . $condition, $params);

		foreach ($list as &$row) {
			$row['replycount'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_sns_post') . ' where pid=:pid and deleted = 0 limit 1', array(':pid' => $row['id']));
			$images = array();
			...
			$row['thumb'] = $thumb;
			$row['needchecks'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_sns_post') . ' where pid=:pid and checked=0 limit 1', array(':pid' => $row['id']));
		}
	...
	public function post()
	{
		...
		$id = intval($_GPC['id']);
		$uniacid = $_W['uniacid'];
		$item = pdo_fetch("select id,bid,title,content,images,istop,isbest,isboardtop,isboardbest,openid\r\n                from " . tablename('wx_shop_sns_post') . ' where id = ' . $id . ' and uniacid = ' . $uniacid . ' ');

		if (!empty($item['images'])) {
			$piclist = array_merge(iunserializer($item['images']));
		}

		$board = pdo_fetchall('select id,title from ' . tablename('wx_shop_sns_board') . ' where uniacid = ' . $uniacid . ' order by id desc ');
		$set = $this->set;
		/*if (isset($set['managers'])) {
			if (!empty($set['managers'])) {
				...
				$managers = pdo_fetchall('select id,nickname,openid from ' . tablename('wx_shop_member') . "\r\n                            where openid in (" . implode(',', $openids) . ') and uniacid = ' . $uniacid . ' ');
			}
		}*/
		$query = load()->object('query');
        $managers = $query->from('wx_shop_member')->where('nickname <>',' ')->where('uniacid =',$uniacid)->getall();
		...
		if ($_W['ispost']) {
			...
			if (!empty($data['openid'])) {
				$user = pdo_fetch('select avatar,nickname from ' . tablename('wx_shop_member') . ' where openid = \'' . $data['openid'] . '\' and uniacid = ' . $uniacid . ' ');
			...
			if (!empty($id)) {
				pdo_update('wx_shop_sns_post', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function delete()
	{
		...
		$items = pdo_fetchall('SELECT id,title,pid,openid, content FROM ' . tablename('wx_shop_sns_post') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		...
		foreach ($items as $item) {
			...
			if ($deleted) {
				...
				pdo_update('wx_shop_sns_post', array('deleted' => 1, 'deletedtime' => time()), array('id' => $item['id']));
				...
				pdo_update('wx_shop_sns_post', array('deleted' => 0, 'deletedtime' => 0), array('id' => $item['id']));
	...
	public function delete1()
	{
		...
		$items = pdo_fetchall('SELECT id,title,pid,openid, content,deleted FROM ' . tablename('wx_shop_sns_post') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			$msg = (empty($item['pid']) ? '话题' : '评论');
			$content = $this->model->replaceContent($item['content']);
			plog('sns.posts.delete', '彻底删除' . $msg . ' ID: ' . $item['id'] . ' 标题: ' . $item['title'] . ' 内容: ' . $content);
			pdo_delete('wx_shop_sns_post', array('id' => $item['id']));
	...
	public function check()
	{
		...
		$items = pdo_fetchall('SELECT id,title,content,openid, pid,bid FROM ' . tablename('wx_shop_sns_post') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		...
		foreach ($items as $item) {
			...
			if ($checked) {
				...
				pdo_update('wx_shop_sns_post', array('checked' => 1), array('id' => $item['id']));
				...
				pdo_update('wx_shop_sns_post', array('checked' => 0), array('id' => $item['id']));
	...
	public function top()
	{
		...
		$items = pdo_fetchall('SELECT id,title,content,openid, pid FROM ' . tablename('wx_shop_sns_post') . ' WHERE id in( ' . $id . ' ) and pid=0 AND uniacid=' . $_W['uniacid']);
		...
		foreach ($items as $item) {
			...
			if ($top) {
				...
				pdo_update('wx_shop_sns_post', array($field => 1), array('id' => $item['id']));
				...
				pdo_update('wx_shop_sns_post', array($field => 0), array('id' => $item['id']));
	...
	public function best()
	{
		...
		$items = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_sns_post') . ' WHERE id in( ' . $id . ' ) and pid=0 AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item) {
			$content = $this->model->replaceContent($item['content']);
			if ($best) {
