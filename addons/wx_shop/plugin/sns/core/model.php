...
if (!class_exists('SnsModel')) {
	class SnsModel extends PluginModel
	{
		public function checkMember()
		{
			...
			if (!empty($_W['openid'])) {
				$member = pdo_fetch('select * from ' . tablename('wx_shop_sns_member') . ' where uniacid=:uniacid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		...
		public function getCategory($all = true)
		{
			...
			return pdo_fetchall('select * from ' . tablename('wx_shop_sns_category') . ' where uniacid=:uniacid ' . $condition . ' and enabled = 1 order by displayorder desc', array(':uniacid' => $_W['uniacid']), 'id');
		}
		...
		{
			global $_W;
			return pdo_fetch('select * from ' . tablename('wx_shop_sns_board') . ' where uniacid=:uniacid and id=:id  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $bid));
		}
		...
		{
			global $_W;
			return pdo_fetch('select * from ' . tablename('wx_shop_sns_post') . ' where uniacid=:uniacid and id=:id  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $pid));
		}
		...
		public function isManager($bid = 0, $openid = '')
		{
			...
			$count = pdo_fetchcolumn('select count(*)  from ' . tablename('wx_shop_sns_manage') . ' where uniacid=:uniacid and bid=:bid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':bid' => $bid, ':openid' => $openid));
			return 0 < $count;
		}
		...
		{
			global $_W;
			$count = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_sns_board_follow') . ' where uniacid=:uniacid and bid=:bid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':bid' => $bid, ':openid' => $_W['openid']));
			return 0 < $count;
		}
		...
		public function getTops($bid = 0)
		{
			...
			$condition .= ')';
			return pdo_fetchall('select id,title,istop,isboardtop from ' . tablename('wx_shop_sns_post') . ' where uniacid=:uniacid ' . $condition . ' and pid=0 and deleted=0  order by istop desc,replytime desc', array(':uniacid' => $_W['uniacid']));
		}
		...
		public function setCredit($openid = '', $bid = 0, $type = -1)
		{
			...
			if (0 < abs($credit)) {
				...
				pdo_update('wx_shop_sns_member', array('credit' => $newcredit), array('id' => $member['sns_id']));
		...
		public function getLevel($openid)
		{
			...
			if (!empty($member) && !empty($member['sns_level'])) {
				$level = pdo_fetch('select * from ' . tablename('wx_shop_sns_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $member['sns_level'], ':uniacid' => $_W['uniacid']));
		...
		public function upgradeLevel($openid)
		{
			...
			$level = false;
			if (empty($leveltype)) {
				$level = pdo_fetch('select * from ' . tablename('wx_shop_sns_level') . ' where uniacid=:uniacid  and enabled=1 and ' . $credit . ' >= credit and credit>0  order by credit desc limit 1', array(':uniacid' => $_W['uniacid']));
			}
			else {
				if ($leveltype == 1) {
					$post = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_sns_post') . ' where uniacid=:uniacid and openid=:openid and pid=0 and checked=1', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
					$level = pdo_fetch('select * from ' . tablename('wx_shop_sns_level') . ' where uniacid=:uniacid and enabled=1 and ' . $post . ' >= `post` and `post`>0  order by `post` desc limit 1', array(':uniacid' => $_W['uniacid']));
				}
			}
			if (empty($level)) {
				return NULL;
			...
			if ($canupgrade) {
				pdo_update('wx_shop_sns_member', array('level' => $level['id']), array('id' => $member['sns_id']));
		...
		public function getLevels($all = true)
		{
			...
			return pdo_fetchall('select * from ' . tablename('wx_shop_sns_level') . ' where uniacid=:uniacid' . $condition . ' order by id asc', array(':uniacid' => $_W['uniacid']));
		}

		public function replaceContent($content)
		{
			global $_W;
			return str_replace("\n", '<br/>', preg_replace('/\\[EM(\\w+)\\]/', '<img src="'.$_W['siteroot'].'addons/wx_shop/plugin/sns/static/images/face/${1}.gif" class="emoji" />', $content));
		}
		...
		public function getAvatar($avatar = '')
		{
			if (empty($avatar)) {
				$set = $this->getSet();
				$avatar = (empty($set['head']) ? '../addons/wx_shop/plugin/sns/static/images/head.jpg' : tomedia($set['head']));
			}
