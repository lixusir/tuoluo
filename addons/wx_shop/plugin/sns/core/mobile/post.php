...
class Post_WxShopPage extends SnsMobilePage
{
	...
	public function getlist() 
	{
		...
		foreach ($list as $key => &$row ) 
		{
			...
			$isgood = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_sns_like') . ' where uniacid=:uniacid and pid=:pid and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':pid' => $row['id'], ':openid' => $_W['openid']));
			$row['isgood'] = $isgood;
	...
	public function like() 
	{
		...
		$isgood = 1;
		$like = pdo_fetch('select id from ' . tablename('wx_shop_sns_like') . ' where pid=:pid and openid=:openid limit 1', array(':pid' => $pid, ':openid' => $_W['openid']));
		if (!(empty($like))) 
		{
			$isgood = 0;
