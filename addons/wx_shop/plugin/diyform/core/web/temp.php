...
class Temp_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$items = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_diyform_type') . ' WHERE uniacid=:uniacid and title like :name order by id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, array(':name' => '%' . $kw . '%', ':uniacid' => $_W['uniacid']));
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_diyform_type') . ' WHERE uniacid=:uniacid and title like :name order by id desc ', array(':uniacid' => $_W['uniacid'], ':name' => '%' . $kw . '%'));
	...
	protected function post()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_diyform_type') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
				...
				pdo_update('wx_shop_diyform_type', $insert, array('id' => $id));
	...
	public function delete()
	{
		...
		$types = pdo_fetchall('SELECT id,title  FROM ' . tablename('wx_shop_diyform_type') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$errmsg = '';
		...
		foreach ($types as $type) {
			...
			if (!empty($err)) {
				...
				pdo_delete('wx_shop_diyform_type', array('id' => $id));
				pdo_delete('wx_shop_diyform_data', array('typeid' => $id));
