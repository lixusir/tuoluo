...
class Category_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		if (!empty($_GPC['catname'])) {
			...
			foreach ($_GPC['catname'] as $id => $catname) {
				...
				if ($id == 'new') {
					pdo_insert('wx_shop_diyform_category', array('name' => $catname, 'uniacid' => $_W['uniacid']));
					$insert_id = pdo_insertid();
					plog('diyform.category.add', '添加自定义表单分类 ID: ' . $insert_id);
				}
				else {
					pdo_update('wx_shop_diyform_category', array('name' => $catname), array('id' => $id));
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_diyform_category') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY id DESC');
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('wx_shop_diyform_category') . ' WHERE id = \'' . $id . '\' AND uniacid=' . $_W['uniacid'] . '');

		if (empty($item)) {
			$this->message('抱歉，分类不存在或是已经被删除！', webUrl('diyform/category', array('op' => 'display')), 'error');
		}

		pdo_delete('wx_shop_diyform_category', array('id' => $id));
