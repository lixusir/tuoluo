...
class DiyformModel extends PluginModel
{
	...
	{
		global $_W;
		return array('data_type_config' => $this->_data_type_config, 'default_data_config' => $this->_default_data_config, 'default_date_config' => $this->_default_date_config, 'category' => pdo_fetchall('select * from ' . tablename('wx_shop_diyform_category') . ' where uniacid=:uniacid order by id desc', array(':uniacid' => $_W['uniacid']), 'id'));
	}
	...
	public function setGoodsLastData($type, $cid, $array, $openid)
	{
		...
		pdo_update('wx_shop_diyform_temp', $change_data, array('cid' => $cid, 'uniacid' => $_W['uniacid'], 'openid' => $openid, 'type' => $type));
	}
	...
	public function getLastData($type, $diymode, $diyformid, $cid, $fields, $member)
	{
		...
		if (!empty($cid)) {
			$table_name = 'wx_shop_diyform_temp';
			...
			if (empty($data)) {
				$table_name = 'wx_shop_order_goods';
	...
	public function getLastCartData($goodsid = 0)
	{
		global $_W;
		$f_data = array();
		$cartdata = pdo_fetch('select * from ' . tablename('wx_shop_member_cart') . ' where goodsid=:goodsid and openid=:openid order by id desc limit 1', array(':openid' => $_W['openid'], ':goodsid' => $goodsid));
	...
	public function getLastOrderData($diyformid, $member)
	{
		...
		if (!empty($diyformid)) {
			...
			if (empty($DiyformInfo['savedata'])) {
				$order = pdo_fetch('select diyformdata,diyformfields from ' . tablename('wx_shop_order') . ' where diyformid=:diyformid and openid=:openid order by id desc limit 1', array(':diyformid' => $diyformid, ':openid' => $member['openid']));
	...
	{
		global $_W;
		pdo_update('wx_shop_diyform_type', 'alldata=alldata+1', array('id' => $id));
	}

	public function getCountData($typeid, $type = 0)
	{
		global $_W;
		$sql = 'select count(*) from ' . tablename('wx_shop_diyform_data') . ' where typeid=:typeid and uniacid=:uniacid';
	...
	{
		global $_W;
		$sql = 'select * from ' . tablename('wx_shop_diyform_temp') . ' where cid=:cid and diyformid=:diyformid and openid=:openid and type=:type and uniacid=:uniacid Limit 1';
	...
	{
		global $_W;
		$sql = 'select count(id) from ' . tablename('wx_shop_goods') . ' where diyformid=:diyformid and uniacid=:uniacid';
	...
	{
		global $_W;
		$form_list = pdo_fetchall('select * from ' . tablename('wx_shop_diyform_type') . ' where uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		return $form_list;
	}
	...
	{
		global $_W;
		$formInfo = pdo_fetch('select * from ' . tablename('wx_shop_diyform_type') . ' where id=:id and uniacid=:uniacid Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	{
		global $_W;
		$data = pdo_fetch('select * from ' . tablename('wx_shop_diyform_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (!empty($data) && $flag) {
	...
	{
		global $_W;
		$data = pdo_fetch('select * from ' . tablename('wx_shop_diyform_temp') . ' where id=:id and uniacid=:uniacid Limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (!empty($data) && $flag) {
			$data['diyformfields'] = iunserializer($data['diyformfields']);
	...
	{
		global $_W;
		$sql = 'select count(1) from ' . tablename('wx_shop_diyform_type') . ' where id=:id and uniacid=:uniacid';
