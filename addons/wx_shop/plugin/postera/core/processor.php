...
class PosteraProcessor extends PluginProcessor
{
	...
	private function responseScan($obj)
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('wx_shop_postera') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $qr['posterid'], ':uniacid' => $_W['uniacid']));
		...
		if ($poster['resptype'] == '0') {
			...
			if ($poster['type'] == 3) {
				$goods = pdo_fetch('select id,title, thumb,description from ' . tablename('wx_shop_goods') . ' where id=:id limit 1', array(':id' => $poster['goodsid']));
	...
	private function responseSubscribe($obj)
	{
		...
		$qrmember = m('member')->getMember($qr['openid']);
		$log = pdo_fetch('select * from ' . tablename('wx_shop_postera_log') . ' where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':posterid' => $poster['id'], ':uniacid' => $_W['uniacid']));
		if (empty($log) && ($openid != $qr['openid'])) {
			...
			pdo_insert('wx_shop_postera_log', $log);
			$log['id'] = pdo_insertid();
	...
	'remark'   => array('value' => "\r\n谢谢您对我们的支持！", 'color' => '#4a5077')
			...
			if (!empty($upgrade)) {
				pdo_update('wx_shop_postera_log', $upgrade, array('id' => $log['id']));
			}
		}
