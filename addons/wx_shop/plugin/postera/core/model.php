...
class PosteraModel extends PluginModel
{
	...
	public function getQR($poster, $member)
	{
		...
		$qr = pdo_fetch('select * from ' . tablename('wx_shop_postera_qr') . ' where openid=:openid and acid=:acid and posterid=:posterid limit 1', array(':openid' => $member['openid'], ':acid' => $acid, ':posterid' => $poster['id']));
		...
		if (empty($qr)) {
			...
			$ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'qrcid' => $scene_id, 'model' => 0, 'name' => 'WX_SHOP_POSTERA_QRCODE', 'keyword' => 'WX_SHOP_POSTERA', 'expire' => $expire, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket']);
			...
			pdo_insert('wx_shop_postera_qr', $qr);
			$qr['id'] = pdo_insertid();
		}
		else {
			...
			if (($qr['endtime'] < time()) || ($qr['qrtime'] != ($poster['timestart'] . $poster['timeend']))) {
				...
				pdo_update('wx_shop_postera_qr', array('ticket' => $ticket, 'qrimg' => $qrimg, 'url' => $result['url'], 'endtime' => $posterendtime, 'qrtime' => $poster['timestart'] . $poster['timeend']), array('id' => $qr['id']));
				$qr['ticket'] = $ticket;
	...
	{
		global $_W;
		$path = IA_ROOT . '/addons/wx_shop/data/postera/' . $_W['uniacid'] . '/';
		...
		if (($qr['qrimg'] != $qr['current_qrimg']) || empty($qr['mediaid']) || empty($qr['createtime']) || ((($qr['createtime'] + (3600 * 24 * 3)) - 7200) < time())) {
			...
			$qr['mediaid'] = $mediaid;
			pdo_update('wx_shop_postera_qr', array('mediaid' => $mediaid, 'createtime' => time()), array('id' => $qr['id']));
		}
	...
	public function getQRByTicket($ticket = '')
	{
		...
		$qrs = pdo_fetchall('select * from ' . tablename('wx_shop_postera_qr') . ' where ticket=:ticket and acid=:acid limit 1', array(':ticket' => $ticket, ':acid' => $_W['acid']));
		$count = count($qrs);
