...
class Log_WxShopPage extends PluginWebPage
{
	...
	protected function getData($type) 
	{
		...
		foreach ($list as $key => &$row ) 
		{
			...
			{
				$option = pdo_fetch('select total,credit,money,title as optiontitle,weight from ' . tablename('wx_shop_creditshop_option') . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $row['optionid'] . ' and goodsid = ' . $row['goodsid'] . ' ');
	...
	public function doexchange() 
	{
		...
		if ($_W['ispost']) 
		{
				...
				if (0 < $goods['isverify']) 
				{
					...
					else if ($goods['verifytype'] == 1) 
					{
						...
						if ($i <= $verifynum) 
						{
							...
							pdo_insert('wx_shop_creditshop_verify', $data);
							++$i;
							pdo_update('wx_shop_creditshop_log', array('status' => 3, 'usetime' => time(), 'time_send' => time(), 'expresscom' => $_GPC['expresscom'], 'expresssn' => $_GPC['expresssn'], 'express' => $_GPC['express']), array('id' => $id));
						}
					}
				}
				else 
			...
			else if ($goods['goodstype'] == 3) 
			{
				...
				$goods = pdo_fetch('select surplusmoney from ' . tablename('wx_shop_creditshop_goods') . ' where id = ' . $log['goodsid'] . ' ');
