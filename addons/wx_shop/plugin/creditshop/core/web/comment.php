...
class Comment_WxShopPage extends PluginWebPage
{
	...
	protected function getList($checked = 0) 
	{
		...
		$list = pdo_fetchall('SELECT  c.*,g.title,g.thumb FROM ' . tablename('wx_shop_creditshop_comment') . ' c  ' . ' left join ' . tablename('wx_shop_creditshop_goods') . ' g on c.goodsid = g.id  ' . ' WHERE 1 ' . $condition . ' ORDER BY c.time desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
	...
	public function delete() 
	{
		...
		{
			pdo_update('wx_shop_creditshop_comment', array('deleted' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			$goods = pdo_fetch('select id,thumb,title from ' . tablename('wx_shop_creditshop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['goodsid'], ':uniacid' => $_W['uniacid']));
