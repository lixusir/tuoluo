...
class Goods_WxShopPage extends PluginWebPage
{
	public function main() 
	{
		...
		$sql = 'SELECT * FROM ' . tablename('wx_shop_creditshop_goods') . ' where  1 and ' . $condition . ' ORDER BY displayorder DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_creditshop_goods') . ' where 1 and ' . $condition, $params);
		if ($merch_plugin) 
		{
		...
		$category = pdo_fetchall('select id,`name`,thumb from ' . tablename('wx_shop_creditshop_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	protected function post() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_creditshop_goods') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		{
			$stores = pdo_fetchall('select id,storename from ' . tablename('wx_shop_store') . ' where id in (' . $item['storeids'] . ' ) and uniacid=' . $_W['uniacid']);
		}
		...
		$category = pdo_fetchall('select id,`name`,thumb from ' . tablename('wx_shop_creditshop_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']));
		if (empty($item['goodstype']) || ($item['goodstype'] == 0)) 
		{
			$goods = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE id =:id and status=1 and deleted=0 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['goodsid']));
		}
		else if ($item['goodstype'] == 1) 
		{
			$coupon = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_coupon') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['couponid']));
		}
		...
		$allspecs = pdo_fetchall('select * from ' . tablename('wx_shop_creditshop_spec') . ' where goodsid=:id order by displayorder asc', array(':id' => $id));
		foreach ($allspecs as &$s ) 
		{
			$s['items'] = pdo_fetchall('select a.id,a.specid,a.title,a.thumb,a.show,a.displayorder,a.valueId,a.virtual,b.title as title2 from ' . tablename('wx_shop_creditshop_spec_item') . ' a left join ' . tablename('wx_shop_virtual_type') . ' b on b.id=a.virtual  where a.specid=:specid order by a.displayorder asc', array(':specid' => $s['id']));
		}
		unset($s);
		$html = '';
		$discounts_html = '';
		...
		$options = pdo_fetchall('select * from ' . tablename('wx_shop_creditshop_option') . ' where goodsid=:id order by id asc', array(':id' => $id));
		$specs = array();
		if (0 < count($options)) 
		{
		...
		$dispatch_data = pdo_fetchall('select * from ' . tablename('wx_shop_dispatch') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				pdo_update('wx_shop_creditshop_goods', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			else 
			{
				...
				pdo_insert('wx_shop_creditshop_goods', $data);
				$id = pdo_insertid();
			...
			if ($data['hasoption']) 
			{
				...
				while ($k < $len) 
				{
					...
					{
						pdo_update('wx_shop_creditshop_spec', $a, array('id' => $get_spec_id));
						$spec_id = $get_spec_id;
					}
					else 
					{
						pdo_insert('wx_shop_creditshop_spec', $a);
					...
					while ($n < $itemlen) 
					{
						...
						{
							pdo_update('wx_shop_creditshop_spec_item', $d, array('id' => $get_item_id));
							$item_id = $get_item_id;
						}
						else 
						{
							pdo_insert('wx_shop_creditshop_spec_item', $d);
					...
					if (0 < count($itemids)) 
					{
						pdo_query('delete from ' . tablename('wx_shop_creditshop_spec_item') . ' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id . ' and id not in (' . implode(',', $itemids) . ')');
					}
					else 
					{
						pdo_query('delete from ' . tablename('wx_shop_creditshop_spec_item') . ' where uniacid=' . $_W['uniacid'] . ' and specid=' . $spec_id);
					}
					pdo_update('wx_shop_creditshop_spec', array('content' => serialize($itemids)), array('id' => $spec_id));
					$specids[] = $spec_id;
					++$k;
				}
				if (0 < count($specids)) 
				{
					pdo_query('delete from ' . tablename('wx_shop_creditshop_spec') . ' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id . ' and id not in (' . implode(',', $specids) . ')');
				}
				else 
				{
					pdo_query('delete from ' . tablename('wx_shop_creditshop_spec') . ' where uniacid=' . $_W['uniacid'] . ' and goodsid=' . $id);
				}
				...
				while ($k < $len) 
				{
					...
					{
						pdo_insert('wx_shop_creditshop_option', $a);
						$option_id = pdo_insertid();
					}
					else 
					{
						pdo_update('wx_shop_creditshop_option', $a, array('id' => $get_option_id));
				...
				{
					pdo_query('delete from ' . tablename('wx_shop_creditshop_option') . ' where goodsid=' . $id . ' and id not in ( ' . implode(',', $optionids) . ')');
					$sql = 'update ' . tablename('wx_shop_creditshop_goods') . ' g set' . "\r\n\t\t\t\t\t" . 'g.mincredit = (select min(credit) from ' . tablename('wx_shop_creditshop_option') . ' where goodsid = ' . $id . '),' . "\r\n\t\t\t\t\t" . 'g.minmoney = (select min(money) from ' . tablename('wx_shop_creditshop_option') . ' where goodsid = ' . $id . '),' . "\r\n\t\t\t\t\t" . 'g.maxcredit = (select max(credit) from ' . tablename('wx_shop_creditshop_option') . ' where goodsid = ' . $id . '),' . "\r\n\t\t\t\t\t" . 'g.maxmoney = (select max(money) from ' . tablename('wx_shop_creditshop_option') . ' where goodsid = ' . $id . ')' . "\r\n\t\t\t\t\t" . 'where g.id = ' . $id . ' and g.hasoption=1';
					pdo_query($sql);
				}
				else 
				{
					pdo_query('delete from ' . tablename('wx_shop_creditshop_option') . ' where goodsid=' . $id);
					$sql = 'update ' . tablename('wx_shop_creditshop_goods') . ' set minmoney = money,maxmoney = money,mincredit = credit,maxcredit = credit where id = ' . $id . ' and hasoption=0;';
					pdo_query($sql);
				}
				if ($data['hasoption'] !== 0) 
				{
					pdo_update('wx_shop_creditshop_goods', array('total' => $totalstocks), array('id' => $id));
				}
			}
	...
	public function delete() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_creditshop_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_creditshop_goods', array('deleted' => 1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function status() 
	{
		...
		$items = pdo_fetchall('SELECT id,status FROM ' . tablename('wx_shop_creditshop_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			$status_update = pdo_update('wx_shop_creditshop_goods', array('status' => $status), array('id' => $item['id']));
			if (!($status_update)) 
			{
	...
	public function deleted() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_delete('wx_shop_creditshop_goods', array('id' => $item['id']));
	...
	public function recycle() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_creditshop_goods', array('status' => 0, 'deleted' => 0), array('id' => $item['id']));
	...
	public function property() 
	{
		...
		{
			pdo_update('wx_shop_creditshop_goods', array($type => $value), array('id' => $id, 'uniacid' => $_W['uniacid']));
			$statusstr = '';
			if ($type == 'istop') 
			{
				$typestr = '置顶';
	...
	public function query() 
	{
		...
		$list = array();
		$list = pdo_fetchall('SELECT id,title,thumb,marketprice,productprice,share_title,share_icon,description,minprice,costprice,total,content,hasoption' . "\r\n" . '              FROM ' . tablename('wx_shop_goods') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
		$list = set_medias($list, array('thumb', 'share_icon'));
		foreach ($list as $key => $value ) 
		{
			if (intval($value['hasoption']) == 1) 
			{
				$allspecs = pdo_fetchall('select * from ' . tablename('wx_shop_goods_spec') . ' where goodsid=:id order by displayorder asc', array(':id' => $value['id']));
				foreach ($allspecs as &$s ) 
				{
					$s['items'] = pdo_fetchall('select a.id,a.specid,a.title,a.thumb,a.show,a.displayorder,a.valueId,a.virtual,b.title as title2 from ' . tablename('wx_shop_goods_spec_item') . ' a left join ' . tablename('wx_shop_virtual_type') . ' b on b.id=a.virtual  where a.specid=:specid order by a.displayorder asc', array(':specid' => $s['id']));
				}
				unset($s);
				$options = pdo_fetchall('select * from ' . tablename('wx_shop_goods_option') . ' where goodsid=:id order by id asc', array(':id' => $value['id']));
			}
	...
	public function querygoods() 
	{
		...
		$list = pdo_fetchall('SELECT *, money as minprice FROM ' . tablename('wx_shop_creditshop_goods') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
