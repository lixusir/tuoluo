...
class Lists_WxShopPage extends CreditshopMobilePage
{
	public function main() 
	{
		...
		if (!(empty($cateid))) 
		{
			$cate = pdo_fetch('select id,name from ' . tablename('wx_shop_creditshop_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cateid, ':uniacid' => $uniacid));
		}
		$category = array();
		if (0 < intval($_GPC['merchid'])) 
		{
			...
			if (!(empty($merch_category))) 
			{
				foreach ($merch_category as $index => $row ) 
				{
					if (0 < $row) 
					{
						$category[$index] = pdo_fetchall('select id,name,thumb,isrecommand from ' . tablename('wx_shop_creditshop_category') . "\r\n\t\t\t\t\t\t" . 'where id = ".$index." and uniacid=:uniacid and  enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
	...
	public function getlist() 
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . tablename('wx_shop_creditshop_goods') . ' where 1 ' . $condition;
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();
		if (!(empty($total))) 
		{
			$sql = 'SELECT id,title,thumb,subtitle,`type`,price,credit,money,goodstype FROM ' . tablename('wx_shop_creditshop_goods') . "\r\n" . '            ' . "\t\t" . 'where 1 ' . $condition . ' ORDER BY displayorder desc,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
