...
class Set_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'wx_shop文章营销入口设置'));
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop文章营销入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			...
			$sys = pdo_fetch('select * from ' . tablename('wx_shop_article_sys') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));

			if (empty($sys)) {
				$arr['uniacid'] = $_W['uniacid'];
				pdo_insert('wx_shop_article_sys', $arr);
			}
			else {
				pdo_update('wx_shop_article_sys', $arr, array('uniacid' => $_W['uniacid']));
			}
