...
class Index_WxShopPage extends PluginMobilePage
{
	public function main()
	{
		...
		if (!empty($openid) && (is_weixin() || is_h5app()) && !empty($myid)) {
			$state = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_article_log') . ' WHERE openid=:openid and aid=:aid and uniacid=:uniacid limit 1 ', array(':openid' => $openid, ':aid' => $article['id'], ':uniacid' => $_W['uniacid']));

			if (empty($state['id'])) {
				$insert = array('aid' => $aid, 'read' => 1, 'uniacid' => $_W['uniacid'], 'openid' => $openid);
				pdo_insert('wx_shop_article_log', $insert);
				$sid = pdo_insertid();
				pdo_update('wx_shop_article', array('article_readnum' => $article['article_readnum'] + 1), array('id' => $article['id']));
			}
			else {
				...
				if ($state['read'] < $readtime) {
					pdo_update('wx_shop_article_log', array('read' => $state['read'] + 1), array('id' => $state['id']));
					pdo_update('wx_shop_article', array('article_readnum' => $article['article_readnum'] + 1), array('id' => $article['id']));
				}
			}
	...
	public function getcontent()
	{
		...
		$article = pdo_fetch('SELECT article_content FROM ' . tablename('wx_shop_article') . ' WHERE id=:aid and article_state=1 and uniacid=:uniacid limit 1 ', array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
	...
	public function like()
	{
		...
		if (!empty($aid) && !empty($openid)) {
			$state = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_article_log') . ' WHERE openid=:openid and aid=:aid and uniacid=:uniacid limit 1 ', array(':openid' => $_W['openid'], ':aid' => $aid, ':uniacid' => $_W['uniacid']));

			if (empty($state['like'])) {
				pdo_update('wx_shop_article', 'article_likenum=article_likenum+1', array('id' => $aid));
				pdo_update('wx_shop_article_log', array('like' => $state['like'] + 1), array('id' => $state['id']));
