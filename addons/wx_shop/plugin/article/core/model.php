...
if (!class_exists('ArticleModel')) {
	class ArticleModel extends PluginModel
	{
		public function doShare($aid, $shareid, $myid)
		{
			...
			$article = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_article') . ' WHERE id=:aid and article_state=1 and uniacid=:uniacid limit 1 ', array(':aid' => $aid, ':uniacid' => $_W['uniacid']));
			...
			$my_click = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_article_share') . ' WHERE aid=:aid and click_user=:click_user and uniacid=:uniacid ', array(':aid' => $article['id'], ':click_user' => $myid, ':uniacid' => $_W['uniacid']));
			...
			$clicktime = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_article_share') . ' WHERE aid=:aid and share_user=:share_user and click_user=:click_user and uniacid=:uniacid ', array(':aid' => $article['id'], ':share_user' => $shareid, ':click_user' => $myid, ':uniacid' => $_W['uniacid']));

			if ($readtime <= $clicktime) {
				return NULL;
			}

			$all_click = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_article_share') . ' WHERE aid=:aid and share_user=:share_user and uniacid=:uniacid ', array(':aid' => $article['id'], ':share_user' => $shareid, ':uniacid' => $_W['uniacid']));
			...
			if ($article['article_rule_allnum'] <= $all_click) {
				...
				$day_click = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_article_share') . ' WHERE aid=:aid and share_user=:share_user and click_date>:day_start and click_date<:day_end and uniacid=:uniacid ', array(':aid' => $article['id'], ':share_user' => $shareid, ':day_start' => $day_start, ':day_end' => $day_end, ':uniacid' => $_W['uniacid']));
			...
			$toto = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_article_share') . ' WHERE aid=:aid and share_user=:click_user and click_user=:share_user and uniacid=:uniacid ', array(':aid' => $article['id'], ':share_user' => $shareid, ':click_user' => $myid, ':uniacid' => $_W['uniacid']));
			...
			if ((0 < $article['article_rule_credittotal']) || (0 < $article['article_rule_moneytotal'])) {
				$creditlast = 0;
				$moneylast = 0;
				$firstreads = pdo_fetchcolumn('select count(distinct click_user) from ' . tablename('wx_shop_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
				$allreads = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
				...
				if (0 < $article['article_rule_credittotal']) {
					if (!empty($article['article_advance'])) {
						...
						$creditout = pdo_fetchcolumn('select sum(add_credit) from ' . tablename('wx_shop_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
				...
				if (0 < $article['article_rule_moneytotal']) {
					if (!empty($article['article_advance'])) {
						...
						$moneyout = pdo_fetchcolumn('select sum(add_money) from ' . tablename('wx_shop_article_share') . ' where aid=:aid and uniacid=:uniacid limit 1', array(':aid' => $article['id'], ':uniacid' => $_W['uniacid']));
			...
			pdo_insert('wx_shop_article_share', $insert);
		...
		public function href_replace($lnk)
		{
			global $_GPC;
			$newlnk = $lnk;
			if (strexists($lnk, 'wx_shop') && !strexists($lnk, '&mid')) {
				if (strexists($lnk, '?')) {
