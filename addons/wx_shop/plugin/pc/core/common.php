...
{
	global $_W;
	$list = pdo_fetchall('SELECT title,link FROM ' . tablename('wx_shop_pc_menu') . ' WHERE type=' . $type . ' AND enabled=1 AND uniacid=' . $_W['uniacid'] . ' ORDER BY displayorder DESC,id ASC');
	if ($type == 0) 
	{
		if ($list) 
		{
...
{
	global $_W;
	$adv = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_pc_adv') . ' WHERE advname=:advname AND width=:width AND height=:height AND uniacid=:uniacid', array(':advname' => $advname, ':width' => $width, ':height' => $height, ':uniacid' => $_W['uniacid']));
	if (empty($adv)) 
	{
		pdo_insert('wx_shop_pc_adv', array('advname' => $advname, 'src' => '', 'link' => '', 'width' => $width, 'height' => $height, 'uniacid' => $_W['uniacid'], 'enabled' => 0));
		return;
	}
...
{
	global $_W;
	$qa_categories = pdo_fetchall('SELECT id,name FROM ' . tablename('wx_shop_qa_category') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY displayorder DESC LIMIT 4');
	foreach ($qa_categories as &$qa_category ) 
	{
		$qa_category['list'] = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_qa_question') . ' where  cate=:cate_id AND uniacid=:uniacid AND status=1 ORDER BY displayorder DESC,id DESC LIMIT 5', array(':cate_id' => $qa_category['id'], ':uniacid' => $_W['uniacid']));
	}
	unset($qa_category);
...
{
	global $_W;
	$links = pdo_fetchall('SELECT linkname,url FROM ' . tablename('wx_shop_pc_link') . ' WHERE  uniacid=:uniacid AND status=1 ORDER BY displayorder', array(':uniacid' => $_W['uniacid']));
	return $links;
}
