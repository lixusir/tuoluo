...
require WX_SHOP_PLUGIN . "pc/core/page_mobile.php";
class Detail_WxShopPage extends PcMobilePage
{
	public function main() 
	{
		...
		$params = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_goods_param') . ' WHERE uniacid=:uniacid and goodsid=:goodsid AND title<>"" order by displayorder asc', array(':uniacid' => $uniacid, ':goodsid' => $goods['id']));
		$goods = set_medias($goods, 'thumb');
		$goods['canbuy'] = !(empty($goods['status'])) && empty($goods['deleted']);
		if (!(empty($goods['hasoption']))) 
		{
			$options = pdo_fetchall('select id,stock from ' . tablename('wx_shop_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $goods['id'], ':uniacid' => $_W['uniacid']), 'stock');
		...
		else 
		{
			if (isset($options) && (0 < count($options)) && ($goods['hasoption'] !== 0)) 
			{
				...
				//pdo_query("delete from " . tablename("wx_shop_goods_option") . " where goodsid=" . $id . ' and id not in ( ' . implode(',', $optionids) . ')');
	...
	public function get_history() 
	{
		...
		$sql = 'SELECT f.id,f.goodsid,g.title,g.thumb,g.marketprice,g.productprice,f.createtime,g.merchid FROM ' . tablename('wx_shop_member_history') . ' f ' . ' left join ' . tablename('wx_shop_goods') . ' g on f.goodsid = g.id ' . ' where 1 ' . $condition . ' ORDER BY f.`id` DESC LIMIT ' . $psize;
