...
class Package_WxShopPage extends PcMobileLoginPage
{
	public function main() 
	{
		...
		foreach ($packages as $key => $value ) 
		{
			...
			{
				$g = pdo_fetch('SELECT id,marketprice FROM ' . tablename('wx_shop_goods') . "\n" . '                    WHERE uniacid = ' . $uniacid . ' and id = ' . $val . '  ORDER BY id DESC');
	...
	public function detail() 
	{
		...
		$uniacid = $_W['uniacid'];
		$pid = intval($_GPC['pid']);
		$package = pdo_fetch('SELECT id,title,price,freight,share_title,share_icon,share_desc FROM ' . tablename('wx_shop_package') . "\n" . '                    WHERE uniacid = ' . $uniacid . ' and id = ' . $pid . ' ');
		$packgoods = array();
		$packgoods = pdo_fetchall('SELECT id,title,thumb,marketprice,packageprice,`option`,goodsid FROM ' . tablename('wx_shop_package_goods') . "\n" . '                    WHERE uniacid = ' . $uniacid . ' and pid = ' . $pid . '  ORDER BY id DESC');
	...
	public function option() 
	{
		...
		$optionid = array();
		$option = array();
		$option = pdo_fetchall('select optionid,title,goodsid,packageprice from ' . tablename('wx_shop_package_goods_option') . ' where pid = ' . $pid . ' and goodsid = ' . $goodsid . ' and uniacid = ' . $uniacid . ' ');
		show_json(1, $option);
	}
}
?>