...
class Pay_Alipay_WxShopPage extends MobilePage
{
	...
	public function complete() 
	{
		...
		else 
		{
			...
			{
				$log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':tid' => $tid));
		...
		if ($log['status'] != 1) 
		{
			...
			$orderid = pdo_fetchcolumn('select id from ' . tablename('wx_shop_order') . ' where ordersn=:ordersn and uniacid=:uniacid', array(':ordersn' => $log['tid'], ':uniacid' => $_W['uniacid']));
			if (!(empty($orderid))) 
			{
				...
				pdo_update("wx_shop_order", $data_alipay, array('id' => $orderid));
			}
			$ret = array();
			$ret['result'] = 'success';
			$ret['type'] = 'alipay';
	...
	public function recharge_complete() 
	{
		...
		if (!(empty($log)) && empty($log['status'])) 
		{
			pdo_update('wx_shop_member_log', array('status' => 1, 'rechargetype' => 'alipay', 'apppay' => (is_h5app() ? 1 : 0), 'transid' => $transid), array('id' => $log['id']));
