...
class Detail_WxShopPage extends PcMobilePage
{
	...
	public function lottery() 
	{
		...
		if ((0 < $goods['credit']) && empty($log['creditpay'])) 
		{
			...
			$update['creditpay'] = 1;
			pdo_query("update " . tablename("wx_shop_creditshop_goods") . " set joins=joins+1 where id=" . $id);
		}
		$status = 1;
		if ($goods['type'] == 1) 
		{
		...
		if ($status == 2) 
		{
			...
			if ($status == 3) 
			{
				pdo_query('update ' . tablename('wx_shop_creditshop_goods') . ' set total=total-1 where id=' . $id);
			}
			...
			{
				pdo_query('update ' . tablename('wx_shop_creditshop_goods') . ' set packetsurplus=packetsurplus-1 where id=' . $id);
			}
