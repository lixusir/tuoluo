...
class Comment_WxShopPage extends PluginMobileLoginPage
{
	...
	public function submit() 
	{
		...
		$uniacid = $_W['uniacid'];
		$logid = intval($_GPC['logid']);
		$log = pdo_fetch('select id,status,iscomment,logno from ' . tablename('wx_shop_creditshop_log') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $logid, ':uniacid' => $uniacid, ':openid' => $openid));
		if (empty($log)) 
		{
			show_json(0, '兑换记录未找到');
		}
		...
		foreach ($comments as $c ) 
		{
			...
			else 
			{
				...
				pdo_update("wx_shop_creditshop_comment", $comment, array('uniacid' => $_W['uniacid'], 'goodsid' => $c['goodsid'], 'logid' => $logid));
			}
		}
		if ($log['iscomment'] <= 0) 
		{
