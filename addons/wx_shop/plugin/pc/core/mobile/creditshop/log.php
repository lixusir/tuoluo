...
class Log_WxShopPage extends PcMobileLoginPage
{
	...
	public function getlist() 
	{
		...
		if (!(empty($total))) 
		{
			$sql = 'SELECT log.id,log.logno,log.goodsid,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,g.credit,g.money,g.isverify,g.goodstype,log.addressid,log.storeid,' . 'g.goodstype,log.time_send,log.time_finish,log.iscomment,op.title as optiontitle ' . ' FROM ' . tablename('wx_shop_creditshop_log') . ' log ' . ' left join ' . tablename('wx_shop_creditshop_goods') . ' g on log.goodsid = g.id ' . ' left join ' . tablename('wx_shop_creditshop_option') . ' op on op.id = log.optionid ' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
	...
	public function detail() 
	{
		...
		$id = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id=:id and openid=:openid and uniacid=:uniacid limit 1', array(':id' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
		if (empty($log)) 
		{
			show_json(-1, '兑换记录不存在!');
		}
		...
		{
			$verifytotal = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_creditshop_verify') . ' where logid = :id and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':id' => $id, ':openid' => $log['openid'], ':uniacid' => $log['uniacid'], ':verifycode' => $log['eno']));
			if ($goods['verifytype'] == 0) 
			{
				$verify = pdo_fetch('select isverify from ' . tablename('wx_shop_creditshop_verify') . ' where logid = :id and openid = :openid and uniacid = :uniacid and verifycode = :verifycode ', array(':id' => $log['id'], ':openid' => $log['openid'], ':uniacid' => $log['uniacid'], ':verifycode' => $log['eno']));
			}
			...
			$isverify = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_verify') . ' where logid = ' . $log['id'] . ' and uniacid = ' . $uniacid . ' and isverify = 1 limit 1 ');
	...
	public function Receivepacket() 
	{
		...
		$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id = ' . $logid . ' and uniacid = ' . $uniacid . ' ');
		if (!($log)) 
		{
		...
		$goods = pdo_fetch('select surplusmoney from ' . tablename('wx_shop_creditshop_goods') . ' where id = ' . $log['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
	...
	public function express() 
	{
		...
		$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id=:logid and uniacid=:uniacid and openid=:openid limit 1', array(':logid' => $logid, ':uniacid' => $uniacid, ':openid' => $openid));
		if (empty($log)) 
		{
			header('location: ' . mobileUrl('creditshop/log'));
			exit();
		...
		$goods = pdo_fetch('select *  from ' . tablename('wx_shop_creditshop_goods') . '  where id=:id and uniacid=:uniacid ', array(':uniacid' => $uniacid, ':id' => $log['goodsid']));
	...
	public function finish() 
	{
		...
		global $_GPC;
		$logid = intval($_GPC['id']);
		$log = pdo_fetch('select * from ' . tablename('wx_shop_creditshop_log') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
		if (empty($log)) 
		{
			show_json(0, '订单未找到');
		}
