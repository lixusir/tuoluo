...
class Bind_WxShopPage extends PcMobileLoginPage
{
	protected $member;
	...
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($member2))) 
			{
				if (strexists($member2['openid'], 'sns_wx_') || strexists($member2['openid'], 'sns_qq_') || strexists($member2['openid'], 'wap_user_')) 
				{
					$tables = pdo_fetchall('SHOW TABLES like \'%_wx_shop_%\'');
					...
					pdo_update("wx_shop_commission_apply", array("mid" => $member['id']), array('uniacid' => $_W['uniacid'], 'mid' => $member2['id']));
					pdo_update("wx_shop_order", array("agentid" => $member['id']), array('agentid' => $member2['id']));
					pdo_update("wx_shop_member", array("agentid" => $member['id']), array('agentid' => $member2['id']));
					...
					pdo_delete("wx_shop_member", array("id" => $member2['id'], 'uniacid' => $_W['uniacid']));
					$changeAgentid = false;
					...
					if ($changeAgentid) 
					{
						pdo_update('wx_shop_member', array('agentid' => $member2['agentid']), array('id' => $member['id']));
					}
					if ($member2['isagent'] && $member2['status']) 
					{
						pdo_update('wx_shop_member', array('isagent' => 1, 'status' => 1, 'agenttime' => $member2['agenttime']), array('id' => $member['id']));
					}
					else if ($member2['isagent']) 
					{
						if (!($member['isagent']) && !($member['status'])) 
						{
							pdo_update('wx_shop_member', array('isagent' => 1, 'status' => 0), array('id' => $member['id']));
						}
					}
					$memberid = $member['id'];
				...
				{
					pdo_update("wx_shop_member", array("pwd" => '', "salt" => '', "mobileverify" => 0), array("id" => $member2['id'], 'uniacid' => $_W['uniacid']));
					$memberid = $member2['id'];
				}
			...
			pdo_update("wx_shop_member", array("mobile" => $mobile, 'pwd' => md5($pwd . $salt), 'salt' => $salt, 'mobileverify' => 1, 'createtime' => $createtime), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
			unset($_SESSION[$key]);
			m("account")->setLogin($member['id']);
			show_json(1);
		}
