...
class Index_WxShopPage extends PcMobilePage 
{
	public function main() 
	{
		...
		$slides = pdo_fetchall('select id,advname,link,thumb,backcolor from ' . tablename('wx_shop_pc_slide') . ' where uniacid=:uniacid and enabled=1 AND type=0 order by displayorder desc', array(':uniacid' => $_W['uniacid']));
		$recommends = pdo_fetchall('select id,advname,link,thumb from ' . tablename('wx_shop_pc_slide') . ' where uniacid=:uniacid and enabled=1 AND type=1 order by displayorder desc', array(':uniacid' => $_W['uniacid']));
		...
		$notice_list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_notice') . ' WHERE uniacid=:uniacid AND status=1  ORDER BY displayorder DESC,id DESC limit 4', array(':uniacid' => $_W['uniacid']));
