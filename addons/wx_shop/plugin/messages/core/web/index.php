...
class Index_WxShopPage extends PluginWebPage
{
	...
	public function build()
	{
		...
		if (!empty($item)) {
			...
			$sql .= ' SELECT  :uniacid,sm.openid,sm.nickname,:taskid,1 from ' . tablename('wx_shop_member') . ' sm  inner join ' . tablename('mc_mapping_fans') . ' mf  on sm.openid=mf.openid and mf.follow=1 where sm.uniacid=:uniacid ';
