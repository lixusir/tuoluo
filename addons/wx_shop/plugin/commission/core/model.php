...
if (!(class_exists('CommissionModel'))) {
    class CommissionModel extends PluginModel
    {
        ...
        public function calculate($orderid = 0, $update = true)
        {
            ...
            $order = pdo_fetch('select agentid,price,goodsprice,deductcredit2,discountprice,isdiscountprice,dispatchprice,changeprice,ispackage,packageid from ' . tablename('wx_shop_order') . ' where id=:id limit 1', array(':id' => $orderid));
                ...
                foreach ($goods as &$cinfo) {
                        ...
                        if (empty($cinfo['nocommission'])) {
                            ...
                            if (0 < $order['ispackage']) {
                                ...
                                if (!(empty($cinfo['optionid']))) {
                                    $packoption = pdo_fetch('select commission1,commission2,commission3 from ' . tablename('wx_shop_package_goods_option') . "\r\n" . '                                            where uniacid = ' . $_W['uniacid'] . ' and pid = ' . $order['packageid'] . ' and optionid = ' . $cinfo['optionid'] . ' ');
                                } else {
                                    $packoption = pdo_fetch('select commission1,commission2,commission3 from ' . tablename('wx_shop_package_goods') . "\r\n" . '                                            where uniacid = ' . $_W['uniacid'] . ' and pid = ' . $order['packageid'] . ' and goodsid = ' . $cinfo['goodsid'] . ' ');
                                }
                    ...
                    if ($update) {
                        ...
                        if (p('task')) {
                            ...
                            p('task')->checkTaskProgress(1, 'pyramid_money', 0, $m1['openid'],0,$order['price']);
                            //p('task')->checkTaskProgress((int)$commissions['level2'], 'pyramid_money', 0, $m2['openid'],0,$order['price']);
                            //p('task')->checkTaskProgress((int)$commissions['level3'], 'pyramid_money', 0, $m3['openid'],$order['price']);
                        }
                        pdo_update('wx_shop_order_goods', array('commission1' => iserializer($cinfo['commission1']), 'commission2' => iserializer($cinfo['commission2']), 'commission3' => iserializer($cinfo['commission3']), 'commissions' => iserializer($commissions), 'nocommission' => $cinfo['nocommission']), array('id' => $cinfo['id']));
                    }
                }
                unset($cinfo);
            }
            if (!($hascommission)) {
                pdo_update('wx_shop_order', array('agentid' => 0), array('id' => $orderid));
            }
            return $goods;
        }
        ...
        public function getOrderCommissions($orderid = 0, $ogid = 0)
        {
            global $_W;
            $set = $this->getSet();
            $agentid = pdo_fetchcolumn('select agentid from ' . tablename('wx_shop_order') . ' where id=:id limit 1', array(':id' => $orderid));
            $goods = pdo_fetch('select commission1,commission2,commission3 from ' . tablename('wx_shop_order_goods') . ' where id=:id and orderid=:orderid and uniacid=:uniacid and nocommission=0 limit 1', array(':id' => $ogid, ':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
            $commissions = array('level1' => 0, 'level2' => 0, 'level3' => 0);
            if (0 < $set['level']) {
        ...
        public function getInfo($openid, $options = NULL)
        {
            ...
            if (1 <= $level) {
                if (in_array('ordercount0', $options)) {
                    $level1_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('wx_shop_order') . ' o ' . ' left join  ' . tablename('wx_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=0 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                ...
                if (in_array('ordercount', $options)) {
                    $level1_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('wx_shop_order') . ' o ' . ' left join  ' . tablename('wx_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=1 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                ...
                if (in_array('ordercount3', $options)) {
                    $level1_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('wx_shop_order') . ' o ' . ' left join  ' . tablename('wx_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid=:agentid and o.status>=3 and og.status1>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                ...
                if (in_array('total', $options)) {
                    $level1_commissions = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                ...
                if (in_array('ok', $options)) {
                    $level1_commissions = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ') and og.status1=0  and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    foreach ($level1_commissions as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                ...
                if (in_array('lock', $options)) {
                    $level1_commissions1 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.nocommission=0 and (' . $time . ' - o.finishtime <= ' . $day_times . ')  and og.status1=0  and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                ...
                if (in_array('apply', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                ...
                if (in_array('check', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=2 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    foreach ($level1_commissions2 as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                ...
                if (in_array('pay', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and o.status>=3 and og.status1=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    foreach ($level1_commissions2 as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                ...
                if (in_array('wait', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid=:agentid and (o.status=2 or o.status=1) and og.status1=0  and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    foreach ($level1_commissions2 as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                ...
                $level1_agentids = pdo_fetchall('select id from ' . tablename('wx_shop_member') . ' where agentid=:agentid and isagent=1 and status=1 and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']), 'id');
            ...
            if (2 <= $level) {
                if (0 < $level1) {
                    if (in_array('ordercount0', $options)) {
                        $level2_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('wx_shop_order') . ' o ' . ' left join  ' . tablename('wx_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=0 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('ordercount3', $options)) {
                        $level2_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct o.id) as ordercount from ' . tablename('wx_shop_order') . ' o ' . ' left join  ' . tablename('wx_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('total', $options)) {
                        $level2_commissions = pdo_fetchall('select og.commission2,og.commissions from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('lock', $options)) {
                        $level2_commissions1 = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and (' . $time . ' - o.finishtime <= ' . $day_times . ') and og.status2=0 and o.status>=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('apply', $options)) {
                        $level2_commissions2 = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('check', $options)) {
                        $level2_commissions3 = pdo_fetchall('select og.commission2,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid ' . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')  and o.status>=3 and og.status2=2 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
                    ...
                    $level2_agentids = pdo_fetchall('select id from ' . tablename('wx_shop_member') . ' where agentid in( ' . implode(',', array_keys($level1_agentids)) . ') and isagent=1 and status=1 and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
            ...
            if (3 <= $level) {
                if (0 < $level2) {
                    if (in_array('ordercount0', $options)) {
                        $level3_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from ' . tablename('wx_shop_order') . ' o ' . ' left join  ' . tablename('wx_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=0 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('ordercount3', $options)) {
                        $level3_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from ' . tablename('wx_shop_order') . ' o ' . ' left join  ' . tablename('wx_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=3 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('total', $options)) {
                        $level3_commissions = pdo_fetchall('select og.commission3,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0', array(':uniacid' => $_W['uniacid']));
                    ...
                    $level3_agentids = pdo_fetchall('select id from ' . tablename('wx_shop_member') . ' where uniacid=:uniacid and agentid in( ' . implode(',', array_keys($level2_agentids)) . ') and isagent=1 and status=1', array(':uniacid' => $_W['uniacid']), 'id');
        ...
        public function getAgents($orderid = 0)
        {
            ...
            global $_GPC;
            $agents = array();
            $order = pdo_fetch('select id,agentid,openid from ' . tablename('wx_shop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
            if (empty($order)) {
                return $agents;
            }
            $set = $this->getSet();
        ...
        public function getAgentsDownNum($openid = NULL)
        {
            ...
            $level1 = (int)pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));
            if ((2 <= $set['level']) && (0 < $levelcount1)) {
                $level2 = (int)pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
            }
            if ((3 <= $set['level']) && (0 < $levelcount2)) {
                $level3 = (int)pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
            }
        ...
        public function createMyShopQrcode($mid = 0, $posterid = 0)
        {
            ...
            return $_W['siteroot'] . 'addons/wx_shop/data/qrcode/' . $_W['uniacid'] . '/' . $file;
        }
        ...
        public function createGoodsImage($goods)
        {
            ...
            $path = IA_ROOT . '/addons/wx_shop/data/poster/' . $_W['uniacid'] . '/';
            if (!(is_dir($path))) {
                load()->func('file');
                mkdirs($path);
            }
            ...
            if (!(is_file($path . $file))) {
                set_time_limit(0);
                $font = IA_ROOT . '/addons/wx_shop/static/fonts/msyh.ttf';
                $target = imagecreatetruecolor(640, 1225);
                $bg = imagecreatefromjpeg(IA_ROOT . '/addons/wx_shop/plugin/commission/static/images/poster.jpg');
            ...
            return $_W['siteroot'] . 'addons/wx_shop/data/poster/' . $_W['uniacid'] . '/' . $file;
        }
        ...
        public function checkAgent($openid = '')
        {
            ...
            if ($parent_is_agent) {
                if ($parent['openid'] != $openid) {
                    ...
                    $clickcount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_commission_clickcount') . ' where uniacid=:uniacid and openid=:openid and from_openid=:from_openid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':from_openid' => $parent['openid']));
                    if ($clickcount <= 0) {
                        $click = array('uniacid' => $_W['uniacid'], 'openid' => $openid, 'from_openid' => $parent['openid'], 'clicktime' => time());
                        pdo_insert('wx_shop_commission_clickcount', $click);
                        pdo_update('wx_shop_member', array('clickcount' => $parent['clickcount'] + 1), array('uniacid' => $_W['uniacid'], 'id' => $parent['id']));
                    }
                }
            }
            if ($member['isagent'] == 1) {
            ...
            $first = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where id<>:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['id']));
            if ($first <= 0) {
                pdo_update('wx_shop_member', array('isagent' => 1, 'status' => 1, 'agenttime' => time(), 'agentblack' => 0), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                return;
            }
            $time = time();
                    ...
                    if (empty($become_child)) {
                        if (empty($member['fixagentid'])) {
                            ...
                            if ($author) {
                                ...
                                pdo_update('wx_shop_member', array('agentid' => $parent['id'], 'childtime' => $time, 'authorid' => $authorid), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                            } else {
                                pdo_update('wx_shop_member', array('agentid' => $parent['id'], 'childtime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                            }
                            if ($author) {
                                ...
                                if (!(empty($author_set['become'])) && (($author_set['become'] == '2') || ($author_set['become'] == '5'))) {
                                    ...
                                    if ($can_author) {
                                        ...
                                        if (empty($member['authorblack'])) {
                                            pdo_update('wx_shop_member', array('authorstatus' => $become_check, 'isauthor' => 1, 'authortime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $parent['id']));
                                            if ($become_check == 1) {
                            ...
                            //代理分红修改begin
                            if (p('weightbonus')) {
                                p('weightbonus')->upgradeLevelByAgent($parent['id']);
                            }
                            //代理分红修改end
                        ...
                        pdo_update('wx_shop_member', array('inviter' => $parent['id']), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                    }
                }
            }
            ...
            if ($to_check_agent) {
                pdo_update('wx_shop_member', array('isagent' => 1, 'status' => $become_check, 'agenttime' => ($become_check == 1 ? $time : 0)), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                if ($become_check == 1) {
        ...
        public function checkOrderConfirm($orderid = '0')
        {
            ...
            $order = pdo_fetch('select id,openid,ordersn,goodsprice,agentid,paytime,officcode from ' . tablename('wx_shop_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
            if (empty($order)) {
                return;
            }
            $openid = $order['openid'];
            ...
            if (empty($become_child)) {
                ...
                $parent = pdo_fetch('select * from ' . tablename('wx_shop_member') . ' where mobile = :mobile and uniacid = :uniacid limit 1 ', array(':mobile' => trim($order['officcode']), ':uniacid' => intval($_W['uniacid'])));
            } else {
            ...
            if (p('offic') && !(empty($order['officcode']))) {
                $agentid = $parent['id'];
                pdo_update('wx_shop_member', array('agentid' => $parent['id']), array('id' => $member['id']));
            }
            if (!(empty($agentid))) {
                pdo_update('wx_shop_order', array('agentid' => $agentid), array('id' => $orderid));
            }
        ...
        public function checkOrderPay($orderid = '0')
        {
            ...
            $order = pdo_fetch('select id,openid,ordersn,goodsprice,agentid,paytime from ' . tablename('wx_shop_order') . ' where id=:id and status>=1 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
            if (empty($order)) {
                return;
            }
            $openid = $order['openid'];
                    ...
                    if (empty($member['agentid']) && ($member['id'] != $parent['id'])) {
                        if (empty($member['fixagentid'])) {
                            ...
                            if (empty($order['agentid'])) {
                                ...
                                pdo_update('wx_shop_order', array('agentid' => $parent['id']), array('id' => $orderid));
                                $this->calculate($orderid);
                            }
                        }
                    }
            ...
            if (!($isagent)) {
                if ((intval($set['become']) == 4) && !(empty($set['become_goodsid']))) {
                    if (empty($set['become_order'])) {
                        $order_goods = pdo_fetchall('select goodsid from ' . tablename('wx_shop_order_goods') . ' where orderid=:orderid and uniacid=:uniacid  ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']), 'goodsid');
                        if (in_array($set['become_goodsid'], array_keys($order_goods))) {
                            if (empty($member['agentblack'])) {
                                pdo_update('wx_shop_member', array('status' => $become_check, 'isagent' => 1, 'agenttime' => ($become_check == 1 ? $time : 0)), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                                if ($become_check == 1) {
                                    $this->sendMessage($openid, array('nickname' => $member['nickname'], 'agenttime' => $time), TM_COMMISSION_BECOME);
                                    if (!(empty($parent))) {
                                        $this->upgradeLevelByAgent($parent['id']);
                    ...
                    if (($set['become'] == 2) || ($set['become'] == 3)) {
                        if (empty($set['become_order'])) {
                            ...
                            $can = false;
                            if ($set['become'] == '2') {
                                $ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where openid=:openid and status>=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                                $can = intval($set['become_ordercount']) <= $ordercount;
                            } else if ($set['become'] == '3') {
                                $moneycount = pdo_fetchcolumn('select sum(og.realprice) from ' . tablename('wx_shop_order_goods') . ' og left join ' . tablename('wx_shop_order') . ' o on og.orderid=o.id  where o.openid=:openid and o.status>=1 and o.uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                                $can = floatval($set['become_moneycount']) <= $moneycount;
                            }
                            if ($can) {
                                if (empty($member['agentblack'])) {
                                    pdo_update('wx_shop_member', array('status' => $become_check, 'isagent' => 1, 'agenttime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                                    if ($become_check == 1) {
                                        $this->sendMessage($openid, array('nickname' => $member['nickname'], 'agenttime' => $time), TM_COMMISSION_BECOME);
                                        if ($parentisagent) {
                                            $this->upgradeLevelByAgent($parent['id']);
            ...
            if (!(empty($member['agentid']))) {
                $parent = m('member')->getMember($member['agentid']);
                if (!(empty($parent)) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
                    $order_goods = pdo_fetchall('select g.id,g.title,og.total,og.price,og.realprice, og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.commission1,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
                    $goods = '';
                    $commission_total1 = 0;
                    $commission_total2 = 0;
                    $commission_total3 = 0;
                ...
                if ((intval($set['become']) == 4) && !(empty($set['become_goodsid']))) {
                    if (empty($set['become_order'])) {
                        ...
                        if (in_array($set['become_goodsid'], array_keys($order_goods))) {
                            if (empty($member['partnerblack'])) {
                                pdo_update('wx_shop_member', array('partnerstatus' => $become_check, 'ispartner' => 1, 'partnertime' => ($become_check == 1 ? $time : 0)), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                                if ($become_check == 1) {
                    ...
                    if (($set['become'] == 2) || ($set['become'] == 3)) {
                        if (empty($set['become_order'])) {
                            ...
                            if ($can) {
                                if (empty($member['partnerblack'])) {
                                    pdo_update('wx_shop_member', array('partnerstatus' => $become_check, 'ispartner' => 1, 'partnertime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                                    if ($become_check == 1) {
                                        $plugin_globonus->sendMessage($openid, array('nickname' => $member['nickname'], 'partnertime' => $time), TM_GLOBONUS_BECOME);
                                    }
                                }
        ...
        public function checkOrderFinish($orderid = '')
        {
            ...
            $order = pdo_fetch('select id,openid, ordersn,goodsprice,agentid,finishtime from ' . tablename('wx_shop_order') . ' where id=:id and status>=3 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
            if (empty($order)) {
                return;
            }
            $set = $this->getSet();
            ...
            if (!($isagent) && ($set['become_order'] == '1')) {
                if (($set['become'] == '4') && !(empty($set['become_goodsid']))) {
                    ...
                    if (($set['become'] == 2) || ($set['become'] == 3)) {
                        $can = false;
                        if ($set['become'] == '2') {
                            $ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where openid=:openid and status>=3 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                            $can = intval($set['become_ordercount']) <= $ordercount;
                        } else if ($set['become'] == '3') {
                            $moneycount = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('wx_shop_order') . ' where openid=:openid and status>=3 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                            $can = floatval($set['become_moneycount']) <= $moneycount;
                        }
                        if ($can) {
                            if (empty($member['agentblack'])) {
            ...
            if (!(empty($member['agentid']))) {
                $parent = m('member')->getMember($member['agentid']);
                if (!(empty($parent)) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
                    $order_goods = pdo_fetchall('select g.id,g.title,og.total,og.realprice,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.commission1,og.commissions from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where og.uniacid=:uniacid and og.orderid=:orderid ', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
                    $goods = '';
                    $commission_total1 = 0;
                    $commission_total2 = 0;
                    $commission_total3 = 0;
            ...
            if ($isagent) {
                    ...
                    if (!(empty($set['open']))) {
                        ...
                        if ($isauthor) {
                            ...
                            if ($set['become_order'] == '1') {
                                ...
                                if ($can) {
                                    if (empty($member['authorblack'])) {
                                        pdo_update('wx_shop_member', array('authorstatus' => $become_check, 'isauthor' => 1, 'authortime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                                        if ($become_check == 1) {
                ...
                //代理分红修改begin(上面的golbouns也做了修改)
                $plugin_weightbonus = p('weightbonus');
                if($plugin_weightbonus){
                    $set = $plugin_weightbonus->getSet();
                    if (!(empty($set['open'])))
                    {
                        $isweight = ($member['isweight'] == 1) && ($member['weightstatus'] == 1);
                        if($isweight) {
                            $plugin_weightbonus->upgradeLevelByOrder($openid);
                        }
                        else{
                            $become_check = intval($set['become_check']);
                            if ($set['become_order'] == '1')
                            {
                                if (($set['become'] == '4') && !(empty($set['become_goodsid'])))
                                {
                                    ...
                                    if (in_array($set['become_goodsid'], array_keys($order_goods)))
                                    {
                                        if (empty($member['weightblack']))
                                        {
                                            pdo_update('wx_shop_member', array('weightstatus' => $become_check, 'isweight' => 1, 'weighttime' => ($become_check == 1 ? $time : 0)), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                                            if ($become_check == 1)
                                            {
                                                $plugin_weightbonus->sendMessage($openid, array('nickname' => $member['nickname'], 'weighttime' => $time), TM_WEIGHTBONUS_BECOME);
                                            }
                                        }
                                    }
                                }
                                else
                                {
                                    if (($set['become'] == 2) || ($set['become'] == 3))
                                    {
                                        ...
                                        if ($can)
                                        {
                                            if (empty($member['weightblack']))
                                            {
                                                pdo_update('wx_shop_member', array('weightstatus' => $become_check, 'isweight' => 1, 'weighttime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                                                if ($become_check == 1)
                                                {
                                                    $plugin_weightbonus->sendMessage($member['openid'], array('nickname' => $member['nickname'], 'weighttime' => $time), TM_WEIGHTBONUS_BECOME);
                                                }
                                            }
                                        }
                                    }
                ...
                //区域代理修改start
                //由于区域代理只能通过后台与申请，无需做其他判断
                $plugin_abonus = p('abonus');
                if ($plugin_abonus) {
                    $set = $plugin_abonus->getSet();
                    if (!(empty($set['open']))) {
                        $isaagent = $member['isaagent'] && $member['aagentstatus'];
                        if ($isaagent) {
                            $plugin_abonus->upgradeLevelByOrder($openid);
                        }
                    }
                }
                //区域代理修改end
        ...
        public function getShop($m)
        {
            ...
            $shop = pdo_fetch('select * from ' . tablename('wx_shop_commission_shop') . ' where uniacid=:uniacid and mid=:mid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
            ...
            if (empty($shop)) {
                //cahoyu关注及分享begin
                $sets = $_W['shopset'];
                $shop = array('name' => (empty($sets['share']['title']) ? $sets['shop']['name'] : $sets['share']['title']), 'logo' => $member['avatar'], 'desc' => $desc, 'img' => tomedia($set['img']));
                //cahoyu关注及分享end
        ...
        public function getLevels($all = true, $default = false)
        {
            ...
            global $_S;
            if ($all) {
                $levels = pdo_fetchall('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid order by commission1 asc', array(':uniacid' => $_W['uniacid']));
            } else {
                $levels = pdo_fetchall('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid and (ordermoney>0 or commissionmoney>0) order by commission1 asc', array(':uniacid' => $_W['uniacid']));
            }
            if ($default) {
        ...
        public function getLevel($openid)
        {
            ...
            $level = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['agentlevel']));
            return $level;
        }
        ...
        public function upgradeLevelByOrder($openid)
        {
            ...
            $levels = pdo_fetchall('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid order by weight desc', ['uniacid' => $_W['uniacid']]);
            if (empty($levels)) return false;
            file_put_contents(IA_ROOT . "/data/logs/commission.log", "【升级方式】 upgradeLevelByOrder                      startime=>" . date('m-d h:i:s') . "[openid]=>{$openid}\n", FILE_APPEND);
            /*异步升级*/
        ...
        public function upgradeLevelByAgent($openid)
        {
            ...
            if (empty($levels)) return false;
            /*异步升级*/
            file_put_contents(IA_ROOT . "/data/logs/commission.log", "【升级方式】 upgradeLevelByAgent                      startime=>" . date('m-d h:i:s') . "[openid]=>{$openid}\n", FILE_APPEND);
        ...
        public function upgradeLevelByCommissionOK($openid)
        {
            ...
            if (empty($levels)) return false;
            /*异步升级*/
            file_put_contents(IA_ROOT . "/data/logs/commission.log", "【升级方式】 upgradeLevelByCommissionOK                      startime=>" . date('m-d h:i:s') . "[openid]=>{$openid}\n", FILE_APPEND);
        ...
        public function upgradeLevelAsyn($openid)
        {
            file_put_contents(IA_ROOT . "/data/logs/commission.log", "upgradeLevelAsyn                            startime=>" . date('m-d h:i:s') . "[openid]=>{$openid}\n", FILE_APPEND);
            global $_W;
            $levels = pdo_fetchall('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid order by weight desc', ['uniacid' => $_W['uniacid']]);
            foreach ($levels as $level) {
                if ($level['levelcondition']==2) {
                    $result = $this->upgradeLevelByAnd($openid, $level);
                    if ($result == true) break;
                } else {
                    $result = $this->upgradeLevelByOr($openid, $level);
                    if ($result == true) break;
                }
            }
            $m = m('member')->getMember($openid);
            ...
            if (!empty($m['agentid']) && $this->upgradeLevelAsynRecursionLevel > 0) {
                --$this->upgradeLevelAsynRecursionLevel;
                $this->upgradeLevelAsyn($m['agentid']);
                file_put_contents(IA_ROOT . "/data/logs/commission.log", "-------递归上一级--------                            startime=>" . date('m-d h:i:s') . "[openid]=>{$openid}\n", FILE_APPEND);
            }
        }

        public function upgradeLevelByOr($openid, $level)
        {
            file_put_contents(IA_ROOT . "/data/logs/commission.log", "upgradeLevelByOr startime=>" . date('m-d h:i:s') . "\n", FILE_APPEND);
            $set = $this->getSet();
            global $_W;
            ...
            if (empty($oldlevel['id'])) {
                $_oldlevel = array('weight' => 0, 'levelname' => empty($set['levelname']) ? '普通等级' : $set['levelname'], 'commission1' => $set['commission1'], 'commission2' => $set['commission2'], 'commission3' => $set['commission3']);
            ...
            if (in_array('10',$leveltypes_json_toarray)) {
                ...
                $newlevel10 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid  and ' . $commissionmoney . ' >= commissionmoney and commissionmoney>0 and id = ' . $level['id'] . '  order by weight desc limit 1', array(':uniacid' => $_W['uniacid']));

                if (!empty($newlevel10) && $newlevel10['weight'] > $newlevel['weight']) {
                    $newlevel = $newlevel10;
                }
            }
            ...
            if (in_array('4',$leveltypes_json_toarray)) {
                ...
                $newlevel4 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid  and ' . $ordermoney . ' >= ordermoney2 and ordermoney2>0 and id = ' . $level['id'] . '  order by weight desc limit 1', array(':uniacid' => $_W['uniacid']));

                if (!empty($newlevel4) && $newlevel4['weight'] > $newlevel['weight']) {
                    $newlevel = $newlevel4;
                }
            ...
            if (in_array('5',$leveltypes_json_toarray)) {
                ...
                $newlevel5 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid  and ' . $ordercount . ' >= ordercount2 and ordercount2>0 and id = ' . $level['id'] . '   order by weight desc limit 1', array(':uniacid' => $_W['uniacid']));

                if (!empty($newlevel5) && $newlevel5['weight'] > $newlevel['weight']) {
                    $newlevel = $newlevel5;
                }
                ...
                if (empty($set['selfbuy']) ? $this->upgradeLevelAsynRecursionLevel < 3 : $this->upgradeLevelAsynRecursionLevel > 0) {
                    if (in_array('0',$leveltypes_json_toarray)) {
                        $ordermoney = $info['ordermoney3'];
                        $newlevel0 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid and ' . $ordermoney . ' >= ordermoney and ordermoney>0 and id = ' . $level['id'] . '  order by weight desc limit 1', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('1',$leveltypes_json_toarray)) {
                        ...
                        $newlevel1 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid and ' . $ordermoney . ' >= ordermoney1 and ordermoney1>0 and id = ' . $level['id'] . '  order by weight desc limit 1', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('2',$leveltypes_json_toarray)) {
                        ...
                        $newlevel2 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid  and ' . $ordercount . ' >= ordercount and ordercount>0 and id = ' . $level['id'] . '  order by weight desc limit 1', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('3',$leveltypes_json_toarray)) {
                        ...
                        $newlevel3 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid  and ' . $ordercount . ' >= ordercount1 and ordercount1>0 and id = ' . $level['id'] . '  order by weight desc limit 1', array(':uniacid' => $_W['uniacid']));
                        if (!empty($newlevel3) && $newlevel3['weight'] > $newlevel['weight']) {
                            $newlevel = $newlevel3;
                        }
                    }
                }
                if ($this->upgradeLevelAsynRecursionLevel > 0) {
                    if (in_array('6',$leveltypes_json_toarray)) {
                        ...
                        $newlevel6 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0 and id = ' . $level['id'] . '  order by weight desc limit 1', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('8',$leveltypes_json_toarray)) {
                        ...
                        $newlevel8 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid  and ' . $downcount8 . ' >= teamcount and teamcount>0 and id = ' . $level['id'] . '  order by weight desc limit 1', array(':uniacid' => $_W['uniacid']));
            ...
            if (in_array('7',$leveltypes_json_toarray)) {
                ...
                $newlevel7 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid  and ' . $downcount7 . ' >= downcount1 and downcount1>0 and id = ' . $level['id'] . '  order by downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
            ...
            if (in_array('9',$leveltypes_json_toarray)) {
                ...
                $newlevel9 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid  and ' . $downcount9 . ' >= teamcount1 and teamcount1>0 and id = ' . $level['id'] . '  order by downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
                if (!empty($newlevel9) && $newlevel9['weight'] > $newlevel['weight']) {
                    $newlevel = $newlevel9;
                }
            }
            if ($newlevel > $oldlevel) {
                pdo_update('wx_shop_member', array('agentlevel' => $newlevel['id']), array('id' => $m['id']));
                $this->sendMessage($m['openid'], array('nickname' => $m['nickname'], 'oldlevel' => $oldlevel, 'newlevel' => $newlevel), TM_COMMISSION_UPGRADE);
                file_put_contents(IA_ROOT . "/data/logs/commission.log", "upgradeLevelByOr success  endtime=>" . date('h:i:s') . "\n", FILE_APPEND);
                return true;
            }
            file_put_contents(IA_ROOT . "/data/logs/commission.log", "upgradeLevelByOr  endtime=>" . date('h:i:s') . "\n", FILE_APPEND);
        }

        public function upgradeLevelByAnd($openid, $level)
        {
            file_put_contents(IA_ROOT . "/data/logs/commission.log", "upgradeLevelByAnd  starttime=>" . date('h:i:s') . "\n", FILE_APPEND);
            ...
            if (in_array('5',$leveltypes_json_toarray)) {
                ...
                if (empty($set['selfbuy']) ? true : $this->upgradeLevelAsynRecursionLevel > 0) {
                    file_put_contents(IA_ROOT . "/data/logs/commission.log", "oldlevel is {$oldlevel['weight']}  starttime=>" . date('m-d h:i:s') . "\n", FILE_APPEND);
                    if (in_array('0',$leveltypes_json_toarray)) {
                        $ordermoney = $info['ordermoney3'];
                        $newlevel0 = pdo_fetch('select * from ' . tablename('wx_shop_commission_level') . ' where uniacid=:uniacid and ' . $ordermoney . ' >= ordermoney and ordermoney>0 and id = ' . $level['id'] . '  order by weight desc limit 1', array(':uniacid' => $_W['uniacid']));
                    ...
                    if (in_array('1',$leveltypes_json_toarray)) {
                        ...
                        file_put_contents(IA_ROOT . "/data/logs/commission.log", "newlevel1 is {$newlevel1['weight']}  starttime=>" . date('m-d h:i:s') . "\n", FILE_APPEND);
            ...
            file_put_contents(IA_ROOT . "/data/logs/commission.log", "++++++++++++++++++++n: {$newlevel['weight']}   o:{$oldlevel['weight']}  endtime=>" . date('h:i:s') . "\n", FILE_APPEND);

            if ($newlevel['weight'] > $oldlevel['weight']) {
                pdo_update('wx_shop_member', array('agentlevel' => $newlevel['id']), array('id' => $m['id']));
                $this->sendMessage($m['openid'], array('nickname' => $m['nickname'], 'oldlevel' => $oldlevel, 'newlevel' => $newlevel), TM_COMMISSION_UPGRADE);
                file_put_contents(IA_ROOT . "/data/logs/commission.log", "upgradeLevelByAnd success!                            startime=>" . date('m-d h:i:s') . "[openid]=>{$openid}\n", FILE_APPEND);

                return true;
            }
            file_put_contents(IA_ROOT . "/data/logs/commission.log", "upgradeLevelByAnd  endtime=>" . date('h:i:s') . "\n", FILE_APPEND);
        }
        ...
        private function upgradeLevelByMultiple($openid)
        {
            global $_W;
            load()->func('communication');
            ihttp_request('http://' . $_SERVER['HTTP_HOST'] . '/app/wx_shop_api.php?r=upgradeasyn&i=' . $_W['uniacid'] . '&openid=' . $openid, [], [], 0);
        }
        ...
        {
            global $_W;
            return array('total1' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_commission_apply') . ' where status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])), 'total2' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_commission_apply') . ' where status=2 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])), 'total3' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_commission_apply') . ' where status=3 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])), 'total_1' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_commission_apply') . ' where status=-1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'])));
        }
        ...
        public function getLastApply($mid, $type = -1)
        {
            ...
            $sql = 'select type,alipay,bankname,bankcard,realname from ' . tablename('wx_shop_commission_apply') . ' where mid=:mid and uniacid=:uniacid';
            if (-1 < $type) {
                $sql .= ' and type=:type';
                $params[':type'] = $type;
            }
        ...
        public function getRepurchase($openid, array $time)
        {
            ...
            foreach ($time as $value) {
                ...
                if (!(empty($repurchase_price))) {
                    $order_price = (double)pdo_fetchcolumn('SELECT SUM(price) as price FROM ' . tablename('wx_shop_order') . ' WHERE `uniacid`=:uniacid AND `openid`=:openid AND `status`>2 AND `createtime` BETWEEN :time1 AND :time2', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':time1' => $time1, ':time2' => $time2));
                    $year_month = explode('-', $value);
                    $year_month[0] = (int)$year_month[0];
                    $year_month[1] = (int)$year_month[1];
                    $residue_price = (double)pdo_fetchcolumn('SELECT SUM(repurchase) FROM ' . tablename('wx_shop_commission_repurchase') . ' WHERE `uniacid`=:uniacid AND `openid`=:openid AND `year`=:year AND `month`=:month', array(':uniacid' => $_W['uniacid'], ':openid' => $openid, ':year' => $year_month[0], ':month' => $year_month[1]));
        ...
        public function getAllDown($openid)
        {
            ...
            $uid = (int)$openid;
            if ($uid == 0) {
                $info = pdo_fetch('select id,openid,uniacid,agentid,isagent,status from ' . tablename('wx_shop_member') . ' where  openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                if (empty($info)) {
                    return false;
                }
                $uid = $info['id'];
            }
            $agents = pdo_fetchall('select id,openid,uniacid,agentid,isagent,nickname,agenttime,createtime,avatar,status,realname,mobile,weixin from ' . tablename('wx_shop_member') . ' where uniacid=:uniacid and agentid=:agentid', array(':uniacid' => $_W['uniacid'], ':agentid' => $uid));
            $ids = array();
            $openids = array();
            $users = array();
        ...
        public function getAllDownOrder($openid, $start = 0, $end = 0)
        {
            ...
            if (!(empty($down['openids']))) {
                $order = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_order') . ' WHERE uniacid=:uniacid AND openid IN (\'' . implode('\',\'', $down['openids']) . '\') AND createtime BETWEEN :time1 AND :time2 AND ccard>0', array(':uniacid' => $_W['uniacid'], ':time1' => $start, ':time2' => $end));
                if ($order) {
