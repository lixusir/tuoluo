...
class User_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$searchstart = (!empty($_GPC['searchstart']) &&  intval($_GPC['searchstart']) == 0)? 0:1;
		$pindex = max(1, intval($_GPC['page']));
		$psize = 100;
		$sql = 'select dm.id,dm.diycommissionfields,dm.diycommissionfields,dm.diymemberfields,dm.diymemberdata,dm.nickname,dm.realname,dm.avatar,l.levelname from ' . tablename('wx_shop_member') . ' dm ' . ' left join ' . tablename('wx_shop_member') . ' lm on lm.agentid = dm.id' . ' left join ' . tablename('wx_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isagent =1 and lm.isagent =1 and dm.status=1 and lm.status=1  group by dm.id ORDER BY dm.agenttime desc';
		$sql .= ' LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		if (!empty($searchstart)) {
			$userlist = pdo_fetchall($sql, $params);
			$total_sql = 'select COUNT(1) from ' . tablename('wx_shop_member') . ' dm ' . ' left join ' . tablename('wx_shop_member') . ' lm on lm.agentid = dm.id' . ' left join ' . tablename('wx_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isagent =1 and lm.isagent =1 and dm.status=1 and lm.status=1';
		...
		if (!empty($userlist)) {
			...
			foreach ($userlist as $k => $v) {
				...
				if (!empty($kwd)) {
					$condition .= " AND ( dm.nickname LIKE '%".$kwd."%' or dm.realname LIKE '%".$kwd."%' or dm.mobile LIKE '%".$kwd."%' )";
				}
				if (!empty($_GPC['mid'])) {
				...
				if ($hasagent) {
					$child_sql = 'select dm.* from ' . tablename('wx_shop_member') . ' dm ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid  and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' ' . $condition . '   ORDER BY dm.agenttime desc';
