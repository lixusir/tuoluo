...
class Rank_WxShopPage extends PluginWebPage
{
	...
	{
		global $_W;
		$result = pdo_fetchall('SELECT openid FROM ' . tablename('wx_shop_member') . ' WHERE uniacid = :uniacid AND status=1 AND agentblack=0', array(':uniacid' => $_W['uniacid']));
		...
		if (!empty($openid)) {
			if (is_array($openid)) {
				foreach ($openid as $val) {
					...
					pdo_update('wx_shop_member', $data, array('openid' => $val, 'uniacid' => $_W['uniacid']));
				}
