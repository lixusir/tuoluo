...

{
	...
	'commission' => array('tm' => $data)
		...
		$template_list = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_message_template') . ' WHERE uniacid=:uniacid and typecode=:typecode ', array(':uniacid' => $_W['uniacid'], ':typecode' => 'commission'));
...

	{
		...
		if ($_W['ispost']) {
			...
			if($_GPC['levelcondition']=='1') $data['leveltype']=implode('_',$_GPC['leveltypes']);

			elseif($_GPC['levelcondition']=='2') $data['leveltype']=implode('&',$_GPC['leveltypes']);
			...
			$data['k_num'] = strtotime($data['k_num']);
