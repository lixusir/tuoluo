...
class Sign_WxShopPage extends PluginWebPage
{
	...
	{
		if (cv('commission/sign.rule')) {
			header('Location: ' . webUrl('commission/sign/rule'));
		}
		else if (cv('commission/sign.records')) {
			header('Location: ' . webUrl('commission/sign/records'));
		}
		else if (cv('commission/sign.set')) {
			header('Location: ' . webUrl('commission/sign/set'));
		}
		else {
	...
	public function rule()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($set)) {
				...
				pdo_insert('wx_shop_sign_set', $data);
			}
			else {
				pdo_update('wx_shop_sign_set', $data, array('id' => $set['id'], 'uniacid' => $_W['uniacid']));
			}
	...
	public function records()
	{
		...
		$psize = 20;
		$list = pdo_fetchall('select r.*, m.nickname, m.id as mid, m.avatar from ' . tablename('wx_shop_sign_records') . 'r left join ' . tablename('wx_shop_member') . 'm on r.openid=m.openid ' . $condition . ' order by r.time desc, r.id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_sign_records') . 'r left join ' . tablename('wx_shop_member') . 'm on r.openid=m.openid ' . $condition);
	...
	public function set()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'wx_shop:sign') {
					show_json(0, '关键字已存在!');
				}
			}

			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'wx_shop:sign'));
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:sign', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			...
			$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'wx_shop', 'title' => $data['title'], 'description' => $data['desc'], 'thumb' => $data['thumb'], 'url' => mobileUrl('sign'));
