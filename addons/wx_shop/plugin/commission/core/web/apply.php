...
class Apply_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select a.*, m.nickname,m.avatar,m.realname,m.mobile,m.agentlevel,l.levelname,l.levelname,a.realname as applyrealname,a.sendmoney from ' . tablename('wx_shop_commission_apply') . ' a ' . ' left join ' . tablename('wx_shop_member') . ' m on m.id = a.mid' . ' left join ' . tablename('wx_shop_commission_level') . ' l on l.id = m.agentlevel' . ' where 1 ' . $condition . ' ORDER BY ' . $orderby . ' desc ';
		...
		if ($status == 3) {
			$realmoney_total = (double) pdo_fetchcolumn('select sum(a.realmoney) from ' . tablename('wx_shop_commission_apply') . ' a ' . ' left join ' . tablename('wx_shop_member') . ' m on m.id = a.mid' . ' left join ' . tablename('wx_shop_commission_level') . ' l on l.id = m.agentlevel' . ' where 1 ' . $condition, $params);
		}
		...
		if ($_GPC['export'] == '1') {
			...
			foreach ($list as &$row) {
				...
				$orders = pdo_fetchall('select id,agentid, ordersn,price,goodsprice,dispatchprice,createtime, paytype from ' . tablename('wx_shop_order') . ' where  id in ( ' . implode(',', $ids) . ' );');
				$totalcommission = 0;
				$totalpay = 0;
				$passmoney = 0;
				$ordergoodscount = 0;
		...
		$total = pdo_fetchcolumn('select count(a.id) from' . tablename('wx_shop_commission_apply') . ' a ' . ' left join ' . tablename('wx_shop_member') . ' m on m.uid = a.mid' . ' left join ' . tablename('wx_shop_commission_level') . ' l on l.id = m.agentlevel' . ' where 1 ' . $condition, $params);
	...
	protected function applyData()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$apply = pdo_fetch('select * from ' . tablename('wx_shop_commission_apply') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		$list = pdo_fetchall('select id,agentid, ordersn,price,goodsprice, dispatchprice,createtime, paytype from ' . tablename('wx_shop_order') . ' where  id in ( ' . implode(',', $ids) . ' );');
		$totalcommission = 0;
		$totalpay = 0;
		...
		unset($row);
		$totalcount = $total = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_member') . ' m on o.openid = m.openid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on a.id = o.addressid ' . ' where o.id in ( ' . implode(',', $ids) . ' );');
		$set_array = array();
	...
	public function check()
	{
		...
		foreach ($list as $row) {
			$goods = pdo_fetchall('SELECT id from ' . tablename('wx_shop_order_goods') . ' where uniacid = :uniacid and orderid=:orderid and nocommission=0', array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
		...
		foreach ($ogids as $ogid) {
			$g = pdo_fetch('SELECT total, commission1,commission2,commission3,commissions from ' . tablename('wx_shop_order_goods') . '  ' . 'where id=:id and uniacid = :uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $ogid));
			...
			if (!empty($update)) {
				pdo_update('wx_shop_order_goods', $update, array('id' => $ogid));
			}
		}

		if ($isAllUncheck) {
			pdo_update('wx_shop_commission_apply', array('status' => -1, 'invalidtime' => $time), array('id' => $id, 'uniacid' => $_W['uniacid']));
		}
		else {
			pdo_update('wx_shop_commission_apply', array('status' => 2, 'checktime' => $time), array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function cancel()
	{
		...
		foreach ($list as $row) {
			...
			foreach ($row['goods'] as $g) {
				...
				if (!empty($update)) {
					pdo_update('wx_shop_order_goods', $update, array('id' => $g['id']));
				}
			}
		}

		pdo_update('wx_shop_commission_apply', array('status' => 1, 'checktime' => 0, 'invalidtime' => 0), array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function refuse()
	{
		...
		pdo_update('wx_shop_commission_apply', array('status' => -2, 'checktime' => 0, 'invalidtime' => 0, 'refusetime' => time()), array('id' => $id, 'uniacid' => $_W['uniacid']));
	...
	public function pay($params = array(), $mine = array())
	{
		...
		if ($apply['type'] < 2) {
			...
			if (!empty($data['paytype']['commission']) && ($apply['type'] == 1)) {
				...
				pdo_update('wx_shop_commission_apply', array('sendmoney' => $result['sendmoney'], 'senddata' => json_encode($result['senddata'])), array('id' => $apply['id']));
		...
		pdo_update('wx_shop_commission_apply', array('status' => 3, 'paytime' => $time, 'commission_pay' => $totalpay, 'realmoney' => $realmoney, 'deductionmoney' => $deductionmoney), array('id' => $id, 'uniacid' => $_W['uniacid']));
		...
		pdo_insert('wx_shop_commission_log', $log);
        ...
        //代理分红修改begin
        if (p('weightbonus')) {
            p('weightbonus')->upgradeLevelByCommissionOK($member['openid']);
        }
        //代理分红修改end

        //区域分红修改begin
        if (p('abonus')) {
            p('abonus')->upgradeLevelByCommissionOK($member['openid']);
        }
        //区域分红修改end
	...
	public function changecommission()
	{
		...
		$id = intval($_GPC['id']);
		$set = $this->set;
		$order = pdo_fetch('select * from ' . tablename('wx_shop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			foreach ($order_goods_change as $og) {
				...
				pdo_update('wx_shop_order_goods', array('commissions' => iserializer($commissions)), array('id' => $og['id']));
			}
