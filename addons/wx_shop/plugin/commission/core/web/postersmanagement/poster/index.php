...
class Index_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_poster') . ' WHERE 1 ' . $condition . ' ORDER BY isdefault desc,createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);

		foreach ($list as &$row) {
			$row['times'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_poster_scan') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
			$row['follows'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_poster_log') . ' where posterid=:posterid and uniacid=:uniacid', array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
		}

		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_poster') . ' where 1 ' . $condition . ' ', $params);
	...
	protected function post()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_poster') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (($item['keyword2'] != $data['keyword2']) && !empty($keyword)) {
				if ($keyword['name'] != ('wx_shop:poster:' . $data['type'])) {
					show_json(0, '关键字已存在!');
				}
			}
			...
			if ($data['isdefault'] == 1) {
				pdo_update('wx_shop_poster', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $data['type']));
			}

			if (!empty($id)) {
				pdo_update('wx_shop_poster', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				pdo_insert('wx_shop_poster', $data);
				$id = pdo_insertid();
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':name' => 'wx_shop:poster:' . $data['type']));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:poster:' . $data['type'], 'module' => 'wx_shop', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'wx_shop', 'content' => $data['type'] == 3 ? '^' . trim($data['keyword2']) . '\\+*[0-9]{1,}$' : trim($data['keyword2']), 'type' => $data['type'] == 3 ? 3 : 1, 'displayorder' => 0, 'status' => 1);
			...
			$ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':name' => 'wx_shop:poster:auto'));

			if (empty($ruleauto)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:poster:auto', 'module' => 'wx_shop', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'wx_shop', 'content' => 'WX_SHOP_POSTER', 'type' => 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}

			show_json(1, array('url' => webUrl('commission/postersmanagement/poster/edit', array('id' => $id, 'tab' => str_replace('#tab_', '', $_GPC['tab'])))));
		}
	...
	public function delete()
	{
		...
		$posters = pdo_fetchall('SELECT id,title,keyword,keyword2 FROM ' . tablename('wx_shop_poster') . ' WHERE id in ( ' . $id . ' ) and uniacid=' . $_W['uniacid']);
		...
		foreach ($posters as $poster) {
			...
			pdo_delete('wx_shop_poster', array('id' => $poster['id'], 'uniacid' => $_W['uniacid']));
			pdo_delete('wx_shop_poster_log', array('posterid' => $poster['id'], 'uniacid' => $_W['uniacid']));
	...
	public function clear()
	{
		...
		global $_GPC;
		load()->func('file');
		@rmdirs(IA_ROOT . '/addons/wx_shop/data/poster/' . $_W['uniacid']);
		@rmdirs(IA_ROOT . '/addons/wx_shop/data/qrcode/' . $_W['uniacid']);
		$acid = pdo_fetchcolumn('SELECT acid FROM ' . tablename('account_wechats') . ' WHERE `uniacid`=:uniacid LIMIT 1', array(':uniacid' => $_W['uniacid']));
		pdo_update('wx_shop_poster_qr', array('mediaid' => ''), array('acid' => $acid));
		plog('poster.clear', '清除海报缓存');
		show_json(1, array('url' => webUrl('commission/postersmanagement/poster')));
	}
	...
	public function setdefault()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$poster = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_poster') . ' WHERE id = \'' . $id . '\'');

		if (empty($poster)) {
			show_json(0, '抱歉，海报不存在或是已经被删除！');
		}

		pdo_update('wx_shop_poster', array('isdefault' => 0), array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $poster['type']));
		pdo_update('wx_shop_poster', array('isdefault' => 1), array('uniacid' => $_W['uniacid'], 'id' => $poster['id']));
