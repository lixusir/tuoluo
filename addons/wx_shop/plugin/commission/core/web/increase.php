...
class Increase_WxShopPage extends PluginWebPage
{
       ...
       // var_dump($years);
		...
		if (!empty($days)) {
			...
			while (0 <= $i) {
				...
				$datas[] = array('date' => $time, 'acount' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where isagent=1 and status=1  ' . $condition, $params));
				--$i;
			}
		}
		else {
			if (!empty($year) && !empty($month)) {
				...
				while ($d <= $lastday) {
					...
					$datas[] = array('date' => $d . '日', 'acount' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where isagent=1  ' . $condition, $params));
					++$d;
				}
			}
			else {
				if (!empty($year)) {
					...
					foreach ($months as $m) {
						...
						$datas[] = array('date' => $m['data'] . '月', 'acount' => pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where isagent=1  ' . $condition, $params));
					}
				}
			}
		}
