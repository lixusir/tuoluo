...
class Agent_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$list = pdo_fetchall($sql, $params);
		 // var_dump($list);
		$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('wx_shop_member') . ' dm  ' . ' left join ' . tablename('wx_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid' . ' where dm.uniacid =' . $_W['uniacid'] . ' and dm.isagent =1 ' . $condition, $params);
        // var_dump($level);
		foreach ($list as &$row) {
	...
	public function export(){
		...
		// echo "<pre>";
		// 	print_r($member['level1_agentids']);
		// echo "</pre>";


		// echo "<pre>";
		// 	print_r(count($member['level1_agentids']));
		// echo "</pre>";
		// exit;
		// echo "<pre>";
		// 	print_r($condition);
		...
		// echo "</pre>";
		// echo "<pre>";
		// 	print_r($member['level2_agentids']);
		// echo "</pre>";
		// exit;
		// echo "<pre>";
		// 	print_r($nubs);
		...
		$sql = 'select dm.*,p.nickname as parentname,p.avatar as parentavatar,l.levelname as levelname  from ' . tablename('wx_shop_member') . ' dm ' . ' left join ' . tablename('wx_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('wx_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' where dm.uniacid = ' . $_W['uniacid'] . '  ' . $condition . '   ORDER BY dm.agenttime desc';
	...
	public function delete()
	{
		...
		foreach ($members as $member) {
			pdo_update('wx_shop_member', array('isagent' => 0, 'status' => 0), array('id' => $member['id']));
	...
	public function user()
	{
		...
		$level11 = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where isagent=0 and agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $agentid, ':uniacid' => $_W['uniacid']));
		$condition = '';
		$params = array();
		$hasagent = false;
		...
		if ($hasagent) {
			$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('wx_shop_member') . ' dm ' . ' left join ' . tablename('wx_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid' . ' where dm.uniacid =' . $_W['uniacid'] . '  ' . $condition, $params);
			$sql = 'select dm.*,p.nickname as parentname,p.avatar as parentavatar,l.levelname as levelname  from ' . tablename('wx_shop_member') . ' dm ' . ' left join ' . tablename('wx_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('wx_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid  and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . '  ' . $condition . '   ORDER BY dm.agenttime desc';
	...
	public function check()
	{
		...
		$members = pdo_fetchall('SELECT id,openid,agentid,nickname,realname,mobile,status FROM ' . tablename('wx_shop_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$time = time();
		...
		foreach ($members as $member) {
			...
			if ($status == 1) {
				pdo_update('wx_shop_member', array('status' => 1, 'agenttime' => $time), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
				...
				pdo_update('wx_shop_member', array('status' => 0, 'agenttime' => 0), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
	...
	public function agentblack()
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,agentblack FROM ' . tablename('wx_shop_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		...
		foreach ($members as $member) {
			...
			if ($agentblack == 1) {
				pdo_update('wx_shop_member', array('isagent' => 1, 'status' => 0, 'agentblack' => 1), array('id' => $member['id']));
				plog('commission.agent.agentblack', '设置黑名单 <br/>分销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else {
				pdo_update('wx_shop_member', array('isagent' => 1, 'status' => 1, 'agentblack' => 0), array('id' => $member['id']));
