...
class Order_WxShopPage extends CommissionMobileLoginPage
{
	...
	public function get_list()
	{
		...
		if (1 <= $level) {
			$level1_memberids = pdo_fetchall('select id from ' . tablename('wx_shop_member') . ' where uniacid=:uniacid and agentid=:agentid', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']), 'id');
			$level1_orders = pdo_fetchall('select commission1,o.id,o.createtime,o.price,og.commissions from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on og.orderid=o.id ' . ' where o.uniacid=:uniacid and o.agentid=:agentid ' . $condition . ' and og.status1>=0 and og.nocommission=0', array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
		...
		if (2 <= $level) {
			if (0 < $level1) {
				$level2_orders = pdo_fetchall('select commission2 ,o.id,o.createtime,o.price,og.commissions   from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on og.orderid=o.id ' . ' where o.uniacid=:uniacid and o.agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ')  ' . $condition . '  and og.status2>=0 and og.nocommission=0 ', array(':uniacid' => $_W['uniacid']));
		...
		if (3 <= $level) {
			if (0 < $level2) {
				$level3_orders = pdo_fetchall('select commission3 ,o.id,o.createtime,o.price,og.commissions  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join  ' . tablename('wx_shop_order') . ' o on og.orderid=o.id ' . ' where o.uniacid=:uniacid and o.agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ')  ' . $condition . ' and og.status3>=0 and og.nocommission=0', array(':uniacid' => $_W['uniacid']));
		...
		if (!empty($orderids)) {
			$list = pdo_fetchall('select id,ordersn,openid,createtime,status from ' . tablename('wx_shop_order') . '  where uniacid =' . $_W['uniacid'] . ' and id in ( ' . implode(',', array_keys($orderids)) . ') order by id desc');
