...
class Index_WxShopPage extends PluginMobilePage
{
	public function main()
	{
		...
		if (empty($shop['selectgoods'])) {
			$goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and status=1 and deleted=0', array(':uniacid' => $_W['uniacid']));
		}
		else {
		...
		$banners = pdo_fetchall('select id,bannername,link,thumb from ' . tablename('wx_shop_banner') . ' where uniacid=:uniacid and enabled=1 and iswxapp=0 order by displayorder desc', array(':uniacid' => $uniacid));
		...
		if (!empty($_W['shopset']['shop']['indexrecommands'])) {
			...
			if (!empty($goodids)) {
				$indexrecommands = pdo_fetchall('select id, title, thumb, marketprice, productprice ,minprice, total from ' . tablename('wx_shop_goods') . ' where id in( ' . $goodids . ' ) and uniacid=:uniacid and status=1 order by displayorder desc', array(':uniacid' => $uniacid));
			}
		}

		$goodsstyle = $_W['shopset']['shop']['goodsstyle'];
		$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('wx_shop_notice') . ' where uniacid=:uniacid and status=1 order by displayorder desc limit 5', array(':uniacid' => $uniacid));
		$shareid = $mid;
		if (($member['isagent'] == 1) && ($member['status'] == 1)) {
			$shareid = $member['id'];
		}
	...
	public function set()
	{
		...
		if ($_W['ispost']) {
			...
			if (empty($shop['id'])) {
				pdo_insert('wx_shop_commission_shop', $shopdata);
			}
			else {
				pdo_update('wx_shop_commission_shop', $shopdata, array('id' => $shop['id']));
			}
	...
	public function select()
	{
		...
		if (!empty($shop['selectgoods'])) {
			...
			if (!empty($goodsids)) {
				$goods = pdo_fetchall('select id,title,marketprice,thumb from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and id in ( ' . trim($shop['goodsids']) . ')', array(':uniacid' => $_W['uniacid']));
