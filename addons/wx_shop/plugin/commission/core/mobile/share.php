...
require WX_SHOP_PLUGIN . 'commission/core/page_mobile.php';
class Share_WxShopPage extends CommissionMobilePage
{
	public function main()
	{
		...
		$qrcode = '';
		$path = $_W['siteroot'] . 'addons/wx_shop/data/poster/' . $_W['uniacid'] . '/';
		...
		if (!empty($goodsid)) {
			...
			if (!empty($goods)) {
				...
				$qrcode = $path . $md5 . '.jpg';
				if (p('poster') && !is_file(IA_ROOT . '/addons/wx_shop/data/poster/' . $_W['uniacid'] . '/' . $md5 . '.jpg')) {
					$poster = pdo_fetch('select bg,data from ' . tablename('wx_shop_poster') . ' where uniacid=:uniacid and type=3 and isdefault=1', array(':uniacid' => $_W['uniacid']));
		...
		if (!$share_goods) {
			...
			$qrcode = $path . $md5 . '.jpg';
			if (p('poster') && !is_file(IA_ROOT . '/addons/wx_shop/data/poster/' . $_W['uniacid'] . '/' . $md5 . '.jpg')) {
				$poster = pdo_fetch('select bg,`data` from ' . tablename('wx_shop_poster') . ' where uniacid=:uniacid and `type`=:type and isdefault=1 limit 1', array(':uniacid' => $_W['uniacid'], ':type' => empty($set['qrcode']) ? 2 : 4));
