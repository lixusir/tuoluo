...
class Log_WxShopPage extends CommissionMobileLoginPage
{
	...
	public function get_list()
	{
		...
		$commissioncount = pdo_fetchcolumn('select sum(commission) from ' . tablename('wx_shop_commission_apply') . ' where mid=:mid and uniacid=:uniacid and status>-1 limit 1', array(':mid' => $member['id'], ':uniacid' => $_W['uniacid']));
		$list = pdo_fetchall('select * from ' . tablename('wx_shop_commission_apply') . ' where 1 ' . $condition . ' order by id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_commission_apply') . ' where 1 ' . $condition, $params);
	...
	public function detail()
	{
		...
		$id = intval($_GPC['id']);
		$member = m('member')->getMember($_W['openid']);
		$apply = pdo_fetch('select * from ' . tablename('wx_shop_commission_apply') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(':id' => $id, ':mid' => $member['id'], ':uniacid' => $_W['uniacid']));
	...
	public function orders()
	{
		...
		$pindex = max(1, intval($_GPC['page']));
		$psize = 5;
		$apply = pdo_fetch('select orderids from ' . tablename('wx_shop_commission_apply') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(':id' => $id, ':mid' => $member['id'], ':uniacid' => $_W['uniacid']));
		...
		$list = pdo_fetchall('select o.id,o.agentid, o.ordersn,o.price,o.goodsprice, o.dispatchprice,o.createtime, o.paytype from ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_member') . ' m on o.openid = m.openid ' . ' where  o.id in ( ' . implode(',', $ids) . ' ) LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' o ' . ' left join ' . tablename('wx_shop_member') . ' m on o.openid = m.openid ' . ' where  o.id in ( ' . implode(',', $ids) . ' ) ');
		$totalcommission = 0;
		$totalpay = 0;
