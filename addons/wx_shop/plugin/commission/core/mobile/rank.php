...
class Rank_WxShopPage extends CommissionMobileLoginPage

{
...

	{
		...
		case '0':
			...
			$user = pdo_fetch('SELECT id,uid,credit1,nickname,avatar,commission_total FROM ' . tablename('wx_shop_member') . ' WHERE uniacid = :uniacid AND openid = :openid', array(':uniacid' => $_W['uniacid'], ':openid' => $_W['mopenid']));

			$user['paiming'] = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_member') . ' WHERE uniacid = :uniacid AND commission_total >= :commission_total', array(':uniacid' => $_W['uniacid'], ':commission_total' => $user['commission_total']));
...

	{
		...
		$result = pdo_fetchall('SELECT id,uid,nickname,avatar,commission_total FROM ' . tablename('wx_shop_member') . ' WHERE uniacid = :uniacid ORDER BY commission_total DESC ' . $limit, array(':uniacid' => $_W['uniacid']));
