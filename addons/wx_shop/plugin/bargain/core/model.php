...
class BargainModel extends PluginModel
{
	public function checkFollowed()
	{
		...
		$sets = pdo_fetchcolumn('SELECT `sets` FROM ' . tablename('wx_shop_sysset') . ' WHERE uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
		$qr = unserialize($sets);
	...
	{
		global $_W;
		$sql = 'SELECT COUNT(*) FROM ' . tablename('wx_shop_bargain_actor') . ' WHERE openid = :openid AND account_id = :uniacid AND goods_id = :goods_id';
