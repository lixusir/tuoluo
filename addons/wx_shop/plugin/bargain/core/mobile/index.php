...
class Index_WxShopPage extends PluginMobileLoginPage
{
	...
	public function purchase()
	{
		...
		$mid = m('member')->getMid();
		$_W['shopshare'] = array('title' => $share['title'], 'desc' => $share['content'], 'link' => mobileUrl('bargain', array('mid' => $mid), true), 'imgUrl' => $share['logo']);
		$act = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_bargain_actor') . ' WHERE openid= :openid AND account_id = :account_id AND status = \'1\' ORDER BY id DESC', array(':openid' => $_W['openid'], ':account_id' => $_W['uniacid']));
		$i = 0;
	...
	public function act()
	{
		...
		$mid = m('member')->getMid();
		$_W['shopshare'] = array('title' => $share['title'], 'desc' => $share['content'], 'link' => mobileUrl('bargain', array('mid' => $mid), true), 'imgUrl' => $share['logo']);
		$act = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_bargain_actor') . ' WHERE openid= :openid AND account_id = :account_id AND status = \'0\' ORDER BY id DESC', array(':openid' => $_W['openid'], ':account_id' => $_W['uniacid']));
		$i = 0;
	...
	public function detail()
	{
		...
		$total = pdo_fetchcolumn('select count(*) from '.tablename('wx_shop_bargain_actor').' where goods_id=:goodsid and account_id=:account_id', array(':goodsid' => $res['id'], 'account_id' => $_W['uniacid']));
		$totalNumber = false;
		if ($total >= $res['total_time']) {
			$totalNumber = true;
		}
		if (!empty($res['initiate'])) {
			$act_id = pdo_get('wx_shop_bargain_actor', array('goods_id' => $id, 'openid' => $_W['openid'], 'status' => 0), 'id');
			if (!empty($act_id)) {
				$act_swi = $act_id;
			}
		}
	...
	public function bargain()
	{
		...
		if ($ajax == 151) {
			...
			$account_set['partin'] *= -1;
			$myself_swi = 0;
			$myself_count = pdo_fetchall('select id from '.tablename('wx_shop_bargain_record').' where openid=:openid and actor_id=:actor_id', array('openid' => $_W['openid'], 'actor_id' => $res['id']));

			// if (empty($myself_count['id']) && count($myself_count) >= $res2['each_time'] && (0 < $res2['myself'])) {
			// 	$myself_swi = 1;
			// }
			if( $res2['each_time'] > count($myself_count) && $res2['myself'] == '1'){
				$myself_swi = 1;
			}
			// var_dump($trade_swi, $myself_swi);die;
			include $this->template();
		}
	}
	...
	public function cut($actor_id, $time3, $min_price, $each_time, $total_time, $max_price, $probability_json)
	{
		...
		$min *= 100;
		$max *= 100;
		$record_res = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_bargain_record') . ' WHERE actor_id=:actor_id AND openid= :openid', array(':actor_id' => $actor_id, ':openid' => $_W['openid']));
