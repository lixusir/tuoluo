...
class Sys_WxShopPage extends PluginWebPage
{
	...
	protected function post($do)
	{
		...
		$diyadvs = pdo_fetchall('select id, `name` from ' . tablename('wx_shop_diypage_plu') . ' where merch=:merch and `type`=1 and status=1 and uniacid=:uniacid  order by id desc', array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid']));
	...
	{
		global $_GPC,$_W;
        if($_GPC['use']==1) pdo_update('wx_shop_diypage',['is_use'=>0],['type'=>$_GPC['type'],'uniacid'=>$_W['uniacid']]);
        pdo_update('wx_shop_diypage',['is_use'=>$_GPC['use']],['id'=>$_GPC['id'],'uniacid'=>$_W['uniacid']]);
		exit();
	}
}
