...
class Index_WxShopPage extends PluginMobilePage
{
	...
	public function getmerch()
	{
		...
		if ($_W['ispost']) {
			...
			$merchs = pdo_fetchall('select id, merchname as `name`, logo as thumb, status, `desc`, address, tel, lng, lat from ' . tablename('wx_shop_merch_user') . ' where 1 ' . $condition . ' order by ' . $orderby . $limitsql, $params);
