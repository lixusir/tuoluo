...
class Set_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'wx_shop:qa') {
					show_json(0, '关键字已存在！');
				}
			}

			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'wx_shop:qa'));
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:qa', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			...
			if (!empty($set)) {
				pdo_update('wx_shop_qa_set', $data, array('id' => $set['id']));
			}
			else {
				$data['uniacid'] = $_W['uniacid'];
				pdo_insert('wx_shop_qa_set', $data);
				$id = pdo_insertid();
			}
