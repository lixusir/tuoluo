...
class Index_WxShopPage extends PluginMobilePage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$category = pdo_fetchall('select * from ' . tablename('wx_shop_qa_category') . ' where isrecommand=1 and enabled=1 and uniacid=:uniacid order by displayorder desc limit 8 ', array(':uniacid' => $_W['uniacid']));
		$advs = pdo_fetchall('select id,advname,link,thumb from ' . tablename('wx_shop_qa_adv') . ' where uniacid=:uniacid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid']));
	...
	public function getlist()
	{
		...
		$sql = 'SELECT q.*, c.name as catename FROM ' . tablename('wx_shop_qa_question') . ' q left join' . tablename('wx_shop_qa_category') . ' `c` on c.id=q.cate and c.uniacid=q.uniacid where  1 and ' . $condition . ' ORDER BY q.displayorder DESC,q.id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_qa_question') . ' q left join' . tablename('wx_shop_qa_category') . ' c on c.id=q.cate and c.uniacid=q.uniacid where  1 and ' . $condition . ' ', $params);
	...
	public function detail()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('select * from ' . tablename('wx_shop_qa_question') . ' where id=:id and status=1 and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function question()
	{
		...
		if (!empty($cate)) {
			$category = pdo_fetch('select * from ' . tablename('wx_shop_qa_category') . ' where id=:id and enabled=1 and uniacid=:uniacid limit 1 ', array(':id' => $cate, ':uniacid' => $_W['uniacid']));
	...
	public function category()
	{
		global $_W;
		global $_GPC;
		$category = pdo_fetchall('select * from ' . tablename('wx_shop_qa_category') . ' where enabled=1 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
