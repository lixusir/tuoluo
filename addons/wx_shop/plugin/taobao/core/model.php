...
class TaobaoModel extends PluginModel
{
	...
	{
		global $_W;
		$g = pdo_fetch('select * from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and merchid=:merchid and catch_id=:catch_id and catch_source=\'taobao\' limit 1', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid, ':catch_id' => $itemid));
...
/* [31m * TODO SEPARATE[0m */
	...
	public function get_item_jingdong($itemid = '', $jingdongurl = '', $cates = '', $merchid = 0)
	{
		error_reporting(0);
		global $_W;
		$g = pdo_fetch('select * from ' . tablename('wx_shop_goods') . ' where uniacid=:uniacid and merchid=:merchid and catch_id=:catch_id and catch_source=\'jingdong\' limit 1', array(':uniacid' => $_W['uniacid'], ':catch_id' => $itemid, ':merchid' => $merchid));
...
/* [31m * TODO SEPARATE[0m */
	...
	public function save_taobao_goods($item = array(), $catch_url = '')
	{
		...
		$goods = pdo_fetch('select * from ' . tablename('wx_shop_goods') . ' where  catch_id=:catch_id and catch_source=\'taobao\' and uniacid=:uniacid and merchid=:merchid', array(':catch_id' => $item['itemId'], ':uniacid' => $_W['uniacid'], ':merchid' => $item['merchid']));
		...
		if (empty($goods)) {
			...
			pdo_update('wx_shop_goods', $data, array('id' => $goodsid));
		}

		$goods_params = pdo_fetchall('select * from ' . tablename('wx_shop_goods_param') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
		$params = $item['params'];
		$paramids = array();
		$displayorder = 0;

		foreach ($params as $p) {
			$oldp = pdo_fetch('select * from ' . tablename('wx_shop_goods_param') . ' where goodsid=:goodsid and title=:title limit 1', array(':goodsid' => $goodsid, ':title' => $p['title']));
			$paramid = 0;
			$d = array('uniacid' => $_W['uniacid'], 'goodsid' => $goodsid, 'title' => $p['title'], 'value' => $p['value'], 'displayorder' => $displayorder);

			if (empty($oldp)) {
				pdo_insert('wx_shop_goods_param', $d);
				$paramid = pdo_insertid();
			}
			else {
				pdo_update('wx_shop_goods_param', $d, array('id' => $oldp['id']));
				$paramid = $oldp['id'];
			}
		...
		if (0 < count($paramids)) {
			pdo_query('delete from ' . tablename('wx_shop_goods_param') . ' where goodsid=:goodsid and id not in (' . implode(',', $paramids) . ')', array(':goodsid' => $goodsid));
		}
		else {
			pdo_query('delete from ' . tablename('wx_shop_goods_param') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
		}
		...
		foreach ($specs as $spec) {
			$oldspec = pdo_fetch('select * from ' . tablename('wx_shop_goods_spec') . ' where goodsid=:goodsid and propId=:propId limit 1', array(':goodsid' => $goodsid, ':propId' => $spec['propId']));
			$specid = 0;
			$d_spec = array('uniacid' => $_W['uniacid'], 'goodsid' => $goodsid, 'title' => $spec['title'], 'displayorder' => $displayorder, 'propId' => $spec['propId']);

			if (empty($oldspec)) {
				pdo_insert('wx_shop_goods_spec', $d_spec);
				$specid = pdo_insertid();
			}
			else {
				pdo_update('wx_shop_goods_spec', $d_spec, array('id' => $oldspec['id']));
				$specid = $oldspec['id'];
			}
			...
			foreach ($spec_items as $spec_item) {
				...
				$oldspecitem = pdo_fetch('select * from ' . tablename('wx_shop_goods_spec_item') . ' where specid=:specid and valueId=:valueId limit 1', array(':specid' => $specid, ':valueId' => $spec_item['valueId']));
				$spec_item_id = 0;

				if (empty($oldspecitem)) {
					pdo_insert('wx_shop_goods_spec_item', $d);
					$spec_item_id = pdo_insertid();
				}
				else {
					pdo_update('wx_shop_goods_spec_item', $d, array('id' => $oldspecitem['id']));
			...
			if (0 < count($spec_itemids)) {
				pdo_query('delete from ' . tablename('wx_shop_goods_spec_item') . ' where specid=:specid and id not in (' . implode(',', $spec_itemids) . ')', array(':specid' => $specid));
			}
			else {
				pdo_query('delete from ' . tablename('wx_shop_goods_spec_item') . ' where specid=:specid ', array(':specid' => $specid));
			}

			pdo_update('wx_shop_goods_spec', array('content' => serialize($spec_itemids)), array('id' => $oldspec['id']));
		}

		if (0 < count($specids)) {
			pdo_query('delete from ' . tablename('wx_shop_goods_spec') . ' where goodsid=:goodsid and id not in (' . implode(',', $specids) . ')', array(':goodsid' => $goodsid));
		}
		else {
			pdo_query('delete from ' . tablename('wx_shop_goods_spec') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
		}
		...
		foreach ($options as $o) {
			...
			$oldoption = pdo_fetch('select * from ' . tablename('wx_shop_goods_option') . ' where goodsid=:goodsid and skuId=:skuId limit 1', array(':goodsid' => $goodsid, ':skuId' => $o['skuId']));
			$option_id = 0;

			if (empty($oldoption)) {
				pdo_insert('wx_shop_goods_option', $do);
				$option_id = pdo_insertid();
			}
			else {
				pdo_update('wx_shop_goods_option', $do, array('id' => $oldoption['id']));
		...
		if (0 < count($optionids)) {
			pdo_query('delete from ' . tablename('wx_shop_goods_option') . ' where goodsid=:goodsid and id not in (' . implode(',', $optionids) . ')', array(':goodsid' => $goodsid));
		}
		else {
			pdo_query('delete from ' . tablename('wx_shop_goods_option') . ' where goodsid=:goodsid ', array(':goodsid' => $goodsid));
		}
		...
		pdo_update('wx_shop_goods', $d, array('id' => $goodsid));

		if ($d['hasoption']) {
			$sql = 'update ' . tablename('wx_shop_goods') . " g set\r\n            g.minprice = (select min(marketprice) from " . tablename('wx_shop_goods_option') . (' where goodsid = ' . $goodsid . " and marketprice > 0),\r\n            g.maxprice = (select max(marketprice) from ") . tablename('wx_shop_goods_option') . (' where goodsid = ' . $goodsid . ")\r\n            where g.id = " . $goodsid . ' and g.hasoption=1');
		}
		else {
			$sql = 'update ' . tablename('wx_shop_goods') . (' set minprice = marketprice,maxprice = marketprice where id = ' . $goodsid . ' and hasoption=0;');
		}
	...
	public function save_jingdong_goods($item = array(), $catch_url = '')
	{
		...
		$data['thumb_url'] = serialize($thumb_url);
		$goods = pdo_fetch('select * from ' . tablename('wx_shop_goods') . ' where  catch_id=:catch_id and catch_source=\'jingdong\' and uniacid=:uniacid and merchid=:merchid', array(':catch_id' => $item['itemId'], ':uniacid' => $_W['uniacid'], ':merchid' => $item['merchid']));
	...
	public function save_1688_goods($item = array(), $catch_url = '')
	{
		...
		$data['thumb_url'] = serialize($thumb_url);
		$goods = pdo_fetch('select * from ' . tablename('wx_shop_goods') . ' where  catch_id=:catch_id and catch_source=\'1688\' and uniacid=:uniacid and merchid=:merchid', array(':catch_id' => $item['itemId'], ':uniacid' => $_W['uniacid'], ':merchid' => $item['merchid']));
		...
		if (true) {
			...
			if ($hasoption) {
				foreach ($json1['skuProps'] as $k => $v) {
					$spec = $v['prop'];
					pdo_insert('wx_shop_goods_spec', array('uniacid' => $_W['uniacid'], 'goodsid' => $goodsid, 'title' => $spec));
					$specId = pdo_insertid();

					foreach ($v['value'] as $key => $val) {
						$thumb = $val['imageUrl'];
						$title = $val['name'];
						pdo_insert('wx_shop_goods_spec_item', array('specid' => $specId, 'title' => $val['name'], 'thumb' => empty($thumb) ? '' : $thumb, 'show' => 1));
						$specs = pdo_insertid();
				...
				foreach ($map as $k => $v) {
					...
					pdo_insert('wx_shop_goods_option', $option);
				}
			}
	...
	private function paramFormat($param, $id)
	{
		...
		foreach ($value as $key => $val) {
			pdo_insert('wx_shop_goods_param', array('goodsid' => $id, 'uniacid' => $_W['uniacid'], 'title' => $key, 'value' => $val));
		}
	}
}
