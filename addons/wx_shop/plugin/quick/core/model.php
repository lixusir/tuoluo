...
class QuickModel extends PluginModel
{
	...
	public function update($data)
	{
		...
		if (!empty($goodsids)) {
			...
			$allGoods = pdo_fetchall('SELECT id, title, subtitle, minprice, total, sales FROM' . tablename('wx_shop_goods') . ' WHERE uniacid=:uniacid AND id in(' . $goodsids . ') AND `deleted`=0 AND `status`=1', array(':uniacid' => $_W['uniacid']), 'id');
		}
		...
		if (!empty($cateids)) {
			...
			$allCates = pdo_fetchall('SELECT * FROM' . tablename('wx_shop_category') . ' WHERE uniacid=:uniacid AND id in(' . $cateids . ') AND enabled=1', array(':uniacid' => $_W['uniacid']), 'id');
		}
		...
		if (!empty($groupids)) {
			...
			$allGroups = pdo_fetchall('SELECT * FROM' . tablename('wx_shop_goods_group') . ' WHERE uniacid=:uniacid AND id in(' . $groupids . ') AND enabled=1', array(':uniacid' => $_W['uniacid']), 'id');
		}
	...
	public function mobile($data, $merchid = 0)
	{
		...
		if ($data['template'] == 0) {
			...
			if ($data['showadv'] == 1) {
				if (0 < $merchid) {
					$returnData['advs'] = pdo_fetchall('SELECT * FROM' . tablename('wx_shop_quick_adv') . 'WHERE uniacid=:uniacid AND merchid=:merchid AND enabled=1', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else {
					$returnData['advs'] = pdo_fetchall('SELECT * FROM' . tablename('wx_shop_quick_adv') . 'WHERE uniacid=:uniacid AND enabled=1', array(':uniacid' => $_W['uniacid']));
				}
			}
			else {
			...
			if ($returnData['style']['notice'] == 1) {
				...
				if (0 < $merchid) {
					$returnData['notices'] = pdo_fetchall('SELECT id, title FROM' . tablename('wx_shop_merch_notice') . 'WHERE uniacid=:uniacid AND status=1 AND merchid=:merchid LIMIT ' . $limit, array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else {
					$returnData['notices'] = pdo_fetchall('SELECT id, title FROM' . tablename('wx_shop_notice') . 'WHERE uniacid=:uniacid AND status=1 AND iswxapp=0 LIMIT ' . $limit, array(':uniacid' => $_W['uniacid']));
				}
			}
			else {
	...
	public function getQuick($id)
	{
		...
		return pdo_fetch('SELECT * FROM' . tablename('wx_shop_quick') . 'WHERE id=:id AND uniacid=:uniacid AND status=1 LIMIT 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	}
	...
	public function getPageList($merch = false)
	{
		...
		return pdo_fetchall('SELECT id, title, status FROM' . tablename('wx_shop_quick') . 'WHERE ' . $condition . ' ORDER BY createtime DESC', $params);
	}
	...
	public function getList($args)
	{
		...
		if (!empty($list) && is_array($list)) {
			foreach ($list as $i => &$g) {
				...
				if (0 < $g['usermaxbuy']) {
					$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $g['id'], ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
				...
				if (p('bargain') && !empty($g['bargain']) && empty($g['gotodetail'])) {
					$bargain = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_bargain_goods') . ' WHERE id = :id AND unix_timestamp(start_time)<' . time() . ' AND unix_timestamp(end_time)>' . time() . ' AND status = 0', array(':id' => $g['bargain']));
	...
	public function getCart($pageid, $json = true)
	{
		...
		$totalprice = 0;
		$ischeckall = true;
		$tablename = (empty($pageid) ? 'wx_shop_member_cart' : 'wx_shop_quick_cart');
