...
class Index_WxShopPage extends PluginMobileLoginPage
{
	...
	public function get_list()
	{
		...
		if ($datatype == 0) {
			...
			if ($datatype == 2) {
				...
				if (!empty($groupid)) {
					$group = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods_group') . ' WHERE id=:id and uniacid=:uniacid and merchid=:merchid and enabled=1 limit 1 ', array(':id' => $groupid, ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
	...
	public function clearCart()
	{
		...
		global $_GPC;
		$quickid = intval($_GPC['quickid']);
		$tablename = (empty($quickid) ? 'wx_shop_member_cart' : 'wx_shop_quick_cart');
	...
	public function update()
	{
		...
		$goods = pdo_fetch('select id,marketprice,diyformid,diyformtype,diyfields, isverify, `type`,merchid, cannotrefund from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsid, ':uniacid' => $_W['uniacid']));
	...
	public function submit()
	{
		...
		foreach ($list as &$g) {
			...
			if ($seckillinfo && ($seckillinfo['status'] == 0)) {
				...
				if (!empty($optionid)) {
					$option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual`,weight from ' . tablename('wx_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $g['id'], ':id' => $optionid));
