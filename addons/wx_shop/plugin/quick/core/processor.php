...
class QuickProcessor extends PluginProcessor
{
	...
	public function respond($obj = NULL)
	{
		...
		if (($msgtype == 'text') || ($event == 'click')) {
			$quick = pdo_fetch('select * from ' . tablename('wx_shop_quick') . ' where keyword=:keyword and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':keyword' => $content));
