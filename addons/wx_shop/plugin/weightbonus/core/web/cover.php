...
class Cover_WxShopPage extends PluginWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'wx_shop代理中心入口设置'));
		...
		$url = mobileUrl('weightbonus', NULL, true);
		$qrcode = m('qrcode')->createQrcode($url);

		if ($_W['ispost']) {
			ca('weightbonus.cover.edit');
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'wx_shop代理中心入口设置') {
					show_json(0, '关键字已存在!');
				}
			}
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop代理中心入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
			...
			$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => $this->modulename, 'title' => trim($data['title']), 'description' => trim($data['desc']), 'thumb' => save_media($data['thumb']), 'url' => mobileUrl('weightbonus'));
			pdo_insert('cover_reply', $cover_data);
			plog('weightbonus.cover.edit', '修改代理中心入口设置');
			show_json(1);
		}
