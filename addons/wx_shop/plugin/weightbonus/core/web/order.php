...
class Order_WxShopPage extends WebPage
{
	...
	public function detail()
	{
		...
		if(empty($_GPC['id'])){
            show_json(0, '暂无法获取该订单分红明细');
		}
        $orderid = $_GPC['id'];

        $sql = 'select * from ' . tablename('wx_shop_weightbonus_order') . '  W inner join ' . tablename('wx_shop_order') . '  O on W.orderid = O.id  where  W.uniacid=' . $_W['uniacid']. ' and O.isweightbonus=1 and O.id = :id';
		$orders = pdo_fetch($sql, array('id'=>$orderid));

		if(empty($orders) || empty($orders['weightbonus_json'])){
            show_json(0, '暂无法获取该订单分红明细');
		}

		$weights = iunserializer($orders['weightbonus_json']);
        if(empty($weights)){
            show_json(0, '该订单暂无分红明细！');
        }

        $bonusmoney = 0;
        foreach ($weights as $weight){
            $bonusmoney+=$weight['bonusmoney'];
		}

        include $this->template('weightbonus/detail');
	}
