...
class ExchangeModel extends PluginModel 
{
	...
	public function sendRedpacket($key) 
	{
		...
		if (!(is_error($result))) 
		{
			pdo_update('wx_shop_exchange_code', array('status' => 2), array('key' => $key, 'uniacid' => $_W['uniacid'], 'status' => 1));
	...
	{
		global $_W;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':name' => 'wx_shop:exchange:' . $id));
		$keyword = m('common')->keyExist($koulingstart);
		if (!(empty($keyword)) && ($keyword['name'] != 'wx_shop:exchange:' . $id))
		{
			show_json(0, '关键字已存在!');
		}
		else if (!(empty($rule))) 
		...
		{
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:exchange:' . $id, 'module' => 'wx_shop', 'displayorder' => 0, 'status' => $status);
			pdo_insert('rule', $rule_data);
			$rid = pdo_insertid();
			$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'wx_shop', 'content' => $koulingstart, 'type' => 1, 'displayorder' => 0, 'status' => $status);
	...
	public function redKeyword($key) 
	{
		...
		$key = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE uniacid = :uniacid AND koulingstart = :koulingstart AND kouling = 1', array(':koulingstart' => $key, ':uniacid' => $_W['uniacid']));
		if (empty($key)) 
		{
			return false;
		}
	...
	{
		global $_W;
		$sql = 'UPDATE ' . tablename('wx_shop_exchange_code') . ' SET repeatcount = repeatcount - 1 WHERE repeatcount >1 AND uniacid = :uniacid AND `key` = :key';
		return pdo_query($sql, array(':uniacid' => $_W['uniacid'], ':key' => $key));
	}
	public function checkRepeatExchange($key) 
	{
		global $_W;
		$logsql = 'SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_record') . ' WHERE `key` = :code AND uniacid = :uniacid AND openid = :openid';
	...
	{
		global $_W;
		$no_qrimg = pdo_fetchcolumn('select no_qrimg from ' . tablename('wx_shop_exchange_setting') . ' where uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
		if ($no_qrimg) 
		{
			return true;
		}
