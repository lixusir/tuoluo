...
class Coupon_WxShopPage extends PluginWebPage
{
	public function main() 
	{
		...
		$psize = 10;
		$pstart = $psize * ($page - 1);
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 5 AND uniacid=:uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 5 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 5 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 5 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	public function nostart() 
	{
		...
		$psize = 10;
		$pstart = $psize * ($page - 1);
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(starttime) >' . time() . ' AND mode = 5 AND uniacid=:uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 5 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 5 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 5 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	public function add() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			{
				$data = array('title' => $title, 'type' => $type, 'mode' => 5, 'endtime' => $endtime, 'starttime' => $starttime, 'coupon' => json_encode($coupon), 'uniacid' => $_W['uniacid'], 'status' => 1, 'img' => '../addons/wx_shop/plugin/exchange/static/img/exchange.jpg', 'title_reply' => '优惠券兑换', 'content' => '欢迎来到兑换中心,点击进入兑换', 'code_type' => intval($_GPC['code_type']), 'binding' => $binding, 'repeat' => $repeat);
	...
	public function creat() 
	{
		...
		if ((0 < intval($_GPC['ajax'])) && (0 < intval($_GPC['loop']))) 
		{
			...
			while (0 < $num) 
			{
				...
				pdo_insert('wx_shop_exchange_code', array('groupid' => $id, 'uniacid' => $_W['uniacid'], 'endtime' => $endtime, 'type' => $res['code_type'], 'repeatcount' => $res['repeat']));
				...
				$insert = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:exchange:coupon', 'module' => 'reply', 'displayorder' => $rand_id, 'status' => 1);
				if (IMS_VERSION == '1.0') 
				{
	...
	public function search() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE mode = 5 AND uniacid=:uniacid  ' . $like . ' ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE mode = 5 AND uniacid=:uniacid  ' . $like, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 5 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 5 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 5 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
