...
class Goods_WxShopPage extends PluginWebPage
{
	public function main() 
	{
		...
		$psize = 20;
		$pstart = $psize * ($page - 1);
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 1 AND uniacid=:uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 1 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 1 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 1 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	public function nostart() 
	{
		...
		$psize = 20;
		$pstart = $psize * ($page - 1);
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(starttime) >' . time() . ' AND mode = 1 AND uniacid=:uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 1 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 1 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 1 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	public function creat() 
	{
		...
		if ((0 < intval($_GPC['ajax'])) && (0 < intval($_GPC['loop']))) 
		{
			...
			while (0 < $num) 
			{
				...
				$insert = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:exchange:goods', 'module' => 'reply', 'displayorder' => $rand_id, 'status' => 1);
				if (IMS_VERSION == '1.0') 
				{
		...
		else 
		{
			...
			if (!(empty($id))) 
			{
				...
				if ($goods['goods'] != false) 
				{
					...
					{
						$all[$k] = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE uniacid = :uniacid AND deleted = 0 AND hasoption = 0 AND id = :id', array(':id' => $v, ':uniacid' => $_W['uniacid']));
					}
				}
	...
	public function search() 
	{
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE mode = 1 AND uniacid=:uniacid  ' . $like . ' ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize, array(':uniacid' => $_W['uniacid']));
		$count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE mode = 1 AND uniacid=:uniacid  ' . $like, array(':uniacid' => $_W['uniacid']));
		$allEnd = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) <' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 1 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allStart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE unix_timestamp(endtime) >' . time() . ' AND unix_timestamp(starttime) <' . time() . ' AND mode = 1 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
		$allNostart = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_exchange_group') . ' WHERE  unix_timestamp(starttime) >' . time() . ' AND mode = 1 AND uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
	...
	public function ajaxlist() 
	{
		...
		{
			$option2[$j] = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_goods_option') . ' WHERE goodsid = :id', array(':id' => $item['id']));
	...
	public function banner() 
	{
		...
		pdo_update('wx_shop_exchange_group', array('banner' => $jsonBanner, 'rule' => $rule, 'diypage' => intval($_GPC['diypage'])), array('uniacid' => $_W['uniacid'], 'id' => $id));
		show_json(1, '保存成功');
	}
	public function selector() 
	{
		...
		$res = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE uniacid = :uniacid AND deleted = 0 ' . $condition . ' LIMIT ' . $pstart . ',8888', array(':uniacid' => $_W['uniacid']));
		if (p('seckill')) 
		{
			foreach ($res as $k => $v ) 
			{
	...
	public function optionset() 
	{
		...
		if ($gtype == 1) 
		{
			$res = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_goods_option') . ' WHERE goodsid = :goodsid AND uniacid = :uniacid', array(':uniacid' => $_W['uniacid'], ':goodsid' => $id));
		}
		else 
		{
			$res = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_goods') . ' WHERE id = :id AND uniacid = :uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		}
		include $this->template();
	}
	public function addreply() 
	{
		...
		$group = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_group') . ' WHERE id = :id AND uniacid = :uniacid ', array(':id' => $groupid, ':uniacid' => $_W['uniacid']));
		if (empty($group)) 
		{
		...
		if (!(empty($group['keyword_reply']))) 
		{
			...
			if (empty($rule['id'])) 
			{
				pdo_insert('rule', array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:exchange', 'module' => 'wx_shop', 'status' => $reply['replystatus']));
				$rid = pdo_insertid();
				pdo_insert('rule_keyword', array('rid' => $rid, 'uniacid' => $_W['uniacid'], 'module' => 'news', 'content' => $reply['keyword'], 'type' => 1, 'status' => 1));
				pdo_insert('news_reply', array('rid' => $rid, 'title' => $reply['title'], 'author' => 'WX_SHOP_V2', 'description' => $reply['description'], 'thumb' => $reply['thumb'], 'url' => $url, 'incontent' => 1, 'createtime' => time(), 'parent_id' => 0));
				pdo_update('wx_shop_exchange_group', array('keyword_reply' => $rid, 'title_reply' => $reply['title'], 'img' => $reply['thumb'], 'content' => $reply['description'], 'reply_keyword' => $reply['keyword'], 'reply_status' => $reply['replystatus']), array('id' => $groupid, 'uniacid' => $_W['uniacid']));
			}
			else 
			{
				...
				pdo_update('news_reply', array('title' => $reply['title'], 'author' => 'WX_SHOP_V2', 'description' => $reply['description'], 'thumb' => $reply['thumb'], 'url' => $url, 'incontent' => 1, 'createtime' => time(), 'parent_id' => 0), array('rid' => $group['keyword_reply']));
				pdo_update('wx_shop_exchange_group', array('title_reply' => $reply['title'], 'img' => $reply['thumb'], 'content' => $reply['description'], 'reply_keyword' => $reply['keyword'], 'reply_status' => $reply['replystatus']), array('id' => $groupid, 'uniacid' => $_W['uniacid']));
			}
		}
		else 
		{
	...
	public function import() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			else 
			{
				...
				$path = '../addons/wx_shop/data/upload/exchange/' . $_W['uniacid'] . '/' . time() . md5($_FILES['f2']['tmp_name']) . '.xls';
				load()->func('file');
				...
				while ($i <= $highestRow) 
				{
					...
					$key = trim($key);
					$exist = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exchange_code') . ' WHERE `key`= :key', array(':key' => $key));
					$pattern = '/^[0-9a-zA-Z_]+$/';
					...
					pdo_insert('wx_shop_exchange_code', array('groupid' => $res['id'], 'uniacid' => $_W['uniacid'], 'endtime' => $endtime, 'type' => $res['code_type'], 'key' => $key));
					$rand_id = pdo_insertid();
					$serial = 'DH' . date('Ymd', time()) . $rand_id;
					pdo_update('wx_shop_exchange_code', array('serial' => $serial), array('key' => $key, 'uniacid' => $_W['uniacid']));
					pdo_query('UPDATE ' . tablename('wx_shop_exchange_group') . ' SET total = total + 1 WHERE uniacid = :uniacid AND id = :id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
					$br = "\r\n";
					if (!(file_exists('../addons/wx_shop/data/qrcode/' . $_W['uniacid'] . '/exchange/' . $dir_pre . '.txt')))
					{
				...
				$excel->getActiveSheet()->setCellValue('B1', '编号');
				$excel->getActiveSheet()->setCellValue('C1', '所属兑换活动');
				if (is_file('../addons/wx_shop/data/qrcode/' . $_W['uniacid'] . '/exchange/' . $dir_pre . '.txt'))
				{
					$file = fopen('../addons/wx_shop/data/qrcode/' . $_W['uniacid'] . '/exchange/' . $dir_pre . '.txt', 'r');
				}
				$line = 2;
