...
class TaskModel extends PluginModel
{
     ...
     * @var array
     */
    public $wx_shop_task_list = array('id', 'uniacid', 'displayorder', 'title', 'image', 'type', 'status', 'picktype', 'starttime', 'endtime', 'stop_type', 'stop_limit', 'stop_time', 'stop_cycle', 'repeat_type', 'repeat_interval', 'repeat_cycle', 'demand', 'reward', 'followreward', 'design_data', 'design_bg', 'goods_limit', 'notice', 'requiregoods', 'native_data', 'native_data2', 'native_data3', 'reward3', 'reward2', 'level2', 'level3', 'member_group', 'auto_pick', 'keyword_pick', 'verb', 'unit', 'member_level');
    public $wx_shop_task_set = array('uniacid', 'entrance', 'keyword', 'cover_title', 'cover_img', 'cover_desc', 'msg_pick', 'msg_progress', 'msg_finish', 'msg_follow', 'isnew', 'bg_img', 'top_notice');
    public $wx_shop_task_record = array('id', 'uniacid', 'taskid', 'tasktitle', 'tasktype', 'task_demand', 'openid', 'nickname', 'picktime', 'stoptime', 'finishtime', 'task_progress', 'reward_data', 'followreward_data', 'taskimage', 'design_data', 'design_bg', 'require_goods', 'level1', 'level2', 'reward_data1', 'reward_data2', 'member_group', 'auto_pick');

    public function get_unread()
    {
        global $_W;
        $sql = 'select count(*) from ' . tablename('wx_shop_task_reward') . ' where openid = :openid and uniacid = :uniacid and `get` = 1 and `sent` = 0 and `read` = 0';
    ...
    {
        global $_W;
        pdo_update('wx_shop_task_reward', array('read' => 1), array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid'], 'get' => 1, 'sent' => 0, 'read' => 0));
    }

    public function isnew()
    {
        global $_W;
        $uniacid = pdo_fetchcolumn('select uniacid from ' . tablename('wx_shop_task_set') . ' where uniacid = ' . $_W['uniacid']);

        if (empty($uniacid)) {
            pdo_insert('wx_shop_task_set', array('uniacid' => $_W['uniacid']));
        }


        $this->set_read();
        return pdo_fetchcolumn('select isnew from ' . tablename('wx_shop_task_set') . ' where uniacid = ' . $_W['uniacid']);
    }
    ...
    public function getQR($poster, $member)
    {
        ...
        $qr = pdo_fetch('select * from ' . tablename('wx_shop_task_poster_qr') . ' where openid=:openid and acid=:acid and posterid=:posterid limit 1', array(':openid' => $member['openid'], ':acid' => $acid, ':posterid' => $poster['id']));
        ...
        if (empty($qr)) {
            ...
            $ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'qrcid' => $scene_id, 'model' => 0, 'name' => 'WX_SHOP_TASK_QRCODE', 'keyword' => $poster['keyword'], 'expire' => $expire, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket']);
            ...
            pdo_insert('wx_shop_task_poster_qr', $qr);
            $qr['id'] = pdo_insertid();
        } else {
            ...
            if ($qr['endtime'] < $time) {
                ...
                pdo_update('wx_shop_task_poster_qr', array('ticket' => $ticket, 'qrimg' => $qrimg, 'url' => $result['url'], 'endtime' => $posterendtime), array('id' => $qr['id']));
                $qr['ticket'] = $ticket;
    ...
    {
        global $_W;
        $path = IA_ROOT . '/addons/wx_shop/data/task/poster/' . $_W['uniacid'] . '/';
        ...
        $img = $_W['siteroot'] . 'addons/wx_shop/data/task/poster/' . $_W['uniacid'] . '/' . $file;
        ...
        if (($qr['qrimg'] != $qr['current_qrimg']) || empty($qr['mediaid']) || empty($qr['createtime']) || ((($qr['createtime'] + (3600 * 24 * 3)) - 7200) < time()) || $is_new) {
            ...
            $qr['mediaid'] = $mediaid;
            $qr['img'] = $mediaid;
            pdo_update('wx_shop_task_poster_qr', array('mediaid' => $mediaid, 'createtime' => time()), array('id' => $qr['id']));
        }
    ...
    public function getQRByTicket($ticket = '')
    {
        ...
        $qrs = pdo_fetchall('select * from ' . tablename('wx_shop_task_poster_qr') . ' where ticket=:ticket and acid=:acid limit 1', array(':ticket' => $ticket, ':acid' => $_W['acid']));
        $count = count($qrs);
    ...
    public function checkMember($openid = '', $acc = '')
    {
        ...
        if (empty($member)) {
            ...
            pdo_update('wx_shop_member', $member, array('id' => $member['id']));
            $member['isnew'] = false;
        }
    ...
    public function responseUnsubscribe($param = '')
    {
        ...
        if (isset($param['openid']) && !(empty($param['openid']))) {
            ...
            $task_info = pdo_fetch('SELECT join_user FROM ' . tablename('wx_shop_task_join') . 'WHERE failtime>' . time() . ' and is_reward=0 and join_id in (SELECT join_id from ' . tablename('wx_shop_task_joiner') . ' where uniacid=:uniacid and joiner_id=:joiner_id and join_status=1)', array(':uniacid' => $_W['uniacid'], ':joiner_id' => $openid));

            if ($task_info) {
                $member = $this->checkMember($openid);
                pdo_update('wx_shop_task_joiner', array('join_status' => 0), $where);
                $updatesql = 'UPDATE ' . tablename('wx_shop_task_join') . ' SET completecount = completecount-1 WHERE failtime>' . time() . ' and is_reward=0 and join_id in (SELECT join_id from ' . tablename('wx_shop_task_joiner') . ' where uniacid=:uniacid and joiner_id=:joiner_id and join_status=1)';
    ...
    public function notice_complain($templete, $member, $poster, $scaner = '', $type = 1)
    {
        ...
        if ($templete) {
            ...
            $db_data = pdo_fetchcolumn('select `data` from ' . tablename('wx_shop_task_default') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
            $res = '';
    ...
    public function getdefault($key)
    {
        ...
        if ($key) {
            $default = pdo_fetchcolumn('select `data` from ' . tablename('wx_shop_task_default') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
    ...
    public function getGoods($param = '')
    {
        ...
        global $_W;
        $search_sql = 'SELECT * FROM ' . tablename('wx_shop_task_join') . ' WHERE join_user= :openid AND uniacid = :uniacid AND `join_id`=:join_id  AND is_reward=1';
        ...
        if (isset($param['goods_num']) && !(empty($param['goods_num']))) {
                ...
                if (!(empty($rec_reward))) {
                    ...
                    if (isset($rec_reward['goods'][$goods_id]) && !(empty($rec_reward['goods'][$goods_id]))) {
                        ...
                        if (0 < $goods_spec) {
                            ...
                            $res = pdo_update('wx_shop_task_join', $update_data, $update_where);
            ...
            if (!(empty($rec_reward))) {
                ...
                if (isset($rec_reward['goods'][$goods_id]) && !(empty($rec_reward['goods'][$goods_id]))) {
                    $createtime_sql = 'SELECT `createtime` FROM ' . tablename('wx_shop_task_log') . ' WHERE openid= :openid AND uniacid = :uniacid AND `join_id`=:join_id  AND (recdata IS NOT NULL AND recdata !="") ';
                    ...
                    $rewardday_sql = 'SELECT `reward_days`,`is_goods` FROM ' . tablename('wx_shop_task_poster') . ' WHERE  uniacid = :uniacid AND `id`=:id  AND poster_type=:poster_type ';
    ...
    public function reward($member_info, $poster, $join_info, $qr, $openid, $qrmember)
    {
        ...
        if (($join_info['needcount'] == $count) && ($join_info['is_reward'] == 0)) {
            ...
            pdo_update('wx_shop_task_join', array('completecount' => $count, 'is_reward' => 1, 'reward_data' => $reward), array('uniacid' => $_W['uniacid'], 'join_id' => $join_info['join_id'], 'join_user' => $qr['openid'], 'task_id' => $poster['id'], 'task_type' => 1));
            pdo_insert('wx_shop_task_log', $reward_log);
            $log_id = pdo_insertid();
            ...
            pdo_insert('wx_shop_task_joiner', $scaner);
                ...
                if ($key == 'rec') {
                    ...
                    if (isset($val['bribery']) && (0 < $val['bribery'])) {
                        ...
                        if (!(is_error($err))) {
                            ...
                            pdo_update('wx_shop_task_log', $upgrade, array('id' => $log_id));
                        }
            ...
            pdo_update('wx_shop_task_join', array('completecount' => $count), array('uniacid' => $_W['uniacid'], 'join_user' => $qr['openid'], 'task_id' => $poster['id'], 'task_type' => 1));
            pdo_insert('wx_shop_task_log', $reward_log);
            $log_id = pdo_insertid();
            $scaner = array('uniacid' => $_W['uniacid'], 'task_user' => $qr['openid'], 'joiner_id' => $openid, 'task_id' => $qr['posterid'], 'join_id' => $join_info['join_id'], 'task_type' => 1, 'join_status' => 1, 'addtime' => time());
            pdo_insert('wx_shop_task_joiner', $scaner);
            ...
            if ($val['needcount'] == $count) {
                if ($is_reward == 0) {
                    ...
                    if (!(isset($val['is_reward'])) || empty($val['is_reward'])) {
                        ...
                        pdo_update('wx_shop_task_join', array('reward_data' => $rec_data, 'is_reward' => 1), array('uniacid' => $_W['uniacid'], 'join_id' => $join_info['join_id'], 'join_user' => $qr['openid'], 'task_id' => $poster['id'], 'task_type' => 2));
                    } else {
    ...
    protected function reward_both($count, $reward_data, $qr, $join_info, $openid, $qrmember, $member_info, $poster)
    {
        ...
        pdo_update('wx_shop_task_join', array('completecount' => $count), array('uniacid' => $_W['uniacid'], 'join_id' => $join_info['join_id'], 'join_user' => $qr['openid'], 'task_id' => $poster['id'], 'task_type' => 2));
        pdo_insert('wx_shop_task_log', $reward_log);
        $log_id = pdo_insertid();
    ...
    protected function reward_scan($count, $reward_data, $qr, $join_info, $openid, $qrmember, $member_info, $poster)
    {
        ...
        pdo_update('wx_shop_task_join', array('completecount' => $count), array('uniacid' => $_W['uniacid'], 'join_user' => $qr['openid'], 'task_id' => $poster['id'], 'task_type' => 2));
        pdo_insert('wx_shop_task_log', $reward_log);
        $log_id = pdo_insertid();
        $scaner = array('uniacid' => $_W['uniacid'], 'task_user' => $qr['openid'], 'joiner_id' => $openid, 'task_id' => $qr['posterid'], 'join_id' => $join_info['join_id'], 'task_type' => 2, 'join_status' => 1, 'addtime' => time());
        pdo_insert('wx_shop_task_joiner', $scaner);
    ...
    public function checkTaskReward($taskclass = '', $num = 1, $openid = '')
    {
        ...
        $sql = 'SELECT * FROM ' . tablename('wx_shop_task_extension_join') . ' WHERE openid = :openid AND uniacid = :uniacid AND completetime = 0 AND endtime > ' . time();
        ...
        foreach ($allTask as $tk => $tv) {
            ...
            pdo_update('wx_shop_task_extension_join', array('progress_data' => $progress_data), array('uniacid' => $_W['uniacid'], 'id' => $tv['id']));
            ...
            if ($isreward) {
                pdo_update('wx_shop_task_extension_join', array('completetime' => time()), array('uniacid' => $_W['uniacid'], 'id' => $tv['id']));
    ...
    public function sendReward($reward_data = array(), $btn = 0, $openid = NULL, $rewardid = 0)
    {
        ...
        pdo_update('wx_shop_task_extension_join', array('rewarded' => $rewarded), array('id' => $rewardid, 'uniacid' => $_W['uniacid']));
    }
    ...
    public function getNewTask($id)
    {
        ...
        $nowtime = time();
        $sql = 'SELECT * FROM ' . tablename('wx_shop_task') . ' WHERE id = :id AND status = 1 AND starttime < ' . $nowtime . ' AND endtime >' . $nowtime . ' AND uniacid = :uniacid';
        ...
        $data['logo'] = $task['logo'];
        pdo_insert('wx_shop_task_extension_join', $data);
    ...
    public function getTaskLixt($action, $page)
    {
        ...
        $psize = 20;
        $pstart = ($page - 1) * $psize;
        $sql = 'SELECT id,title,starttime,endtime,status FROM ' . tablename('wx_shop_task') . ' WHERE `type` = :type AND uniacid = :uniacid ORDER BY endtime DESC LIMIT ' . $pstart . ',' . $psize;
    ...
    public function taskFilter($task)
    {
        ...
        switch ($type) {
            case 1:
                $sql = 'SELECT COUNT(*) FROM ' . tablename('wx_shop_task_extension_join') . ' WHERE taskid = :taskid AND openid = :openid AND uniacid = :uniacid';
            ...
            case 2:
                $sql = 'SELECT COUNT(*) FROM ' . tablename('wx_shop_task_extension_join') . ' WHERE taskid = :taskid AND openid = :openid AND completetime = 0 AND uniacid = :uniacid';
                ...
                $sql1 = 'SELECT completetime FROM ' . tablename('wx_shop_task_extension_join') . ' WHERE taskid = :taskid AND openid = :openid AND uniacid = :uniacid ORDER BY completetime DESC';
                ...
                $sql2 = 'SELECT COUNT(*) FROM ' . tablename('wx_shop_task_extension_join') . ' WHERE taskid = :taskid AND uniacid = :uniacid AND openid = :openid AND completetime > ' . $hourl . ' AND completetime < ' . $hourr . ' AND completetime != 0';
    ...
    public function getRecordsList($page, $taskid)
    {
        ...
        $psize = 20;
        $pstart = ($page - 1) * $psize;
        $sql = 'SELECT * FROM ' . tablename('wx_shop_task_log') . ' WHERE taskid = :taskid AND uniacid = :uniacid ORDER BY id DESC LIMIT ' . $pstart . ',' . $psize;
    ...
    public function getUserTaskList($type)
    {
        ...
        $sql = 'SELECT * FROM ' . tablename('wx_shop_task') . ' WHERE status = 1 ' . $condition . ' AND starttime < ' . $time . ' AND endtime > ' . $time . ' AND uniacid = :uniacid';
    ...
    public function getMyTaskList($condition = '=')
    {
        ...
        // var_dump(time());
        // print_r(pdo_fetchall('SELECT a.* FROM ' . tablename('wx_shop_task_extension_join').'as a'));die;
    ...
    {
        global $_W;
        $sql = 'SELECT a.* FROM ' . tablename('wx_shop_task_extension_join') . ' a JOIN ' . tablename('wx_shop_task') . ' b ON a.taskid = b.id WHERE a.openid = :openid AND a.completetime = 0 AND a.endtime < ' . time() . ' AND a.uniacid = :uniacid';
        return pdo_fetchall($sql, array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
    }
    ...
    public function returnName($taskclass)
    {
        ...
        $sql = 'SELECT taskname FROM ' . tablename('wx_shop_task_extension') . ' WHERE taskclass = :taskclass';
    ...
    {
        global $_W;
        $sql = 'SELECT title FROM ' . tablename('wx_shop_goods') . ' WHERE id = :id AND uniacid = :uniacid';
    ...
    public function __construct($name = '')
    {
        ...
        $this->taskType = pdo_getall('wx_shop_task_type');
    }
    ...
    public function getAllTask(&$page, $psize = 15)
    {
        ...
        $sql = 'select ' . $field . ' from ' . tablename('wx_shop_task_list') . ' where uniacid = :uniacid ' . $condition . '  order by displayorder desc,id desc limit ' . $pstart . ' , ' . $psize;
    ...
    public function deleteTask($ids)
    {
        ...
        if ($isArr) {
            ...
            return pdo_query('delete from ' . tablename('wx_shop_task_list') . ' where ' . $condition);
        }
    ...
    public function getThisTask($id)
    {
        ...
        return pdo_get('wx_shop_task_list', array('id' => $id, 'uniacid' => $_W['uniacid']));
    }
    ...
    public function getAllRecords(&$page, $psize = 20)
    {
        ...
        $field = '*';
        $sql = 'select ' . $field . ' from ' . tablename('wx_shop_task_record') . 'where uniacid = :uniacid ' . $condition . ' order by id desc limit ' . $pstart . ' , ' . $psize;
        $return = pdo_fetchall($sql, $params);
        $countsql = substr($sql, 0, strpos($sql, 'order by'));
        $countsql = str_replace($field, 'count(*)', $countsql);
        $count = pdo_fetchcolumn($countsql, $params);
    ...
    public function getAllRewards(&$page, $psize = 20)
    {
        ...
        $field = '*';
        $sql = 'select ' . $field . ' from ' . tablename('wx_shop_task_reward') . ' where uniacid = :uniacid ' . $condition . ' and `get` = 1 order by id desc limit ' . $pstart . ' , ' . $psize;
        $return = pdo_fetchall($sql, $params);
        $countsql = substr($sql, 0, strpos($sql, 'order by'));
        $countsql = str_replace($field, 'count(*)', $countsql);
        $count = pdo_fetchcolumn($countsql, $params);
    ...
    public function getGoods_new($keyword = '', $page = 1)
    {
        ...
        $sql = 'select ' . $field . ' from ' . tablename('wx_shop_goods') . 'where uniacid = :uniacid and status = 1 and deleted = 0' . $like . ' limit ' . $pstart . ' , ' . $psize;
    ...
    public function getCoupon($keyword = '', $page = 1)
    {
        ...
        $field = 'id,couponname';
        $like = (($keyword === '' ? $keyword : ' and title like %' . $keyword . '%'));
        $sql = 'select ' . $field . ' from ' . tablename('wx_shop_coupon') . 'where uniacid = :uniacid ' . $like . ' limit ' . $pstart . ' , ' . $psize;
        $countsql = substr($sql, 0, strpos($sql, 'limit'));
        $countsql = str_replace($field, 'count(*)', $countsql);
        $params = array(':uniacid' => $_W['uniacid']);
        $count = pdo_fetchcolumn($countsql, $params);
    ...
    protected function checkCanPick($task, $openid)
    {
        ...
        $sql = 'select * from ' . tablename('wx_shop_task_record') . ' where openid = :openid and taskid = :taskid and uniacid = :uniacid order by id desc';
    ...
    protected function createPoster2($poster, $member = 0, $upload = true)
    {
        ...
        if (empty($qr['mediaid'])) {
            ...
            $qr['mediaid'] = $mediaid;
            $qr['img'] = $mediaid;
            pdo_update('wx_shop_task_qr', array('mediaid' => $mediaid), array('id' => $qr['id']));
        }
    ...
    protected function getQR2($poster, $member)
    {
        ...
        $this->createRule();
        $qr = pdo_fetch('select * from ' . tablename('wx_shop_task_qr') . ' where openid = :openid and uniacid = :uniacid and recordid = :recordid limit 1', array(':openid' => $member['openid'], ':uniacid' => $_W['uniacid'], ':recordid' => $poster['id']));
        ...
        if (empty($qr)) {
            ...
            $ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'qrcid' => ($scene_id[0] != 't' ? $scene_id : 0), 'model' => 0, 'name' => 'WX_SHOP_TASKNEW_POSTER', 'keyword' => 'WX_SHOP_TASKNEW', 'expire' => $expire, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket'], 'scene_str' => ($scene_id[0] == 't' ? $scene_id : ''));
            pdo_insert('qrcode', $ims_qrcode);
            $qr = array('uniacid' => $_W['uniacid'], 'openid' => $member['openid'], 'sceneid' => $scene_id, 'ticket' => $result['ticket'], 'recordid' => $poster['id']);
            pdo_insert('wx_shop_task_qr', $qr);
            $qr['id'] = pdo_insertid();
        }
    ...
    {
        global $_W;
        $ruleSql = 'select id from ' . tablename('rule') . ' where `name` = \'wx_shop:task\' and `module` = \'wx_shop\' and uniacid = ' . $_W['uniacid'];
        $ruleCount = pdo_fetchcolumn($ruleSql);

        if (empty($ruleCount)) {
            $rule = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:task', 'module' => 'wx_shop', 'status' => 1);
        ...
        $keywordSql = 'select COUNT(*) from ' . tablename('rule_keyword') . ' where `content` = \'WX_SHOP_TASKNEW\' and uniacid = ' . $_W['uniacid'];
        $keywordCount = pdo_fetchcolumn($keywordSql);

        if (empty($keywordCount)) {
            $keyword = array('rid' => $ruleCount, 'uniacid' => $_W['uniacid'], 'module' => 'wx_shop', 'content' => 'WX_SHOP_TASKNEW', 'type' => 1, 'status' => 1);
     ...
     * @param $typeKey
     */
    public function checkTaskProgress($num, $typeKey, $recordid = 0, $openid = '', $goodsid = 0,$price='')
    {
        global $_W;
        if (empty($openid)) {
            $openid = $_W['openid'];
        ...
        $sqlPassive = 'select * from ' . tablename('wx_shop_task_list') . ' where uniacid = :uniacid and picktype = 1 and starttime < :starttime and endtime > :endtime';
        ...
        if (!(empty($passiveTask))) {
            foreach ($passiveTask as $task) {
                if (!(pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_task_record') . ' where taskid = ' . $task['id'] . ' and uniacid = ' . $_W['uniacid']))) {
        ...
        $sql = 'select * from ' . tablename('wx_shop_task_record') . ' where uniacid = :uniacid ' . $condtion . ' and openid = :openid and tasktype = :tasktype and (stoptime = \'0000-00-00 00:00:00\' or stoptime >\'' . $time . '\') and finishtime = \'0000-00-00 00:00:00\'';
        ...
        if (!(empty($allRecord))) {
            foreach ($allRecord as $record) {
                ...
                //单笔满额奖励
                if ($type['type_key'] == 'order_full' && $price > $record['task_demand'] && $record['task_progress'] < $record['task_demand']) {
                    pdo_update('wx_shop_task_record', array('task_progress' => $record['task_demand']), array('id' => $record['id']));
                    $this->sentReward($record['id'], $openid, 0);
                    $this->taskFinishMessage($record['openid'], $record);
                }

                $record['task_progress'] = $record['task_progress'] + $num;
                if ($type['type_key'] == 'pyramid_money') {
                    if ($record['task_progress'] + $price > $record['task_demand']) {
                        $record['task_progress'] = $record['task_demand'];
                    } else {
                        $record['task_progress'] = ceil($price+$record['task_progress']);
                    }
                }
                if ($record['task_progress'] == $record['task_demand']) {
                    ...
                    $ret = pdo_update('wx_shop_task_record', $update_arr, array('id' => $record['id']));
                    ...
                    if ($type['once'] != 1) {
                        pdo_update('wx_shop_task_record', array('task_progress' => $record['task_progress']), array('id' => $record['id']));
    ...
    public function sentReward($recordid, $openid, $level = 0)
    {
        global $_W;
        $time = date('Y-m-d H:i:s');
        $sql = 'select * from ' . tablename('wx_shop_task_reward') . ' where recordid = :recordid and openid = :openid and uniacid = :uniacid and `get` = 0 and sent = 0 and `level` = ' . $level . ' ';
            ...
            foreach ($rewards as $k => $reward) {
                switch ($reward['reward_type']) {
                    case 'credit':
                        ...
                        pdo_update('wx_shop_task_reward', array('sent' => 1, 'get' => 1, 'gettime' => $time, 'senttime' => $time), array('id' => $reward['id']));
                        break;
                    ...
                    case 'redpacket':
                        pdo_update('wx_shop_task_reward', array('get' => 1, 'gettime' => $time), array('id' => $reward['id']));
                        break;
    ...
    public function followReward($recordid)
    {
        ...
        $record = pdo_fetch('select * from ' . tablename('wx_shop_task_record') . ' where id = :id and uniacid = :uniacid', array(':id' => $recordid, ':uniacid' => $_W['uniacid']));
            ...
            switch ($k) {
                case 'credit':
                    if (0 < $reward) {
                        ...
                        pdo_insert('wx_shop_task_reward', array('uniacid' => $_W['uniacid'], 'taskid' => $record['taskid'], 'tasktitle' => $record['tasktitle'], 'tasktype' => $record['tasktype'], 'taskowner' => $record['openid'], 'ownernickname' => $record['nickname'], 'recordid' => $record['id'], 'nickname' => $info['nickname'], 'headimg' => $info['avatar'], 'openid' => $_W['openid'], 'reward_type' => 'credit', 'reward_title' => $reward . '积分', 'reward_data' => $reward, 'sent' => 1, 'get' => 1, 'gettime' => $time, 'senttime' => $time, 'isjoiner' => 1));
                    }
    ...
    public function taskFinishMessage($openid, $record)
    {
        ...
        $memberList = m('member')->getInfo($openid);
        $url = mobileUrl('task.reward', array(), 1);
        $text = '亲爱的'.$memberList['nickname'].'，您的任务'.$record['tasktitle'].'已经完成！快去查看您的奖励吧~~' . "\n";
        $remark = '<a href=\'' . $url . '\'>点击查看详情</a>';
        $text .= $remark;
        $datas = $this->getNoticeDatas($record);
        m('notice')->sendNotice(array('openid' => $openid, 'tag' => $tag, 'default' => $message, 'cusdefault' => $text, 'url' => $url, 'datas' => $datas));
    ...
    public function getTaskRewardGoodsInfo($taskrewardgoodsid = 0)
    {
        ...
        $reward = pdo_get('wx_shop_task_reward', array('id' => $taskrewardgoodsid, 'openid' => $_W['openid'], 'uniacid' => $_W['uniacid'], 'get' => 1, 'sent' => 0, 'reward_type' => 'goods'));
    ...
    public function setTaskRewardGoodsSent($taskrewardgoodsid = 0)
    {
        ...
        pdo_update('wx_shop_task_reward', array('sent' => 1, 'senttime' => date('Y-m-d H:i:s')), array('id' => $taskrewardgoodsid));
    ...
    {
        global $_W;
        $sql = 'select entrance from ' . tablename('wx_shop_task_set') . ' where uniacid = ' . $_W['uniacid'];
    ...
    {
        global $_W;
        $sql = 'select top_notice from ' . tablename('wx_shop_task_set') . ' where uniacid = ' . $_W['uniacid'];
        return pdo_fetchcolumn($sql);
    }
    ...
    public function getMessageSet($field = array('msg_pick', 'msg_progress', 'msg_finish', 'msg_follow'))
    {
        ...
        $msg = pdo_get('wx_shop_task_set', array('uniacid' => $_W['uniacid']), $field2);
