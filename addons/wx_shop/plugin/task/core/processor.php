...
class TaskProcessor extends PluginProcessor 
{
	...
	private function responseSubscribe_new($obj) 
	{
		...
		$recordid = pdo_fetchcolumn('select recordid from ' . tablename('wx_shop_task_qr') . ' where uniacid = :uniacid and sceneid = :sceneid', array(':uniacid' => $_W['uniacid'], ':sceneid' => $eventkey));
		$openid = pdo_fetchcolumn('select openid from ' . tablename('wx_shop_task_record') . ' where uniacid = :uniacid and id = :recordid', array(':uniacid' => $_W['uniacid'], ':recordid' => $recordid));
		if (empty($recordid)) 
		{
		...
		if ($member_info['isnew']) 
		{
			...
			if ($agent['isagent']) 
			{
				...
				$a = pdo_update('wx_shop_member', array('agentid' => $agent['id'], 'childtime' => time()), array('id' => $my['id']));
			}
	...
	private function responseScan($obj) 
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('wx_shop_task_poster') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $qr['posterid'], ':uniacid' => $_W['uniacid']));
		if (empty($poster)) 
		{
			return $this->responseDefault($obj);
		}
		...
		else 
		{
			...
			$scan_count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_task_joiner') . ' where uniacid=:uniacid and task_user=:task_user and joiner_id=:joiner_id and join_id=:join_id ', $params);
					...
					if (($qrmember['isagent'] == 1) && ($qrmember['status'] == 1)) 
					{
						if (!(empty($poster['bedown']))) 
						{
							if (empty($member['agentid'])) 
							{
								...
								{
									pdo_update('wx_shop_member', array('agentid' => $qrmember['id'], 'childtime' => $time), array('id' => $member['id']));
                                    ...
                                    //代理分红修改begin
                                    if (p('weightbonus')) {
                                        p('weightbonus')->upgradeLevelByAgent($qrmember['id']);
                                    }
                                    //代理分红修改end
						...
						if (!(empty($poster['beagent']))) 
						{
							...
							pdo_update('wx_shop_member', array('isagent' => 1, 'status' => $become_check, 'agenttime' => $time), array('id' => $member['id']));
							if ($become_check == 1) 
							{
