...
class Set_WxShopPage extends PluginWebPage
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'wx_shop签到入口设置'));
			if (!empty($rule)) 
			{
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop签到入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => 1);
			if (empty($rule)) 
			{
			...
			$sys = pdo_fetch('select * from ' . tablename('wx_shop_sign_set') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
			if (empty($sys)) 
			{
				$arr['uniacid'] = $_W['uniacid'];
				pdo_insert('wx_shop_sign_set', $arr);
			}
			else 
			{
				pdo_update('wx_shop_sign_set', $arr, array('uniacid' => $_W['uniacid']));
			}
