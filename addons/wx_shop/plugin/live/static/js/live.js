            ...
            if (obj.text.indexOf("[") > -1 && obj.text.indexOf("]") > -1) {
                ...
                if (res) {
                    $.each(res, function (index, val) {
                        ...
                        if (elm.length > 0) {
                            if (elm.data('index')) {
                                var face = '<img class="face" src="../addons/wx_shop/plugin/live/static/images/face/' + elm.data('index') + '.gif?v=2" />';
    ...
    modal.initRedpackList = function (arr) {
        ...
        if ($.isArray(arr)) {
            $.each(arr, function (index, item) {
                html += '<div class="item"><div class="avatar"><img src="../addons/wx_shop/static/images/customer.jpg"></div><div class="nickname">' + item.nickname + '</div><div class="price"><span>￥</span>' + item.money + '</div></div>'
            })
        }
        elm.html(html)
    };
