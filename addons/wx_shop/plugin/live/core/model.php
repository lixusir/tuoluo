...
class LiveModel extends PluginModel
{
	...
	public function selectOrderPrice($day = 0, $roomid = 0)
	{
		...
		$sql = 'select id,price,createtime from ' . tablename('wx_shop_order') . ' where uniacid = :uniacid ' . $condition . ' and ismr=0 and isparent=0 and (status > 0 or ( status=0 and paytype=3)) and deleted=0 and createtime between :createtime1 and :createtime2';
	...
	// 	global $_W;
	// 	$set = array();
	// 	$set = pdo_fetch('select * from ' . tablename('wx_shop_live_setting') . ' where uniacid = :uniacid  ', array(':uniacid' => $_W['uniacid']));
	// 	$plugin = pdo_fetch('select `name` from ' . tablename('wx_shop_plugin') . ' where `identity` = \'live\' and isv2 = 1 and status = 1 ');
	...
	public function getSet()
	{
		...
		if ($set) {
			$set['pluginname'] = $plugin['name'];
			return array_merge($set,parent::getSet());
		}
		return false;
	}
	...
	public function sendLiveMessage($openid, $type, $roomid)
	{
		...
		$room = pdo_fetch('select * from ' . tablename('wx_shop_live') . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $roomid . ' ');
		...
		if ($type == 'livefollow') {
			...
			if ($type == 'liveroom') {
				...
				$url = str_replace('addons/wx_shop/plugin/live/task/', '', $_W['siteroot']);
				$url = $url . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=wx_shop&do=mobile&r=live.room&id=' . $room['id'];
				$tag = 'livefollow';
				$remark = "\n<a href='" . $url . '\'>点击查看详情</a>';
	...
	public function isLiving($roomid = 0)
	{
		...
		if (!empty($roomid)) {
			$live = pdo_fetch('SELECT id FROM ' . tablename('wx_shop_live') . ' WHERE id=:id AND uniacid=:uniacid AND status=1 AND living=1 LIMIT 1', array(':id' => $roomid, ':uniacid' => $_W['uniacid']));
	...
	public function isFavorite($openid = NULL, $roomid = 0)
	{
		...
		if (!empty($openid) && !empty($roomid)) {
			$favorite = pdo_fetch('SELECT id FROM ' . tablename('wx_shop_live_favorite') . 'WHERE uniacid=:uniacid AND roomid=:roomid AND openid=:openid AND `deleted`=0 LIMIT 1', array(':uniacid' => $_W['uniacid'], ':roomid' => $roomid, ':openid' => $openid));
	...
	public function emoji2html($str)
	{
		...
		foreach ($emojiList as $index => $emoji) {
			if ($emoji == $str) {
				return '<img class="face" src="../addons/wx_shop/plugin/live/static/images/face/' . $index . '.gif?v=2" />';
			}
		}
	...
	{
		return 'wx_shop_live_' . $table . '_' . $roomid;
	}
	...
	public function isLiveGoods($goodsid = 0, $liveid = 0)
	{
		...
		$livegoods = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_live_goods') . ' WHERE goodsid=:goodsid AND liveid=:liveid AND uniacid=:uniacid', array(':goodsid' => $goodsid, ':liveid' => $liveid, ':uniacid' => $_W['uniacid']));
	...
	public function getLiveOptions($goodsid = 0, $liveid = 0, $options = array())
	{
		...
		if (empty($options)) {
			$options = pdo_fetchall('SELECT id, marketprice FROM ' . tablename('wx_shop_goods_option') . ' WHERE goodsid=:goodsid AND uniacid=:uniacid ORDER BY displayorder ASC', array(':goodsid' => $goodsid, ':uniacid' => $_W['uniacid']));
		}
		...
		$liveOptions = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_live_goods_option') . ' WHERE goodsid=:goodsid AND liveid=:liveid AND uniacid=:uniacid', array(':goodsid' => $goodsid, ':liveid' => $liveid, ':uniacid' => $_W['uniacid']), 'optionid');
	...
	{
		$file = WX_SHOP_CORE . 'socket/socket.config.php';
