...
class Room_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'SELECT l.*,lc.name FROM ' . tablename('wx_shop_live') . " as l\n\t\t        left join " . tablename('wx_shop_live_category') . " as lc on lc.id = l.category\n\t\t        where  1 and " . $condition . ' ORDER BY l.displayorder DESC,l.id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_live') . " as l\n\t\t        left join " . tablename('wx_shop_live_category') . " as lc on lc.id = l.category\n\t\t        where  1 and " . $condition, $params);
		...
		$category = pdo_fetchall('select id,`name`,thumb from ' . tablename('wx_shop_live_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	protected function post()
	{
		...
		if (0 < $id) {
			$item = pdo_fetch('select * from ' . tablename('wx_shop_live') . ' where uniacid = ' . $uniacid . ' and id = ' . $id . ' ');
			$coupon = array();

			if (!empty($item['couponid'])) {
				$coupon = pdo_fetchall('select c.couponname,c.thumb,c.id,lc.coupontotal,lc.couponlimit from ' . tablename('wx_shop_live_coupon') . " as lc\n                        left join " . tablename('wx_shop_coupon') . " as c on c.id = lc.couponid\n                        where lc.uniacid = " . $uniacid . ' and lc.roomid = ' . $id . ' and lc.couponid in(' . $item['couponid'] . ')  ');
			}
		...
		if ($_W['ispost']) {
			...
			if (!empty($data['couponid'])) {
				foreach ($_GPC['couponid'] as $key => $value) {
					if (strpos($couponid, $value) !== false) {
						pdo_update('wx_shop_live_coupon', array('coupontotal' => $_GPC['coupontotal' . $value . ''], 'couponlimit' => $_GPC['couponlimit' . $value . '']), array('uniacid' => $uniacid, 'roomid' => $id, 'couponid' => $value));
						...
						pdo_insert('wx_shop_live_coupon', $data_coupon);
					}
				}
			}
			...
			if (!empty($couponid)) {
				foreach ($couponid as $value) {
					pdo_delete('wx_shop_live_coupon', array('couponid' => $value, 'uniacid' => $uniacid, 'roomid' => $id));
				}
			}
			...
			if (!empty($id)) {
				...
				pdo_update('wx_shop_live', $data, array('id' => $id));
				plog('live.room.edit', '编辑直播间 ID: ' . $id . ' <br/>全返名称: ' . $data['title']);
			}
			else {
				$data['createtime'] = time();
				pdo_insert('wx_shop_live', $data);
				$id = pdo_insertid();
			...
			if (!empty($goodsid)) {
				foreach ($goodsid as $key => $value) {
					...
					if (!empty($good_data['option'])) {
						...
						pdo_delete('wx_shop_live_goods_option', array('goodsid' => $value, 'uniacid' => $uniacid, 'liveid' => intval($id)));
						...
						foreach ($liveOption as $val) {
							...
							pdo_insert('wx_shop_live_goods_option', $optionData);
						}

						$livegoods_option = pdo_fetch('select min(liveprice) as minliveprice,max(liveprice) as maxliveprice from ' . tablename('wx_shop_live_goods_option') . ' where goodsid = ' . $value . ' and liveid = ' . $id . ' ');
					...
					$live_goods = pdo_fetch('select * from ' . tablename('wx_shop_live_goods') . ' where goodsid = ' . $value . ' and uniacid = ' . $uniacid . ' and liveid = ' . $id . ' ');
					...
					if (empty($live_goods)) {
						...
						pdo_insert('wx_shop_live_goods', $live_goods_data);
					}
					else {
						pdo_update('wx_shop_live_goods', array('minliveprice' => floatval($minliveprice), 'liveprice' => floatval($minliveprice), 'maxliveprice' => floatval($maxliveprice)), array('uniacid' => $uniacid, 'goodsid' => intval($value), 'liveid' => $id));
					}
				...
				if (!empty($goodsliveid)) {
					foreach ($goodsliveid as $value) {
						pdo_delete('wx_shop_live_goods', array('uniacid' => $uniacid, 'goodsid' => intval($value), 'liveid' => $id));
					}
				}
		...
		if ($isInvitation) {
			$invitation = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_invitation') . ' WHERE status = 1 and uniacid = ' . $uniacid . ' ORDER BY createtime desc');
		}
	...
	array('id' => 0, 'key' => 'default', 'levelname' => empty($_W['shopset']['shop']['levelname']) ? '默认会员' : $_W['shopset']['shop']['levelname'])
		...
		//屏蔽掉获取直播链失败的平台 2018年9月15日14:16:39
		$liveidentity = array(
		...
		$category = pdo_fetchall('select * from ' . tablename('wx_shop_live_category') . ' where uniacid = ' . $uniacid . ' and enabled = 1 ');
		if ((0 < $id) && !empty($item['goodsid'])) {
			$goods = pdo_fetchall('select g.id,g.title,g.marketprice,lg.liveprice,lg.minliveprice,lg.maxliveprice,g.thumb,g.hasoption,lg.liveid from ' . tablename('wx_shop_live_goods') . " as lg \n            left join " . tablename('wx_shop_goods') . " as g on lg.goodsid = g.id and lg.uniacid = g.uniacid\n            where lg.liveid = " . $id . ' and lg.uniacid = ' . $uniacid . ' ');

			foreach ($goods as $key => $value) {
				if (0 < $value['hasoption']) {
					$goods[$key]['option'] = pdo_fetchall('SELECT go.id,go.title,go.marketprice,go.specs,lgo.liveprice,lgo.liveid,lgo.optionid FROM ' . tablename('wx_shop_goods_option') . " as go\n                    left join " . tablename('wx_shop_live_goods_option') . ' as lgo on lgo.optionid = go.id and lgo.goodsid = go.goodsid and lgo.uniacid = go.uniacid and lgo.liveid = ' . $id . "\n                    WHERE go.uniacid = :uniacid and go.goodsid = :goodsid  ORDER BY go.displayorder DESC,go.id DESC ", array(':uniacid' => $uniacid, 'goodsid' => $value['id']));
					$optionid = array();
	...
	public function statistics()
	{
		...
		$end = intval($_GPC['end']);
		$room = pdo_fetch('select * from ' . tablename('wx_shop_live') . ' where uniacid = ' . $uniacid . ' and id = ' . $roomid . ' ');
		...
		$statistics_time = pdo_fetchall('select * from ' . tablename('wx_shop_live_status') . ' where roomid = ' . $roomid . ' and uniacid = ' . $uniacid . ' and endtime != \'\' order by id desc ');
		...
		$list = array();
		$order = array();
		$order = pdo_fetchall('select og.goodsid,og.price,og.total,g.title from ' . tablename('wx_shop_order') . " as o \n                left join " . tablename('wx_shop_order_goods') . " as og on og.orderid = o.id \n                left join " . tablename('wx_shop_goods') . " as g on og.goodsid = g.id\n                where o.uniacid = " . $uniacid . ' and o.liveid = ' . $roomid . ' and o.createtime >= ' . $start . ' and o.createtime <= ' . $end . ' and o.status >= 1 and o.refundid = 0  ');
		$len = count($order);
		$total = 0;
		$allprice = 0;
	...
	public function hasoption()
	{
		...
		if (empty($id)) {
			$goods = pdo_fetch('select id,title,marketprice,hasoption from ' . tablename('wx_shop_goods') . "\n                        where id = " . $goodsid . ' and uniacid = ' . $uniacid . ' ');
		}
		else {
			$goods = pdo_fetch('select g.id,g.title,g.marketprice,lg.liveprice,g.hasoption from ' . tablename('wx_shop_live_goods') . " as lg\n                    left join " . tablename('wx_shop_goods') . " as g on g.id = lg.goodsid and g.uniacid = lg.uniacid\n                    where lg.goodsid = " . $goodsid . ' and lg.uniacid = ' . $uniacid . ' ');
		}
	...
	public function deleted()
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_live') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('wx_shop_live', array('id' => $item['id']));
			plog('live.room.deleted', '删除直播间<br/>ID: ' . $item['id'] . '<br/>直播间名称: ' . $item['title']);
			$favoritetotal = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_live_favorite') . ' where roomid = ' . $id . ' ');

			if (0 < $favoritetotal) {
				pdo_delete('wx_shop_live_favorite', array('roomid' => $item['id']));
			}

			$viewtotal = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_live_view') . ' where roomid = ' . $id . ' ');

			if (0 < $viewtotal) {
				pdo_delete('wx_shop_live_view', array('roomid' => $item['id']));
			}
	...
	public function console()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_live') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		if (in_array($type, array('status', 'displayorder', 'hot', 'recommend'))) {
			...
			if (empty($id) && is_array($_GPC['ids'])) {
				...
				foreach ($_GPC['ids'] as $item) {
					$property_update = pdo_update('wx_shop_live', array($type => $value), array('id' => $item, 'uniacid' => $_W['uniacid']));
					plog('live.room.edit', '修改直播' . $typestr . '状态   ID: ' . $item . ' ' . $statusstr . ' ');
				}
			}
			else {
				$property_update = pdo_update('wx_shop_live', array($type => $value), array('id' => $id, 'uniacid' => $_W['uniacid']));
