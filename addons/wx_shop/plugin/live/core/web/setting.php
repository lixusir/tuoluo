...
class Setting_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$data = pdo_fetch('select * from ' . tablename('wx_shop_live_setting') . ' where uniacid = ' . $uniacid . ' ');
		$sets = $this->getSet();
		if ($_W['ispost']) {
			...
			m('common')->updatePluginset(array('live' => array('on_show_wxapp'=>$_GPC['sets']['on_show_wxapp'])));
			...
			if (!empty($data)) {
				pdo_update('wx_shop_live_setting', $set_data, array('uniacid' => $uniacid));
			}
			else {
				$set_data['uniacid'] = $uniacid;
				pdo_insert('wx_shop_live_setting', $set_data);
			}
