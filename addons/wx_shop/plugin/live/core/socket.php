...
class LiveSocket
{
	...
	public function onMessage($server, $data, $fd)
	{
		...
		if ($data['type'] == 'update') {
			...
			if (($data['type'] == 'text') || ($data['type'] == 'image')) {
				...
				if ($status == 1) {
					pdo_insert2('wx_shop_live_status', array('uniacid' => $data['uniacid'], 'roomid' => $data['roomid'], 'starttime' => $time));
					...
					if ($status == 0) {
						...
						if (!empty($statusid)) {
							pdo_update2('wx_shop_live_status', array('endtime' => $time), array('uniacid' => $data['uniacid'], 'roomid' => $data['roomid'], 'id' => $statusid));
						}
					}
				}
				...
				pdo_update2('wx_shop_live', $update, array('uniacid' => $data['uniacid'], 'id' => $data['roomid']));
	...
	public function sendCoupon($siteroot = NULL, $openid = NULL, $couponid = 0, $total = 1)
	{
		...
		$url = $siteroot . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=wx_shop&do=mobile&r=live.send.coupon';
	...
	public function sendCredit($siteroot = NULL, $openid = NULL, $fee = 0, $log = NULL)
	{
		...
		$url = $siteroot . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=wx_shop&do=mobile&r=live.send.credit';
		ihttp_post($url, array('code' => $code));
	}
...
require_once WX_SHOP_CORE . 'socket/pdo.php';
