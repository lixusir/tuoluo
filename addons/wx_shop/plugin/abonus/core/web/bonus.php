...
class Bonus_WxShopPage extends PluginWebPage
{
	...
	protected function get_list($status = 0) 
	{
		...
		$sql = 'select *  from ' . tablename('wx_shop_abonus_bill') . '  where 1 ' . $condition . ' ORDER BY createtime desc ';
		...
		$total = pdo_fetchcolumn('select count(*) from' . tablename('wx_shop_abonus_bill') . '  where 1 ' . $condition, $params);
		$totalmoneys = pdo_fetch('select sum(bonusmoney_send1) as b1,sum(bonusmoney_send2) as b2 , sum(bonusmoney_send3) as b3 from' . tablename('wx_shop_abonus_bill') . '  where 1 ' . $condition, $params);
	...
	public function detail() 
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$data = pdo_fetch('select * from ' . tablename('wx_shop_abonus_bill') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (empty($data)) 
		{
			$this->message('结算单未找到!');
		}
		$data['aagentcount11'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_abonus_billp') . ' b left join ' . tablename('wx_shop_member') . ' m on b.openid=m.openid and b.uniacid=m.uniacid  where m.aagenttype=1 and  b.billid=:billid and b.status=1 and b.uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		$data['aagentcount22'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_abonus_billp') . ' b left join ' . tablename('wx_shop_member') . ' m on b.openid=m.openid and b.uniacid=m.uniacid  where m.aagenttype=2 and  b.billid=:billid and b.status=1 and b.uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		$data['aagentcount33'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_abonus_billp') . ' b left join ' . tablename('wx_shop_member') . ' m on b.openid=m.openid and b.uniacid=m.uniacid  where m.aagenttype=3 and  b.billid=:billid and b.status=1 and b.uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		...
		$sql = 'select b.*, m.nickname,m.avatar,m.realname,m.weixin,m.mobile,l.levelname,b.bonus1,b.bonus2,b.bonus3,m.aagentlevel,m.aagenttype,m.id as mid from ' . tablename('wx_shop_abonus_billp') . ' b ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid = b.openid and m.uniacid = b.uniacid' . ' left join ' . tablename('wx_shop_abonus_level') . ' l on l.id = m.aagentlevel' . ' where 1 ' . $condition . ' ORDER BY status asc ';
		$list = pdo_fetchall($sql, $params);
	...
	public function delete() 
	{
		...
		pdo_query('delete from ' . tablename('wx_shop_abonus_bill') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		pdo_query('delete from ' . tablename('wx_shop_abonus_billo') . ' where billid=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		pdo_query('delete from ' . tablename('wx_shop_abonus_billp') . ' where billid=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
...
/*			dump($data['aagents']);
			...
			foreach ($data['aagents'] as $a ) 
			{
				...
				{
					pdo_insert('wx_shop_abonus_billp', $bp);
				}
			}
		...
		$bill = pdo_fetch('select * from ' . tablename('wx_shop_abonus_bill') . ' where uniacid=:uniacid order by id desc limit 1 ', array(':uniacid' => $_W['uniacid']));
		include $this->template();
	}
	...
	public function confirm() 
	{
		...
		$time = time();
		pdo_query('update ' . tablename('wx_shop_abonus_bill') . ' set status=1,confirmtime=' . $time . ' where id=:id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function pay($a = array(), $b = array()) 
	{
		...
		if (empty($data['status'])) 
		{
			$orders = pdo_fetchall('select orderid from ' . tablename('wx_shop_abonus_billo') . ' where billid=:billid and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id), 'orderid');
			if (empty($orders)) 
			{
				show_json(0, '未找到任何结算订单!');
			}
			pdo_query('update ' . tablename('wx_shop_order') . ' set isabonus=1 where id in ( ' . implode(',', array_keys($orders)) . ' ) and uniacid=' . $_W['uniacid']);
		}
		$time = time();
		pdo_query('update ' . tablename('wx_shop_abonus_bill') . ' set paytime=' . $time . ' where id=:id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function payp() 
	{
		...
		$aagent = pdo_fetch('select *  from ' . tablename('wx_shop_abonus_billp') . ' where billid=:billid and id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':billid' => $id, ':id' => $baid));
		if (empty($aagent)) 
		{
			show_json(0, '未找到区域代理!');
		}
		...
		if (!(empty($member))) 
		{
			...
			if (is_error($result)) 
			{
				pdo_update('wx_shop_abonus_billp', array('reason' => $result['message'], 'status' => -1), array('billid' => $id, 'id' => $baid));
			}
			else 
			{
				pdo_update('wx_shop_abonus_billp', array('reason' => '', 'status' => 1, 'paytime' => time()), array('billid' => $id, 'id' => $baid));
		...
		{
			pdo_update('wx_shop_abonus_billp', array('reason' => '未找到会员', 'status' => -1), array('billid' => $id, 'id' => $baid));
		}
		$aagentcount11 = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_abonus_billp') . ' b left join ' . tablename('wx_shop_member') . ' m on b.openid=m.openid and b.uniacid=m.uniacid  where m.aagenttype=1 and  b.billid=:billid and b.status=1 and b.uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		$aagentcount22 = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_abonus_billp') . ' b left join ' . tablename('wx_shop_member') . ' m on b.openid=m.openid and b.uniacid=m.uniacid  where m.aagenttype=2 and  b.billid=:billid and b.status=1 and b.uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		$aagentcount33 = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_abonus_billp') . ' b left join ' . tablename('wx_shop_member') . ' m on b.openid=m.openid and b.uniacid=m.uniacid  where m.aagenttype=3 and  b.billid=:billid and b.status=1 and b.uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		$aagentcount44 = $aagentcount11 + $aagentcount22 + $aagentcount33;
		$allaagentcount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_abonus_billp') . ' where billid=:billid and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		$full = ($aagentcount11 + $aagentcount22 + $aagentcount33) == $allaagentcount;
		if ($full) 
		{
			pdo_query('update ' . tablename('wx_shop_abonus_bill') . ' set status=2 where id=:id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		}
		if (is_error($result)) 
		{
	...
	public function paymoney_level() 
	{
		...
		if (!(empty($conditions))) 
		{
			$sql = 'update ' . tablename('wx_shop_abonus_billp') . ' b ,' . tablename('wx_shop_member') . ' m set ' . implode(',', $conditions) . ' where b.openid = m.openid and b.uniacid = m.uniacid and m.aagentlevel=' . $level . ' and m.aagenttype=1 and b.billid=' . $id . ' and b.uniacid=' . $_W['uniacid'];
			pdo_query($sql);
		}
		$conditions = array();
		if ($paymoney2 != '') 
		...
		if (!(empty($conditions))) 
		{
			$sql = 'update ' . tablename('wx_shop_abonus_billp') . ' b ,' . tablename('wx_shop_member') . ' m set  ' . implode(',', $conditions) . '  where b.openid = m.openid and b.uniacid = m.uniacid and m.aagentlevel=' . $level . ' and m.aagenttype=2 and b.billid=' . $id . ' and b.uniacid=' . $_W['uniacid'];
			pdo_query($sql);
		}
		$conditions = array();
		if ($paymoney3 != '') 
		...
		if (!(empty($conditions))) 
		{
			$sql = 'update ' . tablename('wx_shop_abonus_billp') . ' b ,' . tablename('wx_shop_member') . ' m set  ' . implode(',', $conditions) . '  where b.openid = m.openid and b.uniacid = m.uniacid and m.aagentlevel=' . $level . ' and b.billid=' . $id . ' and m.aagenttype=3 and b.uniacid=' . $_W['uniacid'];
			pdo_query($sql);
		}
		$totalmoney = pdo_fetch('select sum(paymoney1) as paymoney1, sum(paymoney2) as paymoney2,sum(paymoney3) as paymoney3 from ' . tablename('wx_shop_abonus_billp') . ' where billid=:billid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		$ret = array('bonusmoney_pay1' => $totalmoney['paymoney1'], 'bonusmoney_pay2' => $totalmoney['paymoney2'], 'bonusmoney_pay3' => $totalmoney['paymoney3']);
		pdo_update('wx_shop_abonus_bill', $ret, array('id' => $id, 'uniacid' => $_W['uniacid']));
		show_json(1, $ret);
	}
	public function paymoney_aagent() 
	{
		...
		$baid = intval($_GPC['baid']);
		$partner = pdo_fetch('select *  from ' . tablename('wx_shop_abonus_billp') . ' where billid=:billid and id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':billid' => $id, ':id' => $baid));
		if (empty($partner)) 
		{
			show_json(0, '未找到代理商!');
		}
		pdo_update('wx_shop_abonus_billp', array('paymoney' . $paymoneytype => $value), array('id' => $baid, 'billid' => $id, 'uniacid' => $_W['uniacid']));
		$totalmoney = pdo_fetch('select sum(paymoney1) as paymoney1, sum(paymoney2) as paymoney2,sum(paymoney3) as paymoney3 from ' . tablename('wx_shop_abonus_billp') . ' where billid=:billid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		$ret = array('bonusmoney_pay1' => $totalmoney['paymoney1'], 'bonusmoney_pay2' => $totalmoney['paymoney2'], 'bonusmoney_pay3' => $totalmoney['paymoney3']);
		pdo_update('wx_shop_abonus_bill', $ret, array('id' => $id, 'uniacid' => $_W['uniacid']));
		show_json(1, $ret);
