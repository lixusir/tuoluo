...
class CashierModel extends PluginModel 
{
	...
	public function updateUserSet($data = array(), $cashierid) 
	{
		...
		return pdo_query('UPDATE ' . tablename('wx_shop_cashier_user') . ' SET `set`=:set WHERE `uniacid` = :uniacid AND `id` = :id', array(':uniacid' => $_W['uniacid'], ':id' => $cashierid, ':set' => $set));
	}
	public function payResult($logid, $return_log = false) 
	{
		...
		if ($id != 0) 
		{
			$log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_cashier_pay_log') . ' WHERE `id`=:id and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		}
		else 
		{
			$log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_cashier_pay_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logid));
		}
		if (!(empty($log))) 
		{
	...
	public function categoryAll($status = 1) 
	{
		...
		$item = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_cashier_category') . ' WHERE 1 ' . $condition . '  ORDER BY displayorder desc, id DESC', $params);
		return $item;
	}
	public function categoryOne($id) 
	{
		global $_W;
		$item = pdo_fetch('select * from ' . tablename('wx_shop_cashier_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		return $item;
	}
	public function savaUser(array $params, $diyform = array()) 
	{
		...
		if (!(isset($params['id']))) 
		{
			...
			$params['deleted'] = 0;
			pdo_insert('wx_shop_cashier_user', $params);
			$params['id'] = pdo_insertid();
		}
		else 
		{
			pdo_update('wx_shop_cashier_user', $params, array('id' => $params['id'], 'uniacid' => $params['uniacid']));
		}
		return $params;
	}
	public function userInfo($openid) 
	{
		global $_W;
		$id = intval($openid);
		$sql = 'SELECT * FROM ' . tablename('wx_shop_cashier_user') . ' WHERE uniacid=:uniacid AND deleted=0';
		$params = array(':uniacid' => $_W['uniacid']);
		if ($id == 0) 
		{
	...
	protected function sendNotice($notice, $tag, $datas) 
	{
		...
		if (!(empty($notice[$tag]))) 
		{
			$advanced_template = pdo_fetch('select * from ' . tablename('wx_shop_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $notice[$tag], ':uniacid' => $_W['uniacid']));
	...
	public function createOrder(array $array, $return = NULL, $can_sale = true) 
	{
		...
		$res = pdo_insert('wx_shop_cashier_pay_log', $data);
		if (!($res)) 
		{
		...
		if (!(empty($usecoupon))) 
		{
			pdo_update('wx_shop_coupon_data', array('used' => 1, 'usetime' => $data['createtime'], 'ordersn' => $data['logno']), array('id' => $array['usecoupon']));
		}
		if ($return !== NULL) 
		{
			return $data;
	...
	public function pay($data, $auth_code = NULL) 
	{
		...
		if (($data['money'] <= 0) || ($data['paytype'] == 3)) 
		{
			...
			$data['paytime'] = time();
			pdo_update('wx_shop_cashier_pay_log', $data, array('id' => $data['id']));
	...
	public function refund($id) 
	{
		global $_W;
		$id = (int) $id;
		$pay_log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_cashier_pay_log') . ' WHERE uniacid=:uniacid AND id=:id', array(':uniacid' => $_W['uniacid'], ':id' => $id));
		if ($pay_log['status'] != 1) 
		{
		...
		pdo_update('wx_shop_cashier_pay_log', array('status' => -1, 'refundsn' => $out_trade_no, 'refunduser' => $refunduser), array('uniacid' => $_W['uniacid'], 'id' => $id));
		...
		{
			pdo_update('wx_shop_order', array('status' => -1), array('uniacid' => $_W['uniacid'], 'id' => $pay_log['orderid']));
		}
		return $res;
	}
	public function updateOrder($log) 
	{
		...
		{
			pdo_update('wx_shop_cashier_pay_log', array('status' => -1), array('uniacid' => $_W['uniacid'], 'id' => $log['id']));
			return false;
		}
		if (($res['total_fee'] == round($realmoney * 100, 2)) || ($res['total_amount'] == round($realmoney, 2))) 
		{
			$log = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_cashier_pay_log') . ' WHERE `id`=:id and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $log['id']));
			if (empty($log['status'])) 
			{
	...
	public function paySuccess($log, $user) 
	{
		...
		pdo_update('wx_shop_cashier_pay_log', array('openid' => $log['openid'], 'payopenid' => $log['openid'], 'money' => $log['money'], 'status' => 1, 'paytime' => (0 < $log['paytime'] ? $log['paytime'] : time()), 'coupon' => $coupon), array('id' => $log['id']));
		...
		if (!(empty($log['orderid']))) 
		{
			...
			pdo_update('wx_shop_order', array('status' => 3, 'paytype' => $paytype, 'paytime' => time(), 'sendtime' => time(), 'finishtime' => time()), array('id' => $log['orderid'], 'uniacid' => $_W['uniacid']));
		...
		{
			$operator = pdo_fetch('select * from ' . tablename('wx_shop_cashier_operator') . ' WHERE id=:id AND cashierid=:cashierid limit 1', array(':id' => $log['operatorid'], ':cashierid' => $log['cashierid']));
		}
		if ($operator) 
		{
		...
		if (!(empty($log['openid']))) 
		{
			...
			{
				pdo_update('wx_shop_cashier_pay_log', array('present_credit1' => $present_credit1), array('id' => $log['id']));
			}
		}
	...
	public function becomeDown($openid, $manageopenid) 
	{
		...
		if ($member) 
		{
			if (empty($member['isagent']) && empty($member['agentid'])) 
			{
				...
				pdo_query('UPDATE ' . tablename('wx_shop_member') . ' SET agentid=' . $store_member['id'] . ' WHERE `id` = :id', array(':id' => $member['id']));
			}
		}
	}
	...
	public function getOrderMoney($starttime = 0, $endtime = 0) 
	{
		...
		{
			$money = (double) pdo_fetchcolumn('SELECT SUM(money+deduction) FROM ' . tablename('wx_shop_cashier_pay_log') . ' WHERE uniacid=:uniacid AND status=1 AND cashierid=:cashierid ', array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid']));
		}
		else 
		{
			$money = (double) pdo_fetchcolumn('SELECT SUM(money+deduction) FROM ' . tablename('wx_shop_cashier_pay_log') . ' WHERE uniacid=:uniacid AND status=1 AND cashierid=:cashierid AND createtime BETWEEN :starttime AND :endtime', array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid'], ':starttime' => $starttime, ':endtime' => $endtime));
		}
		return $money;
	}
	...
	public function createGoodsOrder($goods, $openid = '') 
	{
		...
		foreach ($goods as $g ) 
		{
			...
			if (!(empty($optionid))) 
			{
				$option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual`,weight from ' . tablename('wx_shop_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid, ':id' => $optionid));
				if (!(empty($option))) 
				{
					if ($option['stock'] != -1) 
					{
	...
	public function goodsCalculate($selfgoods = array(), $shopgoods = array(), $params, $return = false) 
	{
		...
		if (!(empty($selfgoods))) 
		{
			foreach ($selfgoods as $key => $val ) 
			{
				...
				$g = pdo_get('wx_shop_cashier_goods', array('cashierid' => $data['cashierid'], 'id' => $data['goodsid']));
				if (($g['total'] < $data['total']) && ($g['total'] != -1)) 
				{
					pdo_delete('wx_shop_cashier_pay_log_goods', array('cashierid' => $data['cashierid'], 'logid' => $data['logid']));
					return array('res' => error('-101', $g['title'] . ' 库存不足, 剩余库存 ' . $g['total']));
				}
				pdo_insert('wx_shop_cashier_pay_log_goods', $data);
			}
		}
		if (!(empty($shopgoods))) 
		{
		...
		{
			pdo_update('wx_shop_cashier_pay_log', $order, array('id' => $order['id']));
			if ($return) 
			{
				return $order;
			}
	...
	public function setStocks($orderid = 0) 
	{
		...
		$order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status,isparent from ' . tablename('wx_shop_order') . ' where id=:id limit 1', array(':id' => $orderid));
		$param = array();
		...
		$goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.credit,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal from ' . tablename('wx_shop_order_goods') . ' og ' . ' left join ' . tablename('wx_shop_goods') . ' g on g.id=og.goodsid ' . ' where ' . $condition . ' and og.uniacid=:uniacid ', $param);
		foreach ($goods as $g ) 
		{
			$stocktype = -1;
			if (!(empty($stocktype))) 
			...
			if (1 <= $order['status']) 
			{
				if ($g['totalcnf'] != 1) 
				{
					pdo_update('wx_shop_goods', array('sales' => $g['sales'] - $g['total']), array('uniacid' => $_W['uniacid'], 'id' => $g['goodsid']));
				}
	...
	public function setSelfGoodsStocks($logid = 0) 
	{
		...
		$goods = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_cashier_pay_log_goods') . ' WHERE cashierid=:cashierid AND logid=:logid', array(':cashierid' => $_W['cashierid'], ':logid' => $logid));
		foreach ($goods as $g ) 
		{
			pdo_query('UPDATE ' . tablename('wx_shop_cashier_goods') . ' SET total=total-' . $g['total'] . ' WHERE cashierid=:cashierid AND id=:id AND total<>-1', array(':cashierid' => $_W['cashierid'], ':id' => $g['goodsid']));
		}
	}
	public function querycoupons($kwd = '') 
	{
		...
		$ds = pdo_fetchall('SELECT id,couponname as title , thumb FROM ' . tablename('wx_shop_coupon') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
	...
	public function refundWechat($openid, $out_trade_no, $out_refund_no, $totalmoney, $refundmoney = 0, $app = false, $refund_account = false) 
	{
		...
		if (is_array($certs)) 
		{
			...
			$certfile = IA_ROOT . '/addons/wx_shop/cert/' . random(128);
			file_put_contents($certfile, $certs['cert']);
			$keyfile = IA_ROOT . '/addons/wx_shop/cert/' . random(128);
			file_put_contents($keyfile, $certs['key']);
			$rootfile = IA_ROOT . '/addons/wx_shop/cert/' . random(128);
