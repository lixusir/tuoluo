...
class User_WxShopPage extends PluginWebPage
{
	public function main() 
	{
		...
		$sql = 'select * from ' . tablename('wx_shop_cashier_user') . ' where uniacid=:uniacid AND deleted=0 ' . $condition . ' ORDER BY id desc limit ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_cashier_user') . ' where uniacid=:uniacid AND deleted=0 ' . $condition, $params);
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$store = pdo_fetch('SELECT id,storeid FROM ' . tablename('wx_shop_cashier_user') . ' WHERE uniacid=:uniacid AND deleted=0 LIMIT 1', array(':uniacid' => $_W['uniacid']));
			$merch = pdo_fetch('SELECT id,merchid FROM ' . tablename('wx_shop_cashier_user') . ' WHERE uniacid=:uniacid AND deleted=0 LIMIT 1', array(':uniacid' => $_W['uniacid']));
			if ($id) 
			{
		...
		{
			$store = pdo_fetch('SELECT id,storename FROM ' . tablename('wx_shop_store') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['storeid']));
		}
		$merch = p('merch');
		if ($merch) 
		{
			...
			{
				$merchres = pdo_fetch('select id,merchname from ' . tablename('wx_shop_merch_user') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['merchid'], ':uniacid' => $_W['uniacid']));
			}
		}
		$wechatpay = json_decode($item['wechatpay'], true);
		$alipay = json_decode($item['alipay'], true);
		$order_template = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member_printer_template') . ' WHERE uniacid=:uniacid  AND merchid=0', array(':uniacid' => $_W['uniacid']));
		$order_printer_array = array();
		if (!(empty($userset['printer']))) 
		{
			$order_printer_array = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_member_printer') . ' WHERE uniacid=:uniacid AND id IN (' . $userset['printer'] . ')', array(':uniacid' => $_W['uniacid']));
		}
		if (!(empty($item['couponid']))) 
		{
			$coupon = pdo_fetchall('SELECT id,couponname as title , thumb  FROM ' . tablename('wx_shop_coupon') . ' WHERE uniacid = ' . $_W['uniacid'] . ' and id in (' . $item['couponid'] . ')');
		}
		include $this->template();
	}
	public function status() 
	{
		...
		$items = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_cashier_user') . ' WHERE id in(' . $id . ') AND deleted=0 AND uniacid=' . $_W['uniacid']);
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_cashier_user', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
	...
	public function delete() 
	{
		...
		foreach ($items as $item ) 
		{
			pdo_update('wx_shop_cashier_user', array('deleted' => 1), array('id' => $item['id']));
