...
class Login_WxShopPage extends CashierWebPage
{
	public function main() 
	{
		...
		{
			$auth_code = authcode(base64_decode($_GPC['auth_code']), 'DECODE', 'wx_shop_cashier');
			if ($auth_code) 
			{
	...
	public function login($username, $password, $salt = NULL, $is_operator = 0) 
	{
		...
		if ($is_operator == 0) 
		{
			$user = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_cashier_user') . ' WHERE username=:username AND uniacid=:uniacid AND status=1 AND deleted=0 LIMIT 1', array(':username' => $username, ':uniacid' => $_W['uniacid']));
		}
		else 
		{
			$operator = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_cashier_operator') . ' WHERE username=:username AND uniacid=:uniacid LIMIT 1', array(':username' => $username, ':uniacid' => $_W['uniacid']));
			if (empty($operator)) 
			{
				show_json(0, '用户名不存在!');
			}
			...
			$user = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_cashier_user') . ' WHERE id=:id AND uniacid=:uniacid AND status=1 AND deleted=0 LIMIT 1', array(':id' => $operator['cashierid'], ':uniacid' => $_W['uniacid']));
			if (!(empty($user))) 
			{
