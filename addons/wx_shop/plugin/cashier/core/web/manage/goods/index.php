...
class Index_WxShopPage extends CashierWebPage
{
	public function main() 
	{
		...
		$cate = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_cashier_goods_category') . ' WHERE uniacid=:uniacid and cashierid=:cashierid ORDER BY displayorder desc, id DESC', array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid']));
		...
		if ((($cate != 'shop') && empty($cate)) || ($cate == 'shop') || empty($cate)) 
		{
			...
			if (!(empty($goodsmanage['goods_ids']))) 
			{
				...
				foreach ($goods as $k => $g ) 
				{
					...
					{
						$options = pdo_fetchall('select * from ' . tablename('wx_shop_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $g['id'], ':uniacid' => $_W['uniacid']), 'id');
						if (!(empty($options))) 
						{
	...
	protected function getSelfGoods($where = '', $params = array()) 
	{
		...
		$selfgoods = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_cashier_goods') . ' WHERE cashierid=:cashierid ' . $where . ' AND status=1 AND total<>0', $params);
		return $selfgoods;
	}
	public function query() 
	{
		...
		$ds = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_member') . ' WHERE 1 ' . $condition . ' order by id asc', $params);
		if ($_GPC['suggest']) 
		{
	...
	public function set_password() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($info['salt']) && empty($info['pwd'])) 
			{
				...
				pdo_update('wx_shop_member', array('pwd' => $pwd, 'salt' => $salt), array('id' => $info['id']));
				show_json(1, $info);
			}
		}
		show_json(0);
	...
	public function goodsquery() 
	{
		...
		$ds = pdo_fetchall('SELECT id,uniacid,title,subtitle,shorttitle,thumb,share_icon FROM ' . tablename('wx_shop_goods') . ' WHERE uniacid=:uniacid AND merchid=:merchid AND cashier=1 ' . $where, $params);
