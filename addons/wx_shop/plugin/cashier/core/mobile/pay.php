...
class Pay_WxShopPage extends CashierMobilePage
{
	public $log;
	...
	public function main() 
	{
		...
		{
			$id = $_GPC['id'];
			$qrid = authcode(base64_decode($id), 'DECODE', 'wx_shop_cashier');
			if (!(empty($qrid))) 
			{
				$item = pdo_get('wx_shop_cashier_qrcode', array('uniacid' => $_W['uniacid'], 'cashierid' => $_W['cashierid'], 'id' => $qrid));
	...
	protected function paytype1() 
	{
		...
		if (is_array($config)) 
		{
			$config['return_url'] = $_W['siteroot'] . 'addons/wx_shop/payment/alipay/return.php';
		}
		else 
		{
			show_json(0);
