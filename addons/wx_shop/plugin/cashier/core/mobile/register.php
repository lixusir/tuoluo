...
class Register_WxShopPage extends CashierMobilePage
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ($id) 
			{
				$user_totle = pdo_fetch('SELECT id FROM ' . tablename('wx_shop_cashier_user') . ' WHERE username=:username AND uniacid=:uniacid AND status=1 LIMIT 1', array(':username' => $params['username'], ':uniacid' => $_W['uniacid']));
