...
require WX_SHOP_PLUGIN . 'cashier/core/inc/mobile_cashier.php';
class Manage_WxShopPage extends CashierMobilePage
{
	...
	public function get_list() 
	{
		...
		$list = pdo_fetchall('SELECT *,money+deduction money,FROM_UNIXTIME(paytime) as paytime FROM ' . tablename('wx_shop_cashier_pay_log') . ' WHERE uniacid=:uniacid AND status=1 AND cashierid=:cashierid AND createtime BETWEEN :starttime AND :endtime ORDER BY id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid'], ':starttime' => $starttime, ':endtime' => $endtime));
		$total = pdo_fetch('SELECT COUNT(*) total,SUM(money+deduction) money FROM ' . tablename('wx_shop_cashier_pay_log') . ' WHERE uniacid=:uniacid AND status=1 AND cashierid=:cashierid AND createtime BETWEEN :starttime AND :endtime', array(':uniacid' => $_W['uniacid'], ':cashierid' => $_W['cashierid'], ':starttime' => $starttime, ':endtime' => $endtime));
