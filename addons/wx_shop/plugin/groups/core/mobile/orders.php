	...
	public function confirm()
	{
		...
		if ($_W['ispost']) {
			...
			if ((0 < intval($_GPC['aid'])) && !$isverify) {
				$order_address = pdo_fetch('select * from ' . tablename('wx_shop_member_address') . ' where id=:id and openid=:openid and uniacid=:uniacid   limit 1', array(':uniacid' => $uniacid, ':openid' => $openid, ':id' => intval($_GPC['aid'])));
