...
class Team_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$teams = pdo_fetchall('SELECT o.* FROM ' . tablename('wx_shop_groups_order') . " as o\r\n\t\t\t\t\tWHERE " . $condition . ' ORDER BY o.createtime DESC limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);

		foreach ($teams as $key => $value) {
			$good = pdo_fetch('select title from ' . tablename('wx_shop_groups_goods') . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $value['goodid']);
			$teams[$key]['title'] = $good['title'];
			$teams[$key]['num'] = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('wx_shop_groups_order') . " as o\r\n\t\t\t\t\t\t\t\t\t\tWHERE o.status > 0 and o.deleted = 0 and o.uniacid = :uniacid and o.teamid = :teamid ", array(':uniacid' => $_W['uniacid'], ':teamid' => $value['teamid']));
	...
	public function detail()
	{
		...
		global $_GPC;
		$teamid = $_GPC['teamid'];
		$teaminfo = pdo_fetch('SELECT o.*,g.id as gid,g.title,g.thumb FROM ' . tablename('wx_shop_groups_order') . " as o\r\n\t\t\t\t\tleft join " . tablename('wx_shop_groups_goods') . " as g on g.id = o.goodid\r\n\t\t\t\t\tWHERE o.teamid =:teamid and o.uniacid=:uniacid and o.is_team = :is_team and heads = :heads", array(':uniacid' => $_W['uniacid'], ':teamid' => $teamid, ':is_team' => 1, ':heads' => 1));
		$total = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_groups_order') . ' as o where o.teamid =:teamid and o.uniacid=:uniacid and o.is_team = :is_team and status > :status', array(':uniacid' => $_W['uniacid'], ':teamid' => $teamid, ':is_team' => 1, ':status' => 0));
		$orders = pdo_fetchall('SELECT o.*,g.thumb FROM ' . tablename('wx_shop_groups_order') . " as o\r\n\t\t\t\t\tleft join " . tablename('wx_shop_groups_goods') . " as g on g.id = o.goodid\r\n\t\t\t\t\tWHERE o.teamid =:teamid and o.uniacid=:uniacid and o.is_team = :is_team and o.status != :status", array(':uniacid' => $_W['uniacid'], ':teamid' => $teamid, ':is_team' => 1, ':status' => 0));
		...
		$dispatch = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_dispatch') . ' WHERE id = :id and uniacid=:uniacid', array(':id' => $teaminfo['dispatchid'], ':uniacid' => $_W['uniacid']));
	...
	public function group()
	{
		...
		foreach ($teamid as $key => $value) {
			$order = pdo_fetch('select id,uniacid,groupnum,goodid,endtime from ' . tablename('wx_shop_groups_order') . "\r\n\t\t\t\t\twhere teamid = :teamid and heads = 1 and success = 0 and uniacid = :uniacid ", array(':teamid' => $value, ':uniacid' => $uniacid));
			$order_count = pdo_fetchcolumn('select COUNT(1) from ' . tablename('wx_shop_groups_order') . ' where teamid = :teamid and status = 1 and success = 0 and uniacid = :uniacid ', array(':teamid' => $value, ':uniacid' => $uniacid));
			...
			while ($i < $num) {
				...
				$order_insert = pdo_insert('wx_shop_groups_order', $system_order_data);
				++$i;
			}

			pdo_update('wx_shop_groups_order', array('success' => 1), array('teamid' => $value, 'uniacid' => $uniacid, 'status' => 1));
			pdo_update('wx_shop_groups_order', array('status' => -1), array('teamid' => $value, 'uniacid' => $uniacid, 'status' => 0));
