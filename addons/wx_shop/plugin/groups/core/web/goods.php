...
class Goods_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'SELECT c.*,g.* FROM ' . tablename('wx_shop_groups_goods') . " AS g\r\n\t\t\t\tLEFT JOIN " . tablename('wx_shop_groups_category') . " AS c ON g.category = c.id\r\n\t\t\t\twhere  1 = 1 and " . $condition . ' ORDER BY g.displayorder DESC,g.id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_groups_goods') . ' AS g where 1 and ' . $condition, $params);
		$pager = pagination2($total, $pindex, $psize);
		$category = pdo_fetchall('select id,name,thumb from ' . tablename('wx_shop_groups_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']), 'id');
	...
	protected function post()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT g.*,c.name as catename FROM ' . tablename('wx_shop_groups_goods') . " as g\r\n\t\t\t\tleft join " . tablename('wx_shop_groups_category') . " as c on c.id = g.category\r\n\t\t\t\tWHERE g.id =:id and g.uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $id));
		$category = pdo_fetchall('select id,name,thumb from ' . tablename('wx_shop_groups_category') . ' where uniacid=:uniacid order by displayorder desc', array(':uniacid' => $_W['uniacid']));
		...
		$dispatch_data = pdo_fetchall('select * from ' . tablename('wx_shop_dispatch') . ' where uniacid=:uniacid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				$goods_update = pdo_update('wx_shop_groups_goods', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				$goods_insert = pdo_insert('wx_shop_groups_goods', $data);
				...
				if ($gid) {
					pdo_update('wx_shop_goods', array('groupstype' => 1), array('id' => $gid, 'uniacid' => $_W['uniacid']));
				}
	...
	public function total()
	{
		...
		$total = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_groups_goods') . ' where ' . $condition . ' ', $params);
	...
	public function delete1()
	{
		...
		$items = pdo_fetchall('SELECT id,title,gid FROM ' . tablename('wx_shop_groups_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('wx_shop_groups_goods', array('id' => $item['id']));
	...
	public function restore()
	{
		...
		foreach ($items as $item) {
			pdo_update('wx_shop_groups_goods', array('deleted' => 0, 'status' => 0), array('id' => $item['id']));

			if (intval($item['gid'])) {
				pdo_update('wx_shop_goods', array('groupstype' => 1), array('id' => $item['gid'], 'uniacid' => $_W['uniacid']));
			}
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
			$delete_update = pdo_update('wx_shop_groups_goods', array('deleted' => 1, 'status' => 0), array('id' => $id, 'uniacid' => $_W['uniacid']));
			...
			if (intval($item['gid'])) {
				pdo_update('wx_shop_goods', array('groupstype' => 0), array('id' => $item['gid'], 'uniacid' => $_W['uniacid']));
			}
	...
	public function status()
	{
		...
		$items = pdo_fetchall('SELECT id,status FROM ' . tablename('wx_shop_groups_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			$status_update = pdo_update('wx_shop_groups_goods', array('status' => $status), array('id' => $item['id']));
	...
	public function property()
	{
		...
		if (in_array($type, array('status', 'displayorder'))) {
			...
			$property_update = pdo_update('wx_shop_groups_goods', array($type => $value), array('id' => $id, 'uniacid' => $_W['uniacid']));
