...
class Exhelper_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		global $_GPC;
		$tab = $_GPC['tab'];
		$sys = pdo_fetch('select * from ' . tablename('wx_shop_exhelper_sys') . ' where uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (!empty($port)) {
				if (empty($sys)) {
					pdo_insert('wx_shop_exhelper_sys', array('port' => $port, 'ip' => $ip, 'uniacid' => $_W['uniacid']));
				}
				else {
					pdo_update('wx_shop_exhelper_sys', array('port' => $port, 'ip' => $ip), array('uniacid' => $_W['uniacid']));
				}
	...
	public function short()
	{
		...
		$sql = 'SELECT id,title,shorttitle,status FROM ' . tablename('wx_shop_groups_goods') . ' where  1 and ' . $condition . ' ORDER BY  shorttitle desc, status desc, id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_groups_goods') . ' where 1 and ' . $condition, $params);
	...
	public function shortedit()
	{
		...
		$goods = pdo_fetchall('SELECT id,title FROM ' . tablename('wx_shop_groups_goods') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($goods as $good) {
			pdo_update('wx_shop_groups_goods', array('shorttitle' => $shorttitle), array('id' => $good['id'], 'uniacid' => $_W['uniacid']));
	...
	protected function expresspost()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('select * from ' . tablename('wx_shop_exhelper_express') . ' where id=:id and type=:type and uniacid=:uniacid limit 1', array(':id' => $id, ':type' => $type, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				pdo_update('wx_shop_exhelper_express', $data, array('id' => $id));
				plog('groups.exhelper.expressedit', '修改快递单信息 ID: ' . $id);
			}
			else {
				$data['uniacid'] = $_W['uniacid'];
				pdo_insert('wx_shop_exhelper_express', $data);
				$id = pdo_insertid();
				plog('groups.exhelper.expressadd', '添加快递单模板 ID: ' . $id);
			}

			if (!empty($data['isdefault'])) {
				pdo_update('wx_shop_exhelper_express', array('isdefault' => 0), array('type' => 1, 'uniacid' => $_W['uniacid']));
				pdo_update('wx_shop_exhelper_express', array('isdefault' => 1), array('type' => 1, 'id' => $id));
			}
	...
	public function invoicepost()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($data['isdefault'])) {
				pdo_update('wx_shop_exhelper_express', array('isdefault' => 0), array('type' => 2, 'uniacid' => $_W['uniacid']));
				pdo_update('wx_shop_exhelper_express', array('isdefault' => 1), array('type' => 2, 'id' => $id));
			}
	...
	public function sender()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('wx_shop_exhelper_senduser') . ' where  1 and ' . $condition . ' ORDER BY isdefault desc,id DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_exhelper_senduser') . ' where 1 and ' . $condition, $params);
		$pager = pagination2($total, $pindex, $psize);
		include $this->template();
	}
	...
	protected function senderpost()
	{
		...
		if (!empty($id)) {
			$item = pdo_fetch('select * from ' . tablename('wx_shop_exhelper_senduser') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				pdo_update('wx_shop_exhelper_senduser', $data, array('id' => $id));
				plog('groups.exhelper.senderedit', '修改发件人模板 ID: ' . $id);
			}
			else {
				pdo_insert('wx_shop_exhelper_senduser', $data);
				$id = pdo_insertid();
				plog('groups.exhelper.senderadd', '添加发件人模板 ID: ' . $id);
			}

			if (!empty($data['isdefault'])) {
				pdo_update('wx_shop_exhelper_senduser', array('isdefault' => 0), array('uniacid' => $_W['uniacid']));
				pdo_update('wx_shop_exhelper_senduser', array('isdefault' => 1), array('id' => $id));
			}
	...
	public function sendersetdefault()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,sendername FROM ' . tablename('wx_shop_exhelper_senduser') . ' WHERE id = \'' . $id . '\' AND uniacid=' . $_W['uniacid'] . '');
	...
	public function senderdelete()
	{
		...
		$items = pdo_fetchall('SELECT id,sendername,sendertel,sendercity,senderaddress FROM ' . tablename('wx_shop_exhelper_senduser') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($items as $item) {
			pdo_delete('wx_shop_exhelper_senduser', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function single()
	{
		...
		$printset = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exhelper_sys') . ' WHERE uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
	...
	public function getdata()
	{
		...
		if ($_W['ispost']) {
			...
			$sql = 'select o.* ,a.realname ,m.nickname, d.dispatchname,m.nickname,r.refundstatus from ' . tablename('wx_shop_groups_order') . ' o' . ' left join ' . tablename('wx_shop_groups_order_refund') . ' r on r.orderid=o.id and ifnull(r.refundstatus,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_dispatch') . ' d on d.id = o.dispatchid ' . ' where ' . $condition . ' ' . $statuscondition . ' ORDER BY o.createtime DESC,o.status DESC  ';
			...
			foreach ($orders as $key => $order) {
				...
				if (!is_array($order_address)) {
					$member_address = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_address') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => $order['addressid'], ':uniacid' => $_W['uniacid']));
	...
	public function getorder()
	{
		...
		if ($_W['ispost']) {
			...
			$sql = 'select o.* , a.realname,a.mobile,a.province,a.city,a.area, d.dispatchname,m.nickname,r.refundstatus from ' . tablename('wx_shop_groups_order') . ' o' . ' left join ' . tablename('wx_shop_groups_order_refund') . ' r on r.orderid=o.id and ifnull(r.refundstatus,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_dispatch') . ' d on d.id = o.dispatchid ' . ' where o.id in ( ' . implode(',', $arr) . ') and o.uniacid=' . $_W['uniacid'] . ' and m.uniacid=' . $_W['uniacid'] . ' ORDER BY o.createtime DESC,o.status DESC  ';
			...
			foreach ($list as &$value) {
				...
				if (!is_array($addressa)) {
					$member_address = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_member_address') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => $value['addressid'], ':uniacid' => $_W['uniacid']));
			...
			unset($value);
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_groups_order') . ' o ' . ' left join ' . tablename('wx_shop_groups_order_refund') . ' r on r.orderid=o.id and ifnull(r.refundstatus,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' WHERE o.id in ( ' . implode(',', $arr) . ') and o.uniacid=' . $_W['uniacid'], $paras);
			$totalmoney = pdo_fetchcolumn('SELECT sum(o.price) FROM ' . tablename('wx_shop_groups_order') . ' o ' . ' left join ' . tablename('wx_shop_groups_order_refund') . ' r on r.orderid=o.id and ifnull(r.refundstatus,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid  ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' WHERE o.id in ( ' . implode(',', $arr) . ') and o.uniacid=' . $_W['uniacid'], $paras);
			$address = false;
		...
		if ($_W['ispost']) {
			...
			if (is_array($ordersns)) {
				...
				foreach ($ordersns as $ordersn) {
					pdo_update('wx_shop_order', array('address_send' => $address_send), array('ordersn' => $ordersn));
				}
	...
	public function getprintTemp()
	{
		...
		if ($_W['ispost']) {
			...
			$tempSender = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exhelper_senduser') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => $printUserId, ':uniacid' => $_W['uniacid']));
			$expTemp = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => $printTempId, ':uniacid' => $_W['uniacid']));
	...
	public function changestate()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($arr as $i => $data) {
				...
				$ordergood = pdo_fetch('SELECT id,goodid,printstate,printstate2 FROM ' . tablename('wx_shop_groups_order') . ' WHERE goodid=:goodid and id=:orderid and uniacid=:uniacid limit 1', array(':orderid' => $orderid, ':goodid' => $ordergoodid, ':uniacid' => $_W['uniacid']));

				if ($type == 1) {
					pdo_update('wx_shop_groups_order', array('printstate' => $ordergood['printstate'] + 1), array('id' => $ordergood['id']));
					$orderprint = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . ' WHERE id=:orderid and printstate=0 and uniacid= :uniacid ', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
					$printstatenum = $ordergood['printstate'] + 1;
				}
				else {
					if ($type == 2) {
						pdo_update('wx_shop_groups_order', array('printstate2' => $ordergood['printstate2'] + 1), array('id' => $ordergood['id']));
						$orderprint = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . ' WHERE id=:orderid and printstate2=0 and uniacid= :uniacid ', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
	...
	public function getorderinfo()
	{
		...
		if ($_W['ispost']) {
			...
			$printTemp = pdo_fetch('SELECT id,type,expressname,express,expresscom FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE id=:id and type=:type and uniacid=:uniacid limit 1', array(':id' => $temp_express, ':type' => 1, ':uniacid' => $_W['uniacid']));
	...
	public function dosend()
	{
		...
		if ($_W['ispost']) {
			...
			$orderinfo = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_groups_order') . ' WHERE id=:orderid and status>-1 and uniacid=:uniacid limit 1', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
			...
			if ($orderinfo['status'] == 1) {
				pdo_update('wx_shop_groups_order', array('express' => trim($express), 'expresssn' => trim($expresssn), 'expresscom' => trim($expresscom), 'sendtime' => time(), 'status' => 2), array('id' => $orderid));

				if (!empty($orderinfo['refundid'])) {
					$refund = pdo_fetch('select * from ' . tablename('wx_shop_groups_order_refund') . ' where id=:id limit 1', array(':id' => $orderinfo['refundid']));

					if (!empty($refund)) {
						pdo_update('wx_shop_groups_order_refund', array('status' => -1), array('id' => $orderinfo['refundid']));
						pdo_update('wx_shop_groups_order', array('refundid' => 0), array('id' => $orderinfo['id']));
					}
				}
	...
	public function getdatabatch()
	{
		...
		if ($_W['ispost']) {
			...
			$sql = 'select o.* ,a.realname ,m.nickname, d.dispatchname,m.nickname,r.refundstatus from ' . tablename('wx_shop_groups_order') . ' o' . ' left join ' . tablename('wx_shop_groups_goods') . ' g on g.id = o.goodid  ' . ' left join ' . tablename('wx_shop_groups_order_refund') . ' r on r.orderid=o.id and ifnull(r.refundstatus,-1)<>-1' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=o.openid ' . ' left join ' . tablename('wx_shop_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('wx_shop_dispatch') . ' d on d.id = o.dispatchid ' . ' where ' . $condition . ' ' . $statuscondition . ' ORDER BY o.createtime DESC  ';
			$orders = pdo_fetchall($sql, $paras);
			$totalmoney = 0;
			...
			foreach ($orders as $i => $order) {
				...
				if (!is_array($order_address)) {
					$member_address = pdo_fetch('SELECT a.*,m.nickname FROM ' . tablename('wx_shop_member_address') . " a\r\n                    left join " . tablename('wx_shop_member') . " m on m.openid=a.openid\r\n                    WHERE a.id=:id and a.uniacid=:uniacid limit 1", array(':id' => $order['addressid'], ':uniacid' => $_W['uniacid']));
	...
	public function changestatebatch()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($arr as $i => $data) {
				$orderid = $data['orderid'];
				$ordergoodid = $data['ordergoodid'];
				$ordergood = pdo_fetch('SELECT id,goodid,printstate,printstate2 FROM ' . tablename('wx_shop_groups_order') . ' WHERE goodid=:goodsid and id=:orderid and uniacid=:uniacid limit 1', array(':orderid' => $orderid, ':goodsid' => $ordergoodid, ':uniacid' => $_W['uniacid']));
