...
class Refund_WxShopPage extends PluginWebPage
{
	protected function opData()
	{
		...
		if (!empty($refundid)) {
			$refund = pdo_fetch('select * from ' . tablename('wx_shop_groups_order_refund') . ' where id=:id limit 1', array(':id' => $refundid));
	...
	public function main()
	{
		...
		try {
			...
			$total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order_refund') . " as ore\r\n                left join " . tablename('wx_shop_groups_order') . " as o on o.id = ore.orderid\r\n\t\t\t\tright join " . tablename('wx_shop_groups_goods') . " as g on g.id = o.goodid\r\n\t\t\t\tright join " . tablename('wx_shop_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid\r\n\t\t\t\tleft join " . tablename('wx_shop_member_address') . " a on a.id=ore.refundaddressid\r\n\t\t\t\tright join " . tablename('wx_shop_groups_category') . " as c on c.id = g.category\r\n\t\t\t\tWHERE 1 " . $condition . ' ', $params);
	...
	public function submit()
	{
		...
		if ($_W['ispost']) {
			...
			if ($item['status'] == 3) {
				pdo_update('wx_shop_groups_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			...
			if (empty($refund['refundno'])) {
				...
				pdo_update('wx_shop_groups_order_refund', array('refundno' => $refund['refundno']), array('id' => $refund['id']));
			}
			...
			if ($refundstatus == 0) {
				...
				if ($raid != 0) {
					$raddress = pdo_fetch('select * from ' . tablename('wx_shop_refund_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $raid, ':uniacid' => $uniacid));
				}
				else {
					$raddress = pdo_fetch('select * from ' . tablename('wx_shop_refund_address') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $raid, ':uniacid' => $uniacid));
				}
				...
				if (empty($raddress)) {
					$raddress = pdo_fetch('select * from ' . tablename('wx_shop_refund_address') . ' where uniacid=:uniacid order by id desc limit 1', array(':uniacid' => $uniacid));
				}
				...
				pdo_update('wx_shop_groups_order_refund', $change_refund, array('id' => $item['refundid']));
				...
				pdo_update('wx_shop_groups_order_refund', $refund_data, array('id' => $item['refundid'], 'uniacid' => $uniacid));
				$order_data = array();
				$order_data['refundstate'] = 0;
				$order_data['status'] = 3;
				$order_data['refundtime'] = $time;
				pdo_update('wx_shop_groups_order', $order_data, array('id' => $item['id'], 'uniacid' => $uniacid));
				p('groups')->sendTeamMessage($item['id'], true);
			}
				...
				$order = pdo_fetch('SELECT id,orderno,credit,creditmoney,price,freight,status,pay_type,is_team,apppay FROM ' . tablename('wx_shop_groups_order') . "\r\n                    WHERE id = :orderid and uniacid=:uniacid", array(':orderid' => $item['id'], ':uniacid' => $uniacid));
				...
				pdo_update('wx_shop_groups_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));
				$sales = pdo_fetch('select id,sales,stock from ' . tablename('wx_shop_groups_goods') . ' where id = :id and uniacid = :uniacid ', array(':id' => $item['goodid'], ':uniacid' => $uniacid));
				pdo_update('wx_shop_groups_goods', array('sales' => $sales['sales'] - 1, 'stock' => $sales['stock'] + 1), array('id' => $sales['id'], 'uniacid' => $uniacid));
				plog('groups.refund', '订单退款 ID: ' . $item['id'] . ' 订单号: ' . $item['orderno']);
			}
			else if ($refundstatus == -1) {
				pdo_update('wx_shop_groups_order_refund', array('reply' => $refundcontent, 'refundstatus' => -1, 'endtime' => $time), array('id' => $item['refundid']));
				p('groups')->sendTeamMessage($item['id'], true);
				plog('groups.refund', '订单退款拒绝 ID: ' . $item['id'] . ' 订单号: ' . $item['orderno'] . ' 原因: ' . $refundcontent);
				pdo_update('wx_shop_groups_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid));
			}
			else {
				if ($refundstatus == 2) {
					$refundtype = 2;
		...
		$refund_address = pdo_fetchall('select * from ' . tablename('wx_shop_refund_address') . ' where uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
