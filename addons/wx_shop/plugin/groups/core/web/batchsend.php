	...
	public function main()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($rows as $rownum => $col) {
				...
				$sql = 'select id,status,refundid from ' . tablename('wx_shop_groups_order') . ' where orderno=:orderno and uniacid=:uniacid ';
				...
				if (!empty($order)) {
					...
					if (!empty($order['refundid'])) {
						$refund = pdo_fetch('select id from ' . tablename('wx_shop_groups_order_refund') . ' where id=:id limit 1', array(':id' => $order['refundid']));

						if (!empty($refund)) {
							$refund_flag = 1;
							pdo_update('wx_shop_groups_order_refund', array('refundstatus' => -1, 'endtime' => $time), array('id' => $order['refundid']));
						}
					}
					...
					pdo_update('wx_shop_groups_order', $data, array('id' => $order['id']));
