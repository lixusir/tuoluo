...
class Set_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$set = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_groups_set') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $uniacid));

		if ($_W['ispost']) {
			$data2 = (is_array($_GPC['data2']) ? $_GPC['data2'] : array());

			m('common')->updatePluginset(array('groups' => array('on_show_wxapp'=>$_GPC['data']['on_show_wxapp'])));
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'wx_shop:groups') {
					show_json(0, '关键字已存在!');
				}
			}

			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':name' => 'wx_shop:groups'));

			if (empty($rule)) {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:groups', 'module' => 'wx_shop', 'displayorder' => 0, 'status' => 1);
			...
			if (!empty($set)) {
				$set_update = pdo_update('wx_shop_groups_set', $data, array('id' => $set['id'], 'uniacid' => $uniacid));
			}
			else {
				$set_insert = pdo_insert('wx_shop_groups_set', $data);
			}

			pdo_update('wx_shop_groups_goods', array('rights' => 1), array('uniacid' => $uniacid));
			$goodsid = explode(',', $data['goodsid']);

			foreach ($goodsid as $value) {
				$goods_update = pdo_update('wx_shop_groups_goods', array('rights' => 0), array('id' => intval($value), 'uniacid' => $uniacid));
			}
		...
		$data2 = $this->set;
		$data = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_groups_set') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $uniacid));

		if ($data['goodsid']) {
			$goods = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('wx_shop_groups_goods') . "\r\n                    WHERE uniacid = :uniacid and id in (" . $data['goodsid'] . ') ', array(':uniacid' => $uniacid));
		}
	...
	public function query()
	{
		...
		$ds = pdo_fetchall('SELECT id,title,rights,thumb FROM ' . tablename('wx_shop_groups_goods') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
