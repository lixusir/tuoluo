...
class GroupsModel extends PluginModel
{
	...
	public function orderstest()
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$sql = 'SELECT * FROM' . tablename('wx_shop_groups_order') . 'where uniacid = :uniacid and status = 0 ';
		...
		if ($allorders) {
			foreach ($allorders as $key => $value) {
				...
				if ($lasttime2 < 0) {
					pdo_update('wx_shop_groups_order', array('status' => -1), array('id' => $value['id']));
				}
			}
		}

		$sql1 = 'SELECT * FROM' . tablename('wx_shop_groups_order') . 'where uniacid = :uniacid and heads = 1 and status = 1 and success = 0 ';
		$allteam = pdo_fetchall($sql1, $params);

		if ($allteam) {
			foreach ($allteam as $key => $value) {
				$total = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_groups_order') . '  where uniacid = :uniacid and teamid = :teamid and heads = :heads and status = :status and success = :success ', array(':uniacid' => $uniacid, ':heads' => 1, ':teamid' => $value['teamid'], ':status' => 1, ':success' => 0));

				if ($value['groupnum'] == $total) {
					pdo_update('wx_shop_groups_order', array('success' => 1), array('teamid' => $value['teamid']));
				}
				else {
					$hours = $value['endtime'];
					$time = time();
					...
					if ($lasttime2 < 0) {
						pdo_update('wx_shop_groups_order', array('success' => -1, 'canceltime' => strtotime($endtime)), array('teamid' => $value['teamid']));
					}
				}
			}
		}
	...
	public function payResult($orderno, $type, $app = false)
	{
		...
		$order = pdo_fetch('select * from ' . tablename('wx_shop_groups_order') . ' where  orderno =:orderno and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':orderno' => $orderno));
		...
		pdo_update('wx_shop_groups_order', array('pay_type' => $type, 'status' => 1, 'paytime' => TIMESTAMP, 'starttime' => TIMESTAMP, 'apppay' => $app ? 1 : 0), array('orderno' => $orderno));
		$this->sendTeamMessage($order['id']);

		if (!empty($order['is_team'])) {
			$total = pdo_fetchcolumn('select count(1) from ' . tablename('wx_shop_groups_order') . ' as o where status = :status and teamid = :teamid and uniacid = :uniacid and success = :success ', $params);

			if ($order['groupnum'] == $total) {
				pdo_update('wx_shop_groups_order', array('success' => 1), array('teamid' => $order['teamid'], 'status' => 1, 'uniacid' => $uniacid));
				pdo_update('wx_shop_groups_order', array('success' => -1, 'status' => -1, 'canceltime' => time()), array('teamid' => $order['teamid'], 'status' => 0, 'uniacid' => $uniacid));
				$this->sendTeamMessage($order['id']);
			}
		}
		...
		pdo_update('wx_shop_groups_goods', array('stock' => $stock, 'sales' => $sales, 'teamnum' => $teamnum), array('id' => $order_goods['id']));
		return true;
	}
	...
	public function getTotals()
	{
		...
		$totals['all'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " o\r\n\t\t\t WHERE o.uniacid = :uniacid and o.isverify = 0 ", $paras);
		$totals['status1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " o\r\n\t\t\t WHERE o.uniacid = :uniacid and o.isverify = 0 and o.status = 1 and (o.success = 1 or o.is_team = 0) ", $paras);
		$totals['status2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " o\r\n\t\t\t WHERE o.uniacid = :uniacid and o.isverify = 0 and o.status=2 ", $paras);
		$totals['status3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " o\r\n\t\t\t WHERE o.uniacid = :uniacid and o.isverify = 0 and o.status = 0 ", $paras);
		$totals['status4'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " o\r\n\t\t\t WHERE o.uniacid = :uniacid and o.isverify = 0 and o.status = 3 ", $paras);
		$totals['status5'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " o\r\n\t\t\t WHERE o.uniacid = :uniacid and o.isverify = 0 and o.status = -1 ", $paras);
		$totals['team1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " o\r\n\t\t\t WHERE o.uniacid = :uniacid and o.heads = 1 and o.paytime > 0 and is_team = 1 and o.success = 1 ", $paras);
		$totals['team2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " o\r\n\t\t\t WHERE o.uniacid = :uniacid and o.heads = 1 and o.paytime > 0 and is_team = 1 and o.success = 0 ", $paras);
		$totals['team3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " o\r\n\t\t\t WHERE o.uniacid = :uniacid and o.heads = 1 and o.paytime > 0 and is_team = 1 and o.success = -1 ", $paras);
		$totals['allteam'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " o\r\n\t\t\t WHERE o.uniacid = :uniacid and o.heads = 1 and o.paytime > 0 and is_team = 1 ", $paras);
		$totals['refund1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order_refund') . " as ore\r\n\t\t\tleft join " . tablename('wx_shop_groups_order') . " as o on o.id = ore.orderid\r\n\t\t\tright join " . tablename('wx_shop_groups_goods') . " as g on g.id = o.goodid\r\n\t\t\tright join " . tablename('wx_shop_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid\r\n\t\t\tleft join " . tablename('wx_shop_member_address') . " a on a.id=ore.refundaddressid\r\n\t\t\tright join " . tablename('wx_shop_groups_category') . " as c on c.id = g.category\r\n\t\t\tWHERE ore.uniacid = :uniacid AND o.refundstate > 0 and o.refundid != 0 and ore.refundstatus >= 0 ", $paras);
		$totals['refund2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order_refund') . " as ore\r\n\t\t\tleft join " . tablename('wx_shop_groups_order') . " as o on o.id = ore.orderid\r\n\t\t\tright join " . tablename('wx_shop_groups_goods') . " as g on g.id = o.goodid\r\n\t\t\tright join " . tablename('wx_shop_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid\r\n\t\t\tleft join " . tablename('wx_shop_member_address') . " a on a.id=ore.refundaddressid\r\n\t\t\tright join " . tablename('wx_shop_groups_category') . " as c on c.id = g.category\r\n\t\t\tWHERE ore.uniacid = :uniacid AND (o.refundtime != 0 or ore.refundstatus < 0) ", $paras);
		$totals['verify1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " as o\r\n\t\t\tWHERE o.uniacid=:uniacid and o.isverify = 1 and o.status =  1 ", $paras);
		$totals['verify2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " as o\r\n\t\t\tWHERE o.uniacid=:uniacid and o.isverify = 1 and o.status = 3 ", $paras);
		$totals['verify3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('wx_shop_groups_order') . " as o\r\n\t\t\tWHERE o.uniacid=:uniacid and o.isverify = 1 and o.status <= 0 ", $paras);
		return $totals;
	}
	...
	public function groupsShare()
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$share = pdo_fetch('select share_title,share_icon,share_desc,share_url from ' . tablename('wx_shop_groups_set') . ' where uniacid=:uniacid ', array(':uniacid' => $uniacid));
	...
	public function sendTeamMessage($orderid = '0', $delRefund = false)
	{
		...
		$order = pdo_fetch('select * from ' . tablename('wx_shop_groups_order') . ' where uniacid = :uniacid and id=:id limit 1', array(':uniacid' => $uniacid, ':id' => $orderid));
		...
		$order_goods = pdo_fetch('select * from ' . tablename('wx_shop_groups_goods') . ' where uniacid=:uniacid and id=:id ', array(':uniacid' => $_W['uniacid'], ':id' => intval($order['goodid'])));
		...
		if ($delRefund == true) {
			$order_refund = pdo_fetch('select * from ' . tablename('wx_shop_groups_order_refund') . ' where uniacid=:uniacid and id=:id ', array(':uniacid' => $_W['uniacid'], ':id' => intval($order['refundid'])));
			$refundtype = '';
			...
			if ($order['success'] == 1) {
				$order = pdo_fetchall('select * from ' . tablename('wx_shop_groups_order') . ' where teamid = :teamid and success = 1 and status = 1 ', array(':teamid' => $order['teamid']));
	...
	public function sendGroupsNotice(array $params)
	{
		...
		if ($tm['is_advanced']) {
			...
			if (!empty($templateid)) {
				$advanced_template = pdo_fetch('select * from ' . tablename('wx_shop_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $templateid, ':uniacid' => $_W['uniacid']));
	...
	public function allow($orderid, $times = 0, $verifycode = '', $openid = '')
	{
		...
		$order = pdo_fetch('select * from ' . tablename('wx_shop_groups_order') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
		...
		if ($order['isverify']) {
			...
			if ($order['verifytype'] == 0) {
				$verifynum = pdo_fetchcolumn('select COUNT(1) from ' . tablename('wx_shop_groups_verify') . ' where uniacid = :uniacid and orderid = :orderid ', array(':uniacid' => $uniacid, ':orderid' => $orderid));
			...
			if (!empty($saler['storeid'])) {
				if (0 < $merchid) {
					$store = pdo_fetch('select * from ' . tablename('wx_shop_merch_store') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $saler['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
				}
				else {
					$store = pdo_fetch('select * from ' . tablename('wx_shop_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $saler['storeid'], ':uniacid' => $_W['uniacid']));
				}
			}
		}
	...
	public function verify($orderid = 0, $times = 0, $verifycode = '', $openid = '')
	{
		...
		if ($order['isverify']) {
			if ($order['verifytype'] == 0) {
				pdo_update('wx_shop_groups_order', array('status' => 3, 'finishtime' => time(), 'sendtime' => $current_time), array('id' => $order['id']));
				...
				pdo_insert('wx_shop_groups_verify', $data);
			}
			else {
				if ($order['verifytype'] == 1) {
					if ($order['status'] != 3) {
	...
	{
		global $_W;
		$item = pdo_fetch('SELECT id,expressname,type FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE id=:id and type=:type AND uniacid=:uniacid', array(':id' => $id, ':type' => $type, ':uniacid' => $_W['uniacid']));

		if (!empty($item)) {
			pdo_update('wx_shop_exhelper_express', array('isdefault' => 0), array('type' => $type, 'uniacid' => $_W['uniacid']));
			pdo_update('wx_shop_exhelper_express', array('isdefault' => 1), array('id' => $id));
	...
	{
		global $_W;
		$items = pdo_fetchall('SELECT id,expressname FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE id in( ' . $id . ' ) and type=:type and uniacid=:uniacid ', array(':type' => $type, ':uniacid' => $_W['uniacid']));

		foreach ($items as $item) {
			pdo_delete('wx_shop_exhelper_express', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function getTemp()
	{
		global $_W;
		global $_GPC;
		$temp_sender = pdo_fetchall('SELECT id,isdefault,sendername,sendertel FROM ' . tablename('wx_shop_exhelper_senduser') . ' WHERE uniacid=:uniacid order by isdefault desc ', array(':uniacid' => $_W['uniacid']));
		$temp_express = pdo_fetchall('SELECT id,type,isdefault,expressname FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE type=1 and uniacid=:uniacid order by isdefault desc ', array(':uniacid' => $_W['uniacid']));
		$temp_invoice = pdo_fetchall('SELECT id,type,isdefault,expressname FROM ' . tablename('wx_shop_exhelper_express') . ' WHERE type=2 and uniacid=:uniacid order by isdefault desc ', array(':uniacid' => $_W['uniacid']));
