...
foreach ($sets as $key => $value) {
	...
	$times = $hours * 60 * 60;
	$orders = pdo_fetchall('select id,orderno,openid,credit,creditmoney,price,freight,status,pay_type,teamid,apppay,isborrow,borrowopenid from ' . tablename('wx_shop_groups_order') . "\r\n            where  uniacid=" . $_W['uniacid'] . ' and status = 1 and success = -1 and refundtime = 0 and canceltime + ' . $times . ' <= ' . time() . ' ');
	...
	foreach ($orders as $k => $val) {
		...
		pdo_update('wx_shop_groups_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => time()), array('id' => $val['id'], 'uniacid' => $_W['uniacid']));
		$sales = pdo_fetch('select id,sales,stock from ' . tablename('wx_shop_groups_goods') . ' where id = :id and uniacid = :uniacid ', array(':id' => $val['goodid'], ':uniacid' => $uniacid));
