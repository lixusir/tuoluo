...
class Index_WxShopPage extends MmanageMobilePage
{
	...
	public function getlist()
	{
		...
		$join = '';
		$sql = 'select * from ' . tablename('wx_shop_member') . ' dm ' . $join . ' where 1 ' . $condition . '  ORDER BY id DESC';
		...
		foreach ($list as &$row) {
			...
			$row['ordermoney'] = pdo_fetchcolumn('select sum(goodsprice) from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
	...
	public function detail()
	{
		...
		$order = pdo_fetch('select finishtime from ' . tablename('wx_shop_order') . ' where uniacid=:uniacid and openid=:openid and status>=1 and finishtime>0 limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
