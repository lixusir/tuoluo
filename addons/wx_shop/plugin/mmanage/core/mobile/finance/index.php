...
class Index_WxShopPage extends MmanageMobilePage
{
	...
	protected function log($type)
	{
		...
		$sql = 'select log.id,m.id as mid, m.realname,m.avatar,m.weixin,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,m.nickname,m.mobile,g.groupname,log.money,log.createtime,l.levelname,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,log.apppay from ' . tablename('wx_shop_member_log') . ' log ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=log.openid' . ' left join ' . tablename('wx_shop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('wx_shop_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
		...
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member_log') . ' log ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid=log.openid and m.uniacid= log.uniacid' . ' left join ' . tablename('wx_shop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('wx_shop_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ', $params);
	...
	protected function credit($type)
	{
		...
		$params = array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':credittype' => $type);
		...
		$sql = 'select log.*,m.id as mid, m.realname,m.avatar,m.nickname,m.avatar, m.mobile, m.weixin,u.username from ' . tablename('mc_credits_record') . ' log ' . ' left join ' . tablename('users') . ' u on log.operator<>0 and log.operator<>log.uid and  log.operator=u.uid' . ' left join ' . tablename('wx_shop_member') . ' m on m.uid=log.uid' . ' left join ' . tablename('wx_shop_member_group') . ' g on m.groupid=g.id' . ' left join ' . tablename('wx_shop_member_level') . ' l on m.level =l.id' . ' where 1 ' . $condition . ' ORDER BY log.createtime DESC ';
