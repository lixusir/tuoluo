...
class Login_WxShopPage extends MmanageMobilePage 
{
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ($type == 'wechat') 
			{
				...
				$roleuser = pdo_fetch('SELECT id, uid, username, status FROM' . tablename('wx_shop_perm_user') . 'WHERE openid=:openid AND uniacid=:uniacid LIMIT 1', array(':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
				if (empty($roleuser)) 
				{
