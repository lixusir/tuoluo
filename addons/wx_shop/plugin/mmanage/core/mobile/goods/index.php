...
class Index_WxShopPage extends MmanageMobilePage
{
	...
	public function getlist()
	{
		...
		if (0 < $total) {
			...
			$sql = 'SELECT g.* FROM ' . tablename('wx_shop_goods') . 'g' . $condition . " ORDER BY g.`status` DESC, g.`displayorder` DESC,\r\n                g.`id` DESC LIMIT " . $presize . ',' . $psize;
	...
	public function restore()
	{
		...
		foreach ($items as $item) {
			pdo_update('wx_shop_goods', array('deleted' => 0, 'status' => 0), array('id' => $item['id']));
