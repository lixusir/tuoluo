	...
	public function setting()
	{
		...
		if ($_W['ispost']) {
			...
			if (!empty($arr['keyword'])) {
				...
				$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':name' => 'wx_shop:mmanage'));

				if (!empty($rule)) {
					pdo_update('rule_keyword', array('content' => $arr['keyword']), array('rid' => $rule['id']));
				}
				else {
					$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:mmanage', 'module' => 'wx_shop', 'displayorder' => 0, 'status' => 1);
