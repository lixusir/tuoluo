...
if ($operation == 'display') {
	...
	$list = pdo_fetchall('select log.*, m.nickname,m.avatar,m.realname,m.mobile,g.title,g.thumb,g.thumb,g.credit,g.money,g.type as goodstype from ' . tablename('wx_shop_creditshop_log') . ' log ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('wx_shop_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('wx_shop_creditshop_goods') . ' g on g.id = log.goodsid' . ' where 1 ' . $condition . ' ORDER BY log.createtime desc limit ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
	$total = pdo_fetchcolumn('select count(log.id) from' . tablename('wx_shop_creditshop_log') . ' log ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid = log.openid and m.uniacid=log.uniacid' . ' left join ' . tablename('wx_shop_member_address') . ' a on a.id = log.addressid' . ' left join ' . tablename('wx_shop_creditshop_goods') . ' g on g.id = log.goodsid' . ' where 1 ' . $condition, $params);
	...
	pdo_update('wx_shop_creditshop_log', array('status' => 3, 'usetime' => time(), 'expresscom' => $_GPC['expresscom'], 'expresssn' => $_GPC['expresssn'], 'express' => $_GPC['express']), array('id' => $id));
