...
class Index_WxShopPage extends PluginMobilePage
{
	public function main()
	{
		...
		if ($item['type'] == 0) {
			...
			$room = pdo_fetch('SELECT id,title,introduce,invitation_id FROM ' . tablename('wx_shop_live') . ' where uniacid = :uniacid and id=:roomid ', array(':uniacid' => $_W['uniacid'], ':roomid' => $roomid));
	...
	protected function getTemp($item)
	{
		...
		foreach ($item['data']['selected'] as $tempid) {
			if (0 < intval($tempid)) {
				$templist[] = array('bg' => '../addons/wx_shop/plugin/invitation/static/templist/image_' . $tempid . '_bg.jpg', 'nail' => '../addons/wx_shop/plugin/invitation/static/templist/image_' . $tempid . '_nail.jpg');
			}
			else {
