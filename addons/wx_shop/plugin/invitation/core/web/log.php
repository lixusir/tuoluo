...
class Log_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$invitation = pdo_fetch('SELECT id,title,scan,follow FROM ' . tablename('wx_shop_invitation') . ' WHERE id=:id AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		$list = pdo_fetchall('SELECT log.*, mc.id as mcid, mc.nickname as nickname_mc, mc.avatar as avatar_mc, ms.id as msid, ms.nickname as nickname_ms, ms.avatar as avatar_ms FROM ' . tablename('wx_shop_invitation_log') . ' log LEFT JOIN ' . tablename('wx_shop_member') . ' mc ON mc.openid=log.openid LEFT JOIN' . tablename('wx_shop_member') . ' ms ON ms.openid=log.invitation_openid WHERE 1' . $condition . ' ORDER BY log.scan_time DESC LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
		$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('wx_shop_invitation_log') . ' log LEFT JOIN ' . tablename('wx_shop_member') . ' mc ON mc.openid=log.openid LEFT JOIN' . tablename('wx_shop_member') . ' ms ON ms.openid=log.invitation_openid WHERE 1' . $condition, $params);
