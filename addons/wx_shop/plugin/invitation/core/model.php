...
class InvitationModel extends PluginModel
{
	public function getQR($params = array())
	{
		...
		$expire = 2592000;
		$create = false;
		$qr = pdo_fetch('select * from ' . tablename('wx_shop_invitation_qr') . ' where openid=:openid and acid=:acid and invitationid=:invitationid and roomid=:roomid limit 1', array(':openid' => $params['openid'], ':acid' => $_W['acid'], ':invitationid' => $params['invitationid'], ':roomid' => $params['roomid']));
		...
		if (empty($qr)) {
			...
			$ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'qrcid' => $scene_id, 'model' => 0, 'name' => 'WX_SHOP_INVITATION_QRCODE', 'keyword' => 'WX_SHOP_INVITATION', 'expire' => $expire, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket']);
			...
			pdo_insert('wx_shop_invitation_qr', $qr);
			$qr['id'] = pdo_insertid();
			$create = true;
		}
		else {
			if (($qr['createtime'] + $qr['expire']) <= time()) {
				...
				pdo_update('wx_shop_invitation_qr', array('ticket' => $result['ticket'], 'qrimg' => $qrimg, 'url' => $result['url']), array('id' => $qr['id']));
	...
	{
		global $_W;
		$path = IA_ROOT . '/addons/wx_shop/data/qrcode/invitation/' . $_W['uniacid'] . '/';
		...
		if (!$create && is_file($filename)) {
			return $_W['siteroot'] . 'addons/wx_shop/data/qrcode/invitation/' . $_W['uniacid'] . '/' . $file;
		}
	...
	public function getQRByTicket($ticket)
	{
		...
		$qr = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_invitation_qr') . ' WHERE ticket=:ticket AND acid=:acid', array(':ticket' => $ticket, ':acid' => $_W['acid']));
	...
	public function scanLog($openid = NULL, $qr = array(), $follow = false)
	{
		...
		pdo_query('UPDATE ' . tablename('wx_shop_invitation') . ' SET scan=scan+1 WHERE id=:id AND uniacid=:uniacid', array(':id' => $qr['invitationid'], ':uniacid' => $_W['uniacid']));

		if ($follow) {
			pdo_query('UPDATE ' . tablename('wx_shop_invitation') . ' SET follow=follow+1 WHERE id=:id AND uniacid=:uniacid', array(':id' => $qr['invitationid'], ':uniacid' => $_W['uniacid']));
		}

		pdo_insert('wx_shop_invitation_log', array('uniacid' => $_W['uniacid'], 'invitation_id' => $qr['invitationid'], 'openid' => $openid, 'invitation_openid' => $qr['openid'], 'scan_time' => time(), 'follow' => $follow ? 1 : 0));
	}
}
