...
require_once WX_SHOP_INC . 'plugin_processor.php';
...
class PosterProcessor extends PluginProcessor
{
	...
	private function responseScan($obj)
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('wx_shop_poster') . ' where type=4 and isdefault=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
		...
		if (0 < $poster['ismembergroup']) {
			if (empty($member['groupid']) && (0 < $poster['membergroupid'])) {
				pdo_update('wx_shop_member', array('groupid' => $poster['membergroupid']), array('openid' => $openid, 'uniacid' => $_W['uniacid']));
			}
		}
	...
	private function responseSubscribe($obj)
	{
		...
		if ($member['isnew']) {
			pdo_update('wx_shop_poster', array('follows' => $poster['follows'] + 1), array('id' => $poster['id']));
		}

		//$qrmember = m('member')->getMember($qr['openid']);
		$qrmember = m('member')->getMember($obj->member['agentid']);
		$qr['openid'] = $qrmember['openid'];
		$log = pdo_fetch('select * from ' . tablename('wx_shop_poster_log') . ' where openid=:openid and posterid=:posterid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':posterid' => $poster['id'], ':uniacid' => $_W['uniacid']));
		if (empty($log) && ($openid != $qr['openid'])) {
			...
			pdo_insert('wx_shop_poster_log', $log);
			$log['id'] = pdo_insertid();
	...
	'remark'   => array('value' => "\r\n谢谢您对我们的支持！", 'color' => '#4a5077')
			...
			if (!empty($upgrade)) {
				pdo_update('wx_shop_poster_log', $upgrade, array('id' => $log['id']));
			}
		}
							...
							if (empty($member['agentid'])) {
								if (empty($member['fixagentid'])) {
									...
									if ($author) {
										...
										pdo_update('wx_shop_member', array('agentid' => $qrmember['id'], 'childtime' => $time, 'authorid' => $authorid), array('id' => $member['id']));
									}
									else {
										...
										if (!empty($author_set['become']) && (($author_set['become'] == '2') || ($author_set['become'] == '5'))) {
											...
											if ($can_author) {
												...
												if (empty($member['authorblack'])) {
													pdo_update('wx_shop_member', array('authorstatus' => $become_check, 'isauthor' => 1, 'authortime' => $time), array('uniacid' => $_W['uniacid'], 'id' => $qrmember['id']));
