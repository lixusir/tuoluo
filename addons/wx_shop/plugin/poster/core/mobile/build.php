...
class build_WxShopPage extends PluginPfMobilePage
{
	public function main()
	{
		...
		if (strexists($content, '+')) {
			$msg = explode('+', $content);
			$poster = pdo_fetch('select * from ' . tablename('wx_shop_poster') . ' where keyword2=:keyword and type=3 and isdefault=1 and uniacid=:uniacid limit 1', array(':keyword' => $msg[0], ':uniacid' => $_W['uniacid']));
			...
			$poster = pdo_fetch('select * from ' . tablename('wx_shop_poster') . ' where keyword2=:keyword and isdefault=1 and uniacid=:uniacid limit 1', array(':keyword' => $content, ':uniacid' => $_W['uniacid']));
