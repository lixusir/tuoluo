...
class PosterModel extends PluginModel
{
	public function checkScan($openid = '')
	{
		...
		$poster = pdo_fetch('select id,times from ' . tablename('wx_shop_poster') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $posterid));
	...
	public function scanTime($openid, $from_openid, $poster)
	{
		...
		global $_W;
		global $_GPC;
		$scancount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_poster_scan') . ' where openid=:openid  and posterid=:posterid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':posterid' => $poster['id'], ':uniacid' => $_W['uniacid']));

		if ($scancount <= 0) {
			$scan = array('uniacid' => $_W['uniacid'], 'posterid' => $poster['id'], 'openid' => $openid, 'from_openid' => $from_openid, 'scantime' => time());
			pdo_insert('wx_shop_poster_scan', $scan);
			pdo_update('wx_shop_poster', array('times' => $poster['times'] + 1), array('id' => $poster['id']));
		}
	}
	...
	public function createCommissionPoster($openid, $goodsid = 0, $type = 0)
	{
		...
		$poster = pdo_fetch('select * from ' . tablename('wx_shop_poster') . ' where uniacid=:uniacid and `type`=:type and isdefault=1 limit 1', array(':uniacid' => $_W['uniacid'], ':type' => $type));
	...
	public function getFixedTicket($poster, $member, $uniaccount)
	{
		global $_W;
		global $_GPC;
		$scene_str = md5('wx_shop_poster:' . $_W['uniacid'] . ':' . $member['openid'] . ':' . $poster['id']);
	...
	public function getQR($poster, $member, $goodsid = 0)
	{
		...
		if ($poster['type'] == 1) {
			...
			$qr = pdo_fetch('select * from ' . tablename('wx_shop_poster_qr') . ' where openid=:openid and acid=:acid and type=:type limit 1', array(':openid' => $member['openid'], ':acid' => $_W['acid'], ':type' => 1));

			if (empty($qr)) {
				$qr = array('acid' => $acid, 'openid' => $member['openid'], 'type' => 1, 'qrimg' => $qrimg);
				pdo_insert('wx_shop_poster_qr', $qr);
				$qr['id'] = pdo_insertid();
			}
		...
		if ($poster['type'] == 2) {
			...
			if ($p) {
				...
				$qr = pdo_fetch('select * from ' . tablename('wx_shop_poster_qr') . ' where openid=:openid and acid=:acid and type=:type limit 1', array(':openid' => $member['openid'], ':acid' => $_W['acid'], ':type' => 2));
			...
			if ($poster['type'] == 3) {
				...
				$qr = pdo_fetch('select * from ' . tablename('wx_shop_poster_qr') . ' where openid=:openid and acid=:acid and type=:type and goodsid=:goodsid limit 1', array(':openid' => $member['openid'], ':acid' => $_W['acid'], ':type' => 3, ':goodsid' => $goodsid));
			...
			if ($poster['type'] == 4) {
				...
				$qr = pdo_fetch('select * from ' . tablename('wx_shop_poster_qr') . ' where openid=:openid and acid=:acid and type=4 limit 1', array(':openid' => $member['openid'], ':acid' => $acid));
				if (empty($qr) || empty($qr['scenestr'])) {
					...
					$ims_qrcode = array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'scene_str' => $barcode['action_info']['scene']['scene_str'], 'model' => 2, 'name' => 'WX_SHOP_POSTER_QRCODE', 'keyword' => 'WX_SHOP_POSTER', 'expire' => 0, 'createtime' => time(), 'status' => 1, 'url' => $result['url'], 'ticket' => $result['ticket']);
				...
				if (empty($qr)) {
					...
					pdo_update('wx_shop_poster_qr', array('scenestr' => $barcode['action_info']['scene']['scene_str'], 'ticket' => $result['ticket'], 'qrimg' => $qrimg), array('id' => $qr['id']));
	...
	public function createPoster($poster, $member, $qr, $upload = true)
	{
		...
		if (!empty($qr['goodsid'])) {
			$goods = pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('wx_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $qr['goodsid'], ':uniacid' => $_W['uniacid']));
		...
		if (!is_file($path . $file) || ($qr['qrimg'] != $qr['current_qrimg'])) {
			...
			if ($qr['qrimg'] != $qr['current_qrimg']) {
				pdo_update('wx_shop_poster_qr', array('qrimg' => $qr['current_qrimg']), array('id' => $qr['id']));
			}
		}

		$img = $_W['siteroot'] . 'addons/wx_shop/data/poster/' . $_W['uniacid'] . '/' . $file;
		...
		if (($qr['qrimg'] != $qr['current_qrimg']) || empty($qr['mediaid']) || empty($qr['createtime']) || ((($qr['createtime'] + (3600 * 24 * 3)) - 7200) < time())) {
			...
			$qr['mediaid'] = $mediaid;
			pdo_update('wx_shop_poster_qr', array('mediaid' => $mediaid, 'createtime' => time()), array('id' => $qr['id']));
		}
	...
	public function getQRByTicket($ticket = '')
	{
		...
		$qrs = pdo_fetchall('select * from ' . tablename('wx_shop_poster_qr') . ' where ticket=:ticket and acid=:acid and type=4 limit 1', array(':ticket' => $ticket, ':acid' => $_W['acid']));
		$count = count($qrs);
