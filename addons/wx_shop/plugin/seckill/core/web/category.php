...
class Category_WxShopPage extends SeckillWebPage
{
	...
	public function main()
	{
		...
		if (!empty($_GPC['catname']) || !empty($_GPC['catname_new'])) {
			...
			if (is_array($_GPC['catname'])) {
				foreach ($_GPC['catname'] as $id => $catname) {
					...
					pdo_update('wx_shop_seckill_category', array('name' => $catname), array('id' => $id));
			...
			if (is_array($_GPC['catname_new'])) {
				foreach ($_GPC['catname_new'] as $id => $catname) {
					...
					pdo_insert('wx_shop_seckill_category', array('name' => $catname, 'uniacid' => $_W['uniacid']));
		...
		$list = pdo_fetchall('SELECT * FROM ' . tablename('wx_shop_seckill_category') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\'  ORDER BY id DESC');
	...
	public function delete()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch('SELECT id,name FROM ' . tablename('wx_shop_seckill_category') . ' WHERE id = \'' . $id . '\'  AND uniacid=' . $_W['uniacid'] . '');

		if (empty($item)) {
			$this->message('抱歉，分类不存在或是已经被删除！', webUrl('seckill/category', array('op' => 'display')), 'error');
		}

		pdo_delete('wx_shop_seckill_category', array('id' => $id));
