...
class Calendar_WxShopPage extends SeckillWebPage
{
	...
	public function dates() 
	{
		...
		// var_dump(redis()->dump($redis_prefix . 'calendar_' . $year . '_' . $month) );die;
		if (empty($calendar)) 
		{
			$calendar = array();
			$i = 1;
		...
		else 
		{
			...
			while ($i <= $day) 
			{
				...
				if (isset($calendar[$date])) 
				{
					...
					if (!(empty($value))) 
					{
						$result[$date] = array('taskid' => $value, 'title' => pdo_fetchcolumn('select title from ' . tablename('wx_shop_seckill_task') . ' where id=:id limit 1', array(':id' => $value)));
					}
				}
				++$i;
			}
	...
	public function set() 
	{
		...
		$year = date('Y', $time);
		$month = date('m', $time);
		$task = pdo_fetch('select id ,title from ' . tablename('wx_shop_seckill_task') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $taskid));
		if (empty($task)) 
		{
			show_json(0, '任务未找到');
		}
		...
		show_json(1, array('taskid' => $task['id'], 'title' => $task['title'],'key'=>$redis_prefix . 'calendar_' . $year . '_' . $month,'_w'=>$_W));
	}
