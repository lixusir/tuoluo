...
class SeckillModel extends PluginModel
{
	public function get_prefix()
	{
		...
		return 'wx_shop_' . $_W['config']['setting']['site']['key'] . '_' . $_W['uniacid'] . '_' . $_W['account']['key'] . '_seckill_';
	}
	...
	public function setTaskCache($id)
	{
		...
		$task = pdo_fetch('select * from ' . tablename('wx_shop_seckill_task') . ' where id=:id limit 1', array(':id' => $id));
		redis()->delete($redis_prefix . 'info_' . $id);
		redis()->hMset($redis_prefix . 'info_' . $id, $task);
		$allrooms = pdo_fetchall('select * from ' . tablename('wx_shop_seckill_task_room') . ' where taskid=:taskid and enabled=1 and uniacid=:uniacid order by `displayorder` desc', array(':taskid' => $id, ':uniacid' => $_W['uniacid']));
		...
		$alltimes = pdo_fetchall('select * from ' . tablename('wx_shop_seckill_task_time') . ' where taskid=:taskid and uniacid=:uniacid order by `time` asc', array(':taskid' => $id, ':uniacid' => $_W['uniacid']));
		$redisgoods = array();

		foreach ($alltimes as &$time) {
			$goods = pdo_fetchall('select * from ' . tablename('wx_shop_seckill_task_goods') . ' where taskid=:taskid and timeid=:timeid and uniacid=:uniacid order by displayorder asc', array(':taskid' => $id, ':timeid' => $time['id'], ':uniacid' => $_W['uniacid']));
		...
		foreach ($keys as $key) {
			...
			if (!empty($orders)) {
				...
				foreach ($orders as $orderid) {
					$o = pdo_fetch('select  id,openid,deductcredit2,ordersn,isparent,deductcredit,deductprice   from ' . tablename('wx_shop_order') . ' where id=:id  limit 1', array(':id' => $orderid));
					if (!empty($o) && ($o['status'] == 0)) {
						...
						pdo_query('update ' . tablename('wx_shop_order') . ' set status=-1,canceltime=' . time() . ' where id=' . $o['id']);
					}
				}
			}
		}
	...
	public function setOrderPay($orderid)
	{
		...
		$redis_prefix = $this->get_prefix();
		$date = date('Y-m-d');
		$order = pdo_fetch('select id,ordersn, price,openid,status, paytype, deductcredit2, couponid,isparent,merchid,agentid,createtime from ' . tablename('wx_shop_order') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
		...
		$goods = pdo_fetchall('select uniacid,total,goodsid,optionid,seckill_timeid,seckill_taskid from  ' . tablename('wx_shop_order_goods') . ' where orderid=' . $order['id'] . ' and  seckill=1 ');
	...
	public function getTaskSeckillInfo()
	{
		...
		$dbgoods = pdo_fetchall('select id,thumb,marketprice from ' . tablename('wx_shop_goods') . ' where id in (' . implode(',', $goodsids) . ') and uniacid=' . $_W['uniacid'], array(), 'id');
		$goods = array();
		...
		foreach ($times as $time) {
			...
			foreach ($goods as $g) {
				if (in_array($g['goodsid'], $goodsids) && ($g['roomid'] != $roomid)) {
					...
					$goodstitle = pdo_fetchcolumn('select title from ' . tablename('wx_shop_goods') . ' where id=:id limit 1', array(':id' => $g['goodsid']));
