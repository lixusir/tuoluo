...
class Bonus_WxShopPage extends PluginWebPage
{
	...
	protected function get_list($status = 0)
	{
		...
		$sql = 'select *  from ' . tablename('wx_shop_globonus_bill') . '  where 1 ' . $condition . ' ORDER BY createtime desc ';
	...
	'columns' => array(
		...
		$total = pdo_fetchcolumn('select count(*) from' . tablename('wx_shop_globonus_bill') . '  where 1 ' . $condition, $params);
		$totalmoney = pdo_fetchcolumn('select sum(bonusmoney_send) from' . tablename('wx_shop_globonus_bill') . '  where 1 ' . $condition, $params);
	...
	public function detail()
	{
		...
		global $_GPC;
		$id = intval($_GPC['id']);
		$data = pdo_fetch('select * from ' . tablename('wx_shop_globonus_bill') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));

		if (empty($data)) {
			$this->message('结算单未找到!');
		}

		$data['partnercount1'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_globonus_billp') . ' where billid=:billid and status=1 and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		...
		$sql = 'select b.*, m.nickname,m.avatar,m.realname,m.weixin,m.mobile,l.levelname,b.bonus,m.partnerlevel,m.id as mid from ' . tablename('wx_shop_globonus_billp') . ' b ' . ' left join ' . tablename('wx_shop_member') . ' m on m.openid = b.openid and m.uniacid = b.uniacid' . ' left join ' . tablename('wx_shop_globonus_level') . ' l on l.id = m.partnerlevel' . ' where 1 ' . $condition . ' ORDER BY status asc ';
		$list = pdo_fetchall($sql, $params);
	...
	public function delete()
	{
		...
		pdo_query('delete from ' . tablename('wx_shop_globonus_bill') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		pdo_query('delete from ' . tablename('wx_shop_globonus_billo') . ' where billid=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		pdo_query('delete from ' . tablename('wx_shop_globonus_billp') . ' where billid=:id and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function build()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($data['partners'] as $partner) {
				...
				pdo_insert('wx_shop_globonus_billp', $bp);
			}
		...
		$bill = pdo_fetch('select * from ' . tablename('wx_shop_globonus_bill') . ' where uniacid=:uniacid order by id desc limit 1 ', array(':uniacid' => $_W['uniacid']));
		include $this->template();
	}
	...
	public function confirm()
	{
		...
		$time = time();
		pdo_query('update ' . tablename('wx_shop_globonus_bill') . ' set status=1,confirmtime=' . $time . ' where id=:id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	public function pay($a = array(), $b = array())
	{
		...
		if (empty($data['status'])) {
			$orders = pdo_fetchall('select orderid from ' . tablename('wx_shop_globonus_billo') . ' where billid=:billid and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id), 'orderid');

			if (empty($orders)) {
				show_json(0, '未找到任何结算订单!');
			}

			pdo_query('update ' . tablename('wx_shop_order') . ' set isglobonus=1 where id in ( ' . implode(',', array_keys($orders)) . ' ) and uniacid=' . $_W['uniacid']);
		}

		$time = time();
		pdo_query('update ' . tablename('wx_shop_globonus_bill') . ' set paytime=' . $time . ' where id=:id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		plog('globonus.bonus.pay', '进行结算单结算 ID:' . $data['id'] . ' 单号: ' . $data['billno']);
		$partners = pdo_fetchall('select id from ' . tablename('wx_shop_globonus_billp') . ' where billid=:billid and status<>1 and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id), 'id');
	...
	public function payp()
	{
		...
		$partner = pdo_fetch('select *  from ' . tablename('wx_shop_globonus_billp') . ' where billid=:billid and id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':billid' => $id, ':id' => $bpid));
		...
		if (!empty($member)) {
			...
			if (is_error($result)) {
				pdo_update('wx_shop_globonus_billp', array('reason' => $result['message'], 'status' => -1), array('billid' => $id, 'id' => $bpid));
			}
			else {
				pdo_update('wx_shop_globonus_billp', array('reason' => '', 'status' => 1, 'paytime' => time()), array('billid' => $id, 'id' => $bpid));
			...
			pdo_update('wx_shop_globonus_billp', array('reason' => '未找到会员', 'status' => -1), array('billid' => $id, 'id' => $bpid));
		}

		$partnercount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_globonus_billp') . ' where billid=:billid and status=1 and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		$allpartnercount = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_globonus_billp') . ' where billid=:billid and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		$full = $partnercount == $allpartnercount;

		if ($full) {
			pdo_query('update ' . tablename('wx_shop_globonus_bill') . ' set status=2 where id=:id and uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		}
	...
	public function paymoney()
	{
		...
		if ($type == 'level') {
			...
			$sql = 'update ' . tablename('wx_shop_globonus_billp') . ' b ,' . tablename('wx_shop_member') . ' m set b.paymoney = ' . $value . ' where b.openid = m.openid and b.uniacid = m.uniacid and m.partnerlevel=' . $level . ' and b.billid=' . $id . ' and b.uniacid=' . $_W['uniacid'];
			pdo_query($sql);
		}
		else {
			if ($type == 'partner') {
				...
				pdo_update('wx_shop_globonus_billp', array('paymoney' => $value), array('id' => $bpid, 'billid' => $id, 'uniacid' => $_W['uniacid']));
			}
		}

		$totalmoney = pdo_fetchcolumn('select sum(paymoney)  from ' . tablename('wx_shop_globonus_billp') . ' where billid=:billid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':billid' => $id));
		pdo_update('wx_shop_globonus_bill', array('bonusmoney_pay' => $totalmoney), array('id' => $id, 'uniacid' => $_W['uniacid']));
		show_json(1);
	}
}
