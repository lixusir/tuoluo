...
class Cover_WxShopPage extends PluginWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'wx_shop股东中心入口设置'));
		...
		if ($_W['ispost']) {
			...
			if (!empty($keyword)) {
				if ($keyword['name'] != 'wx_shop股东中心入口设置') {
					show_json(0, '关键字已存在!');
				}
			}
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop股东中心入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
