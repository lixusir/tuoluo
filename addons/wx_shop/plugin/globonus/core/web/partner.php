...
class Partner_WxShopPage extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar from ' . tablename('wx_shop_member') . ' dm ' . ' left join ' . tablename('wx_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('wx_shop_globonus_level') . ' l on l.id = dm.partnerlevel' . ' where dm.uniacid = ' . $_W['uniacid'] . ' and (dm.ispartner =1 or dm.ispartner=-1)  ' . $condition . ' ORDER BY dm.partnertime desc';
		...
		$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('wx_shop_member') . ' dm  ' . ' left join ' . tablename('wx_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('wx_shop_globonus_level') . ' l on l.id = dm.partnerlevel' . ' where dm.uniacid =' . $_W['uniacid'] . ' and (dm.ispartner =1 or dm.ispartner=-1) ' . $condition, $params);
	...
	public function delete()
	{
		...
		foreach ($members as $member) {
			pdo_update('wx_shop_member', array('ispartner' => 0, 'partnerstatus' => 0), array('id' => $member['id']));
	...
	public function check()
	{
		...
		$members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,partnerstatus FROM ' . tablename('wx_shop_member') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
		$time = time();
		...
		foreach ($members as $member) {
			...
			if ($status == 1) {
				pdo_update('wx_shop_member', array('partnerstatus' => 1, 'partnertime' => $time), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
				...
				pdo_update('wx_shop_member', array('partnerstatus' => 0, 'partnertime' => 0), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
	...
	public function partnerblack()
	{
		...
		foreach ($members as $member) {
			...
			if ($partnerblack == 1) {
				pdo_update('wx_shop_member', array('ispartner' => 1, 'partnerstatus' => 0, 'partnerblack' => 1), array('id' => $member['id']));
				...
				pdo_update('wx_shop_member', array('ispartner' => 1, 'partnerstatus' => 1, 'partnerblack' => 0), array('id' => $member['id']));
