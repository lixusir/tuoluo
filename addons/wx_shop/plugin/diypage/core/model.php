...
class DiypageModel extends PluginModel
{
	...
	public function getPageList($type = 'allpage', $condition = NULL, $page = 0)
	{
		...
		$list = pdo_fetchall('select id, `name`, `type`, createtime, lastedittime, keyword,is_use from ' . tablename('wx_shop_diypage') . ' where merch=:merch ' . $c . ' and uniacid=:uniacid order by `type` desc, id desc ' . $limit, array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid']));
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('wx_shop_diypage') . ' where merch=:merch and uniacid=:uniacid ' . $c, array(':merch' => intval($_W['merchid']), ':uniacid' => $_W['uniacid']));
	...
	public function getPage($id, $mobile = false)
	{
		...
		$page = pdo_fetch('select * from ' . tablename('wx_shop_diypage') . $where . ' limit 1 ', $params);
						...
						if ($item['params']['goodsdata'] == '0') {
							if (!empty($item['data']) && is_array($item['data'])) {
								...
								if (!empty($goodsids) && is_array($goodsids)) {
									...
									if ($creditshop) {
										$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels, showgroups, `type`, goodstype from ' . tablename('wx_shop_creditshop_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									}
									else {
										$goods = pdo_fetchall('select id, title, subtitle, thumb, productprice, minprice, total, showlevels, showgroups, bargain, merchid, sales, salesreal from ' . tablename('wx_shop_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									}
							...
							if (!empty($item['params']['cateid'])) {
								if (empty($item['params']['goodstype'])) {
									$category = pdo_fetch('select id,`name`, enabled from ' . tablename('wx_shop_category') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['cateid'], ':uniacid' => $_W['uniacid']));
								}
								else {
									$category = pdo_fetch('select id,`name`, enabled from ' . tablename('wx_shop_creditshop_category') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['cateid'], ':uniacid' => $_W['uniacid']));
								}
							...
							if (($item['params']['goodsdata'] == '2') && empty($item['params']['goodstype'])) {
								if (!empty($item['params']['groupid'])) {
									$group = pdo_fetch('select id, `name` from ' . tablename('wx_shop_goods_group') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['groupid'], ':uniacid' => $_W['uniacid']));
						...
						if (($item['id'] == 'merchgroup') && p('merch')) {
							if ($item['params']['merchdata'] == '0') {
								...
								if (!empty($merchids) && is_array($merchids)) {
									...
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('wx_shop_merch_user') . ' where id in( ' . $newmerchids . ' ) and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid']));
								...
								if (!empty($item['params']['cateid'])) {
									$category = pdo_fetch('select id, `catename`, status from ' . tablename('wx_shop_merch_category') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['cateid'], ':uniacid' => $_W['uniacid']));
								}
								...
								if (!empty($category) && !empty($category['status'])) {
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('wx_shop_merch_user') . ' where cateid=:cateid and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid'], ':cateid' => $item['params']['cateid']));
									if (!empty($merchs) && is_array($merchs)) {
										$item['data'] = array();
								...
								if (!empty($item['params']['groupid'])) {
									$group = pdo_fetch('select id, groupname, status from ' . tablename('wx_shop_merch_group') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $item['params']['groupid'], ':uniacid' => $_W['uniacid']));
								...
								if (!empty($group) && !empty($group['status'])) {
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('wx_shop_merch_user') . ' where groupid=:groupid and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid'], ':groupid' => $item['params']['groupid']));
									if (!empty($merchs) && is_array($merchs)) {
										$item['data'] = array();
								...
								if ($item['params']['merchdata'] == '3') {
									$merchs = pdo_fetchall('select id, merchname, logo, status, `desc` from ' . tablename('wx_shop_merch_user') . ' where isrecommand=1 and status=1 and uniacid=:uniacid order by isrecommand desc ', array(':uniacid' => $_W['uniacid']));
									if (!empty($merchs) && is_array($merchs)) {
										$item['data'] = array();
							...
							if (($item['id'] == 'picture') || ($item['id'] == 'picturew')) {
								...
								if (!empty($item['data']) && is_array($item['data'])) {
									...
									if (!empty($couponids) && is_array($couponids)) {
										...
										$coupons = pdo_fetchall('select * from ' . tablename('wx_shop_coupon') . ' where id in( ' . $newcouponids . ' ) and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
				...
				foreach ($page['data']['items'] as $itemid => &$item) {
					if ($item['id'] == 'goods') {
						if ($item['params']['goodsdata'] == '0') {
							if (!empty($item['data']) && is_array($item['data'])) {
								...
								if (!empty($goodsids) && is_array($goodsids)) {
									...
									if ($creditshop) {
										$goods = pdo_fetchall('select id, showlevels, showgroups from ' . tablename('wx_shop_creditshop_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									}
									else {
										$goods = pdo_fetchall('select id, showlevels, showgroups from ' . tablename('wx_shop_goods') . ' where id in( ' . $newgoodsids . ' ) and status=1 and deleted=0 and checked=0 and uniacid=:uniacid order by displayorder desc ', array(':uniacid' => $_W['uniacid']));
									}
							...
							if (!empty($cateid)) {
								...
								if (empty($item['params']['goodstype'])) {
									...
									$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels, showgroups, `type`, goodstype from ' . tablename('wx_shop_creditshop_goods') . ' where cate=:cate and status=1 and deleted=0 and uniacid=:uniacid order by ' . $orderby . ' limit ' . $limit, array(':cate' => $cateid, ':uniacid' => $_W['uniacid']));
								}
							...
							if (($item['params']['goodsdata'] == '2') && empty($item['params']['goodstype'])) {
								...
								if (!empty($groupid)) {
									$group = pdo_fetch('SELECT * FROM ' . tablename('wx_shop_goods_group') . ' WHERE id=:id and uniacid=:uniacid limit 1 ', array(':id' => $groupid, ':uniacid' => $_W['uniacid']));
								}
								...
								if (!empty($group) && !empty($group['goodsids'])) {
									...
									$goods = pdo_fetchall('select id, title, subtitle, thumb, minprice, sales, salesreal, total, showlevels, showgroups, bargain, productprice from ' . tablename('wx_shop_goods') . ' where id in( ' . $goodsids . ' ) and status=1 and `deleted`=0 and `status`=1 and uniacid=:uniacid ' . $orderby . ' limit ' . $limit, array(':uniacid' => $_W['uniacid']));
									if (!empty($goods) && is_array($goods)) {
										foreach ($goods as $index => $good) {
											$showgoods = m('goods')->visit($good, $this->member);
								...
								if (2 < $item['params']['goodsdata']) {
									...
									if (empty($item['params']['goodstype'])) {
										...
										$goods = pdo_fetchall('select id, title, thumb, price as productprice, money as minprice, credit, total, showlevels, showgroups, `type`, goodstype from ' . tablename('wx_shop_creditshop_goods') . ' where 1 ' . $condition . ' order by ' . $args['order'] . ' limit ' . $args['pagesize'], $params);
									}
						...
						if ($item['params']['noticedata'] == '0') {
							...
							if (!empty($page['merch'])) {
								$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('wx_shop_merch_notice') . ' where uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc limit ' . $limit, array(':uniacid' => $_W['uniacid'], ':merchid' => intval($page['merch'])));
							}
							else {
								$notices = pdo_fetchall('select id, title, link, thumb from ' . tablename('wx_shop_notice') . ' where uniacid=:uniacid and status=1 order by displayorder desc limit ' . $limit, array(':uniacid' => $_W['uniacid']));
							}
	...
	public function savePage($id, $data, $update = true)
	{
		...
		if (!empty($keyword) && $update) {
			...
			if (!empty($result)) {
				if ($result['name'] != ('wx_shop:diypage:' . $id)) {
					show_json(0, '关键字已存在！');
				}
			}
			else {
		...
		if (!empty($id)) {
			...
			pdo_update('wx_shop_diypage', $diypage, array('id' => $id, 'uniacid' => $_W['uniacid']));
			$savetype = 'update';
		}
		else {
			...
			pdo_insert('wx_shop_diypage', $diypage);
			$id = pdo_insertid();
			$savetype = 'insert';
		}

		if (!empty($keyword) && $update) {
			$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'wx_shop', ':name' => 'wx_shop:diypage:' . $id));

			if (!empty($rule)) {
				pdo_update('rule_keyword', array('content' => $keyword), array('rid' => $rule['id']));
			}
			else {
				$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'wx_shop:diypage:' . $id, 'module' => 'wx_shop', 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule', $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => 'wx_shop', 'content' => $keyword, 'type' => 1, 'displayorder' => 0, 'status' => 1);
				pdo_insert('rule_keyword', $keyword_data);
			}
		}

		if ($update) {
			$item = pdo_fetch('select id, type, name, keyword from ' . tablename('wx_shop_diypage') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
	...
	public function delPage($id)
	{
		...
		$items = pdo_fetchall('SELECT id,`name`,`type`,keyword FROM ' . tablename('wx_shop_diypage') . ' WHERE id in( ' . $id . ' ) and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));

		foreach ($items as $item) {
			pdo_delete('wx_shop_diypage', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			...
			$keyword = pdo_fetch('SELECT * FROM ' . tablename('rule_keyword') . ' WHERE content=:content and module=:module and uniacid=:uniacid limit 1 ', array(':content' => $item['keyword'], ':module' => 'wx_shop', ':uniacid' => $_W['uniacid']));
	...
	public function saveImg($str)
	{
		...
		$str = preg_replace_callback('/"imgurl"\\:"([^\\\'" ]+)"/', function($matches) {
			...
			if (!strexists($preg, '../addons/wx_shop')) {
	...
	public function saveTemp($temp)
	{
		...
		pdo_insert('wx_shop_diypage_template', $temp);
	...
	public function verify($do, $pagetype)
	{
		...
		if ($do == 'add') {
			...
			if (!empty($tid)) {
				$template = pdo_fetch('select * from ' . tablename('wx_shop_diypage_template') . ' where id=:id and (uniacid=:uniacid or uniacid=0) limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $tid));
	...
	public function calculate($str = NULL, $pagetype = false)
	{
		...
		if (strexists($str, 'r=commission')) {
			...
			if ($plugin_commissiom) {
				...
				if (strexists($str, 'r=commission.log')) {
					$this->commission['applycount'] = pdo_fetchcolumn('select count(id) from ' . tablename('wx_shop_commission_apply') . ' where mid=:mid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mid' => $member['id']));
				}

				if (strexists($str, 'r=commission.down')) {
					$level1 = $level2 = $level3 = 0;
					$level1 = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where agentid=:agentid and uniacid=:uniacid limit 1', array(':agentid' => $member['id'], ':uniacid' => $_W['uniacid']));
					if ((2 <= $plugin_commissiom_set['level']) && (0 < count($member['level1_agentids']))) {
						$level2 = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
					}

					if ((3 <= $plugin_commissiom_set['level']) && (0 < count($member['level2_agentids']))) {
						$level3 = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member') . ' where agentid in( ' . implode(',', array_keys($member['level2_agentids'])) . ') and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid']));
					}
		...
		if (strexists($str, 'r=order') || strexists($str, 'r=member.cart') || strexists($str, 'r=coupon.my')) {
			...
			if (strexists($str, 'status=0')) {
				...
				$ordernum['status0'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where ' . $condition . ' and openid=:openid and status=0 and (isparent=1 or (isparent=0 and parentid=0)) and uniacid=:uniacid and istrade=0 and userdeleted=0 limit 1', $params);
			}
			...
			if (strexists($str, 'status=1')) {
				...
				$ordernum['status1'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid limit 1', $params);
			}
			...
			if (strexists($str, 'status=2')) {
				...
				$ordernum['status2'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid limit 1', $params);
			}
			...
			if (strexists($str, 'status=4')) {
				...
				$ordernum['status4'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_order') . ' where ' . $condition . ' and openid=:openid and uniacid=:uniacid limit 1', $params);
			}
			...
			if (strexists($str, 'r=member.cart')) {
				...
				$ordernum['cartnum'] = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('wx_shop_member_cart') . ' where ' . $condition . ' and uniacid=:uniacid and openid=:openid ', $params);
			}
			...
			if (strexists($str, 'r=member.favorite')) {
				...
				$ordernum['favorite'] = pdo_fetchcolumn('select count(*) from ' . tablename('wx_shop_member_favorite') . ' where ' . $condition . ' and uniacid=:uniacid and openid=:openid ', $params);
			}
	...
	public function url($str)
	{
		...
		if (!empty($_W['openid'])) {
			...
			if (!empty($myid)) {
				$member = pdo_fetch('select id,isagent,status from' . tablename('wx_shop_member') . 'where id=' . $myid);
	...
	{
		global $_W;
		$adv = pdo_fetch('SELECT * FROM' . tablename('wx_shop_diypage_plu') . 'WHERE id=:id AND status=1 AND `type`=1 AND uniacid=:uniacid', array(':id' => $id, ':uniacid' => $_W['uniacid']));
	...
	//创建微信公众号返回台位信息
	public function platformRespond($keyword='',$rule_name='wx_shop:merch')
	{
		global $_W;
		if(empty($keyword)){
			return false;
        ...
        //插入ims_rule表
        $rule = pdo_get('rule', array('name' => $rule_name), array('id'));
        $rule_id = $rule['id'];
        if(empty($rule_id)){
            $rule_data = array('uniacid' => $_W['uniacid'], 'name'=> 'wx_shop:merch', 'module' => $_W['current_module']['name'], 'displayorder' => 0, 'status' => 1,'containtype'=>'basic,' ,'reply_type'	=> 0);
            pdo_insert('rule', $rule_data);
            $rule_id = pdo_insertid();
		}

        //插入ims_rule_keywords表
		$rule_keyword_data = array('rid' => $rule_id,'uniacid' => $_W['uniacid'],'module' => $_W['current_module']['name'],'content'=>$keyword,'type'=>1,'displayorder' => 0, 'status' => 1);
        pdo_insert('rule_keyword', $rule_keyword_data);
        $rule_keyword_id = pdo_insertid();

        //生成二维码
        $qr = $this->createQr($keyword);
        if(empty($qr)){
        	return false;
		}
		...
		$qr_data = array(
                ...
                'acid' => intval($_W['acid']),
                'qrcid' => 0,
                'scene_str' => $keyword, //不能为空
                'keyword' => $keyword,
                'name' => $keyword, //这里可才成门店名称+台位名称
                'model' => 2,
                'ticket' => $qr['ticket'],
                'url' => $qr['url'],
                'expire' => 0,
        ...
        pdo_insert('qrcode', $qr_data);
        $qrcode_id = pdo_insertid();

        if($qrcode_id && $rule_keyword_id){
        	$return = array('ticket' => $qr_data['ticket'],'qrid'=>$qrcode_id);
			return $return;
		}
